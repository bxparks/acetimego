// This file was generated by the following script:
//
//   $ /home/brian/src/AceTimeTools/src/acetimetools/tzcompiler.py
//     --input_dir /home/brian/src/AceTimeGo/zonedb/tzfiles
//     --output_dir /home/brian/src/AceTimeGo/zonedb
//     --tz_version 2022g
//     --action zonedb
//     --language go
//     --scope extended
//     --offset_granularity 1
//     --delta_granularity 900
//     --until_at_granularity 60
//     --db_namespace zonedb
//     --start_year 1970
//     --until_year 10000
//
// using the TZ Database files
//
//   africa
//   antarctica
//   asia
//   australasia
//   backward
//   etcetera
//   europe
//   northamerica
//   southamerica
//
// from https://github.com/eggert/tz/releases/tag/2022g
//
// Supported Zones: 596 (351 zones, 245 links)
// Unsupported Zones: 0 (0 zones, 0 links)
// Earliest Year (Original): 1844
// Earliest Year (Generated): 1910
//
// DO NOT EDIT

package zonedb

import (
	"github.com/bxparks/AceTimeGo/zoneinfo"
)

// ---------------------------------------------------------------------------
// String constants.
// ---------------------------------------------------------------------------

const (
	// All ZoneEra.Format entries concatenated together.
	FormatData = "" +
		"%" +
		"+00/+01" +
		"+01" +
		"+01/+00" +
		"+02" +
		"+02/+03" +
		"+03" +
		"+03/+04" +
		"+0330/+0430" +
		"+04" +
		"+04/+05" +
		"+0430" +
		"+05" +
		"+05/+06" +
		"+0530" +
		"+0545" +
		"+06" +
		"+06/+07" +
		"+0630" +
		"+07" +
		"+07/+08" +
		"+0730" +
		"+08" +
		"+08/+09" +
		"+0845/+0945" +
		"+09" +
		"+09/+10" +
		"+10" +
		"+10/+11" +
		"+1030/+11" +
		"+1030/+1130" +
		"+11" +
		"+11/+12" +
		"+1130" +
		"+12" +
		"+12/+13" +
		"+1230" +
		"+1245/+1345" +
		"+13" +
		"+13/+14" +
		"+14" +
		"-00" +
		"-01" +
		"-01/+00" +
		"-02" +
		"-02/-01" +
		"-03" +
		"-03/-0130" +
		"-03/-02" +
		"-03/-0230" +
		"-0330" +
		"-0345" +
		"-04" +
		"-04/-03" +
		"-0430" +
		"-05" +
		"-05/-04" +
		"-06" +
		"-06/-05" +
		"-07" +
		"-07/-06" +
		"-08" +
		"-0830" +
		"-09" +
		"-0930" +
		"-10" +
		"-10/-0930" +
		"-1030" +
		"-1040" +
		"-11" +
		"-11/-10" +
		"-12" +
		"A%T" +
		"AC%T" +
		"ADT" +
		"AE%T" +
		"AEDT" +
		"AEST" +
		"AH%T" +
		"AHST" +
		"AK%T" +
		"AST" +
		"AW%T" +
		"B%T" +
		"BST" +
		"C%T" +
		"CA%T" +
		"CAT" +
		"CDT" +
		"CE%T" +
		"CET" +
		"CST" +
		"ChST" +
		"E%T" +
		"EAT" +
		"EE%T" +
		"EEST" +
		"EET" +
		"EST" +
		"G%T" +
		"GMT" +
		"GMT/BST" +
		"H%T" +
		"HK%T" +
		"HST" +
		"I%T" +
		"IST" +
		"IST/GMT" +
		"J%T" +
		"K%T" +
		"KST" +
		"M%T" +
		"ME%T" +
		"MMT" +
		"MSD" +
		"MSK" +
		"MSK/MSD" +
		"MST" +
		"N%T" +
		"NZ%T" +
		"P%T" +
		"PK%T" +
		"PST" +
		"SAST" +
		"SST" +
		"UTC" +
		"WAST" +
		"WAT" +
		"WE%T" +
		"WEST" +
		"WET" +
		"WIB" +
		"WIT" +
		"WITA" +
		"Y%T" +
		"YST" +
		"~"

	// All ZoneInfo.Name entries concatenated togther.
	NameData = "Africa/Abidjan" +
		"Africa/Accra" +
		"Africa/Addis_Ababa" +
		"Africa/Algiers" +
		"Africa/Asmara" +
		"Africa/Asmera" +
		"Africa/Bamako" +
		"Africa/Bangui" +
		"Africa/Banjul" +
		"Africa/Bissau" +
		"Africa/Blantyre" +
		"Africa/Brazzaville" +
		"Africa/Bujumbura" +
		"Africa/Cairo" +
		"Africa/Casablanca" +
		"Africa/Ceuta" +
		"Africa/Conakry" +
		"Africa/Dakar" +
		"Africa/Dar_es_Salaam" +
		"Africa/Djibouti" +
		"Africa/Douala" +
		"Africa/El_Aaiun" +
		"Africa/Freetown" +
		"Africa/Gaborone" +
		"Africa/Harare" +
		"Africa/Johannesburg" +
		"Africa/Juba" +
		"Africa/Kampala" +
		"Africa/Khartoum" +
		"Africa/Kigali" +
		"Africa/Kinshasa" +
		"Africa/Lagos" +
		"Africa/Libreville" +
		"Africa/Lome" +
		"Africa/Luanda" +
		"Africa/Lubumbashi" +
		"Africa/Lusaka" +
		"Africa/Malabo" +
		"Africa/Maputo" +
		"Africa/Maseru" +
		"Africa/Mbabane" +
		"Africa/Mogadishu" +
		"Africa/Monrovia" +
		"Africa/Nairobi" +
		"Africa/Ndjamena" +
		"Africa/Niamey" +
		"Africa/Nouakchott" +
		"Africa/Ouagadougou" +
		"Africa/Porto-Novo" +
		"Africa/Sao_Tome" +
		"Africa/Timbuktu" +
		"Africa/Tripoli" +
		"Africa/Tunis" +
		"Africa/Windhoek" +
		"America/Adak" +
		"America/Anchorage" +
		"America/Anguilla" +
		"America/Antigua" +
		"America/Araguaina" +
		"America/Argentina/Buenos_Aires" +
		"America/Argentina/Catamarca" +
		"America/Argentina/ComodRivadavia" +
		"America/Argentina/Cordoba" +
		"America/Argentina/Jujuy" +
		"America/Argentina/La_Rioja" +
		"America/Argentina/Mendoza" +
		"America/Argentina/Rio_Gallegos" +
		"America/Argentina/Salta" +
		"America/Argentina/San_Juan" +
		"America/Argentina/San_Luis" +
		"America/Argentina/Tucuman" +
		"America/Argentina/Ushuaia" +
		"America/Aruba" +
		"America/Asuncion" +
		"America/Atikokan" +
		"America/Atka" +
		"America/Bahia" +
		"America/Bahia_Banderas" +
		"America/Barbados" +
		"America/Belem" +
		"America/Belize" +
		"America/Blanc-Sablon" +
		"America/Boa_Vista" +
		"America/Bogota" +
		"America/Boise" +
		"America/Buenos_Aires" +
		"America/Cambridge_Bay" +
		"America/Campo_Grande" +
		"America/Cancun" +
		"America/Caracas" +
		"America/Catamarca" +
		"America/Cayenne" +
		"America/Cayman" +
		"America/Chicago" +
		"America/Chihuahua" +
		"America/Ciudad_Juarez" +
		"America/Coral_Harbour" +
		"America/Cordoba" +
		"America/Costa_Rica" +
		"America/Creston" +
		"America/Cuiaba" +
		"America/Curacao" +
		"America/Danmarkshavn" +
		"America/Dawson" +
		"America/Dawson_Creek" +
		"America/Denver" +
		"America/Detroit" +
		"America/Dominica" +
		"America/Edmonton" +
		"America/Eirunepe" +
		"America/El_Salvador" +
		"America/Ensenada" +
		"America/Fort_Nelson" +
		"America/Fort_Wayne" +
		"America/Fortaleza" +
		"America/Glace_Bay" +
		"America/Godthab" +
		"America/Goose_Bay" +
		"America/Grand_Turk" +
		"America/Grenada" +
		"America/Guadeloupe" +
		"America/Guatemala" +
		"America/Guayaquil" +
		"America/Guyana" +
		"America/Halifax" +
		"America/Havana" +
		"America/Hermosillo" +
		"America/Indiana/Indianapolis" +
		"America/Indiana/Knox" +
		"America/Indiana/Marengo" +
		"America/Indiana/Petersburg" +
		"America/Indiana/Tell_City" +
		"America/Indiana/Vevay" +
		"America/Indiana/Vincennes" +
		"America/Indiana/Winamac" +
		"America/Indianapolis" +
		"America/Inuvik" +
		"America/Iqaluit" +
		"America/Jamaica" +
		"America/Jujuy" +
		"America/Juneau" +
		"America/Kentucky/Louisville" +
		"America/Kentucky/Monticello" +
		"America/Knox_IN" +
		"America/Kralendijk" +
		"America/La_Paz" +
		"America/Lima" +
		"America/Los_Angeles" +
		"America/Louisville" +
		"America/Lower_Princes" +
		"America/Maceio" +
		"America/Managua" +
		"America/Manaus" +
		"America/Marigot" +
		"America/Martinique" +
		"America/Matamoros" +
		"America/Mazatlan" +
		"America/Mendoza" +
		"America/Menominee" +
		"America/Merida" +
		"America/Metlakatla" +
		"America/Mexico_City" +
		"America/Miquelon" +
		"America/Moncton" +
		"America/Monterrey" +
		"America/Montevideo" +
		"America/Montreal" +
		"America/Montserrat" +
		"America/Nassau" +
		"America/New_York" +
		"America/Nipigon" +
		"America/Nome" +
		"America/Noronha" +
		"America/North_Dakota/Beulah" +
		"America/North_Dakota/Center" +
		"America/North_Dakota/New_Salem" +
		"America/Nuuk" +
		"America/Ojinaga" +
		"America/Panama" +
		"America/Pangnirtung" +
		"America/Paramaribo" +
		"America/Phoenix" +
		"America/Port-au-Prince" +
		"America/Port_of_Spain" +
		"America/Porto_Acre" +
		"America/Porto_Velho" +
		"America/Puerto_Rico" +
		"America/Punta_Arenas" +
		"America/Rainy_River" +
		"America/Rankin_Inlet" +
		"America/Recife" +
		"America/Regina" +
		"America/Resolute" +
		"America/Rio_Branco" +
		"America/Rosario" +
		"America/Santa_Isabel" +
		"America/Santarem" +
		"America/Santiago" +
		"America/Santo_Domingo" +
		"America/Sao_Paulo" +
		"America/Scoresbysund" +
		"America/Shiprock" +
		"America/Sitka" +
		"America/St_Barthelemy" +
		"America/St_Johns" +
		"America/St_Kitts" +
		"America/St_Lucia" +
		"America/St_Thomas" +
		"America/St_Vincent" +
		"America/Swift_Current" +
		"America/Tegucigalpa" +
		"America/Thule" +
		"America/Thunder_Bay" +
		"America/Tijuana" +
		"America/Toronto" +
		"America/Tortola" +
		"America/Vancouver" +
		"America/Virgin" +
		"America/Whitehorse" +
		"America/Winnipeg" +
		"America/Yakutat" +
		"America/Yellowknife" +
		"Antarctica/Casey" +
		"Antarctica/Davis" +
		"Antarctica/DumontDUrville" +
		"Antarctica/Macquarie" +
		"Antarctica/Mawson" +
		"Antarctica/McMurdo" +
		"Antarctica/Palmer" +
		"Antarctica/Rothera" +
		"Antarctica/South_Pole" +
		"Antarctica/Syowa" +
		"Antarctica/Troll" +
		"Antarctica/Vostok" +
		"Arctic/Longyearbyen" +
		"Asia/Aden" +
		"Asia/Almaty" +
		"Asia/Amman" +
		"Asia/Anadyr" +
		"Asia/Aqtau" +
		"Asia/Aqtobe" +
		"Asia/Ashgabat" +
		"Asia/Ashkhabad" +
		"Asia/Atyrau" +
		"Asia/Baghdad" +
		"Asia/Bahrain" +
		"Asia/Baku" +
		"Asia/Bangkok" +
		"Asia/Barnaul" +
		"Asia/Beirut" +
		"Asia/Bishkek" +
		"Asia/Brunei" +
		"Asia/Calcutta" +
		"Asia/Chita" +
		"Asia/Choibalsan" +
		"Asia/Chongqing" +
		"Asia/Chungking" +
		"Asia/Colombo" +
		"Asia/Dacca" +
		"Asia/Damascus" +
		"Asia/Dhaka" +
		"Asia/Dili" +
		"Asia/Dubai" +
		"Asia/Dushanbe" +
		"Asia/Famagusta" +
		"Asia/Gaza" +
		"Asia/Harbin" +
		"Asia/Hebron" +
		"Asia/Ho_Chi_Minh" +
		"Asia/Hong_Kong" +
		"Asia/Hovd" +
		"Asia/Irkutsk" +
		"Asia/Istanbul" +
		"Asia/Jakarta" +
		"Asia/Jayapura" +
		"Asia/Jerusalem" +
		"Asia/Kabul" +
		"Asia/Kamchatka" +
		"Asia/Karachi" +
		"Asia/Kashgar" +
		"Asia/Kathmandu" +
		"Asia/Katmandu" +
		"Asia/Khandyga" +
		"Asia/Kolkata" +
		"Asia/Krasnoyarsk" +
		"Asia/Kuala_Lumpur" +
		"Asia/Kuching" +
		"Asia/Kuwait" +
		"Asia/Macao" +
		"Asia/Macau" +
		"Asia/Magadan" +
		"Asia/Makassar" +
		"Asia/Manila" +
		"Asia/Muscat" +
		"Asia/Nicosia" +
		"Asia/Novokuznetsk" +
		"Asia/Novosibirsk" +
		"Asia/Omsk" +
		"Asia/Oral" +
		"Asia/Phnom_Penh" +
		"Asia/Pontianak" +
		"Asia/Pyongyang" +
		"Asia/Qatar" +
		"Asia/Qostanay" +
		"Asia/Qyzylorda" +
		"Asia/Rangoon" +
		"Asia/Riyadh" +
		"Asia/Saigon" +
		"Asia/Sakhalin" +
		"Asia/Samarkand" +
		"Asia/Seoul" +
		"Asia/Shanghai" +
		"Asia/Singapore" +
		"Asia/Srednekolymsk" +
		"Asia/Taipei" +
		"Asia/Tashkent" +
		"Asia/Tbilisi" +
		"Asia/Tehran" +
		"Asia/Tel_Aviv" +
		"Asia/Thimbu" +
		"Asia/Thimphu" +
		"Asia/Tokyo" +
		"Asia/Tomsk" +
		"Asia/Ujung_Pandang" +
		"Asia/Ulaanbaatar" +
		"Asia/Ulan_Bator" +
		"Asia/Urumqi" +
		"Asia/Ust-Nera" +
		"Asia/Vientiane" +
		"Asia/Vladivostok" +
		"Asia/Yakutsk" +
		"Asia/Yangon" +
		"Asia/Yekaterinburg" +
		"Asia/Yerevan" +
		"Atlantic/Azores" +
		"Atlantic/Bermuda" +
		"Atlantic/Canary" +
		"Atlantic/Cape_Verde" +
		"Atlantic/Faeroe" +
		"Atlantic/Faroe" +
		"Atlantic/Jan_Mayen" +
		"Atlantic/Madeira" +
		"Atlantic/Reykjavik" +
		"Atlantic/South_Georgia" +
		"Atlantic/St_Helena" +
		"Atlantic/Stanley" +
		"Australia/ACT" +
		"Australia/Adelaide" +
		"Australia/Brisbane" +
		"Australia/Broken_Hill" +
		"Australia/Canberra" +
		"Australia/Currie" +
		"Australia/Darwin" +
		"Australia/Eucla" +
		"Australia/Hobart" +
		"Australia/LHI" +
		"Australia/Lindeman" +
		"Australia/Lord_Howe" +
		"Australia/Melbourne" +
		"Australia/NSW" +
		"Australia/North" +
		"Australia/Perth" +
		"Australia/Queensland" +
		"Australia/South" +
		"Australia/Sydney" +
		"Australia/Tasmania" +
		"Australia/Victoria" +
		"Australia/West" +
		"Australia/Yancowinna" +
		"Brazil/Acre" +
		"Brazil/DeNoronha" +
		"Brazil/East" +
		"Brazil/West" +
		"CET" +
		"CST6CDT" +
		"Canada/Atlantic" +
		"Canada/Central" +
		"Canada/Eastern" +
		"Canada/Mountain" +
		"Canada/Newfoundland" +
		"Canada/Pacific" +
		"Canada/Saskatchewan" +
		"Canada/Yukon" +
		"Chile/Continental" +
		"Chile/EasterIsland" +
		"Cuba" +
		"EET" +
		"EST" +
		"EST5EDT" +
		"Egypt" +
		"Eire" +
		"Etc/GMT" +
		"Etc/GMT+0" +
		"Etc/GMT+1" +
		"Etc/GMT+10" +
		"Etc/GMT+11" +
		"Etc/GMT+12" +
		"Etc/GMT+2" +
		"Etc/GMT+3" +
		"Etc/GMT+4" +
		"Etc/GMT+5" +
		"Etc/GMT+6" +
		"Etc/GMT+7" +
		"Etc/GMT+8" +
		"Etc/GMT+9" +
		"Etc/GMT-0" +
		"Etc/GMT-1" +
		"Etc/GMT-10" +
		"Etc/GMT-11" +
		"Etc/GMT-12" +
		"Etc/GMT-13" +
		"Etc/GMT-14" +
		"Etc/GMT-2" +
		"Etc/GMT-3" +
		"Etc/GMT-4" +
		"Etc/GMT-5" +
		"Etc/GMT-6" +
		"Etc/GMT-7" +
		"Etc/GMT-8" +
		"Etc/GMT-9" +
		"Etc/GMT0" +
		"Etc/Greenwich" +
		"Etc/UCT" +
		"Etc/UTC" +
		"Etc/Universal" +
		"Etc/Zulu" +
		"Europe/Amsterdam" +
		"Europe/Andorra" +
		"Europe/Astrakhan" +
		"Europe/Athens" +
		"Europe/Belfast" +
		"Europe/Belgrade" +
		"Europe/Berlin" +
		"Europe/Bratislava" +
		"Europe/Brussels" +
		"Europe/Bucharest" +
		"Europe/Budapest" +
		"Europe/Busingen" +
		"Europe/Chisinau" +
		"Europe/Copenhagen" +
		"Europe/Dublin" +
		"Europe/Gibraltar" +
		"Europe/Guernsey" +
		"Europe/Helsinki" +
		"Europe/Isle_of_Man" +
		"Europe/Istanbul" +
		"Europe/Jersey" +
		"Europe/Kaliningrad" +
		"Europe/Kiev" +
		"Europe/Kirov" +
		"Europe/Kyiv" +
		"Europe/Lisbon" +
		"Europe/Ljubljana" +
		"Europe/London" +
		"Europe/Luxembourg" +
		"Europe/Madrid" +
		"Europe/Malta" +
		"Europe/Mariehamn" +
		"Europe/Minsk" +
		"Europe/Monaco" +
		"Europe/Moscow" +
		"Europe/Nicosia" +
		"Europe/Oslo" +
		"Europe/Paris" +
		"Europe/Podgorica" +
		"Europe/Prague" +
		"Europe/Riga" +
		"Europe/Rome" +
		"Europe/Samara" +
		"Europe/San_Marino" +
		"Europe/Sarajevo" +
		"Europe/Saratov" +
		"Europe/Simferopol" +
		"Europe/Skopje" +
		"Europe/Sofia" +
		"Europe/Stockholm" +
		"Europe/Tallinn" +
		"Europe/Tirane" +
		"Europe/Tiraspol" +
		"Europe/Ulyanovsk" +
		"Europe/Uzhgorod" +
		"Europe/Vaduz" +
		"Europe/Vatican" +
		"Europe/Vienna" +
		"Europe/Vilnius" +
		"Europe/Volgograd" +
		"Europe/Warsaw" +
		"Europe/Zagreb" +
		"Europe/Zaporozhye" +
		"Europe/Zurich" +
		"GB" +
		"GB-Eire" +
		"GMT" +
		"GMT+0" +
		"GMT-0" +
		"GMT0" +
		"Greenwich" +
		"HST" +
		"Hongkong" +
		"Iceland" +
		"Indian/Antananarivo" +
		"Indian/Chagos" +
		"Indian/Christmas" +
		"Indian/Cocos" +
		"Indian/Comoro" +
		"Indian/Kerguelen" +
		"Indian/Mahe" +
		"Indian/Maldives" +
		"Indian/Mauritius" +
		"Indian/Mayotte" +
		"Indian/Reunion" +
		"Iran" +
		"Israel" +
		"Jamaica" +
		"Japan" +
		"Kwajalein" +
		"Libya" +
		"MET" +
		"MST" +
		"MST7MDT" +
		"Mexico/BajaNorte" +
		"Mexico/BajaSur" +
		"Mexico/General" +
		"NZ" +
		"NZ-CHAT" +
		"Navajo" +
		"PRC" +
		"PST8PDT" +
		"Pacific/Apia" +
		"Pacific/Auckland" +
		"Pacific/Bougainville" +
		"Pacific/Chatham" +
		"Pacific/Chuuk" +
		"Pacific/Easter" +
		"Pacific/Efate" +
		"Pacific/Enderbury" +
		"Pacific/Fakaofo" +
		"Pacific/Fiji" +
		"Pacific/Funafuti" +
		"Pacific/Galapagos" +
		"Pacific/Gambier" +
		"Pacific/Guadalcanal" +
		"Pacific/Guam" +
		"Pacific/Honolulu" +
		"Pacific/Johnston" +
		"Pacific/Kanton" +
		"Pacific/Kiritimati" +
		"Pacific/Kosrae" +
		"Pacific/Kwajalein" +
		"Pacific/Majuro" +
		"Pacific/Marquesas" +
		"Pacific/Midway" +
		"Pacific/Nauru" +
		"Pacific/Niue" +
		"Pacific/Norfolk" +
		"Pacific/Noumea" +
		"Pacific/Pago_Pago" +
		"Pacific/Palau" +
		"Pacific/Pitcairn" +
		"Pacific/Pohnpei" +
		"Pacific/Ponape" +
		"Pacific/Port_Moresby" +
		"Pacific/Rarotonga" +
		"Pacific/Saipan" +
		"Pacific/Samoa" +
		"Pacific/Tahiti" +
		"Pacific/Tarawa" +
		"Pacific/Tongatapu" +
		"Pacific/Truk" +
		"Pacific/Wake" +
		"Pacific/Wallis" +
		"Pacific/Yap" +
		"Poland" +
		"Portugal" +
		"ROC" +
		"ROK" +
		"Singapore" +
		"Turkey" +
		"UCT" +
		"US/Alaska" +
		"US/Aleutian" +
		"US/Arizona" +
		"US/Central" +
		"US/East-Indiana" +
		"US/Eastern" +
		"US/Hawaii" +
		"US/Indiana-Starke" +
		"US/Michigan" +
		"US/Mountain" +
		"US/Pacific" +
		"US/Samoa" +
		"UTC" +
		"Universal" +
		"W-SU" +
		"WET" +
		"Zulu" +
		"~"
)

var (
	// Byte offset into FormatData for each index. The actual Format string
	// at index `i` given by the `ZoneEra.Format` field is
	// `FormatData[FormatOffsets[i]:FormatOffsets[i+1]]`.
	FormatOffsets = []uint16{
		0, 0, 1, 8, 11, 18, 21, 28, 31, 38,
		49, 52, 59, 64, 67, 74, 79, 84, 87, 94,
		99, 102, 109, 114, 117, 124, 135, 138, 145, 148,
		155, 164, 175, 178, 185, 190, 193, 200, 205, 216,
		219, 226, 229, 232, 235, 242, 245, 252, 255, 264,
		271, 280, 285, 290, 293, 300, 305, 308, 315, 318,
		325, 328, 335, 338, 343, 346, 351, 354, 363, 368,
		373, 376, 383, 386, 389, 393, 396, 400, 404, 408,
		412, 416, 420, 423, 427, 430, 433, 436, 440, 443,
		446, 450, 453, 456, 460, 463, 466, 470, 474, 477,
		480, 483, 486, 493, 496, 500, 503, 506, 509, 516,
		519, 522, 525, 528, 532, 535, 538, 541, 548, 551,
		554, 558, 561, 565, 568, 572, 575, 578, 582, 585,
		589, 593, 596, 599, 602, 606, 609, 612,
}

	// Byte offset into NameData for each index. The actual Letter string
	// at index `i` given by the `ZoneRule.Name` field is
	// `NameData[NameOffsets[i]:NameOffsets[i+1]]`.
	NameOffsets = []uint16{
		0, 14, 26, 44, 58, 71, 84, 97, 110, 123,
		136, 151, 169, 185, 197, 214, 226, 240, 252, 272,
		287, 300, 315, 330, 345, 358, 377, 388, 402, 417,
		430, 445, 457, 474, 485, 498, 515, 528, 541, 554,
		567, 581, 597, 612, 626, 641, 654, 671, 689, 706,
		721, 736, 750, 762, 777, 789, 806, 822, 837, 854,
		884, 911, 943, 968, 991, 1017, 1042, 1072, 1095, 1121,
		1147, 1172, 1197, 1210, 1226, 1242, 1254, 1267, 1289, 1305,
		1318, 1332, 1352, 1369, 1383, 1396, 1416, 1437, 1457, 1471,
		1486, 1503, 1518, 1532, 1547, 1564, 1585, 1606, 1621, 1639,
		1654, 1668, 1683, 1703, 1717, 1737, 1751, 1766, 1782, 1798,
		1814, 1833, 1849, 1868, 1886, 1903, 1920, 1935, 1952, 1970,
		1985, 2003, 2020, 2037, 2051, 2066, 2080, 2098, 2126, 2146,
		2169, 2195, 2220, 2241, 2266, 2289, 2309, 2323, 2338, 2353,
		2366, 2380, 2407, 2434, 2449, 2467, 2481, 2493, 2512, 2530,
		2551, 2565, 2580, 2594, 2609, 2627, 2644, 2660, 2675, 2692,
		2706, 2724, 2743, 2759, 2774, 2791, 2809, 2825, 2843, 2857,
		2873, 2888, 2900, 2915, 2942, 2969, 2999, 3011, 3026, 3040,
		3059, 3077, 3092, 3114, 3135, 3153, 3172, 3191, 3211, 3230,
		3250, 3264, 3278, 3294, 3312, 3327, 3347, 3363, 3379, 3400,
		3417, 3437, 3453, 3466, 3487, 3503, 3519, 3535, 3552, 3570,
		3591, 3610, 3623, 3642, 3657, 3672, 3687, 3704, 3718, 3736,
		3752, 3767, 3786, 3802, 3818, 3843, 3863, 3880, 3898, 3915,
		3933, 3954, 3970, 3986, 4003, 4022, 4031, 4042, 4052, 4063,
		4073, 4084, 4097, 4111, 4122, 4134, 4146, 4155, 4167, 4179,
		4190, 4202, 4213, 4226, 4236, 4251, 4265, 4279, 4291, 4301,
		4314, 4324, 4333, 4343, 4356, 4370, 4379, 4390, 4401, 4417,
		4431, 4440, 4452, 4465, 4477, 4490, 4504, 4514, 4528, 4540,
		4552, 4566, 4579, 4592, 4604, 4620, 4637, 4649, 4660, 4670,
		4680, 4692, 4705, 4716, 4727, 4739, 4756, 4772, 4781, 4790,
		4805, 4819, 4833, 4843, 4856, 4870, 4882, 4893, 4904, 4917,
		4931, 4941, 4954, 4968, 4986, 4997, 5010, 5022, 5033, 5046,
		5057, 5069, 5079, 5089, 5107, 5123, 5138, 5149, 5162, 5176,
		5192, 5204, 5215, 5233, 5245, 5260, 5276, 5291, 5310, 5325,
		5339, 5357, 5373, 5391, 5413, 5431, 5447, 5460, 5478, 5496,
		5517, 5535, 5551, 5567, 5582, 5598, 5611, 5629, 5648, 5667,
		5680, 5695, 5710, 5730, 5745, 5761, 5779, 5797, 5811, 5831,
		5842, 5858, 5869, 5880, 5883, 5890, 5905, 5919, 5933, 5948,
		5967, 5981, 6000, 6012, 6029, 6047, 6051, 6054, 6057, 6064,
		6069, 6073, 6080, 6089, 6098, 6108, 6118, 6128, 6137, 6146,
		6155, 6164, 6173, 6182, 6191, 6200, 6209, 6218, 6228, 6238,
		6248, 6258, 6268, 6277, 6286, 6295, 6304, 6313, 6322, 6331,
		6340, 6348, 6361, 6368, 6375, 6388, 6396, 6412, 6426, 6442,
		6455, 6469, 6484, 6497, 6514, 6529, 6545, 6560, 6575, 6590,
		6607, 6620, 6636, 6651, 6666, 6684, 6699, 6712, 6730, 6741,
		6753, 6764, 6777, 6793, 6806, 6823, 6836, 6848, 6864, 6876,
		6889, 6902, 6916, 6927, 6939, 6955, 6968, 6979, 6990, 7003,
		7020, 7035, 7049, 7066, 7079, 7091, 7107, 7121, 7134, 7149,
		7165, 7180, 7192, 7206, 7219, 7233, 7249, 7262, 7275, 7292,
		7305, 7307, 7314, 7317, 7322, 7327, 7331, 7340, 7343, 7351,
		7358, 7377, 7390, 7406, 7418, 7431, 7447, 7458, 7473, 7489,
		7503, 7517, 7521, 7527, 7534, 7539, 7548, 7553, 7556, 7559,
		7566, 7582, 7596, 7610, 7612, 7619, 7625, 7628, 7635, 7647,
		7663, 7683, 7698, 7711, 7725, 7738, 7755, 7770, 7782, 7798,
		7815, 7830, 7849, 7861, 7877, 7893, 7907, 7925, 7939, 7956,
		7970, 7987, 8001, 8014, 8026, 8041, 8055, 8072, 8085, 8101,
		8116, 8130, 8150, 8167, 8181, 8194, 8208, 8222, 8239, 8251,
		8263, 8277, 8288, 8294, 8302, 8305, 8308, 8317, 8323, 8326,
		8335, 8346, 8356, 8366, 8381, 8391, 8400, 8417, 8428, 8439,
		8449, 8457, 8460, 8469, 8473, 8476, 8480,
	}
)

// ---------------------------------------------------------------------------
// ZoneEraRecords is an array of zoneinfo.ZoneEraRecord items concatenated
// together.
//
// Supported zones: 351
// numEras: 1081
// ---------------------------------------------------------------------------

var ZoneEraRecords = []zoneinfo.ZoneEraRecord{
	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Abidjan
	// EraIndex: 0
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              0:00    -    GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 101, // "GMT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Algiers
	// EraIndex: 1
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             0:00    Algeria    WE%sT    1977 Oct 21
	{
		PolicyIndex: 8, // PolicyName: Algeria
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1977,
		UntilMonth: 10,
		UntilDay: 21,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    Algeria    CE%sT    1979 Oct 26
	{
		PolicyIndex: 8, // PolicyName: Algeria
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1979,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             0:00    Algeria    WE%sT    1981 May
	{
		PolicyIndex: 8, // PolicyName: Algeria
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1981,
		UntilMonth: 5,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    -    CET
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 91, // "CET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Bissau
	// EraIndex: 5
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -1:00    -    -01    1975
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 43, // "-01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -240, // (offset_seconds=-3600)/15
		UntilYear: 1975,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              0:00    -    GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 101, // "GMT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Cairo
	// EraIndex: 7
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    Egypt    EE%sT
	{
		PolicyIndex: 38, // PolicyName: Egypt
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Casablanca
	// EraIndex: 8
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//              0:00    Morocco    +00/+01    1984 Mar 16
	{
		PolicyIndex: 73, // PolicyName: Morocco
		FormatIndex: 2, // "+00/+01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1984,
		UntilMonth: 3,
		UntilDay: 16,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              1:00    -    +01    1986
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 3, // "+01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1986,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              0:00    Morocco    +00/+01    2018 Oct 28  3:00
	{
		PolicyIndex: 73, // PolicyName: Morocco
		FormatIndex: 2, // "+00/+01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 2018,
		UntilMonth: 10,
		UntilDay: 28,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              1:00    Morocco    +01/+00
	{
		PolicyIndex: 73, // PolicyName: Morocco
		FormatIndex: 4, // "+01/+00"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Ceuta
	// EraIndex: 12
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//              0:00 SpainAfrica WE%sT    1984 Mar 16
	{
		PolicyIndex: 95, // PolicyName: SpainAfrica
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1984,
		UntilMonth: 3,
		UntilDay: 16,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              1:00    -    CET    1986
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 91, // "CET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1986,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/El_Aaiun
	// EraIndex: 15
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -1:00    -    -01    1976 Apr 14
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 43, // "-01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -240, // (offset_seconds=-3600)/15
		UntilYear: 1976,
		UntilMonth: 4,
		UntilDay: 14,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              0:00    Morocco    +00/+01    2018 Oct 28  3:00
	{
		PolicyIndex: 73, // PolicyName: Morocco
		FormatIndex: 2, // "+00/+01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 2018,
		UntilMonth: 10,
		UntilDay: 28,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              1:00    Morocco    +01/+00
	{
		PolicyIndex: 73, // PolicyName: Morocco
		FormatIndex: 4, // "+01/+00"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Johannesburg
	// EraIndex: 18
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    SA    SAST
	{
		PolicyIndex: 91, // PolicyName: SA
		FormatIndex: 124, // "SAST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Juba
	// EraIndex: 19
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             2:00    Sudan    CA%sT    2000 Jan 15 12:00
	{
		PolicyIndex: 97, // PolicyName: Sudan
		FormatIndex: 87, // "CA%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2000,
		UntilMonth: 1,
		UntilDay: 15,
		UntilTimeCode: 48,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00    -    EAT    2021 Feb  1 00:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 95, // "EAT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2021,
		UntilMonth: 2,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    CAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 88, // "CAT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Khartoum
	// EraIndex: 22
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             2:00    Sudan    CA%sT    2000 Jan 15 12:00
	{
		PolicyIndex: 97, // PolicyName: Sudan
		FormatIndex: 87, // "CA%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2000,
		UntilMonth: 1,
		UntilDay: 15,
		UntilTimeCode: 48,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00    -    EAT    2017 Nov  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 95, // "EAT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2017,
		UntilMonth: 11,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    CAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 88, // "CAT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Lagos
	// EraIndex: 25
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    -    WAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 128, // "WAT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Maputo
	// EraIndex: 26
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    -    CAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 88, // "CAT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Monrovia
	// EraIndex: 27
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -0:44:30 -    MMT    1972 Jan 7
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 114, // "MMT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -178, // (offset_seconds=-2670)/15
		UntilYear: 1972,
		UntilMonth: 1,
		UntilDay: 7,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              0:00    -    GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 101, // "GMT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Nairobi
	// EraIndex: 29
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:00    -    EAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 95, // "EAT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Ndjamena
	// EraIndex: 30
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             1:00    -    WAT    1979 Oct 14
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 128, // "WAT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1979,
		UntilMonth: 10,
		UntilDay: 14,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    1:00    WAST    1980 Mar  8
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 127, // "WAST"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1980,
		UntilMonth: 3,
		UntilDay: 8,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    -    WAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 128, // "WAT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Sao_Tome
	// EraIndex: 33
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//              0:00    -    GMT    2018 Jan  1 01:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 101, // "GMT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 2018,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 4,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              1:00    -    WAT    2019 Jan  1 02:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 128, // "WAT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 2019,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              0:00    -    GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 101, // "GMT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Tripoli
	// EraIndex: 36
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             2:00    -    EET    1982
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1982,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    Libya    CE%sT    1990 May  4
	{
		PolicyIndex: 64, // PolicyName: Libya
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1990,
		UntilMonth: 5,
		UntilDay: 4,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    EET    1996 Sep 30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1996,
		UntilMonth: 9,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    Libya    CE%sT    1997 Oct  4
	{
		PolicyIndex: 64, // PolicyName: Libya
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1997,
		UntilMonth: 10,
		UntilDay: 4,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    EET    2012 Nov 10  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2012,
		UntilMonth: 11,
		UntilDay: 10,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    Libya    CE%sT    2013 Oct 25  2:00
	{
		PolicyIndex: 64, // PolicyName: Libya
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 2013,
		UntilMonth: 10,
		UntilDay: 25,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    EET
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Tunis
	// EraIndex: 43
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    Tunisia    CE%sT
	{
		PolicyIndex: 106, // PolicyName: Tunisia
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Windhoek
	// EraIndex: 44
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             2:00    -    SAST    1990 Mar 21
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 124, // "SAST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1990,
		UntilMonth: 3,
		UntilDay: 21,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    Namibia    %s
	{
		PolicyIndex: 77, // PolicyName: Namibia
		FormatIndex: 1, // "%"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Adak
	// EraIndex: 46
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -11:00    -    BST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 85, // "BST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2640, // (offset_seconds=-39600)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -11:00    US    B%sT    1983 Oct 30  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 84, // "B%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2640, // (offset_seconds=-39600)/15
		UntilYear: 1983,
		UntilMonth: 10,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -10:00    US    AH%sT    1983 Nov 30
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 79, // "AH%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2400, // (offset_seconds=-36000)/15
		UntilYear: 1983,
		UntilMonth: 11,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -10:00    US    H%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 103, // "H%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2400, // (offset_seconds=-36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Anchorage
	// EraIndex: 50
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -10:00    -    AHST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 80, // "AHST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2400, // (offset_seconds=-36000)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -10:00    US    AH%sT    1983 Oct 30  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 79, // "AH%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2400, // (offset_seconds=-36000)/15
		UntilYear: 1983,
		UntilMonth: 10,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    Y%sT    1983 Nov 30
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 135, // "Y%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 1983,
		UntilMonth: 11,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 81, // "AK%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Araguaina
	// EraIndex: 54
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             -3:00    Brazil    -03/-02    1990 Sep 17
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1990,
		UntilMonth: 9,
		UntilDay: 17,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    1995 Sep 14
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1995,
		UntilMonth: 9,
		UntilDay: 14,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Brazil    -03/-02    2003 Sep 24
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2003,
		UntilMonth: 9,
		UntilDay: 24,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2012 Oct 21
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2012,
		UntilMonth: 10,
		UntilDay: 21,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Brazil    -03/-02    2013 Sep
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2013,
		UntilMonth: 9,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Buenos_Aires
	// EraIndex: 60
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Catamarca
	// EraIndex: 64
	// EraCount: 9
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1991 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1991 Oct 20
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 10,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2004 Jun  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2004,
		UntilMonth: 6,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2004 Jun 20
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2004,
		UntilMonth: 6,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    2008 Oct 18
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2008,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Cordoba
	// EraIndex: 73
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1991 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1991 Oct 20
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 10,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Jujuy
	// EraIndex: 79
	// EraCount: 10
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1990 Mar  4
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1990,
		UntilMonth: 3,
		UntilDay: 4,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1990 Oct 28
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1990,
		UntilMonth: 10,
		UntilDay: 28,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    1:00    -03    1991 Mar 17
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 17,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1991 Oct  6
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 10,
		UntilDay: 6,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    1:00    -02    1992
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "-02"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    2008 Oct 18
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2008,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/La_Rioja
	// EraIndex: 89
	// EraCount: 9
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1991 Mar  1
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1991 May  7
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 5,
		UntilDay: 7,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2004 Jun  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2004,
		UntilMonth: 6,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2004 Jun 20
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2004,
		UntilMonth: 6,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    2008 Oct 18
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2008,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Mendoza
	// EraIndex: 98
	// EraCount: 13
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1990 Mar  4
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1990,
		UntilMonth: 3,
		UntilDay: 4,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1990 Oct 15
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1990,
		UntilMonth: 10,
		UntilDay: 15,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    1:00    -03    1991 Mar  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1991 Oct 15
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 10,
		UntilDay: 15,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    1:00    -03    1992 Mar  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1992,
		UntilMonth: 3,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1992 Oct 18
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1992,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2004 May 23
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2004,
		UntilMonth: 5,
		UntilDay: 23,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2004 Sep 26
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2004,
		UntilMonth: 9,
		UntilDay: 26,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    2008 Oct 18
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2008,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Rio_Gallegos
	// EraIndex: 111
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2004 Jun  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2004,
		UntilMonth: 6,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2004 Jun 20
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2004,
		UntilMonth: 6,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    2008 Oct 18
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2008,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Salta
	// EraIndex: 118
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1991 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1991 Oct 20
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 10,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    2008 Oct 18
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2008,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/San_Juan
	// EraIndex: 125
	// EraCount: 9
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1991 Mar  1
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1991 May  7
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 5,
		UntilDay: 7,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2004 May 31
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2004,
		UntilMonth: 5,
		UntilDay: 31,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2004 Jul 25
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2004,
		UntilMonth: 7,
		UntilDay: 25,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    2008 Oct 18
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2008,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/San_Luis
	// EraIndex: 134
	// EraCount: 13
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1990
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1990,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    1:00    -02    1990 Mar 14
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "-02"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1990,
		UntilMonth: 3,
		UntilDay: 14,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1990 Oct 15
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1990,
		UntilMonth: 10,
		UntilDay: 15,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    1:00    -03    1991 Mar  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1991 Jun  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 6,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    1999 Oct  3
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    1:00    -03    2000 Mar  3
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2004 May 31
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2004,
		UntilMonth: 5,
		UntilDay: 31,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2004 Jul 25
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2004,
		UntilMonth: 7,
		UntilDay: 25,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    2008 Jan 21
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2008,
		UntilMonth: 1,
		UntilDay: 21,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    SanLuis    -04/-03    2009 Oct 11
	{
		PolicyIndex: 93, // PolicyName: SanLuis
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2009,
		UntilMonth: 10,
		UntilDay: 11,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Tucuman
	// EraIndex: 147
	// EraCount: 8
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1991 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1991 Oct 20
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1991,
		UntilMonth: 10,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2004 Jun  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2004,
		UntilMonth: 6,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2004 Jun 13
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2004,
		UntilMonth: 6,
		UntilDay: 13,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Ushuaia
	// EraIndex: 155
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1999 Oct  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Arg    -04/-03    2000 Mar  3
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 3,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2004 May 30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2004,
		UntilMonth: 5,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2004 Jun 20
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2004,
		UntilMonth: 6,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    2008 Oct 18
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2008,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Asuncion
	// EraIndex: 162
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -4:00    -    -04    1972 Oct
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1972,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    1974 Apr
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1974,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Para    -04/-03
	{
		PolicyIndex: 82, // PolicyName: Para
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Bahia
	// EraIndex: 165
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -3:00    Brazil    -03/-02    2003 Sep 24
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2003,
		UntilMonth: 9,
		UntilDay: 24,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2011 Oct 16
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2011,
		UntilMonth: 10,
		UntilDay: 16,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Brazil    -03/-02    2012 Oct 21
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2012,
		UntilMonth: 10,
		UntilDay: 21,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Bahia_Banderas
	// EraIndex: 169
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -8:00    -    PST    1970
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1970,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    Mexico    M%sT    2010 Apr  4  2:00
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 2010,
		UntilMonth: 4,
		UntilDay: 4,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Mexico    C%sT
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Barbados
	// EraIndex: 172
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Barb    A%sT
	{
		PolicyIndex: 14, // PolicyName: Barb
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Belem
	// EraIndex: 173
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -3:00    Brazil    -03/-02    1988 Sep 12
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1988,
		UntilMonth: 9,
		UntilDay: 12,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Belize
	// EraIndex: 175
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Belize    %s
	{
		PolicyIndex: 16, // PolicyName: Belize
		FormatIndex: 1, // "%"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Boa_Vista
	// EraIndex: 176
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -4:00    Brazil    -04/-03    1988 Sep 12
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1988,
		UntilMonth: 9,
		UntilDay: 12,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1999 Sep 30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1999,
		UntilMonth: 9,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Brazil    -04/-03    2000 Oct 15
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 15,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Bogota
	// EraIndex: 180
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    CO    -05/-04
	{
		PolicyIndex: 21, // PolicyName: CO
		FormatIndex: 57, // "-05/-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Boise
	// EraIndex: 181
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -7:00    US    M%sT    1974
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 1974,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    -    MST    1974 Feb  3  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 118, // "MST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 1974,
		UntilMonth: 2,
		UntilDay: 3,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    US    M%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Cambridge_Bay
	// EraIndex: 184
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             -7:00    NT_YK    M%sT    1999 Oct 31  2:00
	{
		PolicyIndex: 75, // PolicyName: NT_YK
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Canada    C%sT    2000 Oct 29  2:00
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2000 Nov  5  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2000,
		UntilMonth: 11,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    -    CST    2001 Apr  1  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2001,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    Canada    M%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Campo_Grande
	// EraIndex: 189
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Brazil    -04/-03
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Cancun
	// EraIndex: 190
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST    1981 Dec 23
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1981,
		UntilMonth: 12,
		UntilDay: 23,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    Mexico    E%sT    1998 Aug  2  2:00
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1998,
		UntilMonth: 8,
		UntilDay: 2,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Mexico    C%sT    2015 Feb  1  2:00
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2015,
		UntilMonth: 2,
		UntilDay: 1,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Caracas
	// EraIndex: 194
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -4:00    -    -04    2007 Dec  9  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2007,
		UntilMonth: 12,
		UntilDay: 9,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:30    -    -0430    2016 May  1  2:30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 55, // "-0430"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1080, // (offset_seconds=-16200)/15
		UntilYear: 2016,
		UntilMonth: 5,
		UntilDay: 1,
		UntilTimeCode: 10,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Cayenne
	// EraIndex: 197
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Chicago
	// EraIndex: 198
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Chihuahua
	// EraIndex: 199
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST    1996
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1996,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Mexico    C%sT    1998
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1998,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    -    CST    1998 Apr Sun>=1  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1998,
		UntilMonth: 4,
		UntilDay: 5,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    Mexico    M%sT    2022 Oct 30  2:00
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 2022,
		UntilMonth: 10,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    -    CST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Ciudad_Juarez
	// EraIndex: 204
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST    1996
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1996,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Mexico    C%sT    1998
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1998,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    -    CST    1998 Apr Sun>=1  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1998,
		UntilMonth: 4,
		UntilDay: 5,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    Mexico    M%sT    2010
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 2010,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    US    M%sT    2022 Oct 30  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 2022,
		UntilMonth: 10,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    -    CST    2022 Nov 30  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2022,
		UntilMonth: 11,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    US    M%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Costa_Rica
	// EraIndex: 211
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    CR    C%sT
	{
		PolicyIndex: 22, // PolicyName: CR
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Cuiaba
	// EraIndex: 212
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -4:00    Brazil    -04/-03    2003 Sep 24
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2003,
		UntilMonth: 9,
		UntilDay: 24,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2004 Oct  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2004,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Brazil    -04/-03
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Danmarkshavn
	// EraIndex: 215
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -3:00    -    -03    1980 Apr  6  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1980,
		UntilMonth: 4,
		UntilDay: 6,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    EU    -03/-02    1996
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1996,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             0:00    -    GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 101, // "GMT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Dawson
	// EraIndex: 218
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -9:00    Yukon    Y%sT    1973 Oct 28  0:00
	{
		PolicyIndex: 115, // PolicyName: Yukon
		FormatIndex: 135, // "Y%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 1973,
		UntilMonth: 10,
		UntilDay: 28,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    -    PST    1980
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1980,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    Canada    P%sT    2020 Nov  1
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 2020,
		UntilMonth: 11,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 118, // "MST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Dawson_Creek
	// EraIndex: 222
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -8:00    Vanc    P%sT    1972 Aug 30  2:00
	{
		PolicyIndex: 110, // PolicyName: Vanc
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1972,
		UntilMonth: 8,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 118, // "MST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Denver
	// EraIndex: 224
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    US    M%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Detroit
	// EraIndex: 225
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT    1969
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    1973
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1973,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    1975
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1975,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    1975 Apr 27  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1975,
		UntilMonth: 4,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Edmonton
	// EraIndex: 230
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -7:00    Edm    M%sT    1987
	{
		PolicyIndex: 37, // PolicyName: Edm
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 1987,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    Canada    M%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Eirunepe
	// EraIndex: 232
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             -5:00    Brazil    -05/-04    1988 Sep 12
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 57, // "-05/-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1988,
		UntilMonth: 9,
		UntilDay: 12,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    -05    1993 Sep 28
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 56, // "-05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1993,
		UntilMonth: 9,
		UntilDay: 28,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    Brazil    -05/-04    1994 Sep 22
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 57, // "-05/-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1994,
		UntilMonth: 9,
		UntilDay: 22,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    -05    2008 Jun 24  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 56, // "-05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2008,
		UntilMonth: 6,
		UntilDay: 24,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2013 Nov 10
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2013,
		UntilMonth: 11,
		UntilDay: 10,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    -05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 56, // "-05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/El_Salvador
	// EraIndex: 238
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Salv    C%sT
	{
		PolicyIndex: 92, // PolicyName: Salv
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Fort_Nelson
	// EraIndex: 239
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -8:00    Vanc    P%sT    1987
	{
		PolicyIndex: 110, // PolicyName: Vanc
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1987,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    Canada    P%sT    2015 Mar  8  2:00
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 2015,
		UntilMonth: 3,
		UntilDay: 8,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 118, // "MST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Fortaleza
	// EraIndex: 242
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             -3:00    Brazil    -03/-02    1990 Sep 17
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1990,
		UntilMonth: 9,
		UntilDay: 17,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    1999 Sep 30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 9,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Brazil    -03/-02    2000 Oct 22
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 22,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2001 Sep 13
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2001,
		UntilMonth: 9,
		UntilDay: 13,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Brazil    -03/-02    2002 Oct  1
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2002,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Glace_Bay
	// EraIndex: 248
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -4:00    -    AST    1972
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 82, // "AST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1972,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Halifax    A%sT    1974
	{
		PolicyIndex: 51, // PolicyName: Halifax
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1974,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Canada    A%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Goose_Bay
	// EraIndex: 251
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -4:00    StJohns    A%sT    2011 Nov
	{
		PolicyIndex: 96, // PolicyName: StJohns
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2011,
		UntilMonth: 11,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Canada    A%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Grand_Turk
	// EraIndex: 253
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST    1979
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1979,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    2015 Mar  8  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2015,
		UntilMonth: 3,
		UntilDay: 8,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    AST    2018 Mar 11  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 82, // "AST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2018,
		UntilMonth: 3,
		UntilDay: 11,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Guatemala
	// EraIndex: 257
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Guat    C%sT
	{
		PolicyIndex: 48, // PolicyName: Guat
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Guayaquil
	// EraIndex: 258
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    Ecuador    -05/-04
	{
		PolicyIndex: 36, // PolicyName: Ecuador
		FormatIndex: 57, // "-05/-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Guyana
	// EraIndex: 259
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -3:45    -    -0345    1975 Aug  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 52, // "-0345"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -900, // (offset_seconds=-13500)/15
		UntilYear: 1975,
		UntilMonth: 8,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    1992 Mar 29  1:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1992,
		UntilMonth: 3,
		UntilDay: 29,
		UntilTimeCode: 4,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Halifax
	// EraIndex: 262
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -4:00    Halifax    A%sT    1974
	{
		PolicyIndex: 51, // PolicyName: Halifax
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1974,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Canada    A%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Havana
	// EraIndex: 264
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    Cuba    C%sT
	{
		PolicyIndex: 27, // PolicyName: Cuba
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Hermosillo
	// EraIndex: 265
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -8:00    -    PST    1970
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1970,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    Mexico    M%sT    1999
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 1999,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 118, // "MST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Indianapolis
	// EraIndex: 268
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    1971
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1971,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2006
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2006,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Knox
	// EraIndex: 272
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT    1991 Oct 27  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1991,
		UntilMonth: 10,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2006 Apr  2  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2006,
		UntilMonth: 4,
		UntilDay: 2,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Marengo
	// EraIndex: 275
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    1974 Jan  6  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1974,
		UntilMonth: 1,
		UntilDay: 6,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    1:00    CDT    1974 Oct 27  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 89, // "CDT"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1974,
		UntilMonth: 10,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    1976
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1976,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2006
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2006,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Petersburg
	// EraIndex: 281
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT    1977 Oct 30  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1977,
		UntilMonth: 10,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2006 Apr  2  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2006,
		UntilMonth: 4,
		UntilDay: 2,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT    2007 Nov  4  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2007,
		UntilMonth: 11,
		UntilDay: 4,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Tell_City
	// EraIndex: 285
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT    1969 Apr 27  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1969,
		UntilMonth: 4,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    1971
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1971,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2006 Apr  2  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2006,
		UntilMonth: 4,
		UntilDay: 2,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Vevay
	// EraIndex: 289
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    1973
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1973,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2006
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2006,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Vincennes
	// EraIndex: 293
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    1971
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1971,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2006 Apr  2  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2006,
		UntilMonth: 4,
		UntilDay: 2,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT    2007 Nov  4  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2007,
		UntilMonth: 11,
		UntilDay: 4,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Winamac
	// EraIndex: 298
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    1971
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1971,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2006 Apr  2  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2006,
		UntilMonth: 4,
		UntilDay: 2,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT    2007 Mar 11  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2007,
		UntilMonth: 3,
		UntilDay: 11,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Inuvik
	// EraIndex: 303
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -8:00    NT_YK    P%sT    1979 Apr lastSun  2:00
	{
		PolicyIndex: 75, // PolicyName: NT_YK
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1979,
		UntilMonth: 4,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    NT_YK    M%sT    1980
	{
		PolicyIndex: 75, // PolicyName: NT_YK
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 1980,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    Canada    M%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Iqaluit
	// EraIndex: 306
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -5:00    NT_YK    E%sT    1999 Oct 31  2:00
	{
		PolicyIndex: 75, // PolicyName: NT_YK
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Canada    C%sT    2000 Oct 29  2:00
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    Canada    E%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Jamaica
	// EraIndex: 309
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST    1974
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1974,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    1984
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1984,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Juneau
	// EraIndex: 312
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//              -8:00    -    PST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -8:00    US    P%sT    1980 Apr 27  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1980,
		UntilMonth: 4,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    Y%sT    1980 Oct 26  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 135, // "Y%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 1980,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -8:00    US    P%sT    1983 Oct 30  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1983,
		UntilMonth: 10,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    Y%sT    1983 Nov 30
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 135, // "Y%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 1983,
		UntilMonth: 11,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 81, // "AK%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Kentucky/Louisville
	// EraIndex: 318
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT    1974 Jan  6  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1974,
		UntilMonth: 1,
		UntilDay: 6,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    1:00    CDT    1974 Oct 27  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 89, // "CDT"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1974,
		UntilMonth: 10,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Kentucky/Monticello
	// EraIndex: 321
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT    2000 Oct 29  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/La_Paz
	// EraIndex: 323
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    -04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Lima
	// EraIndex: 324
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    Peru    -05/-04
	{
		PolicyIndex: 83, // PolicyName: Peru
		FormatIndex: 57, // "-05/-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Los_Angeles
	// EraIndex: 325
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -8:00    US    P%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Maceio
	// EraIndex: 326
	// EraCount: 8
	// ---------------------------------------------------------------------------

	//             -3:00    Brazil    -03/-02    1990 Sep 17
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1990,
		UntilMonth: 9,
		UntilDay: 17,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    1995 Oct 13
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1995,
		UntilMonth: 10,
		UntilDay: 13,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Brazil    -03/-02    1996 Sep  4
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1996,
		UntilMonth: 9,
		UntilDay: 4,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    1999 Sep 30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 9,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Brazil    -03/-02    2000 Oct 22
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 22,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2001 Sep 13
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2001,
		UntilMonth: 9,
		UntilDay: 13,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Brazil    -03/-02    2002 Oct  1
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2002,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Managua
	// EraIndex: 334
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST    1973 May
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1973,
		UntilMonth: 5,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    1975 Feb 16
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1975,
		UntilMonth: 2,
		UntilDay: 16,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Nic    C%sT    1992 Jan  1  4:00
	{
		PolicyIndex: 78, // PolicyName: Nic
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 16,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    1992 Sep 24
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1992,
		UntilMonth: 9,
		UntilDay: 24,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    -    CST    1993
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1993,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    1997
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1997,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Nic    C%sT
	{
		PolicyIndex: 78, // PolicyName: Nic
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Manaus
	// EraIndex: 341
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -4:00    Brazil    -04/-03    1988 Sep 12
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1988,
		UntilMonth: 9,
		UntilDay: 12,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    1993 Sep 28
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1993,
		UntilMonth: 9,
		UntilDay: 28,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Brazil    -04/-03    1994 Sep 22
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1994,
		UntilMonth: 9,
		UntilDay: 22,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Martinique
	// EraIndex: 345
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -4:00    -    AST    1980 Apr  6
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 82, // "AST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1980,
		UntilMonth: 4,
		UntilDay: 6,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    1:00    ADT    1980 Sep 28
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 75, // "ADT"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1980,
		UntilMonth: 9,
		UntilDay: 28,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    AST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 82, // "AST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Matamoros
	// EraIndex: 348
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST    1988
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1988,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT    1989
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1989,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Mexico    C%sT    2010
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2010,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Mazatlan
	// EraIndex: 352
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -8:00    -    PST    1970
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1970,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    Mexico    M%sT
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Menominee
	// EraIndex: 354
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -6:00 Menominee    C%sT    1969 Apr 27  2:00
	{
		PolicyIndex: 68, // PolicyName: Menominee
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1969,
		UntilMonth: 4,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    1973 Apr 29  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1973,
		UntilMonth: 4,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Merida
	// EraIndex: 357
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST    1981 Dec 23
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1981,
		UntilMonth: 12,
		UntilDay: 23,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    1982 Dec  2
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1982,
		UntilMonth: 12,
		UntilDay: 2,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Mexico    C%sT
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Metlakatla
	// EraIndex: 360
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//              -8:00    -    PST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -8:00    US    P%sT    1983 Oct 30  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1983,
		UntilMonth: 10,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -8:00    -    PST    2015 Nov  1  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 2015,
		UntilMonth: 11,
		UntilDay: 1,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    AK%sT    2018 Nov  4  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 81, // "AK%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 2018,
		UntilMonth: 11,
		UntilDay: 4,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -8:00    -    PST    2019 Jan 20  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 2019,
		UntilMonth: 1,
		UntilDay: 20,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 81, // "AK%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Mexico_City
	// EraIndex: 366
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -6:00    Mexico    C%sT    2001 Sep 30  2:00
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2001,
		UntilMonth: 9,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    -    CST    2002 Feb 20
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2002,
		UntilMonth: 2,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Mexico    C%sT
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Miquelon
	// EraIndex: 369
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -4:00    -    AST    1980 May
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 82, // "AST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1980,
		UntilMonth: 5,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    1987
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1987,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Canada    -03/-02
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Moncton
	// EraIndex: 372
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -4:00    Moncton    A%sT    1973
	{
		PolicyIndex: 71, // PolicyName: Moncton
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1973,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Canada    A%sT    1993
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1993,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Moncton    A%sT    2007
	{
		PolicyIndex: 71, // PolicyName: Moncton
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2007,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Canada    A%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Monterrey
	// EraIndex: 376
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST    1988
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1988,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT    1989
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1989,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Mexico    C%sT
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Montevideo
	// EraIndex: 379
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             -3:00    Uruguay    -03/-0230 1970
	{
		PolicyIndex: 109, // PolicyName: Uruguay
		FormatIndex: 50, // "-03/-0230"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1970,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Uruguay    -03/-02    1974
	{
		PolicyIndex: 109, // PolicyName: Uruguay
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1974,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Uruguay    -03/-0130 1974 Mar 10
	{
		PolicyIndex: 109, // PolicyName: Uruguay
		FormatIndex: 48, // "-03/-0130"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1974,
		UntilMonth: 3,
		UntilDay: 10,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Uruguay    -03/-0230 1974 Dec 22
	{
		PolicyIndex: 109, // PolicyName: Uruguay
		FormatIndex: 50, // "-03/-0230"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1974,
		UntilMonth: 12,
		UntilDay: 22,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Uruguay    -03/-02
	{
		PolicyIndex: 109, // PolicyName: Uruguay
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/New_York
	// EraIndex: 384
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Nome
	// EraIndex: 385
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -11:00    -    BST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 85, // "BST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2640, // (offset_seconds=-39600)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -11:00    US    B%sT    1983 Oct 30  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 84, // "B%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2640, // (offset_seconds=-39600)/15
		UntilYear: 1983,
		UntilMonth: 10,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    Y%sT    1983 Nov 30
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 135, // "Y%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 1983,
		UntilMonth: 11,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 81, // "AK%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Noronha
	// EraIndex: 389
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             -2:00    Brazil    -02/-01    1990 Sep 17
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 46, // "-02/-01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 1990,
		UntilMonth: 9,
		UntilDay: 17,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -2:00    -    -02    1999 Sep 30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 1999,
		UntilMonth: 9,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -2:00    Brazil    -02/-01    2000 Oct 15
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 46, // "-02/-01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 15,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -2:00    -    -02    2001 Sep 13
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 2001,
		UntilMonth: 9,
		UntilDay: 13,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -2:00    Brazil    -02/-01    2002 Oct  1
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 46, // "-02/-01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 2002,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -2:00    -    -02
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/North_Dakota/Beulah
	// EraIndex: 395
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -7:00    US    M%sT    2010 Nov  7  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 2010,
		UntilMonth: 11,
		UntilDay: 7,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/North_Dakota/Center
	// EraIndex: 397
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -7:00    US    M%sT    1992 Oct 25  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 1992,
		UntilMonth: 10,
		UntilDay: 25,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/North_Dakota/New_Salem
	// EraIndex: 399
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -7:00    US    M%sT    2003 Oct 26  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 2003,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Nuuk
	// EraIndex: 401
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -3:00    -    -03    1980 Apr  6  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1980,
		UntilMonth: 4,
		UntilDay: 6,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    EU    -03/-02    2023 Mar 25 22:00
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2023,
		UntilMonth: 3,
		UntilDay: 25,
		UntilTimeCode: 88,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -2:00    -    -02
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Ojinaga
	// EraIndex: 404
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST    1996
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1996,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Mexico    C%sT    1998
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1998,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    -    CST    1998 Apr Sun>=1  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 1998,
		UntilMonth: 4,
		UntilDay: 5,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    Mexico    M%sT    2010
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 2010,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    US    M%sT    2022 Oct 30  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 2022,
		UntilMonth: 10,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    -    CST    2022 Nov 30  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2022,
		UntilMonth: 11,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    US    C%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Panama
	// EraIndex: 411
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Paramaribo
	// EraIndex: 412
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -3:30    -    -0330    1984 Oct
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 51, // "-0330"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -840, // (offset_seconds=-12600)/15
		UntilYear: 1984,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Phoenix
	// EraIndex: 414
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 118, // "MST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Port-au-Prince
	// EraIndex: 415
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    Haiti    E%sT
	{
		PolicyIndex: 50, // PolicyName: Haiti
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Porto_Velho
	// EraIndex: 416
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -4:00    Brazil    -04/-03    1988 Sep 12
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1988,
		UntilMonth: 9,
		UntilDay: 12,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Puerto_Rico
	// EraIndex: 418
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    AST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 82, // "AST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Punta_Arenas
	// EraIndex: 419
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -4:00    Chile    -04/-03    2016 Dec  4
	{
		PolicyIndex: 25, // PolicyName: Chile
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2016,
		UntilMonth: 12,
		UntilDay: 4,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Rankin_Inlet
	// EraIndex: 421
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -6:00    NT_YK    C%sT    2000 Oct 29  2:00
	{
		PolicyIndex: 75, // PolicyName: NT_YK
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2001 Apr  1  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2001,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Canada    C%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Recife
	// EraIndex: 424
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             -3:00    Brazil    -03/-02    1990 Sep 17
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1990,
		UntilMonth: 9,
		UntilDay: 17,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    1999 Sep 30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1999,
		UntilMonth: 9,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Brazil    -03/-02    2000 Oct 15
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 15,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03    2001 Sep 13
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2001,
		UntilMonth: 9,
		UntilDay: 13,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Brazil    -03/-02    2002 Oct  1
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 2002,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Regina
	// EraIndex: 430
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Resolute
	// EraIndex: 431
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             -6:00    NT_YK    C%sT    2000 Oct 29  2:00
	{
		PolicyIndex: 75, // PolicyName: NT_YK
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2001 Apr  1  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2001,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Canada    C%sT    2006 Oct 29  2:00
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2006,
		UntilMonth: 10,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    EST    2007 Mar 11  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2007,
		UntilMonth: 3,
		UntilDay: 11,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Canada    C%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Rio_Branco
	// EraIndex: 436
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -5:00    Brazil    -05/-04    1988 Sep 12
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 57, // "-05/-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1988,
		UntilMonth: 9,
		UntilDay: 12,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    -05    2008 Jun 24  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 56, // "-05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2008,
		UntilMonth: 6,
		UntilDay: 24,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2013 Nov 10
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2013,
		UntilMonth: 11,
		UntilDay: 10,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    -    -05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 56, // "-05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Santarem
	// EraIndex: 440
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -4:00    Brazil    -04/-03    1988 Sep 12
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1988,
		UntilMonth: 9,
		UntilDay: 12,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    -04    2008 Jun 24  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2008,
		UntilMonth: 6,
		UntilDay: 24,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Santiago
	// EraIndex: 443
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Chile    -04/-03
	{
		PolicyIndex: 25, // PolicyName: Chile
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Santo_Domingo
	// EraIndex: 444
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -5:00    DR    %s    1974 Oct 27
	{
		PolicyIndex: 30, // PolicyName: DR
		FormatIndex: 1, // "%"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1974,
		UntilMonth: 10,
		UntilDay: 27,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    AST    2000 Oct 29  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 82, // "AST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2000,
		UntilMonth: 10,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    US    E%sT    2000 Dec  3  1:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 2000,
		UntilMonth: 12,
		UntilDay: 3,
		UntilTimeCode: 4,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    -    AST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 82, // "AST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Sao_Paulo
	// EraIndex: 448
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    Brazil    -03/-02
	{
		PolicyIndex: 18, // PolicyName: Brazil
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Scoresbysund
	// EraIndex: 449
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -2:00    -    -02    1980 Apr  6  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 1980,
		UntilMonth: 4,
		UntilDay: 6,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -2:00    C-Eur    -02/-01    1981 Mar 29
	{
		PolicyIndex: 20, // PolicyName: C-Eur
		FormatIndex: 46, // "-02/-01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 1981,
		UntilMonth: 3,
		UntilDay: 29,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -1:00    EU    -01/+00
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 44, // "-01/+00"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -240, // (offset_seconds=-3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Sitka
	// EraIndex: 452
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//              -8:00    -    PST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -8:00    US    P%sT    1983 Oct 30  2:00
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1983,
		UntilMonth: 10,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    Y%sT    1983 Nov 30
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 135, // "Y%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 1983,
		UntilMonth: 11,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 81, // "AK%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/St_Johns
	// EraIndex: 456
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -3:30    StJohns    N%sT    2011 Nov
	{
		PolicyIndex: 96, // PolicyName: StJohns
		FormatIndex: 119, // "N%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -840, // (offset_seconds=-12600)/15
		UntilYear: 2011,
		UntilMonth: 11,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:30    Canada    N%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 119, // "N%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -840, // (offset_seconds=-12600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Swift_Current
	// EraIndex: 458
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -7:00    Swift    M%sT    1972 Apr lastSun  2:00
	{
		PolicyIndex: 98, // PolicyName: Swift
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 1972,
		UntilMonth: 4,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    -    CST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 92, // "CST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Tegucigalpa
	// EraIndex: 460
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Hond    C%sT
	{
		PolicyIndex: 53, // PolicyName: Hond
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Thule
	// EraIndex: 461
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Thule    A%sT
	{
		PolicyIndex: 102, // PolicyName: Thule
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Tijuana
	// EraIndex: 462
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             -8:00    -    PST    1976
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1976,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    US    P%sT    1996
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1996,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    Mexico    P%sT    2001
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 2001,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    US    P%sT    2002 Feb 20
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 2002,
		UntilMonth: 2,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    Mexico    P%sT    2010
	{
		PolicyIndex: 69, // PolicyName: Mexico
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 2010,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    US    P%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Toronto
	// EraIndex: 468
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -5:00    Toronto    E%sT    1974
	{
		PolicyIndex: 104, // PolicyName: Toronto
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1974,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -5:00    Canada    E%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Vancouver
	// EraIndex: 470
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -8:00    Vanc    P%sT    1987
	{
		PolicyIndex: 110, // PolicyName: Vanc
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1987,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    Canada    P%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Whitehorse
	// EraIndex: 472
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -8:00    -    PST    1980
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 123, // "PST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 1980,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    Canada    P%sT    2020 Nov  1
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 2020,
		UntilMonth: 11,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 118, // "MST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Winnipeg
	// EraIndex: 475
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -6:00    Winn    C%sT    2006
	{
		PolicyIndex: 114, // PolicyName: Winn
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 2006,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Canada    C%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Yakutat
	// EraIndex: 477
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//              -9:00    -    YST    1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 136, // "YST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    Y%sT    1983 Nov 30
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 135, // "Y%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 1983,
		UntilMonth: 11,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 81, // "AK%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Yellowknife
	// EraIndex: 480
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -7:00    NT_YK    M%sT    1980
	{
		PolicyIndex: 75, // PolicyName: NT_YK
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 1980,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -7:00    Canada    M%sT
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Casey
	// EraIndex: 482
	// EraCount: 13
	// ---------------------------------------------------------------------------

	// 0 - -00 1969
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 42, // "-00"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1969,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              8:00    -    +08    2009 Oct 18  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 2009,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             11:00    -    +11    2010 Mar  5  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2010,
		UntilMonth: 3,
		UntilDay: 5,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              8:00    -    +08    2011 Oct 28  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 2011,
		UntilMonth: 10,
		UntilDay: 28,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             11:00    -    +11    2012 Feb 21 17:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2012,
		UntilMonth: 2,
		UntilDay: 21,
		UntilTimeCode: 68,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//              8:00    -    +08    2016 Oct 22
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 2016,
		UntilMonth: 10,
		UntilDay: 22,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             11:00    -    +11    2018 Mar 11  4:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2018,
		UntilMonth: 3,
		UntilDay: 11,
		UntilTimeCode: 16,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              8:00    -    +08    2018 Oct  7  4:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 2018,
		UntilMonth: 10,
		UntilDay: 7,
		UntilTimeCode: 16,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             11:00    -    +11    2019 Mar 17  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2019,
		UntilMonth: 3,
		UntilDay: 17,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              8:00    -    +08    2019 Oct  4  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 2019,
		UntilMonth: 10,
		UntilDay: 4,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             11:00    -    +11    2020 Mar  8  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2020,
		UntilMonth: 3,
		UntilDay: 8,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              8:00    -    +08    2020 Oct  4  0:01
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 2020,
		UntilMonth: 10,
		UntilDay: 4,
		UntilTimeCode: 0,
		UntilTimeModifier: 1, // SuffixW + minute=1
	},

	//             11:00    -    +11
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Davis
	// EraIndex: 495
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             0    -    -00    1969 Feb
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 42, // "-00"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1969,
		UntilMonth: 2,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             7:00    -    +07    2009 Oct 18  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 2009,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    -    +05    2010 Mar 10 20:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 2010,
		UntilMonth: 3,
		UntilDay: 10,
		UntilTimeCode: 80,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             7:00    -    +07    2011 Oct 28  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 2011,
		UntilMonth: 10,
		UntilDay: 28,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    -    +05    2012 Feb 21 20:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 2012,
		UntilMonth: 2,
		UntilDay: 21,
		UntilTimeCode: 80,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             7:00    -    +07
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Macquarie
	// EraIndex: 501
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             10:00    AT    AE%sT    2010
	{
		PolicyIndex: 4, // PolicyName: AT
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2010,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:00    1:00    AEDT    2011
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 77, // "AEDT"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2011,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:00    AT    AE%sT
	{
		PolicyIndex: 4, // PolicyName: AT
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Mawson
	// EraIndex: 504
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             6:00    -    +06    2009 Oct 18  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2009,
		UntilMonth: 10,
		UntilDay: 18,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Palmer
	// EraIndex: 506
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -4:00    Arg    -04/-03    1969 Oct  5
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 5,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Arg    -03/-02    1982 May
	{
		PolicyIndex: 9, // PolicyName: Arg
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1982,
		UntilMonth: 5,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Chile    -04/-03    2016 Dec  4
	{
		PolicyIndex: 25, // PolicyName: Chile
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2016,
		UntilMonth: 12,
		UntilDay: 4,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Rothera
	// EraIndex: 510
	// EraCount: 2
	// ---------------------------------------------------------------------------

	// 0 - -00 1976 Dec 1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 42, // "-00"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1976,
		UntilMonth: 12,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Troll
	// EraIndex: 512
	// EraCount: 2
	// ---------------------------------------------------------------------------

	// 0 - -00 2005 Feb 12
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 42, // "-00"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 2005,
		UntilMonth: 2,
		UntilDay: 12,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             0:00    Troll    %s
	{
		PolicyIndex: 105, // PolicyName: Troll
		FormatIndex: 1, // "%"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Almaty
	// EraIndex: 514
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             6:00 RussiaAsia +06/+07    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1992 Jan 19  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             6:00 RussiaAsia    +06/+07    2004 Oct 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2004,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             6:00    -    +06
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Amman
	// EraIndex: 518
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             2:00    Jordan    EE%sT    2022 Oct 28 0:00s
	{
		PolicyIndex: 59, // PolicyName: Jordan
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2022,
		UntilMonth: 10,
		UntilDay: 28,
		UntilTimeCode: 0,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             3:00    -    +03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Anadyr
	// EraIndex: 520
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             13:00    Russia    +13/+14    1982 Apr  1  0:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 40, // "+13/+14"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 3120, // (offset_seconds=46800)/15
		UntilYear: 1982,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             12:00    Russia    +12/+13    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 36, // "+12/+13"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    Russia    +11/+12    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             12:00    Russia    +12/+13    2010 Mar 28  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 36, // "+12/+13"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 2010,
		UntilMonth: 3,
		UntilDay: 28,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    Russia    +11/+12    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             12:00    -    +12
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 35, // "+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Aqtau
	// EraIndex: 526
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             5:00    -    +05    1981 Oct  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06    1982 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1982,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    1992 Jan 19  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1994 Sep 25  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1994,
		UntilMonth: 9,
		UntilDay: 25,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    2004 Oct 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2004,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Aqtobe
	// EraIndex: 533
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             5:00    -    +05    1981 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    1:00    +06    1981 Oct  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06    1982 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1982,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    1992 Jan 19  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    2004 Oct 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 2004,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Ashgabat
	// EraIndex: 540
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             5:00 RussiaAsia    +05/+06    1991 Mar 31  2:00
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    1992 Jan 19  2:00
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Atyrau
	// EraIndex: 543
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             5:00    -    +05    1981 Oct  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06    1982 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1982,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    1992 Jan 19  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1999 Mar 28  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1999,
		UntilMonth: 3,
		UntilDay: 28,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    2004 Oct 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2004,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Baghdad
	// EraIndex: 550
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             3:00    -    +03    1982 May
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1982,
		UntilMonth: 5,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00    Iraq    +03/+04
	{
		PolicyIndex: 56, // PolicyName: Iraq
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Baku
	// EraIndex: 552
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             4:00 RussiaAsia +04/+05    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             3:00 RussiaAsia    +03/+04    1992 Sep lastSun  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1992,
		UntilMonth: 9,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00    -    +04    1996
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1996,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             4:00    EUAsia    +04/+05    1997
	{
		PolicyIndex: 35, // PolicyName: EUAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1997,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             4:00    Azer    +04/+05
	{
		PolicyIndex: 13, // PolicyName: Azer
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Bangkok
	// EraIndex: 557
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             7:00    -    +07
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Barnaul
	// EraIndex: 558
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//              7:00    Russia    +07/+08    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    Russia    +06/+07    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    Russia    +07/+08    1995 May 28
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 1995,
		UntilMonth: 5,
		UntilDay: 28,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              6:00    Russia    +06/+07    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    -    +07    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    -    +06    2016 Mar 27  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2016,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    -    +07
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Beirut
	// EraIndex: 565
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    Lebanon    EE%sT
	{
		PolicyIndex: 63, // PolicyName: Lebanon
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Bishkek
	// EraIndex: 566
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             6:00 RussiaAsia +06/+07    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1991 Aug 31  2:00
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1991,
		UntilMonth: 8,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    Kyrgyz    +05/+06    2005 Aug 12
	{
		PolicyIndex: 60, // PolicyName: Kyrgyz
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 2005,
		UntilMonth: 8,
		UntilDay: 12,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Chita
	// EraIndex: 570
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//              9:00    Russia    +09/+10    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 27, // "+09/+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              8:00    Russia    +08/+09    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 24, // "+08/+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              9:00    Russia    +09/+10    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 27, // "+09/+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    -    +10    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 28, // "+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              8:00    -    +08    2016 Mar 27  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 2016,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              9:00    -    +09
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 26, // "+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Choibalsan
	// EraIndex: 576
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             7:00    -    +07    1978
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 1978,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             8:00    -    +08    1983 Apr
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 1983,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             9:00    Mongol    +09/+10    2008 Mar 31
	{
		PolicyIndex: 72, // PolicyName: Mongol
		FormatIndex: 27, // "+09/+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 2008,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             8:00    Mongol    +08/+09
	{
		PolicyIndex: 72, // PolicyName: Mongol
		FormatIndex: 24, // "+08/+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Colombo
	// EraIndex: 580
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             5:30    -    +0530    1996 May 25  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 15, // "+0530"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1320, // (offset_seconds=19800)/15
		UntilYear: 1996,
		UntilMonth: 5,
		UntilDay: 25,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:30    -    +0630    1996 Oct 26  0:30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 19, // "+0630"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1560, // (offset_seconds=23400)/15
		UntilYear: 1996,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 2,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06    2006 Apr 15  0:30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2006,
		UntilMonth: 4,
		UntilDay: 15,
		UntilTimeCode: 2,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:30    -    +0530
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 15, // "+0530"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1320, // (offset_seconds=19800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Damascus
	// EraIndex: 584
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             2:00    Syria    EE%sT    2022 Oct 28 0:00
	{
		PolicyIndex: 100, // PolicyName: Syria
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2022,
		UntilMonth: 10,
		UntilDay: 28,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00    -    +03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Dhaka
	// EraIndex: 586
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             6:00    -    +06    2009
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2009,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    Dhaka    +06/+07
	{
		PolicyIndex: 31, // PolicyName: Dhaka
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Dili
	// EraIndex: 588
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             9:00    -    +09    1976 May  3
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 26, // "+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 1976,
		UntilMonth: 5,
		UntilDay: 3,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             8:00    -    +08    2000 Sep 17  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 2000,
		UntilMonth: 9,
		UntilDay: 17,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             9:00    -    +09
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 26, // "+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Dubai
	// EraIndex: 591
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             4:00    -    +04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Dushanbe
	// EraIndex: 592
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             6:00 RussiaAsia +06/+07    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00    1:00    +06    1991 Sep  9  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1991,
		UntilMonth: 9,
		UntilDay: 9,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Famagusta
	// EraIndex: 595
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             2:00    Cyprus    EE%sT    1998 Sep
	{
		PolicyIndex: 28, // PolicyName: Cyprus
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1998,
		UntilMonth: 9,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EUAsia    EE%sT    2016 Sep  8
	{
		PolicyIndex: 35, // PolicyName: EUAsia
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2016,
		UntilMonth: 9,
		UntilDay: 8,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00    -    +03    2017 Oct 29 1:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2017,
		UntilMonth: 10,
		UntilDay: 29,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             2:00    EUAsia    EE%sT
	{
		PolicyIndex: 35, // PolicyName: EUAsia
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Gaza
	// EraIndex: 599
	// EraCount: 9
	// ---------------------------------------------------------------------------

	//             2:00    Zion    I%sT    1996
	{
		PolicyIndex: 116, // PolicyName: Zion
		FormatIndex: 106, // "I%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1996,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    Jordan    EE%sT    1999
	{
		PolicyIndex: 59, // PolicyName: Jordan
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1999,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00 Palestine    EE%sT    2008 Aug 29  0:00
	{
		PolicyIndex: 81, // PolicyName: Palestine
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2008,
		UntilMonth: 8,
		UntilDay: 29,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    EET    2008 Sep
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2008,
		UntilMonth: 9,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00 Palestine    EE%sT    2010
	{
		PolicyIndex: 81, // PolicyName: Palestine
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2010,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    EET    2010 Mar 27  0:01
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2010,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 0,
		UntilTimeModifier: 1, // SuffixW + minute=1
	},

	//             2:00 Palestine    EE%sT    2011 Aug  1
	{
		PolicyIndex: 81, // PolicyName: Palestine
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2011,
		UntilMonth: 8,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    EET    2012
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2012,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00 Palestine    EE%sT
	{
		PolicyIndex: 81, // PolicyName: Palestine
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Hebron
	// EraIndex: 608
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             2:00    Zion    I%sT    1996
	{
		PolicyIndex: 116, // PolicyName: Zion
		FormatIndex: 106, // "I%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1996,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    Jordan    EE%sT    1999
	{
		PolicyIndex: 59, // PolicyName: Jordan
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1999,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00 Palestine    EE%sT
	{
		PolicyIndex: 81, // PolicyName: Palestine
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Ho_Chi_Minh
	// EraIndex: 611
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             8:00    -    +08    1975 Jun 13
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 1975,
		UntilMonth: 6,
		UntilDay: 13,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             7:00    -    +07
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Hong_Kong
	// EraIndex: 613
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    HK    HK%sT
	{
		PolicyIndex: 49, // PolicyName: HK
		FormatIndex: 104, // "HK%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Hovd
	// EraIndex: 614
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             6:00    -    +06    1978
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1978,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             7:00    Mongol    +07/+08
	{
		PolicyIndex: 72, // PolicyName: Mongol
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Irkutsk
	// EraIndex: 616
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//              8:00    Russia    +08/+09    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 24, // "+08/+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    Russia    +07/+08    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              8:00    Russia    +08/+09    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 24, // "+08/+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              9:00    -    +09    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 26, // "+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              8:00    -    +08
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Jakarta
	// EraIndex: 621
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             7:00    -    WIB
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 132, // "WIB"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Jayapura
	// EraIndex: 622
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             9:00    -    WIT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 133, // "WIT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Jerusalem
	// EraIndex: 623
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    Zion    I%sT
	{
		PolicyIndex: 116, // PolicyName: Zion
		FormatIndex: 106, // "I%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Kabul
	// EraIndex: 624
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             4:30    -    +0430
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 12, // "+0430"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1080, // (offset_seconds=16200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Kamchatka
	// EraIndex: 625
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             12:00    Russia    +12/+13    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 36, // "+12/+13"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    Russia    +11/+12    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             12:00    Russia    +12/+13    2010 Mar 28  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 36, // "+12/+13"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 2010,
		UntilMonth: 3,
		UntilDay: 28,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    Russia    +11/+12    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             12:00    -    +12
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 35, // "+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Karachi
	// EraIndex: 630
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             5:00    -    +05    1971 Mar 26
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1971,
		UntilMonth: 3,
		UntilDay: 26,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00 Pakistan    PK%sT
	{
		PolicyIndex: 80, // PolicyName: Pakistan
		FormatIndex: 122, // "PK%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Kathmandu
	// EraIndex: 632
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             5:30    -    +0530    1986
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 15, // "+0530"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1320, // (offset_seconds=19800)/15
		UntilYear: 1986,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:45    -    +0545
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 16, // "+0545"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1380, // (offset_seconds=20700)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Khandyga
	// EraIndex: 634
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//              9:00    Russia    +09/+10    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 27, // "+09/+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              8:00    Russia    +08/+09    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 24, // "+08/+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              9:00    Russia    +09/+10    2004
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 27, // "+09/+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 2004,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:00    Russia    +10/+11    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 29, // "+10/+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    -    +11    2011 Sep 13  0:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2011,
		UntilMonth: 9,
		UntilDay: 13,
		UntilTimeCode: 0,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    -    +10    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 28, // "+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              9:00    -    +09
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 26, // "+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Kolkata
	// EraIndex: 641
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:30    -    IST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 107, // "IST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1320, // (offset_seconds=19800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Krasnoyarsk
	// EraIndex: 642
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//              7:00    Russia    +07/+08    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    Russia    +06/+07    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    Russia    +07/+08    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              8:00    -    +08    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    -    +07
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Kuching
	// EraIndex: 647
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    -    +08
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Macau
	// EraIndex: 648
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    Macau    C%sT
	{
		PolicyIndex: 65, // PolicyName: Macau
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Magadan
	// EraIndex: 649
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             11:00    Russia    +11/+12    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    Russia    +10/+11    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 29, // "+10/+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    Russia    +11/+12    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             12:00    -    +12    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 35, // "+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    -    +10    2016 Apr 24  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 28, // "+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2016,
		UntilMonth: 4,
		UntilDay: 24,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    -    +11
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Makassar
	// EraIndex: 655
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    -    WITA
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 134, // "WITA"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Manila
	// EraIndex: 656
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    Phil    P%sT
	{
		PolicyIndex: 84, // PolicyName: Phil
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Nicosia
	// EraIndex: 657
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             2:00    Cyprus    EE%sT    1998 Sep
	{
		PolicyIndex: 28, // PolicyName: Cyprus
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1998,
		UntilMonth: 9,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EUAsia    EE%sT
	{
		PolicyIndex: 35, // PolicyName: EUAsia
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Novokuznetsk
	// EraIndex: 659
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//              7:00    Russia    +07/+08    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    Russia    +06/+07    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    Russia    +07/+08    2010 Mar 28  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 2010,
		UntilMonth: 3,
		UntilDay: 28,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    Russia    +06/+07    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    -    +07
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Novosibirsk
	// EraIndex: 664
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//              7:00    Russia    +07/+08    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    Russia    +06/+07    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    Russia    +07/+08    1993 May 23
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 1993,
		UntilMonth: 5,
		UntilDay: 23,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              6:00    Russia    +06/+07    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    -    +07    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    -    +06    2016 Jul 24  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2016,
		UntilMonth: 7,
		UntilDay: 24,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    -    +07
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Omsk
	// EraIndex: 671
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//              6:00    Russia    +06/+07    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              5:00    Russia    +05/+06    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    Russia    +06/+07    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    -    +07    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    -    +06
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Oral
	// EraIndex: 676
	// EraCount: 8
	// ---------------------------------------------------------------------------

	//             5:00    -    +05    1981 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    1:00    +06    1981 Oct  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06    1982 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1982,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1989 Mar 26  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1989,
		UntilMonth: 3,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    1992 Jan 19  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1992 Mar 29  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1992,
		UntilMonth: 3,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    2004 Oct 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2004,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Pontianak
	// EraIndex: 684
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             8:00    -    WITA    1988 Jan  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 134, // "WITA"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 1988,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             7:00    -    WIB
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 132, // "WIB"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Pyongyang
	// EraIndex: 686
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             9:00    -    KST    2015 Aug 15 00:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 111, // "KST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 2015,
		UntilMonth: 8,
		UntilDay: 15,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             8:30    -    KST    2018 May  4 23:30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 111, // "KST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2040, // (offset_seconds=30600)/15
		UntilYear: 2018,
		UntilMonth: 5,
		UntilDay: 4,
		UntilTimeCode: 94,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             9:00    -    KST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 111, // "KST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Qatar
	// EraIndex: 689
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             4:00    -    +04    1972 Jun
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1972,
		UntilMonth: 6,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00    -    +03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Qostanay
	// EraIndex: 691
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             5:00    -    +05    1981 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    1:00    +06    1981 Oct  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06    1982 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1982,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    1992 Jan 19  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    2004 Oct 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 2004,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             6:00    -    +06
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Qyzylorda
	// EraIndex: 698
	// EraCount: 10
	// ---------------------------------------------------------------------------

	//             5:00    -    +05    1981 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    1:00    +06    1981 Oct  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06    1982 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1982,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    1991 Sep 29  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1991,
		UntilMonth: 9,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1992 Jan 19  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             6:00 RussiaAsia    +06/+07    1992 Mar 29  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1992,
		UntilMonth: 3,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    2004 Oct 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 2004,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             6:00    -    +06    2018 Dec 21  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2018,
		UntilMonth: 12,
		UntilDay: 21,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Riyadh
	// EraIndex: 708
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:00    -    +03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Sakhalin
	// EraIndex: 709
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             11:00    Russia    +11/+12    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    Russia    +10/+11    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 29, // "+10/+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    Russia    +11/+12    1997 Mar lastSun  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 1997,
		UntilMonth: 3,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    Russia    +10/+11    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 29, // "+10/+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    -    +11    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    -    +10    2016 Mar 27  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 28, // "+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2016,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    -    +11
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Samarkand
	// EraIndex: 716
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             5:00    -    +05    1981 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    1:00    +06    1981 Oct  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1981,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06    1982 Apr  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1982,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1992
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Seoul
	// EraIndex: 721
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             9:00    ROK    K%sT
	{
		PolicyIndex: 87, // PolicyName: ROK
		FormatIndex: 110, // "K%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Shanghai
	// EraIndex: 722
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    PRC    C%sT
	{
		PolicyIndex: 79, // PolicyName: PRC
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Singapore
	// EraIndex: 723
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             7:30    -    +0730    1981 Dec 31 16:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 22, // "+0730"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1800, // (offset_seconds=27000)/15
		UntilYear: 1981,
		UntilMonth: 12,
		UntilDay: 31,
		UntilTimeCode: 64,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             8:00    -    +08
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Srednekolymsk
	// EraIndex: 725
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             11:00    Russia    +11/+12    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    Russia    +10/+11    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 29, // "+10/+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    Russia    +11/+12    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             12:00    -    +12    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 35, // "+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    -    +11
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Taipei
	// EraIndex: 730
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    Taiwan    C%sT
	{
		PolicyIndex: 101, // PolicyName: Taiwan
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Tashkent
	// EraIndex: 731
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             6:00 RussiaAsia    +06/+07    1991 Mar 31  2:00
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00 RussiaAsia    +05/+06    1992
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Tbilisi
	// EraIndex: 734
	// EraCount: 8
	// ---------------------------------------------------------------------------

	//             4:00 RussiaAsia +04/+05    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             3:00 RussiaAsia +03/+04    1992
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00 E-EurAsia    +03/+04    1994 Sep lastSun
	{
		PolicyIndex: 33, // PolicyName: E-EurAsia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1994,
		UntilMonth: 9,
		UntilDay: 25,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             4:00 E-EurAsia    +04/+05    1996 Oct lastSun
	{
		PolicyIndex: 33, // PolicyName: E-EurAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1996,
		UntilMonth: 10,
		UntilDay: 27,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             4:00    1:00    +05    1997 Mar lastSun
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1997,
		UntilMonth: 3,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             4:00 E-EurAsia    +04/+05    2004 Jun 27
	{
		PolicyIndex: 33, // PolicyName: E-EurAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2004,
		UntilMonth: 6,
		UntilDay: 27,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00 RussiaAsia    +03/+04    2005 Mar lastSun  2:00
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2005,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             4:00    -    +04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Tehran
	// EraIndex: 742
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             3:30    Iran    +0330/+0430 1977 Oct 20 24:00
	{
		PolicyIndex: 55, // PolicyName: Iran
		FormatIndex: 9, // "+0330/+0430"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 840, // (offset_seconds=12600)/15
		UntilYear: 1977,
		UntilMonth: 10,
		UntilDay: 20,
		UntilTimeCode: 96,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             4:00    Iran    +04/+05    1979
	{
		PolicyIndex: 55, // PolicyName: Iran
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1979,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:30    Iran    +0330/+0430
	{
		PolicyIndex: 55, // PolicyName: Iran
		FormatIndex: 9, // "+0330/+0430"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 840, // (offset_seconds=12600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Thimphu
	// EraIndex: 745
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             5:30    -    +0530    1987 Oct
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 15, // "+0530"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1320, // (offset_seconds=19800)/15
		UntilYear: 1987,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Tokyo
	// EraIndex: 747
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             9:00    Japan    J%sT
	{
		PolicyIndex: 58, // PolicyName: Japan
		FormatIndex: 109, // "J%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Tomsk
	// EraIndex: 748
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//              7:00    Russia    +07/+08    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    Russia    +06/+07    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    Russia    +07/+08    2002 May  1  3:00
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 21, // "+07/+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 2002,
		UntilMonth: 5,
		UntilDay: 1,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              6:00    Russia    +06/+07    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 18, // "+06/+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    -    +07    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    -    +06    2016 May 29  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2016,
		UntilMonth: 5,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              7:00    -    +07
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Ulaanbaatar
	// EraIndex: 755
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             7:00    -    +07    1978
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 1978,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             8:00    Mongol    +08/+09
	{
		PolicyIndex: 72, // PolicyName: Mongol
		FormatIndex: 24, // "+08/+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Urumqi
	// EraIndex: 757
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             6:00    -    +06
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Ust-Nera
	// EraIndex: 758
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//              9:00    Russia    +09/+10    1981 Apr  1
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 27, // "+09/+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 1981,
		UntilMonth: 4,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             11:00    Russia    +11/+12    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    Russia    +10/+11    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 29, // "+10/+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    Russia    +11/+12    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             12:00    -    +12    2011 Sep 13  0:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 35, // "+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 2011,
		UntilMonth: 9,
		UntilDay: 13,
		UntilTimeCode: 0,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    -    +11    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    -    +10
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 28, // "+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Vladivostok
	// EraIndex: 765
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             10:00    Russia    +10/+11    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 29, // "+10/+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              9:00    Russia    +09/+10    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 27, // "+09/+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    Russia    +10/+11    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 29, // "+10/+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    -    +11    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    -    +10
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 28, // "+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Yakutsk
	// EraIndex: 770
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//              9:00    Russia    +09/+10    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 27, // "+09/+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              8:00    Russia    +08/+09    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 24, // "+08/+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              9:00    Russia    +09/+10    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 27, // "+09/+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             10:00    -    +10    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 28, // "+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              9:00    -    +09
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 26, // "+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Yangon
	// EraIndex: 775
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             6:30    -    +0630
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 19, // "+0630"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1560, // (offset_seconds=23400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Yekaterinburg
	// EraIndex: 776
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//              5:00    Russia    +05/+06    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    Russia    +04/+05    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              5:00    Russia    +05/+06    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 14, // "+05/+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              6:00    -    +06    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Yerevan
	// EraIndex: 781
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             4:00 RussiaAsia +04/+05    1991 Mar 31  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             3:00 RussiaAsia    +03/+04    1995 Sep 24  2:00s
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1995,
		UntilMonth: 9,
		UntilDay: 24,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             4:00    -    +04    1997
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1997,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             4:00 RussiaAsia    +04/+05    2011
	{
		PolicyIndex: 90, // PolicyName: RussiaAsia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2011,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             4:00    Armenia    +04/+05
	{
		PolicyIndex: 10, // PolicyName: Armenia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Azores
	// EraIndex: 786
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -1:00    Port    -01/+00    1983 Sep 25  1:00s
	{
		PolicyIndex: 86, // PolicyName: Port
		FormatIndex: 44, // "-01/+00"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -240, // (offset_seconds=-3600)/15
		UntilYear: 1983,
		UntilMonth: 9,
		UntilDay: 25,
		UntilTimeCode: 4,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             -1:00    W-Eur    -01/+00    1992 Sep 27  1:00s
	{
		PolicyIndex: 112, // PolicyName: W-Eur
		FormatIndex: 44, // "-01/+00"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -240, // (offset_seconds=-3600)/15
		UntilYear: 1992,
		UntilMonth: 9,
		UntilDay: 27,
		UntilTimeCode: 4,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              0:00    EU    WE%sT    1993 Mar 28  1:00u
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1993,
		UntilMonth: 3,
		UntilDay: 28,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             -1:00    EU    -01/+00
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 44, // "-01/+00"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -240, // (offset_seconds=-3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Bermuda
	// EraIndex: 790
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -4:00    Bermuda    A%sT    1974 Apr 28  2:00
	{
		PolicyIndex: 17, // PolicyName: Bermuda
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1974,
		UntilMonth: 4,
		UntilDay: 28,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Canada    A%sT    1976
	{
		PolicyIndex: 23, // PolicyName: Canada
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1976,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    US    A%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 73, // "A%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Canary
	// EraIndex: 793
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//              0:00    -    WET    1980 Apr  6  0:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 131, // "WET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1980,
		UntilMonth: 4,
		UntilDay: 6,
		UntilTimeCode: 0,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              0:00    1:00    WEST    1980 Sep 28  1:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 130, // "WEST"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1980,
		UntilMonth: 9,
		UntilDay: 28,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//              0:00    EU    WE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Cape_Verde
	// EraIndex: 796
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -2:00    -    -02    1975 Nov 25  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 1975,
		UntilMonth: 11,
		UntilDay: 25,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -1:00    -    -01
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 43, // "-01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -240, // (offset_seconds=-3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Faroe
	// EraIndex: 798
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//              0:00    -    WET    1981
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 131, // "WET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1981,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              0:00    EU    WE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Madeira
	// EraIndex: 800
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//              0:00    Port    WE%sT    1983 Sep 25  1:00s
	{
		PolicyIndex: 86, // PolicyName: Port
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1983,
		UntilMonth: 9,
		UntilDay: 25,
		UntilTimeCode: 4,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              0:00    EU    WE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/South_Georgia
	// EraIndex: 802
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -2:00    -    -02
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Stanley
	// EraIndex: 803
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             -4:00    Falk    -04/-03    1983 May
	{
		PolicyIndex: 40, // PolicyName: Falk
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 1983,
		UntilMonth: 5,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    Falk    -03/-02    1985 Sep 15
	{
		PolicyIndex: 40, // PolicyName: Falk
		FormatIndex: 49, // "-03/-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 1985,
		UntilMonth: 9,
		UntilDay: 15,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -4:00    Falk    -04/-03    2010 Sep  5  2:00
	{
		PolicyIndex: 40, // PolicyName: Falk
		FormatIndex: 54, // "-04/-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 2010,
		UntilMonth: 9,
		UntilDay: 5,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -3:00    -    -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Adelaide
	// EraIndex: 807
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             9:30    Aus    AC%sT    1971
	{
		PolicyIndex: 11, // PolicyName: Aus
		FormatIndex: 74, // "AC%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2280, // (offset_seconds=34200)/15
		UntilYear: 1971,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             9:30    AS    AC%sT
	{
		PolicyIndex: 3, // PolicyName: AS
		FormatIndex: 74, // "AC%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2280, // (offset_seconds=34200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Brisbane
	// EraIndex: 809
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             10:00    Aus    AE%sT    1971
	{
		PolicyIndex: 11, // PolicyName: Aus
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1971,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:00    AQ    AE%sT
	{
		PolicyIndex: 2, // PolicyName: AQ
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Broken_Hill
	// EraIndex: 811
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             9:30    Aus    AC%sT    1971
	{
		PolicyIndex: 11, // PolicyName: Aus
		FormatIndex: 74, // "AC%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2280, // (offset_seconds=34200)/15
		UntilYear: 1971,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             9:30    AN    AC%sT    2000
	{
		PolicyIndex: 1, // PolicyName: AN
		FormatIndex: 74, // "AC%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2280, // (offset_seconds=34200)/15
		UntilYear: 2000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             9:30    AS    AC%sT
	{
		PolicyIndex: 3, // PolicyName: AS
		FormatIndex: 74, // "AC%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2280, // (offset_seconds=34200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Darwin
	// EraIndex: 814
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              9:30    Aus    AC%sT
	{
		PolicyIndex: 11, // PolicyName: Aus
		FormatIndex: 74, // "AC%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2280, // (offset_seconds=34200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Eucla
	// EraIndex: 815
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              8:45    AW  +0845/+0945
	{
		PolicyIndex: 6, // PolicyName: AW
		FormatIndex: 25, // "+0845/+0945"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2100, // (offset_seconds=31500)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Hobart
	// EraIndex: 816
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    AT    AE%sT
	{
		PolicyIndex: 4, // PolicyName: AT
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Lindeman
	// EraIndex: 817
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             10:00    Aus    AE%sT    1971
	{
		PolicyIndex: 11, // PolicyName: Aus
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1971,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:00    AQ    AE%sT    1992 Jul
	{
		PolicyIndex: 2, // PolicyName: AQ
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1992,
		UntilMonth: 7,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:00    Holiday    AE%sT
	{
		PolicyIndex: 52, // PolicyName: Holiday
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Lord_Howe
	// EraIndex: 820
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             10:00    -    AEST    1981 Mar
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 78, // "AEST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1981,
		UntilMonth: 3,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:30    LH    +1030/+1130 1985 Jul
	{
		PolicyIndex: 61, // PolicyName: LH
		FormatIndex: 31, // "+1030/+1130"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2520, // (offset_seconds=37800)/15
		UntilYear: 1985,
		UntilMonth: 7,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:30    LH    +1030/+11
	{
		PolicyIndex: 61, // PolicyName: LH
		FormatIndex: 30, // "+1030/+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2520, // (offset_seconds=37800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Melbourne
	// EraIndex: 823
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             10:00    Aus    AE%sT    1971
	{
		PolicyIndex: 11, // PolicyName: Aus
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1971,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:00    AV    AE%sT
	{
		PolicyIndex: 5, // PolicyName: AV
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Perth
	// EraIndex: 825
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              8:00    AW    AW%sT
	{
		PolicyIndex: 6, // PolicyName: AW
		FormatIndex: 83, // "AW%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Sydney
	// EraIndex: 826
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             10:00    Aus    AE%sT    1971
	{
		PolicyIndex: 11, // PolicyName: Aus
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 1971,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:00    AN    AE%sT
	{
		PolicyIndex: 1, // PolicyName: AN
		FormatIndex: 76, // "AE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: CET
	// EraIndex: 828
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 1:00 C-Eur CE%sT
	{
		PolicyIndex: 20, // PolicyName: C-Eur
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: CST6CDT
	// EraIndex: 829
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -6:00 US C%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 86, // "C%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: EET
	// EraIndex: 830
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 2:00 EU EE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: EST
	// EraIndex: 831
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -5:00 - EST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 99, // "EST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: EST5EDT
	// EraIndex: 832
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -5:00 US E%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 94, // "E%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT
	// EraIndex: 833
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 0 - GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 101, // "GMT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+1
	// EraIndex: 834
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -1 - -01
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 43, // "-01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -240, // (offset_seconds=-3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+10
	// EraIndex: 835
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -10 - -10
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 66, // "-10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2400, // (offset_seconds=-36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+11
	// EraIndex: 836
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -11 - -11
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 70, // "-11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2640, // (offset_seconds=-39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+12
	// EraIndex: 837
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -12 - -12
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 72, // "-12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2880, // (offset_seconds=-43200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+2
	// EraIndex: 838
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -2 - -02
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "-02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -480, // (offset_seconds=-7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+3
	// EraIndex: 839
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -3 - -03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 47, // "-03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -720, // (offset_seconds=-10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+4
	// EraIndex: 840
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -4 - -04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 53, // "-04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -960, // (offset_seconds=-14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+5
	// EraIndex: 841
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -5 - -05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 56, // "-05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+6
	// EraIndex: 842
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -6 - -06
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 58, // "-06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+7
	// EraIndex: 843
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -7 - -07
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 60, // "-07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+8
	// EraIndex: 844
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -8 - -08
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 62, // "-08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+9
	// EraIndex: 845
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -9 - -09
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 64, // "-09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-1
	// EraIndex: 846
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 1 - +01
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 3, // "+01"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-10
	// EraIndex: 847
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 10 - +10
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 28, // "+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-11
	// EraIndex: 848
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 11 - +11
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-12
	// EraIndex: 849
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 12 - +12
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 35, // "+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-13
	// EraIndex: 850
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 13 - +13
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 39, // "+13"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 3120, // (offset_seconds=46800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-14
	// EraIndex: 851
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 14 - +14
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 41, // "+14"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 3360, // (offset_seconds=50400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-2
	// EraIndex: 852
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 2 - +02
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 5, // "+02"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-3
	// EraIndex: 853
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 3 - +03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-4
	// EraIndex: 854
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 4 - +04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-5
	// EraIndex: 855
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 5 - +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-6
	// EraIndex: 856
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 6 - +06
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-7
	// EraIndex: 857
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 7 - +07
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "+07"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1680, // (offset_seconds=25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-8
	// EraIndex: 858
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 8 - +08
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 23, // "+08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1920, // (offset_seconds=28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-9
	// EraIndex: 859
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 9 - +09
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 26, // "+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/UTC
	// EraIndex: 860
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 0 - UTC
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 126, // "UTC"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Andorra
	// EraIndex: 861
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    -    CET    1985 Mar 31  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 91, // "CET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1985,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Astrakhan
	// EraIndex: 863
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//              4:00    Russia    +04/+05    1989 Mar 26  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1989,
		UntilMonth: 3,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04    1992 Mar 29  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 3,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    +03    2016 Mar 27  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2016,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Athens
	// EraIndex: 870
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             2:00    Greece    EE%sT    1981
	{
		PolicyIndex: 46, // PolicyName: Greece
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1981,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Belgrade
	// EraIndex: 872
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    -    CET    1982 Nov 27
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 91, // "CET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1982,
		UntilMonth: 11,
		UntilDay: 27,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Berlin
	// EraIndex: 874
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    Germany    CE%sT    1980
	{
		PolicyIndex: 45, // PolicyName: Germany
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1980,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Brussels
	// EraIndex: 876
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    Belgium    CE%sT    1977
	{
		PolicyIndex: 15, // PolicyName: Belgium
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1977,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Bucharest
	// EraIndex: 878
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             2:00    Romania    EE%sT    1981 Mar 29  2:00s
	{
		PolicyIndex: 88, // PolicyName: Romania
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1981,
		UntilMonth: 3,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             2:00    C-Eur    EE%sT    1991
	{
		PolicyIndex: 20, // PolicyName: C-Eur
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1991,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    Romania    EE%sT    1994
	{
		PolicyIndex: 88, // PolicyName: Romania
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1994,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    E-Eur    EE%sT    1997
	{
		PolicyIndex: 32, // PolicyName: E-Eur
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1997,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Budapest
	// EraIndex: 883
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    Hungary    CE%sT    1984
	{
		PolicyIndex: 54, // PolicyName: Hungary
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1984,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Chisinau
	// EraIndex: 885
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             3:00    Russia    MSK/MSD    1990 May  6  2:00
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1990,
		UntilMonth: 5,
		UntilDay: 6,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    Russia    EE%sT    1992
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    E-Eur    EE%sT    1997
	{
		PolicyIndex: 32, // PolicyName: E-Eur
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1997,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    Moldova    EE%sT
	{
		PolicyIndex: 70, // PolicyName: Moldova
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Dublin
	// EraIndex: 889
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              1:00    Eire    IST/GMT
	{
		PolicyIndex: 39, // PolicyName: Eire
		FormatIndex: 108, // "IST/GMT"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Gibraltar
	// EraIndex: 890
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    -    CET    1982
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 91, // "CET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1982,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Helsinki
	// EraIndex: 892
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             2:00    Finland    EE%sT    1983
	{
		PolicyIndex: 42, // PolicyName: Finland
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1983,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Istanbul
	// EraIndex: 894
	// EraCount: 11
	// ---------------------------------------------------------------------------

	//             2:00    Turkey    EE%sT    1978 Jun 29
	{
		PolicyIndex: 107, // PolicyName: Turkey
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1978,
		UntilMonth: 6,
		UntilDay: 29,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00    Turkey    +03/+04    1984 Nov  1  2:00
	{
		PolicyIndex: 107, // PolicyName: Turkey
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1984,
		UntilMonth: 11,
		UntilDay: 1,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    Turkey    EE%sT    2007
	{
		PolicyIndex: 107, // PolicyName: Turkey
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2007,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT    2011 Mar 27  1:00u
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             2:00    -    EET    2011 Mar 28  1:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 28,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             2:00    EU    EE%sT    2014 Mar 30  1:00u
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2014,
		UntilMonth: 3,
		UntilDay: 30,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             2:00    -    EET    2014 Mar 31  1:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2014,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             2:00    EU    EE%sT    2015 Oct 25  1:00u
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2015,
		UntilMonth: 10,
		UntilDay: 25,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             2:00    1:00    EEST    2015 Nov  8  1:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 97, // "EEST"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2015,
		UntilMonth: 11,
		UntilDay: 8,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             2:00    EU    EE%sT    2016 Sep  7
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2016,
		UntilMonth: 9,
		UntilDay: 7,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00    -    +03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Kaliningrad
	// EraIndex: 905
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//              3:00    Russia    MSK/MSD    1989 Mar 26  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1989,
		UntilMonth: 3,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              2:00    Russia    EE%sT    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    +03    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              2:00    -    EET
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Kirov
	// EraIndex: 909
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//              4:00    Russia    +04/+05    1989 Mar 26  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1989,
		UntilMonth: 3,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04    1992 Mar 29  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 3,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    +03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Kyiv
	// EraIndex: 915
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             3:00    Russia    MSK/MSD    1990 Jul  1  2:00
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1990,
		UntilMonth: 7,
		UntilDay: 1,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    1:00    EEST    1991 Sep 29  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 97, // "EEST"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1991,
		UntilMonth: 9,
		UntilDay: 29,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    C-Eur    EE%sT    1996 May 13
	{
		PolicyIndex: 20, // PolicyName: C-Eur
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1996,
		UntilMonth: 5,
		UntilDay: 13,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Lisbon
	// EraIndex: 919
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//              1:00    -    CET    1976 Sep 26  1:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 91, // "CET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1976,
		UntilMonth: 9,
		UntilDay: 26,
		UntilTimeCode: 4,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              0:00    Port    WE%sT    1983 Sep 25  1:00s
	{
		PolicyIndex: 86, // PolicyName: Port
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1983,
		UntilMonth: 9,
		UntilDay: 25,
		UntilTimeCode: 4,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              0:00    W-Eur    WE%sT    1992 Sep 27  1:00s
	{
		PolicyIndex: 112, // PolicyName: W-Eur
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1992,
		UntilMonth: 9,
		UntilDay: 27,
		UntilTimeCode: 4,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              1:00    EU    CE%sT    1996 Mar 31  1:00u
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1996,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//              0:00    EU    WE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/London
	// EraIndex: 924
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//              1:00    -    BST    1971 Oct 31  2:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 85, // "BST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1971,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//              0:00    GB-Eire    %s    1996
	{
		PolicyIndex: 44, // PolicyName: GB-Eire
		FormatIndex: 1, // "%"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 1996,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              0:00    EU    GMT/BST
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 102, // "GMT/BST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Madrid
	// EraIndex: 927
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//              1:00    Spain    CE%sT    1979
	{
		PolicyIndex: 94, // PolicyName: Spain
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1979,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Malta
	// EraIndex: 929
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             1:00    Italy    CE%sT    1973 Mar 31
	{
		PolicyIndex: 57, // PolicyName: Italy
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1973,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    Malta    CE%sT    1981
	{
		PolicyIndex: 66, // PolicyName: Malta
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1981,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Minsk
	// EraIndex: 932
	// EraCount: 4
	// ---------------------------------------------------------------------------

	//             3:00    Russia    MSK/MSD    1990
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1990,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             3:00    -    MSK    1991 Mar 31  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 116, // "MSK"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             2:00    Russia    EE%sT    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             3:00    -    +03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Moscow
	// EraIndex: 936
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//              3:00    Russia    MSK/MSD    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              2:00    Russia    EE%sT    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    MSK/MSD    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    MSK    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 116, // "MSK"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    MSK
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 116, // "MSK"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Paris
	// EraIndex: 941
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    France    CE%sT    1977
	{
		PolicyIndex: 43, // PolicyName: France
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1977,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Prague
	// EraIndex: 943
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    Czech    CE%sT    1979
	{
		PolicyIndex: 29, // PolicyName: Czech
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1979,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Riga
	// EraIndex: 945
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             3:00    Russia    MSK/MSD    1989 Mar lastSun  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1989,
		UntilMonth: 3,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             2:00    1:00    EEST    1989 Sep lastSun  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 97, // "EEST"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1989,
		UntilMonth: 9,
		UntilDay: 24,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             2:00    Latvia    EE%sT    1997 Jan 21
	{
		PolicyIndex: 62, // PolicyName: Latvia
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1997,
		UntilMonth: 1,
		UntilDay: 21,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT    2000 Feb 29
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2000,
		UntilMonth: 2,
		UntilDay: 29,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    EET    2001 Jan  2
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2001,
		UntilMonth: 1,
		UntilDay: 2,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Rome
	// EraIndex: 951
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    Italy    CE%sT    1980
	{
		PolicyIndex: 57, // PolicyName: Italy
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1980,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Samara
	// EraIndex: 953
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//              4:00    Russia    +04/+05    1989 Mar 26  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1989,
		UntilMonth: 3,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              2:00    Russia    +02/+03    1991 Sep 29  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 6, // "+02/+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1991,
		UntilMonth: 9,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    +03    1991 Oct 20  3:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1991,
		UntilMonth: 10,
		UntilDay: 20,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              4:00    Russia    +04/+05    2010 Mar 28  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2010,
		UntilMonth: 3,
		UntilDay: 28,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Saratov
	// EraIndex: 960
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//              4:00    Russia    +04/+05    1988 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1988,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04    1992 Mar 29  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 3,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    +03    2016 Dec  4  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2016,
		UntilMonth: 12,
		UntilDay: 4,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Simferopol
	// EraIndex: 967
	// EraCount: 10
	// ---------------------------------------------------------------------------

	//              3:00    Russia    MSK/MSD    1990
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1990,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              3:00    -    MSK    1990 Jul  1  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 116, // "MSK"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1990,
		UntilMonth: 7,
		UntilDay: 1,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              2:00    -    EET    1992 Mar 20
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1992,
		UntilMonth: 3,
		UntilDay: 20,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              2:00    C-Eur    EE%sT    1994 May
	{
		PolicyIndex: 20, // PolicyName: C-Eur
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1994,
		UntilMonth: 5,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              3:00    C-Eur    MSK/MSD    1996 Mar 31  0:00s
	{
		PolicyIndex: 20, // PolicyName: C-Eur
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1996,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 0,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    1:00    MSD    1996 Oct 27  3:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 115, // "MSD"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1996,
		UntilMonth: 10,
		UntilDay: 27,
		UntilTimeCode: 12,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    MSK    1997 Mar lastSun  1:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 116, // "MSK"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1997,
		UntilMonth: 3,
		UntilDay: 30,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//              2:00    EU    EE%sT    2014 Mar 30  2:00
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2014,
		UntilMonth: 3,
		UntilDay: 30,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              4:00    -    MSK    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 116, // "MSK"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    MSK
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 116, // "MSK"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Sofia
	// EraIndex: 977
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             2:00    -    EET    1979 Mar 31 23:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1979,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 92,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    Bulg    EE%sT    1982 Sep 26  3:00
	{
		PolicyIndex: 19, // PolicyName: Bulg
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1982,
		UntilMonth: 9,
		UntilDay: 26,
		UntilTimeCode: 12,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    C-Eur    EE%sT    1991
	{
		PolicyIndex: 20, // PolicyName: C-Eur
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1991,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    E-Eur    EE%sT    1997
	{
		PolicyIndex: 32, // PolicyName: E-Eur
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1997,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Tallinn
	// EraIndex: 982
	// EraCount: 6
	// ---------------------------------------------------------------------------

	//             3:00    Russia    MSK/MSD    1989 Mar 26  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1989,
		UntilMonth: 3,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             2:00    1:00    EEST    1989 Sep 24  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 97, // "EEST"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1989,
		UntilMonth: 9,
		UntilDay: 24,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             2:00    C-Eur    EE%sT    1998 Sep 22
	{
		PolicyIndex: 20, // PolicyName: C-Eur
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1998,
		UntilMonth: 9,
		UntilDay: 22,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT    1999 Oct 31  4:00
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 16,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    EET    2002 Feb 21
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2002,
		UntilMonth: 2,
		UntilDay: 21,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Tirane
	// EraIndex: 988
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    Albania    CE%sT    1984 Jul
	{
		PolicyIndex: 7, // PolicyName: Albania
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1984,
		UntilMonth: 7,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Ulyanovsk
	// EraIndex: 990
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//              4:00    Russia    +04/+05    1989 Mar 26  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1989,
		UntilMonth: 3,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              2:00    Russia    +02/+03    1992 Jan 19  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 6, // "+02/+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1992,
		UntilMonth: 1,
		UntilDay: 19,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    +03    2016 Mar 27  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2016,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Vienna
	// EraIndex: 997
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    Austria    CE%sT    1981
	{
		PolicyIndex: 12, // PolicyName: Austria
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1981,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Vilnius
	// EraIndex: 999
	// EraCount: 7
	// ---------------------------------------------------------------------------

	//             3:00    Russia    MSK/MSD    1989 Mar 26  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 117, // "MSK/MSD"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1989,
		UntilMonth: 3,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             2:00    Russia    EE%sT    1991 Sep 29  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1991,
		UntilMonth: 9,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             2:00    C-Eur    EE%sT    1998
	{
		PolicyIndex: 20, // PolicyName: C-Eur
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1998,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    -    EET    1998 Mar 29  1:00u
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 1998,
		UntilMonth: 3,
		UntilDay: 29,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             1:00    EU    CE%sT    1999 Oct 31  1:00u
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1999,
		UntilMonth: 10,
		UntilDay: 31,
		UntilTimeCode: 4,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             2:00    -    EET    2003 Jan  1
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 98, // "EET"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 2003,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 96, // "EE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 480, // (offset_seconds=7200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Volgograd
	// EraIndex: 1006
	// EraCount: 8
	// ---------------------------------------------------------------------------

	//              4:00    Russia    +04/+05    1988 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1988,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    1991 Mar 31  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 1991,
		UntilMonth: 3,
		UntilDay: 31,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04    1992 Mar 29  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 1992,
		UntilMonth: 3,
		UntilDay: 29,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    Russia    +03/+04    2011 Mar 27  2:00s
	{
		PolicyIndex: 89, // PolicyName: Russia
		FormatIndex: 8, // "+03/+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2011,
		UntilMonth: 3,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04    2014 Oct 26  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2014,
		UntilMonth: 10,
		UntilDay: 26,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    +03    2018 Oct 28  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 2018,
		UntilMonth: 10,
		UntilDay: 28,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              4:00    -    +04    2020 Dec 27  2:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "+04"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 2020,
		UntilMonth: 12,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//              3:00    -    +03
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "+03"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 720, // (offset_seconds=10800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Warsaw
	// EraIndex: 1014
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             1:00    Poland    CE%sT    1977
	{
		PolicyIndex: 85, // PolicyName: Poland
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1977,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    W-Eur    CE%sT    1988
	{
		PolicyIndex: 112, // PolicyName: W-Eur
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1988,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Zurich
	// EraIndex: 1017
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             1:00    Swiss    CE%sT    1981
	{
		PolicyIndex: 99, // PolicyName: Swiss
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 1981,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 90, // "CE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: HST
	// EraIndex: 1019
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -10:00 - HST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 105, // "HST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2400, // (offset_seconds=-36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Indian/Chagos
	// EraIndex: 1020
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             5:00    -    +05    1996
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 1996,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             6:00    -    +06
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 17, // "+06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1440, // (offset_seconds=21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Indian/Maldives
	// EraIndex: 1022
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    +05
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 1200, // (offset_seconds=18000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Indian/Mauritius
	// EraIndex: 1023
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             4:00 Mauritius    +04/+05
	{
		PolicyIndex: 67, // PolicyName: Mauritius
		FormatIndex: 11, // "+04/+05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 960, // (offset_seconds=14400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: MET
	// EraIndex: 1024
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 1:00 C-Eur ME%sT
	{
		PolicyIndex: 20, // PolicyName: C-Eur
		FormatIndex: 113, // "ME%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 240, // (offset_seconds=3600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: MST
	// EraIndex: 1025
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -7:00 - MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 118, // "MST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: MST7MDT
	// EraIndex: 1026
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -7:00 US M%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 112, // "M%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: PST8PDT
	// EraIndex: 1027
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -8:00 US P%sT
	{
		PolicyIndex: 108, // PolicyName: US
		FormatIndex: 121, // "P%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Apia
	// EraIndex: 1028
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -11:00    WS    -11/-10    2011 Dec 29 24:00
	{
		PolicyIndex: 113, // PolicyName: WS
		FormatIndex: 71, // "-11/-10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2640, // (offset_seconds=-39600)/15
		UntilYear: 2011,
		UntilMonth: 12,
		UntilDay: 29,
		UntilTimeCode: 96,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              13:00    WS    +13/+14
	{
		PolicyIndex: 113, // PolicyName: WS
		FormatIndex: 40, // "+13/+14"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 3120, // (offset_seconds=46800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Auckland
	// EraIndex: 1030
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             12:00    NZ    NZ%sT
	{
		PolicyIndex: 76, // PolicyName: NZ
		FormatIndex: 120, // "NZ%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Bougainville
	// EraIndex: 1031
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             10:00    -    +10    2014 Dec 28  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 28, // "+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2014,
		UntilMonth: 12,
		UntilDay: 28,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             11:00    -    +11
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Chatham
	// EraIndex: 1033
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             12:45    Chatham    +1245/+1345
	{
		PolicyIndex: 24, // PolicyName: Chatham
		FormatIndex: 38, // "+1245/+1345"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 3060, // (offset_seconds=45900)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Easter
	// EraIndex: 1034
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -7:00    Chile    -07/-06    1982 Mar 14 3:00u
	{
		PolicyIndex: 25, // PolicyName: Chile
		FormatIndex: 61, // "-07/-06"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1680, // (offset_seconds=-25200)/15
		UntilYear: 1982,
		UntilMonth: 3,
		UntilDay: 14,
		UntilTimeCode: 12,
		UntilTimeModifier: 32, // SuffixU + minute=0
	},

	//             -6:00    Chile    -06/-05
	{
		PolicyIndex: 25, // PolicyName: Chile
		FormatIndex: 59, // "-06/-05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Efate
	// EraIndex: 1036
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    Vanuatu    +11/+12
	{
		PolicyIndex: 111, // PolicyName: Vanuatu
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Fakaofo
	// EraIndex: 1037
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -11:00    -    -11    2011 Dec 30
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 70, // "-11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2640, // (offset_seconds=-39600)/15
		UntilYear: 2011,
		UntilMonth: 12,
		UntilDay: 30,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             13:00    -    +13
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 39, // "+13"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 3120, // (offset_seconds=46800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Fiji
	// EraIndex: 1039
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             12:00    Fiji    +12/+13
	{
		PolicyIndex: 41, // PolicyName: Fiji
		FormatIndex: 36, // "+12/+13"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Galapagos
	// EraIndex: 1040
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -5:00    -    -05    1986
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 56, // "-05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1200, // (offset_seconds=-18000)/15
		UntilYear: 1986,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -6:00    Ecuador    -06/-05
	{
		PolicyIndex: 36, // PolicyName: Ecuador
		FormatIndex: 59, // "-06/-05"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1440, // (offset_seconds=-21600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Gambier
	// EraIndex: 1042
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              -9:00    -    -09
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 64, // "-09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2160, // (offset_seconds=-32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Guadalcanal
	// EraIndex: 1043
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    -    +11
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Guam
	// EraIndex: 1044
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             10:00    Guam    G%sT    2000 Dec 23
	{
		PolicyIndex: 47, // PolicyName: Guam
		FormatIndex: 100, // "G%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 2000,
		UntilMonth: 12,
		UntilDay: 23,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             10:00    -    ChST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 93, // "ChST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Honolulu
	// EraIndex: 1046
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -10:00    -    HST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 105, // "HST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2400, // (offset_seconds=-36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Kanton
	// EraIndex: 1047
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -12:00    -    -12    1979 Oct
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 72, // "-12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2880, // (offset_seconds=-43200)/15
		UntilYear: 1979,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -11:00    -    -11    1994 Dec 31
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 70, // "-11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2640, // (offset_seconds=-39600)/15
		UntilYear: 1994,
		UntilMonth: 12,
		UntilDay: 31,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              13:00    -    +13
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 39, // "+13"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 3120, // (offset_seconds=46800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Kiritimati
	// EraIndex: 1050
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//             -10:40    -    -1040    1979 Oct
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 69, // "-1040"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2560, // (offset_seconds=-38400)/15
		UntilYear: 1979,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -10:00    -    -10    1994 Dec 31
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 66, // "-10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2400, // (offset_seconds=-36000)/15
		UntilYear: 1994,
		UntilMonth: 12,
		UntilDay: 31,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              14:00    -    +14
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 41, // "+14"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 3360, // (offset_seconds=50400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Kosrae
	// EraIndex: 1053
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//              11:00    -    +11    1969 Oct
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              12:00    -    +12    1999
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 35, // "+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 1999,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              11:00    -    +11
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Kwajalein
	// EraIndex: 1056
	// EraCount: 3
	// ---------------------------------------------------------------------------

	//              11:00    -    +11    1969 Oct
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 1969,
		UntilMonth: 10,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -12:00    -    -12    1993 Aug 20 24:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 72, // "-12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2880, // (offset_seconds=-43200)/15
		UntilYear: 1993,
		UntilMonth: 8,
		UntilDay: 20,
		UntilTimeCode: 96,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//              12:00    -    +12
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 35, // "+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Marquesas
	// EraIndex: 1059
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              -9:30    -    -0930
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 65, // "-0930"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2280, // (offset_seconds=-34200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Nauru
	// EraIndex: 1060
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             11:30    -    +1130    1979 Feb 10  2:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 34, // "+1130"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2760, // (offset_seconds=41400)/15
		UntilYear: 1979,
		UntilMonth: 2,
		UntilDay: 10,
		UntilTimeCode: 8,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             12:00    -    +12
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 35, // "+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Niue
	// EraIndex: 1062
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -11:00    -    -11
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 70, // "-11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2640, // (offset_seconds=-39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Norfolk
	// EraIndex: 1063
	// EraCount: 5
	// ---------------------------------------------------------------------------

	//             11:30    -    +1130    1974 Oct 27 02:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 34, // "+1130"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2760, // (offset_seconds=41400)/15
		UntilYear: 1974,
		UntilMonth: 10,
		UntilDay: 27,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:30    1:00    +1230    1975 Mar  2 02:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 37, // "+1230"
		DeltaCode: 8, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=60)/15 + 4)
		OffsetSecondsCode: 2760, // (offset_seconds=41400)/15
		UntilYear: 1975,
		UntilMonth: 3,
		UntilDay: 2,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:30    -    +1130    2015 Oct  4 02:00s
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 34, // "+1130"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2760, // (offset_seconds=41400)/15
		UntilYear: 2015,
		UntilMonth: 10,
		UntilDay: 4,
		UntilTimeCode: 8,
		UntilTimeModifier: 16, // SuffixS + minute=0
	},

	//             11:00    -    +11    2019 Jul
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "+11"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 2019,
		UntilMonth: 7,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             11:00    AN    +11/+12
	{
		PolicyIndex: 1, // PolicyName: AN
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Noumea
	// EraIndex: 1068
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    NC    +11/+12
	{
		PolicyIndex: 74, // PolicyName: NC
		FormatIndex: 33, // "+11/+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2640, // (offset_seconds=39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Pago_Pago
	// EraIndex: 1069
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -11:00    -    SST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 125, // "SST"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2640, // (offset_seconds=-39600)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Palau
	// EraIndex: 1070
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//               9:00    -    +09
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 26, // "+09"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2160, // (offset_seconds=32400)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Pitcairn
	// EraIndex: 1071
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -8:30    -    -0830    1998 Apr 27  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 63, // "-0830"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2040, // (offset_seconds=-30600)/15
		UntilYear: 1998,
		UntilMonth: 4,
		UntilDay: 27,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -8:00    -    -08
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 62, // "-08"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -1920, // (offset_seconds=-28800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Port_Moresby
	// EraIndex: 1073
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    -    +10
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 28, // "+10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2400, // (offset_seconds=36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Rarotonga
	// EraIndex: 1074
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -10:30    -    -1030    1978 Nov 12
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 68, // "-1030"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2520, // (offset_seconds=-37800)/15
		UntilYear: 1978,
		UntilMonth: 11,
		UntilDay: 12,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             -10:00    Cook    -10/-0930
	{
		PolicyIndex: 26, // PolicyName: Cook
		FormatIndex: 67, // "-10/-0930"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2400, // (offset_seconds=-36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Tahiti
	// EraIndex: 1076
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -10:00    -    -10
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 66, // "-10"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: -2400, // (offset_seconds=-36000)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Tarawa
	// EraIndex: 1077
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              12:00    -    +12
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 35, // "+12"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 2880, // (offset_seconds=43200)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Tongatapu
	// EraIndex: 1078
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             13:00    -    +13    1999
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 39, // "+13"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 3120, // (offset_seconds=46800)/15
		UntilYear: 1999,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	//             13:00    Tonga    +13/+14
	{
		PolicyIndex: 103, // PolicyName: Tonga
		FormatIndex: 40, // "+13/+14"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 3120, // (offset_seconds=46800)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: WET
	// EraIndex: 1080
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 0:00 EU WE%sT
	{
		PolicyIndex: 34, // PolicyName: EU
		FormatIndex: 129, // "WE%T"
		DeltaCode: 4, // ((offset_seconds_remainder=0) << 4) + ((delta_minutes=0)/15 + 4)
		OffsetSecondsCode: 0, // (offset_seconds=0)/15
		UntilYear: 10000,
		UntilMonth: 1,
		UntilDay: 1,
		UntilTimeCode: 0,
		UntilTimeModifier: 0, // SuffixW + minute=0
	},


}

const ZoneEraCount = 1081

const ZoneEraChunkSize = 12

// ZoneErasData contains the ZoneEraRecords data as a hex encoded string.
const ZoneErasData = "\x65\x00\x00\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x81\x00\x08\x04\x00\x00\xb9\x07\x0a\x15\x00\x00" +
		"\x5a\x00\x08\x04\xf0\x00\xbb\x07\x0a\x1a\x00\x00" +
		"\x81\x00\x08\x04\x00\x00\xbd\x07\x05\x01\x00\x00" +
		"\x5b\x00\x00\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x2b\x00\x00\x04\x10\xff\xb7\x07\x01\x01\x00\x00" +
		"\x65\x00\x00\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x26\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x02\x00\x49\x04\x00\x00\xc0\x07\x03\x10\x00\x00" +
		"\x03\x00\x00\x04\xf0\x00\xc2\x07\x01\x01\x00\x00" +
		"\x02\x00\x49\x04\x00\x00\xe2\x07\x0a\x1c\x0c\x00" +
		"\x04\x00\x49\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x81\x00\x5f\x04\x00\x00\xc0\x07\x03\x10\x00\x00" +
		"\x5b\x00\x00\x04\xf0\x00\xc2\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x2b\x00\x00\x04\x10\xff\xb8\x07\x04\x0e\x00\x00" +
		"\x02\x00\x49\x04\x00\x00\xe2\x07\x0a\x1c\x0c\x00" +
		"\x04\x00\x49\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x7c\x00\x5b\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x57\x00\x61\x04\xe0\x01\xd0\x07\x01\x0f\x30\x00" +
		"\x5f\x00\x00\x04\xd0\x02\xe5\x07\x02\x01\x00\x00" +
		"\x58\x00\x00\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x57\x00\x61\x04\xe0\x01\xd0\x07\x01\x0f\x30\x00" +
		"\x5f\x00\x00\x04\xd0\x02\xe1\x07\x0b\x01\x00\x00" +
		"\x58\x00\x00\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x80\x00\x00\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x58\x00\x00\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x72\x00\x00\x04\x4e\xff\xb4\x07\x01\x07\x00\x00" +
		"\x65\x00\x00\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x5f\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x80\x00\x00\x04\xf0\x00\xbb\x07\x0a\x0e\x00\x00" +
		"\x7f\x00\x00\x08\xf0\x00\xbc\x07\x03\x08\x00\x00" +
		"\x80\x00\x00\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x65\x00\x00\x04\x00\x00\xe2\x07\x01\x01\x04\x00" +
		"\x80\x00\x00\x04\xf0\x00\xe3\x07\x01\x01\x08\x00" +
		"\x65\x00\x00\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x62\x00\x00\x04\xe0\x01\xbe\x07\x01\x01\x00\x00" +
		"\x5a\x00\x40\x04\xf0\x00\xc6\x07\x05\x04\x00\x00" +
		"\x62\x00\x00\x04\xe0\x01\xcc\x07\x09\x1e\x00\x00" +
		"\x5a\x00\x40\x04\xf0\x00\xcd\x07\x0a\x04\x00\x00" +
		"\x62\x00\x00\x04\xe0\x01\xdc\x07\x0b\x0a\x08\x00" +
		"\x5a\x00\x40\x04\xf0\x00\xdd\x07\x0a\x19\x08\x00" +
		"\x62\x00\x00\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x6a\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x7c\x00\x00\x04\xe0\x01\xc6\x07\x03\x15\x00\x00" +
		"\x01\x00\x4d\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x55\x00\x00\x04\xb0\xf5\xb1\x07\x01\x01\x00\x00" +
		"\x54\x00\x6c\x04\xb0\xf5\xbf\x07\x0a\x1e\x08\x00" +
		"\x4f\x00\x6c\x04\xa0\xf6\xbf\x07\x0b\x1e\x00\x00" +
		"\x67\x00\x6c\x04\xa0\xf6\x10\x27\x01\x01\x00\x00" +
		"\x50\x00\x00\x04\xa0\xf6\xb1\x07\x01\x01\x00\x00" +
		"\x4f\x00\x6c\x04\xa0\xf6\xbf\x07\x0a\x1e\x08\x00" +
		"\x87\x00\x6c\x04\x90\xf7\xbf\x07\x0b\x1e\x00\x00" +
		"\x51\x00\x6c\x04\x90\xf7\x10\x27\x01\x01\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xc6\x07\x09\x11\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xcb\x07\x09\x0e\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xd3\x07\x09\x18\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xdc\x07\x0a\x15\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xdd\x07\x09\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xc7\x07\x03\x03\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc7\x07\x0a\x14\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd4\x07\x06\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd4\x07\x06\x14\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xd8\x07\x0a\x12\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xc7\x07\x03\x03\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc7\x07\x0a\x14\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xc6\x07\x03\x04\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc6\x07\x0a\x1c\x00\x00" +
		"\x2f\x00\x00\x08\x40\xfc\xc7\x07\x03\x11\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc7\x07\x0a\x06\x00\x00" +
		"\x2d\x00\x00\x08\x30\xfd\xc8\x07\x01\x01\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xd8\x07\x0a\x12\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xc7\x07\x03\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc7\x07\x05\x07\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd4\x07\x06\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd4\x07\x06\x14\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xd8\x07\x0a\x12\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xc6\x07\x03\x04\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc6\x07\x0a\x0f\x00\x00" +
		"\x2f\x00\x00\x08\x40\xfc\xc7\x07\x03\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc7\x07\x0a\x0f\x00\x00" +
		"\x2f\x00\x00\x08\x40\xfc\xc8\x07\x03\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc8\x07\x0a\x12\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd4\x07\x05\x17\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd4\x07\x09\x1a\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xd8\x07\x0a\x12\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd4\x07\x06\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd4\x07\x06\x14\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xd8\x07\x0a\x12\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xc7\x07\x03\x03\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc7\x07\x0a\x14\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xd8\x07\x0a\x12\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xc7\x07\x03\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc7\x07\x05\x07\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd4\x07\x05\x1f\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd4\x07\x07\x19\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xd8\x07\x0a\x12\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xc6\x07\x01\x01\x00\x00" +
		"\x2d\x00\x00\x08\x30\xfd\xc6\x07\x03\x0e\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc6\x07\x0a\x0f\x00\x00" +
		"\x2f\x00\x00\x08\x40\xfc\xc7\x07\x03\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc7\x07\x06\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x2f\x00\x00\x08\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd4\x07\x05\x1f\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd4\x07\x07\x19\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xd8\x07\x01\x15\x00\x00" +
		"\x36\x00\x5d\x04\x40\xfc\xd9\x07\x0a\x0b\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xc7\x07\x03\x03\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc7\x07\x0a\x14\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd4\x07\x06\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd4\x07\x06\x0d\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xcf\x07\x0a\x03\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xd0\x07\x03\x03\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd4\x07\x05\x1e\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd4\x07\x06\x14\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xd8\x07\x0a\x12\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xb4\x07\x0a\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xb6\x07\x04\x01\x00\x00" +
		"\x36\x00\x52\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xd3\x07\x09\x18\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xdb\x07\x0a\x10\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xdc\x07\x0a\x15\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xb2\x07\x01\x01\x00\x00" +
		"\x70\x00\x45\x04\x70\xf9\xda\x07\x04\x04\x08\x00" +
		"\x56\x00\x45\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x49\x00\x0e\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xc4\x07\x09\x0c\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x01\x00\x10\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x12\x04\x40\xfc\xc4\x07\x09\x0c\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xcf\x07\x09\x1e\x00\x00" +
		"\x36\x00\x12\x04\x40\xfc\xd0\x07\x0a\x0f\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x39\x00\x15\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x70\x00\x6c\x04\x70\xf9\xb6\x07\x01\x01\x00\x00" +
		"\x76\x00\x00\x04\x70\xf9\xb6\x07\x02\x03\x08\x00" +
		"\x70\x00\x6c\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x70\x00\x4b\x04\x70\xf9\xcf\x07\x0a\x1f\x08\x00" +
		"\x56\x00\x17\x04\x60\xfa\xd0\x07\x0a\x1d\x08\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd0\x07\x0b\x05\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xd1\x07\x04\x01\x0c\x00" +
		"\x70\x00\x17\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x12\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xbd\x07\x0c\x17\x00\x00" +
		"\x5e\x00\x45\x04\x50\xfb\xce\x07\x08\x02\x08\x00" +
		"\x56\x00\x45\x04\x60\xfa\xdf\x07\x02\x01\x08\x00" +
		"\x63\x00\x00\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd7\x07\x0c\x09\x0c\x00" +
		"\x37\x00\x00\x04\xc8\xfb\xe0\x07\x05\x01\x0a\x00" +
		"\x35\x00\x00\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x6c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xcc\x07\x01\x01\x00\x00" +
		"\x56\x00\x45\x04\x60\xfa\xce\x07\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xce\x07\x04\x05\x0c\x00" +
		"\x70\x00\x45\x04\x70\xf9\xe6\x07\x0a\x1e\x08\x00" +
		"\x5c\x00\x00\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xcc\x07\x01\x01\x00\x00" +
		"\x56\x00\x45\x04\x60\xfa\xce\x07\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xce\x07\x04\x05\x0c\x00" +
		"\x70\x00\x45\x04\x70\xf9\xda\x07\x01\x01\x00\x00" +
		"\x70\x00\x6c\x04\x70\xf9\xe6\x07\x0a\x1e\x08\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xe6\x07\x0b\x1e\x00\x00" +
		"\x70\x00\x6c\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x16\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x12\x04\x40\xfc\xd3\x07\x09\x18\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd4\x07\x0a\x01\x00\x00" +
		"\x36\x00\x12\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xbc\x07\x04\x06\x08\x00" +
		"\x31\x00\x22\x04\x30\xfd\xcc\x07\x01\x01\x00\x00" +
		"\x65\x00\x00\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x87\x00\x73\x04\x90\xf7\xb5\x07\x0a\x1c\x00\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xbc\x07\x01\x01\x00\x00" +
		"\x79\x00\x17\x04\x80\xf8\xe4\x07\x0b\x01\x00\x00" +
		"\x76\x00\x00\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x79\x00\x6e\x04\x80\xf8\xb4\x07\x08\x1e\x08\x00" +
		"\x76\x00\x00\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x70\x00\x6c\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xb1\x07\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xb5\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xb7\x07\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xb7\x07\x04\x1b\x08\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x70\x00\x25\x04\x70\xf9\xc3\x07\x01\x01\x00\x00" +
		"\x70\x00\x17\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x39\x00\x12\x04\x50\xfb\xc4\x07\x09\x0c\x00\x00" +
		"\x38\x00\x00\x04\x50\xfb\xc9\x07\x09\x1c\x00\x00" +
		"\x39\x00\x12\x04\x50\xfb\xca\x07\x09\x16\x00\x00" +
		"\x38\x00\x00\x04\x50\xfb\xd8\x07\x06\x18\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xdd\x07\x0b\x0a\x00\x00" +
		"\x38\x00\x00\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x5c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x79\x00\x6e\x04\x80\xf8\xc3\x07\x01\x01\x00\x00" +
		"\x79\x00\x17\x04\x80\xf8\xdf\x07\x03\x08\x08\x00" +
		"\x76\x00\x00\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xc6\x07\x09\x11\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xcf\x07\x09\x1e\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xd0\x07\x0a\x16\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd1\x07\x09\x0d\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xd2\x07\x0a\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x52\x00\x00\x04\x40\xfc\xb4\x07\x01\x01\x00\x00" +
		"\x49\x00\x33\x04\x40\xfc\xb6\x07\x01\x01\x00\x00" +
		"\x49\x00\x17\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x49\x00\x60\x04\x40\xfc\xdb\x07\x0b\x01\x00\x00" +
		"\x49\x00\x17\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xbb\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xdf\x07\x03\x08\x08\x00" +
		"\x52\x00\x00\x04\x40\xfc\xe2\x07\x03\x0b\x0c\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x30\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x39\x00\x24\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x34\x00\x00\x04\x7c\xfc\xb7\x07\x08\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xc8\x07\x03\x1d\x04\x00" +
		"\x35\x00\x00\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x49\x00\x33\x04\x40\xfc\xb6\x07\x01\x01\x00\x00" +
		"\x49\x00\x17\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x1b\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xb2\x07\x01\x01\x00\x00" +
		"\x70\x00\x45\x04\x70\xf9\xcf\x07\x01\x01\x00\x00" +
		"\x76\x00\x00\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xb1\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xb3\x07\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd6\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x6c\x04\x60\xfa\xc7\x07\x0a\x1b\x08\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd6\x07\x04\x02\x08\x00" +
		"\x56\x00\x6c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xb1\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xb6\x07\x01\x06\x08\x00" +
		"\x59\x00\x00\x08\x60\xfa\xb6\x07\x0a\x1b\x08\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xb8\x07\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd6\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x6c\x04\x60\xfa\xb9\x07\x0a\x1e\x08\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd6\x07\x04\x02\x08\x00" +
		"\x56\x00\x6c\x04\x60\xfa\xd7\x07\x0b\x04\x08\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x6c\x04\x60\xfa\xb1\x07\x04\x1b\x08\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xb3\x07\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd6\x07\x04\x02\x08\x00" +
		"\x56\x00\x6c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xb1\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xb5\x07\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd6\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xb1\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xb3\x07\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd6\x07\x04\x02\x08\x00" +
		"\x56\x00\x6c\x04\x60\xfa\xd7\x07\x0b\x04\x08\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xb1\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xb3\x07\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd6\x07\x04\x02\x08\x00" +
		"\x56\x00\x6c\x04\x60\xfa\xd7\x07\x03\x0b\x08\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x79\x00\x4b\x04\x80\xf8\xbb\x07\x04\x1d\x08\x00" +
		"\x70\x00\x4b\x04\x70\xf9\xbc\x07\x01\x01\x00\x00" +
		"\x70\x00\x17\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x5e\x00\x4b\x04\x50\xfb\xcf\x07\x0a\x1f\x08\x00" +
		"\x56\x00\x17\x04\x60\xfa\xd0\x07\x0a\x1d\x08\x00" +
		"\x5e\x00\x17\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xb6\x07\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xc0\x07\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xb1\x07\x01\x01\x00\x00" +
		"\x79\x00\x6c\x04\x80\xf8\xbc\x07\x04\x1b\x08\x00" +
		"\x87\x00\x6c\x04\x90\xf7\xbc\x07\x0a\x1a\x08\x00" +
		"\x79\x00\x6c\x04\x80\xf8\xbf\x07\x0a\x1e\x08\x00" +
		"\x87\x00\x6c\x04\x90\xf7\xbf\x07\x0b\x1e\x00\x00" +
		"\x51\x00\x6c\x04\x90\xf7\x10\x27\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xb6\x07\x01\x06\x08\x00" +
		"\x59\x00\x00\x08\x60\xfa\xb6\x07\x0a\x1b\x08\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x6c\x04\x60\xfa\xd0\x07\x0a\x1d\x08\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x39\x00\x53\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x79\x00\x6c\x04\x80\xf8\x10\x27\x01\x01\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xc6\x07\x09\x11\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xcb\x07\x0a\x0d\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xcc\x07\x09\x04\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xcf\x07\x09\x1e\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xd0\x07\x0a\x16\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd1\x07\x09\x0d\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xd2\x07\x0a\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xb5\x07\x05\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xb7\x07\x02\x10\x00\x00" +
		"\x56\x00\x4e\x04\x60\xfa\xc8\x07\x01\x01\x10\x00" +
		"\x63\x00\x00\x04\x50\xfb\xc8\x07\x09\x18\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xc9\x07\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xcd\x07\x01\x01\x00\x00" +
		"\x56\x00\x4e\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x12\x04\x40\xfc\xc4\x07\x09\x0c\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xc9\x07\x09\x1c\x00\x00" +
		"\x36\x00\x12\x04\x40\xfc\xca\x07\x09\x16\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x52\x00\x00\x04\x40\xfc\xbc\x07\x04\x06\x00\x00" +
		"\x4b\x00\x00\x08\x40\xfc\xbc\x07\x09\x1c\x00\x00" +
		"\x52\x00\x00\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xc4\x07\x01\x01\x00\x00" +
		"\x56\x00\x6c\x04\x60\xfa\xc5\x07\x01\x01\x00\x00" +
		"\x56\x00\x45\x04\x60\xfa\xda\x07\x01\x01\x00\x00" +
		"\x56\x00\x6c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xb2\x07\x01\x01\x00\x00" +
		"\x70\x00\x45\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x44\x04\x60\xfa\xb1\x07\x04\x1b\x08\x00" +
		"\x63\x00\x00\x04\x50\xfb\xb5\x07\x04\x1d\x08\x00" +
		"\x56\x00\x6c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xbd\x07\x0c\x17\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\xbe\x07\x0c\x02\x00\x00" +
		"\x56\x00\x45\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xb1\x07\x01\x01\x00\x00" +
		"\x79\x00\x6c\x04\x80\xf8\xbf\x07\x0a\x1e\x08\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xdf\x07\x0b\x01\x08\x00" +
		"\x51\x00\x6c\x04\x90\xf7\xe2\x07\x0b\x04\x08\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xe3\x07\x01\x14\x08\x00" +
		"\x51\x00\x6c\x04\x90\xf7\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x45\x04\x60\xfa\xd1\x07\x09\x1e\x08\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xd2\x07\x02\x14\x00\x00" +
		"\x56\x00\x45\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x52\x00\x00\x04\x40\xfc\xbc\x07\x05\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xc3\x07\x01\x01\x00\x00" +
		"\x31\x00\x17\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x49\x00\x47\x04\x40\xfc\xb5\x07\x01\x01\x00\x00" +
		"\x49\x00\x17\x04\x40\xfc\xc9\x07\x01\x01\x00\x00" +
		"\x49\x00\x47\x04\x40\xfc\xd7\x07\x01\x01\x00\x00" +
		"\x49\x00\x17\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xc4\x07\x01\x01\x00\x00" +
		"\x56\x00\x6c\x04\x60\xfa\xc5\x07\x01\x01\x00\x00" +
		"\x56\x00\x45\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x32\x00\x6d\x04\x30\xfd\xb2\x07\x01\x01\x00\x00" +
		"\x31\x00\x6d\x04\x30\xfd\xb6\x07\x01\x01\x00\x00" +
		"\x30\x00\x6d\x04\x30\xfd\xb6\x07\x03\x0a\x00\x00" +
		"\x32\x00\x6d\x04\x30\xfd\xb6\x07\x0c\x16\x00\x00" +
		"\x31\x00\x6d\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x55\x00\x00\x04\xb0\xf5\xb1\x07\x01\x01\x00\x00" +
		"\x54\x00\x6c\x04\xb0\xf5\xbf\x07\x0a\x1e\x08\x00" +
		"\x87\x00\x6c\x04\x90\xf7\xbf\x07\x0b\x1e\x00\x00" +
		"\x51\x00\x6c\x04\x90\xf7\x10\x27\x01\x01\x00\x00" +
		"\x2e\x00\x12\x04\x20\xfe\xc6\x07\x09\x11\x00\x00" +
		"\x2d\x00\x00\x04\x20\xfe\xcf\x07\x09\x1e\x00\x00" +
		"\x2e\x00\x12\x04\x20\xfe\xd0\x07\x0a\x0f\x00\x00" +
		"\x2d\x00\x00\x04\x20\xfe\xd1\x07\x09\x0d\x00\x00" +
		"\x2e\x00\x12\x04\x20\xfe\xd2\x07\x0a\x01\x00\x00" +
		"\x2d\x00\x00\x04\x20\xfe\x10\x27\x01\x01\x00\x00" +
		"\x70\x00\x6c\x04\x70\xf9\xda\x07\x0b\x07\x08\x00" +
		"\x56\x00\x6c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x70\x00\x6c\x04\x70\xf9\xc8\x07\x0a\x19\x08\x00" +
		"\x56\x00\x6c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x70\x00\x6c\x04\x70\xf9\xd3\x07\x0a\x1a\x08\x00" +
		"\x56\x00\x6c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xbc\x07\x04\x06\x08\x00" +
		"\x31\x00\x22\x04\x30\xfd\xe7\x07\x03\x19\x58\x00" +
		"\x2d\x00\x00\x04\x20\xfe\x10\x27\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xcc\x07\x01\x01\x00\x00" +
		"\x56\x00\x45\x04\x60\xfa\xce\x07\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xce\x07\x04\x05\x0c\x00" +
		"\x70\x00\x45\x04\x70\xf9\xda\x07\x01\x01\x00\x00" +
		"\x70\x00\x6c\x04\x70\xf9\xe6\x07\x0a\x1e\x08\x00" +
		"\x5c\x00\x00\x04\x60\xfa\xe6\x07\x0b\x1e\x00\x00" +
		"\x56\x00\x6c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x33\x00\x00\x04\xb8\xfc\xc0\x07\x0a\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x76\x00\x00\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x5e\x00\x32\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x12\x04\x40\xfc\xc4\x07\x09\x0c\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x52\x00\x00\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x19\x04\x40\xfc\xe0\x07\x0c\x04\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x4b\x04\x60\xfa\xd0\x07\x0a\x1d\x08\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd1\x07\x04\x01\x0c\x00" +
		"\x56\x00\x17\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xc6\x07\x09\x11\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xcf\x07\x09\x1e\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xd0\x07\x0a\x0f\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\xd1\x07\x09\x0d\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\xd2\x07\x0a\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x5c\x00\x00\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x4b\x04\x60\xfa\xd0\x07\x0a\x1d\x08\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd1\x07\x04\x01\x0c\x00" +
		"\x56\x00\x17\x04\x60\xfa\xd6\x07\x0a\x1d\x08\x00" +
		"\x63\x00\x00\x04\x50\xfb\xd7\x07\x03\x0b\x0c\x00" +
		"\x56\x00\x17\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x39\x00\x12\x04\x50\xfb\xc4\x07\x09\x0c\x00\x00" +
		"\x38\x00\x00\x04\x50\xfb\xd8\x07\x06\x18\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xdd\x07\x0b\x0a\x00\x00" +
		"\x38\x00\x00\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x12\x04\x40\xfc\xc4\x07\x09\x0c\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\xd8\x07\x06\x18\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x19\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x01\x00\x1e\x04\x50\xfb\xb6\x07\x0a\x1b\x00\x00" +
		"\x52\x00\x00\x04\x40\xfc\xd0\x07\x0a\x1d\x08\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\xd0\x07\x0c\x03\x04\x00" +
		"\x52\x00\x00\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x31\x00\x12\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x2d\x00\x00\x04\x20\xfe\xbc\x07\x04\x06\x08\x00" +
		"\x2e\x00\x14\x04\x20\xfe\xbd\x07\x03\x1d\x00\x00" +
		"\x2c\x00\x22\x04\x10\xff\x10\x27\x01\x01\x00\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xb1\x07\x01\x01\x00\x00" +
		"\x79\x00\x6c\x04\x80\xf8\xbf\x07\x0a\x1e\x08\x00" +
		"\x87\x00\x6c\x04\x90\xf7\xbf\x07\x0b\x1e\x00\x00" +
		"\x51\x00\x6c\x04\x90\xf7\x10\x27\x01\x01\x00\x00" +
		"\x77\x00\x60\x04\xb8\xfc\xdb\x07\x0b\x01\x00\x00" +
		"\x77\x00\x17\x04\xb8\xfc\x10\x27\x01\x01\x00\x00" +
		"\x70\x00\x62\x04\x70\xf9\xb4\x07\x04\x1e\x08\x00" +
		"\x5c\x00\x00\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x35\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x49\x00\x66\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xb8\x07\x01\x01\x00\x00" +
		"\x79\x00\x6c\x04\x80\xf8\xcc\x07\x01\x01\x00\x00" +
		"\x79\x00\x45\x04\x80\xf8\xd1\x07\x01\x01\x00\x00" +
		"\x79\x00\x6c\x04\x80\xf8\xd2\x07\x02\x14\x00\x00" +
		"\x79\x00\x45\x04\x80\xf8\xda\x07\x01\x01\x00\x00" +
		"\x79\x00\x6c\x04\x80\xf8\x10\x27\x01\x01\x00\x00" +
		"\x5e\x00\x68\x04\x50\xfb\xb6\x07\x01\x01\x00\x00" +
		"\x5e\x00\x17\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x79\x00\x6e\x04\x80\xf8\xc3\x07\x01\x01\x00\x00" +
		"\x79\x00\x17\x04\x80\xf8\x10\x27\x01\x01\x00\x00" +
		"\x7b\x00\x00\x04\x80\xf8\xbc\x07\x01\x01\x00\x00" +
		"\x79\x00\x17\x04\x80\xf8\xe4\x07\x0b\x01\x00\x00" +
		"\x76\x00\x00\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x72\x04\x60\xfa\xd6\x07\x01\x01\x00\x00" +
		"\x56\x00\x17\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x88\x00\x00\x04\x90\xf7\xb1\x07\x01\x01\x00\x00" +
		"\x87\x00\x6c\x04\x90\xf7\xbf\x07\x0b\x1e\x00\x00" +
		"\x51\x00\x6c\x04\x90\xf7\x10\x27\x01\x01\x00\x00" +
		"\x70\x00\x4b\x04\x70\xf9\xbc\x07\x01\x01\x00\x00" +
		"\x70\x00\x17\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x2a\x00\x00\x04\x00\x00\xb1\x07\x01\x01\x00\x00" +
		"\x17\x00\x00\x04\x80\x07\xd9\x07\x0a\x12\x08\x00" +
		"\x20\x00\x00\x04\x50\x0a\xda\x07\x03\x05\x08\x00" +
		"\x17\x00\x00\x04\x80\x07\xdb\x07\x0a\x1c\x08\x00" +
		"\x20\x00\x00\x04\x50\x0a\xdc\x07\x02\x15\x44\x20" +
		"\x17\x00\x00\x04\x80\x07\xe0\x07\x0a\x16\x00\x00" +
		"\x20\x00\x00\x04\x50\x0a\xe2\x07\x03\x0b\x10\x00" +
		"\x17\x00\x00\x04\x80\x07\xe2\x07\x0a\x07\x10\x00" +
		"\x20\x00\x00\x04\x50\x0a\xe3\x07\x03\x11\x0c\x00" +
		"\x17\x00\x00\x04\x80\x07\xe3\x07\x0a\x04\x0c\x00" +
		"\x20\x00\x00\x04\x50\x0a\xe4\x07\x03\x08\x0c\x00" +
		"\x17\x00\x00\x04\x80\x07\xe4\x07\x0a\x04\x00\x01" +
		"\x20\x00\x00\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x2a\x00\x00\x04\x00\x00\xb1\x07\x02\x01\x00\x00" +
		"\x14\x00\x00\x04\x90\x06\xd9\x07\x0a\x12\x08\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xda\x07\x03\x0a\x50\x20" +
		"\x14\x00\x00\x04\x90\x06\xdb\x07\x0a\x1c\x08\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xdc\x07\x02\x15\x50\x20" +
		"\x14\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x4c\x00\x04\x04\x60\x09\xda\x07\x01\x01\x00\x00" +
		"\x4d\x00\x00\x08\x60\x09\xdb\x07\x01\x01\x00\x00" +
		"\x4c\x00\x04\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\xd9\x07\x0a\x12\x08\x00" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x09\x04\x40\xfc\xb1\x07\x0a\x05\x00\x00" +
		"\x31\x00\x09\x04\x30\xfd\xbe\x07\x05\x01\x00\x00" +
		"\x36\x00\x19\x04\x40\xfc\xe0\x07\x0c\x04\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x2a\x00\x00\x04\x00\x00\xb8\x07\x0c\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x2a\x00\x00\x04\x00\x00\xd5\x07\x02\x0c\x00\x00" +
		"\x01\x00\x69\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x12\x00\x5a\x04\xa0\x05\xc7\x07\x03\x1f\x08\x10" +
		"\x0e\x00\x5a\x04\xb0\x04\xc8\x07\x01\x13\x08\x10" +
		"\x12\x00\x5a\x04\xa0\x05\xd4\x07\x0a\x1f\x08\x10" +
		"\x11\x00\x00\x04\xa0\x05\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x3b\x04\xe0\x01\xe6\x07\x0a\x1c\x00\x10" +
		"\x07\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x28\x00\x59\x04\x30\x0c\xbe\x07\x04\x01\x00\x10" +
		"\x24\x00\x59\x04\x40\x0b\xc7\x07\x03\x1f\x08\x10" +
		"\x21\x00\x59\x04\x50\x0a\xc8\x07\x01\x13\x08\x10" +
		"\x24\x00\x59\x04\x40\x0b\xda\x07\x03\x1c\x08\x10" +
		"\x21\x00\x59\x04\x50\x0a\xdb\x07\x03\x1b\x08\x10" +
		"\x23\x00\x00\x04\x40\x0b\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xbd\x07\x0a\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\xbe\x07\x04\x01\x00\x00" +
		"\x0e\x00\x5a\x04\xb0\x04\xc7\x07\x03\x1f\x08\x10" +
		"\x0b\x00\x5a\x04\xc0\x03\xc8\x07\x01\x13\x08\x10" +
		"\x0e\x00\x5a\x04\xb0\x04\xca\x07\x09\x19\x08\x10" +
		"\x0b\x00\x5a\x04\xc0\x03\xd4\x07\x0a\x1f\x08\x10" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xbd\x07\x04\x01\x00\x00" +
		"\x11\x00\x00\x08\xb0\x04\xbd\x07\x0a\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\xbe\x07\x04\x01\x00\x00" +
		"\x0e\x00\x5a\x04\xb0\x04\xc7\x07\x03\x1f\x08\x10" +
		"\x0b\x00\x5a\x04\xc0\x03\xc8\x07\x01\x13\x08\x10" +
		"\x0e\x00\x5a\x04\xb0\x04\xd4\x07\x0a\x1f\x08\x10" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x0e\x00\x5a\x04\xb0\x04\xc7\x07\x03\x1f\x08\x00" +
		"\x0b\x00\x5a\x04\xc0\x03\xc8\x07\x01\x13\x08\x00" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xbd\x07\x0a\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\xbe\x07\x04\x01\x00\x00" +
		"\x0e\x00\x5a\x04\xb0\x04\xc7\x07\x03\x1f\x08\x10" +
		"\x0b\x00\x5a\x04\xc0\x03\xc8\x07\x01\x13\x08\x10" +
		"\x0e\x00\x5a\x04\xb0\x04\xcf\x07\x03\x1c\x08\x10" +
		"\x0b\x00\x5a\x04\xc0\x03\xd4\x07\x0a\x1f\x08\x10" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x07\x00\x00\x04\xd0\x02\xbe\x07\x05\x01\x00\x00" +
		"\x08\x00\x38\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x0b\x00\x5a\x04\xc0\x03\xc7\x07\x03\x1f\x08\x10" +
		"\x08\x00\x5a\x04\xd0\x02\xc8\x07\x09\x1b\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xcc\x07\x01\x01\x00\x00" +
		"\x0b\x00\x23\x04\xc0\x03\xcd\x07\x01\x01\x00\x00" +
		"\x0b\x00\x0d\x04\xc0\x03\x10\x27\x01\x01\x00\x00" +
		"\x14\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x15\x00\x59\x04\x90\x06\xc7\x07\x03\x1f\x08\x10" +
		"\x12\x00\x59\x04\xa0\x05\xc8\x07\x01\x13\x08\x10" +
		"\x15\x00\x59\x04\x90\x06\xcb\x07\x05\x1c\x00\x00" +
		"\x12\x00\x59\x04\xa0\x05\xdb\x07\x03\x1b\x08\x10" +
		"\x14\x00\x00\x04\x90\x06\xde\x07\x0a\x1a\x08\x10" +
		"\x11\x00\x00\x04\xa0\x05\xe0\x07\x03\x1b\x08\x10" +
		"\x14\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x3f\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x12\x00\x5a\x04\xa0\x05\xc7\x07\x03\x1f\x08\x10" +
		"\x0e\x00\x5a\x04\xb0\x04\xc7\x07\x08\x1f\x08\x00" +
		"\x0e\x00\x3c\x04\xb0\x04\xd5\x07\x08\x0c\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\x10\x27\x01\x01\x00\x00" +
		"\x1b\x00\x59\x04\x70\x08\xc7\x07\x03\x1f\x08\x10" +
		"\x18\x00\x59\x04\x80\x07\xc8\x07\x01\x13\x08\x10" +
		"\x1b\x00\x59\x04\x70\x08\xdb\x07\x03\x1b\x08\x10" +
		"\x1c\x00\x00\x04\x60\x09\xde\x07\x0a\x1a\x08\x10" +
		"\x17\x00\x00\x04\x80\x07\xe0\x07\x03\x1b\x08\x00" +
		"\x1a\x00\x00\x04\x70\x08\x10\x27\x01\x01\x00\x00" +
		"\x14\x00\x00\x04\x90\x06\xba\x07\x01\x01\x00\x00" +
		"\x17\x00\x00\x04\x80\x07\xbf\x07\x04\x01\x00\x00" +
		"\x1b\x00\x48\x04\x70\x08\xd8\x07\x03\x1f\x00\x00" +
		"\x18\x00\x48\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x0f\x00\x00\x04\x28\x05\xcc\x07\x05\x19\x00\x00" +
		"\x13\x00\x00\x04\x18\x06\xcc\x07\x0a\x1a\x02\x00" +
		"\x11\x00\x00\x04\xa0\x05\xd6\x07\x04\x0f\x02\x00" +
		"\x0f\x00\x00\x04\x28\x05\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x64\x04\xe0\x01\xe6\x07\x0a\x1c\x00\x00" +
		"\x07\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\xd9\x07\x01\x01\x00\x00" +
		"\x12\x00\x1f\x04\xa0\x05\x10\x27\x01\x01\x00\x00" +
		"\x1a\x00\x00\x04\x70\x08\xb8\x07\x05\x03\x00\x00" +
		"\x17\x00\x00\x04\x80\x07\xd0\x07\x09\x11\x00\x00" +
		"\x1a\x00\x00\x04\x70\x08\x10\x27\x01\x01\x00\x00" +
		"\x0a\x00\x00\x04\xc0\x03\x10\x27\x01\x01\x00\x00" +
		"\x12\x00\x5a\x04\xa0\x05\xc7\x07\x03\x1f\x08\x10" +
		"\x11\x00\x00\x08\xb0\x04\xc7\x07\x09\x09\x08\x10" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x1c\x04\xe0\x01\xce\x07\x09\x01\x00\x00" +
		"\x60\x00\x23\x04\xe0\x01\xe0\x07\x09\x08\x00\x00" +
		"\x07\x00\x00\x04\xd0\x02\xe1\x07\x0a\x1d\x04\x20" +
		"\x60\x00\x23\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x6a\x00\x74\x04\xe0\x01\xcc\x07\x01\x01\x00\x00" +
		"\x60\x00\x3b\x04\xe0\x01\xcf\x07\x01\x01\x00\x00" +
		"\x60\x00\x51\x04\xe0\x01\xd8\x07\x08\x1d\x00\x00" +
		"\x62\x00\x00\x04\xe0\x01\xd8\x07\x09\x01\x00\x00" +
		"\x60\x00\x51\x04\xe0\x01\xda\x07\x01\x01\x00\x00" +
		"\x62\x00\x00\x04\xe0\x01\xda\x07\x03\x1b\x00\x01" +
		"\x60\x00\x51\x04\xe0\x01\xdb\x07\x08\x01\x00\x00" +
		"\x62\x00\x00\x04\xe0\x01\xdc\x07\x01\x01\x00\x00" +
		"\x60\x00\x51\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x6a\x00\x74\x04\xe0\x01\xcc\x07\x01\x01\x00\x00" +
		"\x60\x00\x3b\x04\xe0\x01\xcf\x07\x01\x01\x00\x00" +
		"\x60\x00\x51\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x17\x00\x00\x04\x80\x07\xb7\x07\x06\x0d\x00\x00" +
		"\x14\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x68\x00\x31\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\xba\x07\x01\x01\x00\x00" +
		"\x15\x00\x48\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x18\x00\x59\x04\x80\x07\xc7\x07\x03\x1f\x08\x10" +
		"\x15\x00\x59\x04\x90\x06\xc8\x07\x01\x13\x08\x10" +
		"\x18\x00\x59\x04\x80\x07\xdb\x07\x03\x1b\x08\x10" +
		"\x1a\x00\x00\x04\x70\x08\xde\x07\x0a\x1a\x08\x10" +
		"\x17\x00\x00\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x84\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x85\x00\x00\x04\x70\x08\x10\x27\x01\x01\x00\x00" +
		"\x6a\x00\x74\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x0c\x00\x00\x04\x38\x04\x10\x27\x01\x01\x00\x00" +
		"\x24\x00\x59\x04\x40\x0b\xc7\x07\x03\x1f\x08\x10" +
		"\x21\x00\x59\x04\x50\x0a\xc8\x07\x01\x13\x08\x10" +
		"\x24\x00\x59\x04\x40\x0b\xda\x07\x03\x1c\x08\x10" +
		"\x21\x00\x59\x04\x50\x0a\xdb\x07\x03\x1b\x08\x10" +
		"\x23\x00\x00\x04\x40\x0b\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xb3\x07\x03\x1a\x00\x00" +
		"\x7a\x00\x50\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x0f\x00\x00\x04\x28\x05\xc2\x07\x01\x01\x00\x00" +
		"\x10\x00\x00\x04\x64\x05\x10\x27\x01\x01\x00\x00" +
		"\x1b\x00\x59\x04\x70\x08\xc7\x07\x03\x1f\x08\x10" +
		"\x18\x00\x59\x04\x80\x07\xc8\x07\x01\x13\x08\x10" +
		"\x1b\x00\x59\x04\x70\x08\xd4\x07\x01\x01\x00\x00" +
		"\x1d\x00\x59\x04\x60\x09\xdb\x07\x03\x1b\x08\x10" +
		"\x20\x00\x00\x04\x50\x0a\xdb\x07\x09\x0d\x00\x10" +
		"\x1c\x00\x00\x04\x60\x09\xde\x07\x0a\x1a\x08\x10" +
		"\x1a\x00\x00\x04\x70\x08\x10\x27\x01\x01\x00\x00" +
		"\x6b\x00\x00\x04\x28\x05\x10\x27\x01\x01\x00\x00" +
		"\x15\x00\x59\x04\x90\x06\xc7\x07\x03\x1f\x08\x10" +
		"\x12\x00\x59\x04\xa0\x05\xc8\x07\x01\x13\x08\x10" +
		"\x15\x00\x59\x04\x90\x06\xdb\x07\x03\x1b\x08\x10" +
		"\x17\x00\x00\x04\x80\x07\xde\x07\x0a\x1a\x08\x10" +
		"\x14\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x17\x00\x00\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x41\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x21\x00\x59\x04\x50\x0a\xc7\x07\x03\x1f\x08\x10" +
		"\x1d\x00\x59\x04\x60\x09\xc8\x07\x01\x13\x08\x10" +
		"\x21\x00\x59\x04\x50\x0a\xdb\x07\x03\x1b\x08\x10" +
		"\x23\x00\x00\x04\x40\x0b\xde\x07\x0a\x1a\x08\x10" +
		"\x1c\x00\x00\x04\x60\x09\xe0\x07\x04\x18\x08\x10" +
		"\x20\x00\x00\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x86\x00\x00\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x79\x00\x54\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x1c\x04\xe0\x01\xce\x07\x09\x01\x00\x00" +
		"\x60\x00\x23\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x15\x00\x59\x04\x90\x06\xc7\x07\x03\x1f\x08\x10" +
		"\x12\x00\x59\x04\xa0\x05\xc8\x07\x01\x13\x08\x10" +
		"\x15\x00\x59\x04\x90\x06\xda\x07\x03\x1c\x08\x10" +
		"\x12\x00\x59\x04\xa0\x05\xdb\x07\x03\x1b\x08\x10" +
		"\x14\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x15\x00\x59\x04\x90\x06\xc7\x07\x03\x1f\x08\x10" +
		"\x12\x00\x59\x04\xa0\x05\xc8\x07\x01\x13\x08\x10" +
		"\x15\x00\x59\x04\x90\x06\xc9\x07\x05\x17\x00\x00" +
		"\x12\x00\x59\x04\xa0\x05\xdb\x07\x03\x1b\x08\x10" +
		"\x14\x00\x00\x04\x90\x06\xde\x07\x0a\x1a\x08\x10" +
		"\x11\x00\x00\x04\xa0\x05\xe0\x07\x07\x18\x08\x10" +
		"\x14\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x12\x00\x59\x04\xa0\x05\xc7\x07\x03\x1f\x08\x10" +
		"\x0e\x00\x59\x04\xb0\x04\xc8\x07\x01\x13\x08\x10" +
		"\x12\x00\x59\x04\xa0\x05\xdb\x07\x03\x1b\x08\x10" +
		"\x14\x00\x00\x04\x90\x06\xde\x07\x0a\x1a\x08\x10" +
		"\x11\x00\x00\x04\xa0\x05\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xbd\x07\x04\x01\x00\x00" +
		"\x11\x00\x00\x08\xb0\x04\xbd\x07\x0a\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\xbe\x07\x04\x01\x00\x00" +
		"\x0e\x00\x5a\x04\xb0\x04\xc5\x07\x03\x1a\x08\x10" +
		"\x0b\x00\x5a\x04\xc0\x03\xc8\x07\x01\x13\x08\x10" +
		"\x0e\x00\x5a\x04\xb0\x04\xc8\x07\x03\x1d\x08\x10" +
		"\x0b\x00\x5a\x04\xc0\x03\xd4\x07\x0a\x1f\x08\x10" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x86\x00\x00\x04\x80\x07\xc4\x07\x01\x01\x00\x00" +
		"\x84\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x6f\x00\x00\x04\x70\x08\xdf\x07\x08\x0f\x00\x00" +
		"\x6f\x00\x00\x04\xf8\x07\xe2\x07\x05\x04\x5e\x00" +
		"\x6f\x00\x00\x04\x70\x08\x10\x27\x01\x01\x00\x00" +
		"\x0a\x00\x00\x04\xc0\x03\xb4\x07\x06\x01\x00\x00" +
		"\x07\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xbd\x07\x04\x01\x00\x00" +
		"\x11\x00\x00\x08\xb0\x04\xbd\x07\x0a\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\xbe\x07\x04\x01\x00\x00" +
		"\x0e\x00\x5a\x04\xb0\x04\xc7\x07\x03\x1f\x08\x10" +
		"\x0b\x00\x5a\x04\xc0\x03\xc8\x07\x01\x13\x08\x10" +
		"\x0e\x00\x5a\x04\xb0\x04\xd4\x07\x0a\x1f\x08\x10" +
		"\x11\x00\x00\x04\xa0\x05\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xbd\x07\x04\x01\x00\x00" +
		"\x11\x00\x00\x08\xb0\x04\xbd\x07\x0a\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\xbe\x07\x04\x01\x00\x00" +
		"\x0e\x00\x5a\x04\xb0\x04\xc7\x07\x03\x1f\x08\x10" +
		"\x0b\x00\x5a\x04\xc0\x03\xc7\x07\x09\x1d\x08\x10" +
		"\x0e\x00\x5a\x04\xb0\x04\xc8\x07\x01\x13\x08\x10" +
		"\x12\x00\x5a\x04\xa0\x05\xc8\x07\x03\x1d\x08\x10" +
		"\x0e\x00\x5a\x04\xb0\x04\xd4\x07\x0a\x1f\x08\x10" +
		"\x11\x00\x00\x04\xa0\x05\xe2\x07\x0c\x15\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x07\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x21\x00\x59\x04\x50\x0a\xc7\x07\x03\x1f\x08\x10" +
		"\x1d\x00\x59\x04\x60\x09\xc8\x07\x01\x13\x08\x10" +
		"\x21\x00\x59\x04\x50\x0a\xcd\x07\x03\x1e\x08\x10" +
		"\x1d\x00\x59\x04\x60\x09\xdb\x07\x03\x1b\x08\x10" +
		"\x20\x00\x00\x04\x50\x0a\xde\x07\x0a\x1a\x08\x10" +
		"\x1c\x00\x00\x04\x60\x09\xe0\x07\x03\x1b\x08\x10" +
		"\x20\x00\x00\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xbd\x07\x04\x01\x00\x00" +
		"\x11\x00\x00\x08\xb0\x04\xbd\x07\x0a\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\xbe\x07\x04\x01\x00\x00" +
		"\x0e\x00\x5a\x04\xb0\x04\xc8\x07\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x6e\x00\x57\x04\x70\x08\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x4f\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x16\x00\x00\x04\x08\x07\xbd\x07\x0c\x1f\x40\x20" +
		"\x17\x00\x00\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x21\x00\x59\x04\x50\x0a\xc7\x07\x03\x1f\x08\x10" +
		"\x1d\x00\x59\x04\x60\x09\xc8\x07\x01\x13\x08\x10" +
		"\x21\x00\x59\x04\x50\x0a\xdb\x07\x03\x1b\x08\x10" +
		"\x23\x00\x00\x04\x40\x0b\xde\x07\x0a\x1a\x08\x10" +
		"\x20\x00\x00\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x65\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x12\x00\x5a\x04\xa0\x05\xc7\x07\x03\x1f\x08\x00" +
		"\x0e\x00\x5a\x04\xb0\x04\xc8\x07\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x0b\x00\x5a\x04\xc0\x03\xc7\x07\x03\x1f\x08\x10" +
		"\x08\x00\x5a\x04\xd0\x02\xc8\x07\x01\x01\x00\x00" +
		"\x08\x00\x21\x04\xd0\x02\xca\x07\x09\x19\x00\x00" +
		"\x0b\x00\x21\x04\xc0\x03\xcc\x07\x0a\x1b\x00\x00" +
		"\x0d\x00\x00\x08\xc0\x03\xcd\x07\x03\x1e\x00\x00" +
		"\x0b\x00\x21\x04\xc0\x03\xd4\x07\x06\x1b\x00\x00" +
		"\x08\x00\x5a\x04\xd0\x02\xd5\x07\x03\x1b\x08\x00" +
		"\x0a\x00\x00\x04\xc0\x03\x10\x27\x01\x01\x00\x00" +
		"\x09\x00\x37\x04\x48\x03\xb9\x07\x0a\x14\x60\x00" +
		"\x0b\x00\x37\x04\xc0\x03\xbb\x07\x01\x01\x00\x00" +
		"\x09\x00\x37\x04\x48\x03\x10\x27\x01\x01\x00\x00" +
		"\x0f\x00\x00\x04\x28\x05\xc3\x07\x0a\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\x10\x27\x01\x01\x00\x00" +
		"\x6d\x00\x3a\x04\x70\x08\x10\x27\x01\x01\x00\x00" +
		"\x15\x00\x59\x04\x90\x06\xc7\x07\x03\x1f\x08\x10" +
		"\x12\x00\x59\x04\xa0\x05\xc8\x07\x01\x13\x08\x10" +
		"\x15\x00\x59\x04\x90\x06\xd2\x07\x05\x01\x0c\x00" +
		"\x12\x00\x59\x04\xa0\x05\xdb\x07\x03\x1b\x08\x10" +
		"\x14\x00\x00\x04\x90\x06\xde\x07\x0a\x1a\x08\x10" +
		"\x11\x00\x00\x04\xa0\x05\xe0\x07\x05\x1d\x08\x10" +
		"\x14\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x14\x00\x00\x04\x90\x06\xba\x07\x01\x01\x00\x00" +
		"\x18\x00\x48\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\x10\x27\x01\x01\x00\x00" +
		"\x1b\x00\x59\x04\x70\x08\xbd\x07\x04\x01\x00\x00" +
		"\x21\x00\x59\x04\x50\x0a\xc7\x07\x03\x1f\x08\x10" +
		"\x1d\x00\x59\x04\x60\x09\xc8\x07\x01\x13\x08\x10" +
		"\x21\x00\x59\x04\x50\x0a\xdb\x07\x03\x1b\x08\x10" +
		"\x23\x00\x00\x04\x40\x0b\xdb\x07\x09\x0d\x00\x10" +
		"\x20\x00\x00\x04\x50\x0a\xde\x07\x0a\x1a\x08\x10" +
		"\x1c\x00\x00\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x1d\x00\x59\x04\x60\x09\xc7\x07\x03\x1f\x08\x10" +
		"\x1b\x00\x59\x04\x70\x08\xc8\x07\x01\x13\x08\x10" +
		"\x1d\x00\x59\x04\x60\x09\xdb\x07\x03\x1b\x08\x10" +
		"\x20\x00\x00\x04\x50\x0a\xde\x07\x0a\x1a\x08\x10" +
		"\x1c\x00\x00\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x1b\x00\x59\x04\x70\x08\xc7\x07\x03\x1f\x08\x10" +
		"\x18\x00\x59\x04\x80\x07\xc8\x07\x01\x13\x08\x10" +
		"\x1b\x00\x59\x04\x70\x08\xdb\x07\x03\x1b\x08\x10" +
		"\x1c\x00\x00\x04\x60\x09\xde\x07\x0a\x1a\x08\x10" +
		"\x1a\x00\x00\x04\x70\x08\x10\x27\x01\x01\x00\x00" +
		"\x13\x00\x00\x04\x18\x06\x10\x27\x01\x01\x00\x00" +
		"\x0e\x00\x59\x04\xb0\x04\xc7\x07\x03\x1f\x08\x10" +
		"\x0b\x00\x59\x04\xc0\x03\xc8\x07\x01\x13\x08\x10" +
		"\x0e\x00\x59\x04\xb0\x04\xdb\x07\x03\x1b\x08\x10" +
		"\x11\x00\x00\x04\xa0\x05\xde\x07\x0a\x1a\x08\x10" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x0b\x00\x5a\x04\xc0\x03\xc7\x07\x03\x1f\x08\x10" +
		"\x08\x00\x5a\x04\xd0\x02\xcb\x07\x09\x18\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xcd\x07\x01\x01\x00\x00" +
		"\x0b\x00\x5a\x04\xc0\x03\xdb\x07\x01\x01\x00\x00" +
		"\x0b\x00\x0a\x04\xc0\x03\x10\x27\x01\x01\x00\x00" +
		"\x2c\x00\x56\x04\x10\xff\xbf\x07\x09\x19\x04\x10" +
		"\x2c\x00\x70\x04\x10\xff\xc8\x07\x09\x1b\x04\x10" +
		"\x81\x00\x22\x04\x00\x00\xc9\x07\x03\x1c\x04\x20" +
		"\x2c\x00\x22\x04\x10\xff\x10\x27\x01\x01\x00\x00" +
		"\x49\x00\x11\x04\x40\xfc\xb6\x07\x04\x1c\x08\x00" +
		"\x49\x00\x17\x04\x40\xfc\xb8\x07\x01\x01\x00\x00" +
		"\x49\x00\x6c\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x83\x00\x00\x04\x00\x00\xbc\x07\x04\x06\x00\x10" +
		"\x82\x00\x00\x08\x00\x00\xbc\x07\x09\x1c\x04\x20" +
		"\x81\x00\x22\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x2d\x00\x00\x04\x20\xfe\xb7\x07\x0b\x19\x08\x00" +
		"\x2b\x00\x00\x04\x10\xff\x10\x27\x01\x01\x00\x00" +
		"\x83\x00\x00\x04\x00\x00\xbd\x07\x01\x01\x00\x00" +
		"\x81\x00\x22\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x81\x00\x56\x04\x00\x00\xbf\x07\x09\x19\x04\x10" +
		"\x81\x00\x22\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x2d\x00\x00\x04\x20\xfe\x10\x27\x01\x01\x00\x00" +
		"\x36\x00\x28\x04\x40\xfc\xbf\x07\x05\x01\x00\x00" +
		"\x31\x00\x28\x04\x30\xfd\xc1\x07\x09\x0f\x00\x00" +
		"\x36\x00\x28\x04\x40\xfc\xda\x07\x09\x05\x08\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x4a\x00\x0b\x04\xe8\x08\xb3\x07\x01\x01\x00\x00" +
		"\x4a\x00\x03\x04\xe8\x08\x10\x27\x01\x01\x00\x00" +
		"\x4c\x00\x0b\x04\x60\x09\xb3\x07\x01\x01\x00\x00" +
		"\x4c\x00\x02\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x4a\x00\x0b\x04\xe8\x08\xb3\x07\x01\x01\x00\x00" +
		"\x4a\x00\x01\x04\xe8\x08\xd0\x07\x01\x01\x00\x00" +
		"\x4a\x00\x03\x04\xe8\x08\x10\x27\x01\x01\x00\x00" +
		"\x4a\x00\x0b\x04\xe8\x08\x10\x27\x01\x01\x00\x00" +
		"\x19\x00\x06\x04\x34\x08\x10\x27\x01\x01\x00\x00" +
		"\x4c\x00\x04\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x4c\x00\x0b\x04\x60\x09\xb3\x07\x01\x01\x00\x00" +
		"\x4c\x00\x02\x04\x60\x09\xc8\x07\x07\x01\x00\x00" +
		"\x4c\x00\x34\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x4e\x00\x00\x04\x60\x09\xbd\x07\x03\x01\x00\x00" +
		"\x1f\x00\x3d\x04\xd8\x09\xc1\x07\x07\x01\x00\x00" +
		"\x1e\x00\x3d\x04\xd8\x09\x10\x27\x01\x01\x00\x00" +
		"\x4c\x00\x0b\x04\x60\x09\xb3\x07\x01\x01\x00\x00" +
		"\x4c\x00\x05\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x53\x00\x06\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x4c\x00\x0b\x04\x60\x09\xb3\x07\x01\x01\x00\x00" +
		"\x4c\x00\x01\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x14\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x56\x00\x6c\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x63\x00\x00\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x5e\x00\x6c\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x65\x00\x00\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x2b\x00\x00\x04\x10\xff\x10\x27\x01\x01\x00\x00" +
		"\x42\x00\x00\x04\xa0\xf6\x10\x27\x01\x01\x00\x00" +
		"\x46\x00\x00\x04\xb0\xf5\x10\x27\x01\x01\x00\x00" +
		"\x48\x00\x00\x04\xc0\xf4\x10\x27\x01\x01\x00\x00" +
		"\x2d\x00\x00\x04\x20\xfe\x10\x27\x01\x01\x00\x00" +
		"\x2f\x00\x00\x04\x30\xfd\x10\x27\x01\x01\x00\x00" +
		"\x35\x00\x00\x04\x40\xfc\x10\x27\x01\x01\x00\x00" +
		"\x38\x00\x00\x04\x50\xfb\x10\x27\x01\x01\x00\x00" +
		"\x3a\x00\x00\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x3c\x00\x00\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x3e\x00\x00\x04\x80\xf8\x10\x27\x01\x01\x00\x00" +
		"\x40\x00\x00\x04\x90\xf7\x10\x27\x01\x01\x00\x00" +
		"\x03\x00\x00\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x1c\x00\x00\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x20\x00\x00\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x23\x00\x00\x04\x40\x0b\x10\x27\x01\x01\x00\x00" +
		"\x27\x00\x00\x04\x30\x0c\x10\x27\x01\x01\x00\x00" +
		"\x29\x00\x00\x04\x20\x0d\x10\x27\x01\x01\x00\x00" +
		"\x05\x00\x00\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x07\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x0a\x00\x00\x04\xc0\x03\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\x10\x27\x01\x01\x00\x00" +
		"\x14\x00\x00\x04\x90\x06\x10\x27\x01\x01\x00\x00" +
		"\x17\x00\x00\x04\x80\x07\x10\x27\x01\x01\x00\x00" +
		"\x1a\x00\x00\x04\x70\x08\x10\x27\x01\x01\x00\x00" +
		"\x7e\x00\x00\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x5b\x00\x00\x04\xf0\x00\xc1\x07\x03\x1f\x08\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x0b\x00\x59\x04\xc0\x03\xc5\x07\x03\x1a\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xc7\x07\x03\x1f\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xc8\x07\x03\x1d\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xdb\x07\x03\x1b\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xde\x07\x0a\x1a\x08\x10" +
		"\x07\x00\x00\x04\xd0\x02\xe0\x07\x03\x1b\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x2e\x04\xe0\x01\xbd\x07\x01\x01\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x5b\x00\x00\x04\xf0\x00\xbe\x07\x0b\x1b\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x2d\x04\xf0\x00\xbc\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x0f\x04\xf0\x00\xb9\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x58\x04\xe0\x01\xbd\x07\x03\x1d\x08\x10" +
		"\x60\x00\x14\x04\xe0\x01\xc7\x07\x01\x01\x00\x00" +
		"\x60\x00\x58\x04\xe0\x01\xca\x07\x01\x01\x00\x00" +
		"\x60\x00\x20\x04\xe0\x01\xcd\x07\x01\x01\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x36\x04\xf0\x00\xc0\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x75\x00\x59\x04\xd0\x02\xc6\x07\x05\x06\x08\x00" +
		"\x60\x00\x59\x04\xe0\x01\xc8\x07\x01\x01\x00\x00" +
		"\x60\x00\x20\x04\xe0\x01\xcd\x07\x01\x01\x00\x00" +
		"\x60\x00\x46\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x6c\x00\x27\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x5b\x00\x00\x04\xf0\x00\xbe\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x2a\x04\xe0\x01\xbf\x07\x01\x01\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x60\x00\x6b\x04\xe0\x01\xba\x07\x06\x1d\x00\x00" +
		"\x08\x00\x6b\x04\xd0\x02\xc0\x07\x0b\x01\x08\x00" +
		"\x60\x00\x6b\x04\xe0\x01\xd7\x07\x01\x01\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\xdb\x07\x03\x1b\x04\x20" +
		"\x62\x00\x00\x04\xe0\x01\xdb\x07\x03\x1c\x04\x20" +
		"\x60\x00\x22\x04\xe0\x01\xde\x07\x03\x1e\x04\x20" +
		"\x62\x00\x00\x04\xe0\x01\xde\x07\x03\x1f\x04\x20" +
		"\x60\x00\x22\x04\xe0\x01\xdf\x07\x0a\x19\x04\x20" +
		"\x61\x00\x00\x08\xe0\x01\xdf\x07\x0b\x08\x04\x20" +
		"\x60\x00\x22\x04\xe0\x01\xe0\x07\x09\x07\x00\x00" +
		"\x07\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x75\x00\x59\x04\xd0\x02\xc5\x07\x03\x1a\x08\x10" +
		"\x60\x00\x59\x04\xe0\x01\xdb\x07\x03\x1b\x08\x10" +
		"\x07\x00\x00\x04\xd0\x02\xde\x07\x0a\x1a\x08\x10" +
		"\x62\x00\x00\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x0b\x00\x59\x04\xc0\x03\xc5\x07\x03\x1a\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xc7\x07\x03\x1f\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xc8\x07\x03\x1d\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xdb\x07\x03\x1b\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xde\x07\x0a\x1a\x08\x10" +
		"\x07\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x75\x00\x59\x04\xd0\x02\xc6\x07\x07\x01\x08\x00" +
		"\x61\x00\x00\x08\xe0\x01\xc7\x07\x09\x1d\x0c\x00" +
		"\x60\x00\x14\x04\xe0\x01\xcc\x07\x05\x0d\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x5b\x00\x00\x04\xf0\x00\xb8\x07\x09\x1a\x04\x00" +
		"\x81\x00\x56\x04\x00\x00\xbf\x07\x09\x19\x04\x10" +
		"\x81\x00\x70\x04\x00\x00\xc8\x07\x09\x1b\x04\x10" +
		"\x5a\x00\x22\x04\xf0\x00\xcc\x07\x03\x1f\x04\x20" +
		"\x81\x00\x22\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x55\x00\x00\x04\xf0\x00\xb3\x07\x0a\x1f\x08\x20" +
		"\x01\x00\x2c\x04\x00\x00\xcc\x07\x01\x01\x00\x00" +
		"\x66\x00\x22\x04\x00\x00\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x5e\x04\xf0\x00\xbb\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x39\x04\xf0\x00\xb5\x07\x03\x1f\x00\x00" +
		"\x5a\x00\x42\x04\xf0\x00\xbd\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x75\x00\x59\x04\xd0\x02\xc6\x07\x01\x01\x00\x00" +
		"\x74\x00\x00\x04\xd0\x02\xc7\x07\x03\x1f\x08\x10" +
		"\x60\x00\x59\x04\xe0\x01\xdb\x07\x03\x1b\x08\x10" +
		"\x07\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x75\x00\x59\x04\xd0\x02\xc7\x07\x03\x1f\x08\x10" +
		"\x60\x00\x59\x04\xe0\x01\xc8\x07\x01\x13\x08\x10" +
		"\x75\x00\x59\x04\xd0\x02\xdb\x07\x03\x1b\x08\x10" +
		"\x74\x00\x00\x04\xc0\x03\xde\x07\x0a\x1a\x08\x10" +
		"\x74\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x2b\x04\xf0\x00\xb9\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x1d\x04\xf0\x00\xbb\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x75\x00\x59\x04\xd0\x02\xc5\x07\x03\x1a\x08\x10" +
		"\x61\x00\x00\x08\xe0\x01\xc5\x07\x09\x18\x08\x10" +
		"\x60\x00\x3e\x04\xe0\x01\xcd\x07\x01\x15\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\xd0\x07\x02\x1d\x00\x00" +
		"\x62\x00\x00\x04\xe0\x01\xd1\x07\x01\x02\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x39\x04\xf0\x00\xbc\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x0b\x00\x59\x04\xc0\x03\xc5\x07\x03\x1a\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xc7\x07\x03\x1f\x08\x10" +
		"\x06\x00\x59\x04\xe0\x01\xc7\x07\x09\x1d\x08\x10" +
		"\x07\x00\x00\x04\xd0\x02\xc7\x07\x0a\x14\x0c\x00" +
		"\x0b\x00\x59\x04\xc0\x03\xda\x07\x03\x1c\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xdb\x07\x03\x1b\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\x10\x27\x01\x01\x00\x00" +
		"\x0b\x00\x59\x04\xc0\x03\xc4\x07\x03\x1b\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xc7\x07\x03\x1f\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xc8\x07\x03\x1d\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xdb\x07\x03\x1b\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xde\x07\x0a\x1a\x08\x10" +
		"\x07\x00\x00\x04\xd0\x02\xe0\x07\x0c\x04\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\x10\x27\x01\x01\x00\x00" +
		"\x75\x00\x59\x04\xd0\x02\xc6\x07\x01\x01\x00\x00" +
		"\x74\x00\x00\x04\xd0\x02\xc6\x07\x07\x01\x08\x00" +
		"\x62\x00\x00\x04\xe0\x01\xc8\x07\x03\x14\x00\x00" +
		"\x60\x00\x14\x04\xe0\x01\xca\x07\x05\x01\x00\x00" +
		"\x75\x00\x14\x04\xd0\x02\xcc\x07\x03\x1f\x00\x10" +
		"\x73\x00\x00\x08\xd0\x02\xcc\x07\x0a\x1b\x0c\x10" +
		"\x74\x00\x00\x04\xd0\x02\xcd\x07\x03\x1e\x04\x20" +
		"\x60\x00\x22\x04\xe0\x01\xde\x07\x03\x1e\x08\x00" +
		"\x74\x00\x00\x04\xc0\x03\xde\x07\x0a\x1a\x08\x10" +
		"\x74\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x62\x00\x00\x04\xe0\x01\xbb\x07\x03\x1f\x5c\x00" +
		"\x60\x00\x13\x04\xe0\x01\xbe\x07\x09\x1a\x0c\x00" +
		"\x60\x00\x14\x04\xe0\x01\xc7\x07\x01\x01\x00\x00" +
		"\x60\x00\x20\x04\xe0\x01\xcd\x07\x01\x01\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x75\x00\x59\x04\xd0\x02\xc5\x07\x03\x1a\x08\x10" +
		"\x61\x00\x00\x08\xe0\x01\xc5\x07\x09\x18\x08\x10" +
		"\x60\x00\x14\x04\xe0\x01\xce\x07\x09\x16\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\xcf\x07\x0a\x1f\x10\x00" +
		"\x62\x00\x00\x04\xe0\x01\xd2\x07\x02\x15\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x07\x04\xf0\x00\xc0\x07\x07\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x0b\x00\x59\x04\xc0\x03\xc5\x07\x03\x1a\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xc7\x07\x03\x1f\x08\x10" +
		"\x06\x00\x59\x04\xe0\x01\xc8\x07\x01\x13\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xdb\x07\x03\x1b\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xde\x07\x0a\x1a\x08\x10" +
		"\x07\x00\x00\x04\xd0\x02\xe0\x07\x03\x1b\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x0c\x04\xf0\x00\xbd\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x75\x00\x59\x04\xd0\x02\xc5\x07\x03\x1a\x08\x10" +
		"\x60\x00\x59\x04\xe0\x01\xc7\x07\x09\x1d\x08\x10" +
		"\x60\x00\x14\x04\xe0\x01\xce\x07\x01\x01\x00\x00" +
		"\x62\x00\x00\x04\xe0\x01\xce\x07\x03\x1d\x04\x20" +
		"\x5a\x00\x22\x04\xf0\x00\xcf\x07\x0a\x1f\x04\x20" +
		"\x62\x00\x00\x04\xe0\x01\xd3\x07\x01\x01\x00\x00" +
		"\x60\x00\x22\x04\xe0\x01\x10\x27\x01\x01\x00\x00" +
		"\x0b\x00\x59\x04\xc0\x03\xc4\x07\x03\x1b\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xc7\x07\x03\x1f\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xc8\x07\x03\x1d\x08\x10" +
		"\x08\x00\x59\x04\xd0\x02\xdb\x07\x03\x1b\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xde\x07\x0a\x1a\x08\x10" +
		"\x07\x00\x00\x04\xd0\x02\xe2\x07\x0a\x1c\x08\x10" +
		"\x0a\x00\x00\x04\xc0\x03\xe4\x07\x0c\x1b\x08\x10" +
		"\x07\x00\x00\x04\xd0\x02\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x55\x04\xf0\x00\xb9\x07\x01\x01\x00\x00" +
		"\x5a\x00\x70\x04\xf0\x00\xc4\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x5a\x00\x63\x04\xf0\x00\xbd\x07\x01\x01\x00\x00" +
		"\x5a\x00\x22\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x69\x00\x00\x04\xa0\xf6\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\xcc\x07\x01\x01\x00\x00" +
		"\x11\x00\x00\x04\xa0\x05\x10\x27\x01\x01\x00\x00" +
		"\x0d\x00\x00\x04\xb0\x04\x10\x27\x01\x01\x00\x00" +
		"\x0b\x00\x43\x04\xc0\x03\x10\x27\x01\x01\x00\x00" +
		"\x71\x00\x14\x04\xf0\x00\x10\x27\x01\x01\x00\x00" +
		"\x76\x00\x00\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x70\x00\x6c\x04\x70\xf9\x10\x27\x01\x01\x00\x00" +
		"\x79\x00\x6c\x04\x80\xf8\x10\x27\x01\x01\x00\x00" +
		"\x47\x00\x71\x04\xb0\xf5\xdb\x07\x0c\x1d\x60\x00" +
		"\x28\x00\x71\x04\x30\x0c\x10\x27\x01\x01\x00\x00" +
		"\x78\x00\x4c\x04\x40\x0b\x10\x27\x01\x01\x00\x00" +
		"\x1c\x00\x00\x04\x60\x09\xde\x07\x0c\x1c\x08\x00" +
		"\x20\x00\x00\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x26\x00\x18\x04\xf4\x0b\x10\x27\x01\x01\x00\x00" +
		"\x3d\x00\x19\x04\x70\xf9\xbe\x07\x03\x0e\x0c\x20" +
		"\x3b\x00\x19\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x21\x00\x6f\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x46\x00\x00\x04\xb0\xf5\xdb\x07\x0c\x1e\x00\x00" +
		"\x27\x00\x00\x04\x30\x0c\x10\x27\x01\x01\x00\x00" +
		"\x24\x00\x29\x04\x40\x0b\x10\x27\x01\x01\x00\x00" +
		"\x38\x00\x00\x04\x50\xfb\xc2\x07\x01\x01\x00\x00" +
		"\x3b\x00\x24\x04\x60\xfa\x10\x27\x01\x01\x00\x00" +
		"\x40\x00\x00\x04\x90\xf7\x10\x27\x01\x01\x00\x00" +
		"\x20\x00\x00\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x64\x00\x2f\x04\x60\x09\xd0\x07\x0c\x17\x00\x00" +
		"\x5d\x00\x00\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x69\x00\x00\x04\xa0\xf6\x10\x27\x01\x01\x00\x00" +
		"\x48\x00\x00\x04\xc0\xf4\xbb\x07\x0a\x01\x00\x00" +
		"\x46\x00\x00\x04\xb0\xf5\xca\x07\x0c\x1f\x00\x00" +
		"\x27\x00\x00\x04\x30\x0c\x10\x27\x01\x01\x00\x00" +
		"\x45\x00\x00\x04\x00\xf6\xbb\x07\x0a\x01\x00\x00" +
		"\x42\x00\x00\x04\xa0\xf6\xca\x07\x0c\x1f\x00\x00" +
		"\x29\x00\x00\x04\x20\x0d\x10\x27\x01\x01\x00\x00" +
		"\x20\x00\x00\x04\x50\x0a\xb1\x07\x0a\x01\x00\x00" +
		"\x23\x00\x00\x04\x40\x0b\xcf\x07\x01\x01\x00\x00" +
		"\x20\x00\x00\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x20\x00\x00\x04\x50\x0a\xb1\x07\x0a\x01\x00\x00" +
		"\x48\x00\x00\x04\xc0\xf4\xc9\x07\x08\x14\x60\x00" +
		"\x23\x00\x00\x04\x40\x0b\x10\x27\x01\x01\x00\x00" +
		"\x41\x00\x00\x04\x18\xf7\x10\x27\x01\x01\x00\x00" +
		"\x22\x00\x00\x04\xc8\x0a\xbb\x07\x02\x0a\x08\x00" +
		"\x23\x00\x00\x04\x40\x0b\x10\x27\x01\x01\x00\x00" +
		"\x46\x00\x00\x04\xb0\xf5\x10\x27\x01\x01\x00\x00" +
		"\x22\x00\x00\x04\xc8\x0a\xb6\x07\x0a\x1b\x08\x10" +
		"\x25\x00\x00\x08\xc8\x0a\xb7\x07\x03\x02\x08\x10" +
		"\x22\x00\x00\x04\xc8\x0a\xdf\x07\x0a\x04\x08\x10" +
		"\x20\x00\x00\x04\x50\x0a\xe3\x07\x07\x01\x00\x00" +
		"\x21\x00\x01\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x21\x00\x4a\x04\x50\x0a\x10\x27\x01\x01\x00\x00" +
		"\x7d\x00\x00\x04\xb0\xf5\x10\x27\x01\x01\x00\x00" +
		"\x1a\x00\x00\x04\x70\x08\x10\x27\x01\x01\x00\x00" +
		"\x3f\x00\x00\x04\x08\xf8\xce\x07\x04\x1b\x00\x00" +
		"\x3e\x00\x00\x04\x80\xf8\x10\x27\x01\x01\x00\x00" +
		"\x1c\x00\x00\x04\x60\x09\x10\x27\x01\x01\x00\x00" +
		"\x44\x00\x00\x04\x28\xf6\xba\x07\x0b\x0c\x00\x00" +
		"\x43\x00\x1a\x04\xa0\xf6\x10\x27\x01\x01\x00\x00" +
		"\x42\x00\x00\x04\xa0\xf6\x10\x27\x01\x01\x00\x00" +
		"\x23\x00\x00\x04\x40\x0b\x10\x27\x01\x01\x00\x00" +
		"\x27\x00\x00\x04\x30\x0c\xcf\x07\x01\x01\x00\x00" +
		"\x28\x00\x67\x04\x30\x0c\x10\x27\x01\x01\x00\x00" +
		"\x81\x00\x22\x04\x00\x00\x10\x27\x01\x01\x00\x00"

// ---------------------------------------------------------------------------
// ZoneInfoRecords is an array of zoneinfo.ZoneInfoRecord items concatenated
// together.
//
// Total: 596 (351 zones, 245 links)
// ---------------------------------------------------------------------------

var ZoneInfoRecords = []zoneinfo.ZoneInfoRecord{
	// 0: Link GB -> Europe/London
	{
		ZoneID: 0x005973ae,
		NameIndex: 490, // "GB"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 1: Link NZ -> Pacific/Auckland
	{
		ZoneID: 0x005974ad,
		NameIndex: 523, // "NZ"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 98, // Pacific/Auckland
	},
	// 2: Link Asia/Kuala_Lumpur -> Asia/Singapore
	{
		ZoneID: 0x014763c4,
		NameIndex: 285, // "Asia/Kuala_Lumpur"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 494, // Asia/Singapore
	},
	// 3: Link Africa/Libreville -> Africa/Lagos
	{
		ZoneID: 0x01d96de4,
		NameIndex: 32, // "Africa/Libreville"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 4: Link Indian/Cocos -> Asia/Yangon
	{
		ZoneID: 0x021e86de,
		NameIndex: 503, // "Indian/Cocos"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 524, // Asia/Yangon
	},
	// 5: Link Australia/Victoria -> Australia/Melbourne
	{
		ZoneID: 0x0260d5db,
		NameIndex: 366, // "Australia/Victoria"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 49, // Australia/Melbourne
	},
	// 6: Link Atlantic/Faeroe -> Atlantic/Faroe
	{
		ZoneID: 0x031ec516,
		NameIndex: 338, // "Atlantic/Faeroe"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 539, // Atlantic/Faroe
	},
	// 7: Zone America/St_Johns
	{
		ZoneID: 0x04b14e6e,
		NameIndex: 204, // "America/St_Johns"
		EraIndex: 456,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 8: Link America/St_Kitts -> America/Puerto_Rico
	{
		ZoneID: 0x04c0507b,
		NameIndex: 205, // "America/St_Kitts"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 9: Link Africa/Ouagadougou -> Africa/Abidjan
	{
		ZoneID: 0x04d7219a,
		NameIndex: 47, // "Africa/Ouagadougou"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 10: Link America/St_Lucia -> America/Puerto_Rico
	{
		ZoneID: 0x04d8b3ba,
		NameIndex: 206, // "America/St_Lucia"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 11: Zone America/North_Dakota/New_Salem
	{
		ZoneID: 0x04f9958e,
		NameIndex: 175, // "America/North_Dakota/New_Salem"
		EraIndex: 399,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 12: Zone Asia/Jakarta
	{
		ZoneID: 0x0506ab50,
		NameIndex: 273, // "Asia/Jakarta"
		EraIndex: 621,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 13: Link Africa/Bujumbura -> Africa/Maputo
	{
		ZoneID: 0x05232a47,
		NameIndex: 12, // "Africa/Bujumbura"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 14: Zone America/Mazatlan
	{
		ZoneID: 0x0532189e,
		NameIndex: 156, // "America/Mazatlan"
		EraIndex: 352,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 15: Link America/St_Barthelemy -> America/Puerto_Rico
	{
		ZoneID: 0x054e6a79,
		NameIndex: 203, // "America/St_Barthelemy"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 16: Link Africa/Addis_Ababa -> Africa/Nairobi
	{
		ZoneID: 0x05ae1e65,
		NameIndex: 2, // "Africa/Addis_Ababa"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 17: Zone Pacific/Fakaofo
	{
		ZoneID: 0x06532bba,
		NameIndex: 536, // "Pacific/Fakaofo"
		EraIndex: 1037,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 18: Zone America/Hermosillo
	{
		ZoneID: 0x065d21c4,
		NameIndex: 126, // "America/Hermosillo"
		EraIndex: 265,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 19: Link Mexico/BajaSur -> America/Mazatlan
	{
		ZoneID: 0x08ee3641,
		NameIndex: 521, // "Mexico/BajaSur"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 14, // America/Mazatlan
	},
	// 20: Zone Asia/Tbilisi
	{
		ZoneID: 0x0903e442,
		NameIndex: 316, // "Asia/Tbilisi"
		EraIndex: 734,
		EraCount: 8,
		TargetIndex: 0,
	},
	// 21: Zone America/Indiana/Tell_City
	{
		ZoneID: 0x09263612,
		NameIndex: 131, // "America/Indiana/Tell_City"
		EraIndex: 285,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 22: Link US/Hawaii -> Pacific/Honolulu
	{
		ZoneID: 0x09c8de2f,
		NameIndex: 585, // "US/Hawaii"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 552, // Pacific/Honolulu
	},
	// 23: Zone America/Boa_Vista
	{
		ZoneID: 0x0a7b7efe,
		NameIndex: 82, // "America/Boa_Vista"
		EraIndex: 176,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 24: Zone Asia/Colombo
	{
		ZoneID: 0x0af0e91d,
		NameIndex: 257, // "Asia/Colombo"
		EraIndex: 580,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 25: Zone CET
	{
		ZoneID: 0x0b87d921,
		NameIndex: 373, // "CET"
		EraIndex: 828,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 26: Zone EET
	{
		ZoneID: 0x0b87e1a3,
		NameIndex: 386, // "EET"
		EraIndex: 830,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 27: Zone EST
	{
		ZoneID: 0x0b87e371,
		NameIndex: 387, // "EST"
		EraIndex: 831,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 28: Link GMT -> Etc/GMT
	{
		ZoneID: 0x0b87eb2d,
		NameIndex: 492, // "GMT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 513, // Etc/GMT
	},
	// 29: Zone HST
	{
		ZoneID: 0x0b87f034,
		NameIndex: 497, // "HST"
		EraIndex: 1019,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 30: Zone MET
	{
		ZoneID: 0x0b8803ab,
		NameIndex: 517, // "MET"
		EraIndex: 1024,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 31: Zone MST
	{
		ZoneID: 0x0b880579,
		NameIndex: 518, // "MST"
		EraIndex: 1025,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 32: Link PRC -> Asia/Shanghai
	{
		ZoneID: 0x0b88120a,
		NameIndex: 526, // "PRC"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 572, // Asia/Shanghai
	},
	// 33: Link ROC -> Asia/Taipei
	{
		ZoneID: 0x0b881a29,
		NameIndex: 574, // "ROC"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 499, // Asia/Taipei
	},
	// 34: Link ROK -> Asia/Seoul
	{
		ZoneID: 0x0b881a31,
		NameIndex: 575, // "ROK"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 67, // Asia/Seoul
	},
	// 35: Link UCT -> Etc/UTC
	{
		ZoneID: 0x0b882571,
		NameIndex: 578, // "UCT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 515, // Etc/UTC
	},
	// 36: Link UTC -> Etc/UTC
	{
		ZoneID: 0x0b882791,
		NameIndex: 591, // "UTC"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 515, // Etc/UTC
	},
	// 37: Zone WET
	{
		ZoneID: 0x0b882e35,
		NameIndex: 594, // "WET"
		EraIndex: 1080,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 38: Zone America/Guatemala
	{
		ZoneID: 0x0c8259f7,
		NameIndex: 121, // "America/Guatemala"
		EraIndex: 257,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 39: Link Europe/Mariehamn -> Europe/Helsinki
	{
		ZoneID: 0x0caa6496,
		NameIndex: 457, // "Europe/Mariehamn"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 233, // Europe/Helsinki
	},
	// 40: Zone Africa/Monrovia
	{
		ZoneID: 0x0ce90385,
		NameIndex: 42, // "Africa/Monrovia"
		EraIndex: 27,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 41: Link Egypt -> Africa/Cairo
	{
		ZoneID: 0x0d1a278e,
		NameIndex: 389, // "Egypt"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 270, // Africa/Cairo
	},
	// 42: Link GMT+0 -> Etc/GMT
	{
		ZoneID: 0x0d2f7028,
		NameIndex: 493, // "GMT+0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 513, // Etc/GMT
	},
	// 43: Link GMT-0 -> Etc/GMT
	{
		ZoneID: 0x0d2f706a,
		NameIndex: 494, // "GMT-0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 513, // Etc/GMT
	},
	// 44: Link Japan -> Asia/Tokyo
	{
		ZoneID: 0x0d712f8f,
		NameIndex: 514, // "Japan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 68, // Asia/Tokyo
	},
	// 45: Link Libya -> Africa/Tripoli
	{
		ZoneID: 0x0d998b16,
		NameIndex: 516, // "Libya"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 398, // Africa/Tripoli
	},
	// 46: Link Kwajalein -> Pacific/Kwajalein
	{
		ZoneID: 0x0e57afbb,
		NameIndex: 515, // "Kwajalein"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 324, // Pacific/Kwajalein
	},
	// 47: Zone Antarctica/Rothera
	{
		ZoneID: 0x0e86d203,
		NameIndex: 229, // "Antarctica/Rothera"
		EraIndex: 510,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 48: Zone America/Yellowknife
	{
		ZoneID: 0x0f76c76f,
		NameIndex: 221, // "America/Yellowknife"
		EraIndex: 480,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 49: Zone Australia/Melbourne
	{
		ZoneID: 0x0fe559a3,
		NameIndex: 358, // "Australia/Melbourne"
		EraIndex: 823,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 50: Zone America/Sao_Paulo
	{
		ZoneID: 0x1063bfc9,
		NameIndex: 199, // "America/Sao_Paulo"
		EraIndex: 448,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 51: Link Europe/Amsterdam -> Europe/Brussels
	{
		ZoneID: 0x109395c2,
		NameIndex: 426, // "Europe/Amsterdam"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 529, // Europe/Brussels
	},
	// 52: Zone America/Indiana/Vevay
	{
		ZoneID: 0x10aca054,
		NameIndex: 132, // "America/Indiana/Vevay"
		EraIndex: 289,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 53: Zone America/Scoresbysund
	{
		ZoneID: 0x123f8d2a,
		NameIndex: 200, // "America/Scoresbysund"
		EraIndex: 449,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 54: Zone Asia/Samarkand
	{
		ZoneID: 0x13ae5104,
		NameIndex: 309, // "Asia/Samarkand"
		EraIndex: 716,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 55: Zone Asia/Amman
	{
		ZoneID: 0x148d21bc,
		NameIndex: 237, // "Asia/Amman"
		EraIndex: 518,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 56: Zone Asia/Aqtau
	{
		ZoneID: 0x148f710e,
		NameIndex: 239, // "Asia/Aqtau"
		EraIndex: 526,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 57: Zone Asia/Chita
	{
		ZoneID: 0x14ae863b,
		NameIndex: 253, // "Asia/Chita"
		EraIndex: 570,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 58: Link Asia/Dacca -> Asia/Dhaka
	{
		ZoneID: 0x14bcac5e,
		NameIndex: 258, // "Asia/Dacca"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 59, // Asia/Dhaka
	},
	// 59: Zone Asia/Dhaka
	{
		ZoneID: 0x14c07b8b,
		NameIndex: 260, // "Asia/Dhaka"
		EraIndex: 586,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 60: Zone Asia/Dubai
	{
		ZoneID: 0x14c79f77,
		NameIndex: 262, // "Asia/Dubai"
		EraIndex: 591,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 61: Zone America/Bahia_Banderas
	{
		ZoneID: 0x14f6329a,
		NameIndex: 77, // "America/Bahia_Banderas"
		EraIndex: 169,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 62: Zone Asia/Kabul
	{
		ZoneID: 0x153b5601,
		NameIndex: 276, // "Asia/Kabul"
		EraIndex: 624,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 63: Link Asia/Ashkhabad -> Asia/Ashgabat
	{
		ZoneID: 0x15454f09,
		NameIndex: 242, // "Asia/Ashkhabad"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 453, // Asia/Ashgabat
	},
	// 64: Link Asia/Macao -> Asia/Macau
	{
		ZoneID: 0x155f88b3,
		NameIndex: 288, // "Asia/Macao"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 65, // Asia/Macau
	},
	// 65: Zone Asia/Macau
	{
		ZoneID: 0x155f88b9,
		NameIndex: 289, // "Asia/Macau"
		EraIndex: 648,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 66: Zone Asia/Qatar
	{
		ZoneID: 0x15a8330b,
		NameIndex: 302, // "Asia/Qatar"
		EraIndex: 689,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 67: Zone Asia/Seoul
	{
		ZoneID: 0x15ce82da,
		NameIndex: 310, // "Asia/Seoul"
		EraIndex: 721,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 68: Zone Asia/Tokyo
	{
		ZoneID: 0x15e606a8,
		NameIndex: 321, // "Asia/Tokyo"
		EraIndex: 747,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 69: Zone Asia/Tomsk
	{
		ZoneID: 0x15e60e60,
		NameIndex: 322, // "Asia/Tomsk"
		EraIndex: 748,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 70: Link Asia/Tel_Aviv -> Asia/Jerusalem
	{
		ZoneID: 0x166d7c2c,
		NameIndex: 318, // "Asia/Tel_Aviv"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 193, // Asia/Jerusalem
	},
	// 71: Zone Asia/Thimphu
	{
		ZoneID: 0x170380d1,
		NameIndex: 320, // "Asia/Thimphu"
		EraIndex: 745,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 72: Zone America/Guayaquil
	{
		ZoneID: 0x17e64958,
		NameIndex: 122, // "America/Guayaquil"
		EraIndex: 258,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 73: Link America/Montserrat -> America/Puerto_Rico
	{
		ZoneID: 0x199b0a35,
		NameIndex: 167, // "America/Montserrat"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 74: Zone America/Kentucky/Louisville
	{
		ZoneID: 0x1a21024b,
		NameIndex: 141, // "America/Kentucky/Louisville"
		EraIndex: 318,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 75: Zone Asia/Pontianak
	{
		ZoneID: 0x1a76c057,
		NameIndex: 300, // "Asia/Pontianak"
		EraIndex: 684,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 76: Link Europe/Podgorica -> Europe/Belgrade
	{
		ZoneID: 0x1c1a499c,
		NameIndex: 464, // "Europe/Podgorica"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 535, // Europe/Belgrade
	},
	// 77: Link Atlantic/Reykjavik -> Africa/Abidjan
	{
		ZoneID: 0x1c2b4f74,
		NameIndex: 342, // "Atlantic/Reykjavik"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 78: Zone America/New_York
	{
		ZoneID: 0x1e2a7654,
		NameIndex: 169, // "America/New_York"
		EraIndex: 384,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 79: Link Europe/Luxembourg -> Europe/Brussels
	{
		ZoneID: 0x1f8bc6ce,
		NameIndex: 454, // "Europe/Luxembourg"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 529, // Europe/Brussels
	},
	// 80: Link Asia/Aden -> Asia/Riyadh
	{
		ZoneID: 0x1fa7084a,
		NameIndex: 235, // "Asia/Aden"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 489, // Asia/Riyadh
	},
	// 81: Zone Asia/Baku
	{
		ZoneID: 0x1fa788b5,
		NameIndex: 246, // "Asia/Baku"
		EraIndex: 552,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 82: Zone Asia/Dili
	{
		ZoneID: 0x1fa8c394,
		NameIndex: 261, // "Asia/Dili"
		EraIndex: 588,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 83: Zone Asia/Gaza
	{
		ZoneID: 0x1faa4875,
		NameIndex: 265, // "Asia/Gaza"
		EraIndex: 599,
		EraCount: 9,
		TargetIndex: 0,
	},
	// 84: Zone Asia/Hovd
	{
		ZoneID: 0x1fab0fe3,
		NameIndex: 270, // "Asia/Hovd"
		EraIndex: 614,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 85: Zone Asia/Omsk
	{
		ZoneID: 0x1faeddac,
		NameIndex: 297, // "Asia/Omsk"
		EraIndex: 671,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 86: Zone Asia/Oral
	{
		ZoneID: 0x1faef0a0,
		NameIndex: 298, // "Asia/Oral"
		EraIndex: 676,
		EraCount: 8,
		TargetIndex: 0,
	},
	// 87: Link America/Montreal -> America/Toronto
	{
		ZoneID: 0x203a1ea8,
		NameIndex: 166, // "America/Montreal"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 276, // America/Toronto
	},
	// 88: Zone Asia/Ho_Chi_Minh
	{
		ZoneID: 0x20f2d127,
		NameIndex: 268, // "Asia/Ho_Chi_Minh"
		EraIndex: 611,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 89: Zone Asia/Damascus
	{
		ZoneID: 0x20fbb063,
		NameIndex: 259, // "Asia/Damascus"
		EraIndex: 584,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 90: Link America/Argentina/ComodRivadavia -> America/Argentina/Catamarca
	{
		ZoneID: 0x22758877,
		NameIndex: 61, // "America/Argentina/ComodRivadavia"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 318, // America/Argentina/Catamarca
	},
	// 91: Zone Pacific/Apia
	{
		ZoneID: 0x23359b5e,
		NameIndex: 528, // "Pacific/Apia"
		EraIndex: 1028,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 92: Zone Pacific/Fiji
	{
		ZoneID: 0x23383ba5,
		NameIndex: 537, // "Pacific/Fiji"
		EraIndex: 1039,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 93: Zone Pacific/Guam
	{
		ZoneID: 0x2338f9ed,
		NameIndex: 542, // "Pacific/Guam"
		EraIndex: 1044,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 94: Zone Pacific/Niue
	{
		ZoneID: 0x233ca014,
		NameIndex: 553, // "Pacific/Niue"
		EraIndex: 1062,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 95: Link Pacific/Truk -> Pacific/Port_Moresby
	{
		ZoneID: 0x234010a9,
		NameIndex: 568, // "Pacific/Truk"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 419, // Pacific/Port_Moresby
	},
	// 96: Link Pacific/Wake -> Pacific/Tarawa
	{
		ZoneID: 0x23416c2b,
		NameIndex: 569, // "Pacific/Wake"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 563, // Pacific/Tarawa
	},
	// 97: Zone Australia/Adelaide
	{
		ZoneID: 0x2428e8a3,
		NameIndex: 347, // "Australia/Adelaide"
		EraIndex: 807,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 98: Zone Pacific/Auckland
	{
		ZoneID: 0x25062f86,
		NameIndex: 529, // "Pacific/Auckland"
		EraIndex: 1030,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 99: Zone Pacific/Tongatapu
	{
		ZoneID: 0x262ca836,
		NameIndex: 567, // "Pacific/Tongatapu"
		EraIndex: 1078,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 100: Zone America/Monterrey
	{
		ZoneID: 0x269a1deb,
		NameIndex: 164, // "America/Monterrey"
		EraIndex: 376,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 101: Link Etc/Greenwich -> Etc/GMT
	{
		ZoneID: 0x26daa98c,
		NameIndex: 421, // "Etc/Greenwich"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 513, // Etc/GMT
	},
	// 102: Link Australia/Currie -> Australia/Hobart
	{
		ZoneID: 0x278b6a24,
		NameIndex: 351, // "Australia/Currie"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 124, // Australia/Hobart
	},
	// 103: Zone Australia/Darwin
	{
		ZoneID: 0x2876bdff,
		NameIndex: 352, // "Australia/Darwin"
		EraIndex: 814,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 104: Link Pacific/Pohnpei -> Pacific/Guadalcanal
	{
		ZoneID: 0x28929f96,
		NameIndex: 559, // "Pacific/Pohnpei"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 568, // Pacific/Guadalcanal
	},
	// 105: Zone Asia/Famagusta
	{
		ZoneID: 0x289b4f8b,
		NameIndex: 264, // "Asia/Famagusta"
		EraIndex: 595,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 106: Zone America/Indiana/Vincennes
	{
		ZoneID: 0x28a0b212,
		NameIndex: 133, // "America/Indiana/Vincennes"
		EraIndex: 293,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 107: Zone America/Indiana/Indianapolis
	{
		ZoneID: 0x28a669a4,
		NameIndex: 127, // "America/Indiana/Indianapolis"
		EraIndex: 268,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 108: Zone Asia/Vladivostok
	{
		ZoneID: 0x29de34a8,
		NameIndex: 329, // "Asia/Vladivostok"
		EraIndex: 765,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 109: Link Australia/Canberra -> Australia/Sydney
	{
		ZoneID: 0x2a09ae58,
		NameIndex: 350, // "Australia/Canberra"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 168, // Australia/Sydney
	},
	// 110: Zone America/Fortaleza
	{
		ZoneID: 0x2ad018ee,
		NameIndex: 114, // "America/Fortaleza"
		EraIndex: 242,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 111: Zone America/Vancouver
	{
		ZoneID: 0x2c6f6b1f,
		NameIndex: 216, // "America/Vancouver"
		EraIndex: 470,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 112: Link America/Pangnirtung -> America/Iqaluit
	{
		ZoneID: 0x2d999193,
		NameIndex: 179, // "America/Pangnirtung"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 113, // America/Iqaluit
	},
	// 113: Zone America/Iqaluit
	{
		ZoneID: 0x2de310bf,
		NameIndex: 137, // "America/Iqaluit"
		EraIndex: 306,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 114: Link Jamaica -> America/Jamaica
	{
		ZoneID: 0x2e44fdab,
		NameIndex: 513, // "Jamaica"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 182, // America/Jamaica
	},
	// 115: Zone Pacific/Chatham
	{
		ZoneID: 0x2f0de999,
		NameIndex: 531, // "Pacific/Chatham"
		EraIndex: 1033,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 116: Link Etc/Universal -> Etc/UTC
	{
		ZoneID: 0x2f8cb9a9,
		NameIndex: 424, // "Etc/Universal"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 515, // Etc/UTC
	},
	// 117: Zone America/Indiana/Marengo
	{
		ZoneID: 0x2feeee72,
		NameIndex: 129, // "America/Indiana/Marengo"
		EraIndex: 275,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 118: Zone Europe/Tallinn
	{
		ZoneID: 0x30c4e096,
		NameIndex: 476, // "Europe/Tallinn"
		EraIndex: 982,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 119: Link Africa/Djibouti -> Africa/Nairobi
	{
		ZoneID: 0x30ea01d4,
		NameIndex: 19, // "Africa/Djibouti"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 120: Zone Asia/Ulaanbaatar
	{
		ZoneID: 0x30f0cc4e,
		NameIndex: 324, // "Asia/Ulaanbaatar"
		EraIndex: 755,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 121: Link Africa/Gaborone -> Africa/Maputo
	{
		ZoneID: 0x317c0aa7,
		NameIndex: 23, // "Africa/Gaborone"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 122: Zone America/Argentina/Ushuaia
	{
		ZoneID: 0x320dcdde,
		NameIndex: 71, // "America/Argentina/Ushuaia"
		EraIndex: 155,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 123: Link Asia/Calcutta -> Asia/Kolkata
	{
		ZoneID: 0x328a44c3,
		NameIndex: 252, // "Asia/Calcutta"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 251, // Asia/Kolkata
	},
	// 124: Zone Australia/Hobart
	{
		ZoneID: 0x32bf951a,
		NameIndex: 354, // "Australia/Hobart"
		EraIndex: 816,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 125: Zone Asia/Dushanbe
	{
		ZoneID: 0x32fc5c3c,
		NameIndex: 263, // "Asia/Dushanbe"
		EraIndex: 592,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 126: Zone Atlantic/South_Georgia
	{
		ZoneID: 0x33013174,
		NameIndex: 343, // "Atlantic/South_Georgia"
		EraIndex: 802,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 127: Zone America/Phoenix
	{
		ZoneID: 0x34b5af01,
		NameIndex: 181, // "America/Phoenix"
		EraIndex: 414,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 128: Link Asia/Istanbul -> Europe/Istanbul
	{
		ZoneID: 0x382e7894,
		NameIndex: 272, // "Asia/Istanbul"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 399, // Europe/Istanbul
	},
	// 129: Link Asia/Ulan_Bator -> Asia/Ulaanbaatar
	{
		ZoneID: 0x394db4d9,
		NameIndex: 325, // "Asia/Ulan_Bator"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 120, // Asia/Ulaanbaatar
	},
	// 130: Zone Antarctica/Mawson
	{
		ZoneID: 0x399cd863,
		NameIndex: 226, // "Antarctica/Mawson"
		EraIndex: 504,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 131: Link Africa/Brazzaville -> Africa/Lagos
	{
		ZoneID: 0x39cda760,
		NameIndex: 11, // "Africa/Brazzaville"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 132: Zone America/Caracas
	{
		ZoneID: 0x3be064f4,
		NameIndex: 89, // "America/Caracas"
		EraIndex: 194,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 133: Zone America/Cayenne
	{
		ZoneID: 0x3c617269,
		NameIndex: 91, // "America/Cayenne"
		EraIndex: 197,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 134: Link Africa/Porto-Novo -> Africa/Lagos
	{
		ZoneID: 0x3d1bf95d,
		NameIndex: 48, // "Africa/Porto-Novo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 135: Zone Atlantic/Bermuda
	{
		ZoneID: 0x3d4bb1c4,
		NameIndex: 335, // "Atlantic/Bermuda"
		EraIndex: 790,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 136: Zone America/Managua
	{
		ZoneID: 0x3d5e7600,
		NameIndex: 151, // "America/Managua"
		EraIndex: 334,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 137: Link America/Marigot -> America/Puerto_Rico
	{
		ZoneID: 0x3dab3a59,
		NameIndex: 153, // "America/Marigot"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 138: Link Europe/Guernsey -> Europe/London
	{
		ZoneID: 0x3db12c16,
		NameIndex: 442, // "Europe/Guernsey"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 139: Link Africa/Nouakchott -> Africa/Abidjan
	{
		ZoneID: 0x3dc49dba,
		NameIndex: 46, // "Africa/Nouakchott"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 140: Link America/Louisville -> America/Kentucky/Louisville
	{
		ZoneID: 0x3dcb47ee,
		NameIndex: 148, // "America/Louisville"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 74, // America/Kentucky/Louisville
	},
	// 141: Zone America/Argentina/San_Juan
	{
		ZoneID: 0x3e1009bd,
		NameIndex: 68, // "America/Argentina/San_Juan"
		EraIndex: 125,
		EraCount: 9,
		TargetIndex: 0,
	},
	// 142: Zone America/Argentina/San_Luis
	{
		ZoneID: 0x3e11238c,
		NameIndex: 69, // "America/Argentina/San_Luis"
		EraIndex: 134,
		EraCount: 13,
		TargetIndex: 0,
	},
	// 143: Zone Europe/Volgograd
	{
		ZoneID: 0x3ed0f389,
		NameIndex: 485, // "Europe/Volgograd"
		EraIndex: 1006,
		EraCount: 8,
		TargetIndex: 0,
	},
	// 144: Zone America/Fort_Nelson
	{
		ZoneID: 0x3f437e0f,
		NameIndex: 112, // "America/Fort_Nelson"
		EraIndex: 239,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 145: Zone Etc/GMT+10
	{
		ZoneID: 0x3f8f1cc4,
		NameIndex: 394, // "Etc/GMT+10"
		EraIndex: 835,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 146: Zone Etc/GMT+11
	{
		ZoneID: 0x3f8f1cc5,
		NameIndex: 395, // "Etc/GMT+11"
		EraIndex: 836,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 147: Zone Etc/GMT+12
	{
		ZoneID: 0x3f8f1cc6,
		NameIndex: 396, // "Etc/GMT+12"
		EraIndex: 837,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 148: Zone Etc/GMT-10
	{
		ZoneID: 0x3f8f2546,
		NameIndex: 407, // "Etc/GMT-10"
		EraIndex: 847,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 149: Zone Etc/GMT-11
	{
		ZoneID: 0x3f8f2547,
		NameIndex: 408, // "Etc/GMT-11"
		EraIndex: 848,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 150: Zone Etc/GMT-12
	{
		ZoneID: 0x3f8f2548,
		NameIndex: 409, // "Etc/GMT-12"
		EraIndex: 849,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 151: Zone Etc/GMT-13
	{
		ZoneID: 0x3f8f2549,
		NameIndex: 410, // "Etc/GMT-13"
		EraIndex: 850,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 152: Zone Etc/GMT-14
	{
		ZoneID: 0x3f8f254a,
		NameIndex: 411, // "Etc/GMT-14"
		EraIndex: 851,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 153: Zone Antarctica/Palmer
	{
		ZoneID: 0x40962f4f,
		NameIndex: 228, // "Antarctica/Palmer"
		EraIndex: 506,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 154: Link Canada/Pacific -> America/Vancouver
	{
		ZoneID: 0x40fa3c7b,
		NameIndex: 380, // "Canada/Pacific"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 111, // America/Vancouver
	},
	// 155: Zone Europe/Athens
	{
		ZoneID: 0x4318fa27,
		NameIndex: 429, // "Europe/Athens"
		EraIndex: 870,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 156: Link Indian/Kerguelen -> Indian/Maldives
	{
		ZoneID: 0x4351b389,
		NameIndex: 505, // "Indian/Kerguelen"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 360, // Indian/Maldives
	},
	// 157: Zone America/Indiana/Winamac
	{
		ZoneID: 0x4413fa69,
		NameIndex: 134, // "America/Indiana/Winamac"
		EraIndex: 298,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 158: Zone Europe/Berlin
	{
		ZoneID: 0x44644c20,
		NameIndex: 432, // "Europe/Berlin"
		EraIndex: 874,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 159: Link Atlantic/St_Helena -> Africa/Abidjan
	{
		ZoneID: 0x451fc5f7,
		NameIndex: 344, // "Atlantic/St_Helena"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 160: Zone Indian/Chagos
	{
		ZoneID: 0x456f7c3c,
		NameIndex: 501, // "Indian/Chagos"
		EraIndex: 1020,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 161: Link Indian/Mahe -> Asia/Dubai
	{
		ZoneID: 0x45e725e2,
		NameIndex: 506, // "Indian/Mahe"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 60, // Asia/Dubai
	},
	// 162: Link Indian/Comoro -> Africa/Nairobi
	{
		ZoneID: 0x45f4deb6,
		NameIndex: 504, // "Indian/Comoro"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 163: Link America/Mendoza -> America/Argentina/Mendoza
	{
		ZoneID: 0x46b4e054,
		NameIndex: 157, // "America/Mendoza"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 427, // America/Argentina/Mendoza
	},
	// 164: Zone Asia/Ust-Nera
	{
		ZoneID: 0x4785f921,
		NameIndex: 327, // "Asia/Ust-Nera"
		EraIndex: 758,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 165: Zone Europe/Dublin
	{
		ZoneID: 0x4a275f62,
		NameIndex: 440, // "Europe/Dublin"
		EraIndex: 889,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 166: Zone Asia/Nicosia
	{
		ZoneID: 0x4b0fcf78,
		NameIndex: 294, // "Asia/Nicosia"
		EraIndex: 657,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 167: Zone America/Chicago
	{
		ZoneID: 0x4b92b5d4,
		NameIndex: 93, // "America/Chicago"
		EraIndex: 198,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 168: Zone Australia/Sydney
	{
		ZoneID: 0x4d1e9776,
		NameIndex: 364, // "Australia/Sydney"
		EraIndex: 826,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 169: Link NZ-CHAT -> Pacific/Chatham
	{
		ZoneID: 0x4d42afda,
		NameIndex: 524, // "NZ-CHAT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 115, // Pacific/Chatham
	},
	// 170: Link US/Arizona -> America/Phoenix
	{
		ZoneID: 0x4ec52670,
		NameIndex: 581, // "US/Arizona"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 127, // America/Phoenix
	},
	// 171: Link Antarctica/Vostok -> Asia/Urumqi
	{
		ZoneID: 0x4f966fd4,
		NameIndex: 233, // "Antarctica/Vostok"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 509, // Asia/Urumqi
	},
	// 172: Link US/Aleutian -> America/Adak
	{
		ZoneID: 0x4fe013ef,
		NameIndex: 580, // "US/Aleutian"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 355, // America/Adak
	},
	// 173: Zone Australia/Brisbane
	{
		ZoneID: 0x4fedc9c0,
		NameIndex: 348, // "Australia/Brisbane"
		EraIndex: 809,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 174: Link America/Catamarca -> America/Argentina/Catamarca
	{
		ZoneID: 0x5036e963,
		NameIndex: 90, // "America/Catamarca"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 318, // America/Argentina/Catamarca
	},
	// 175: Zone America/Asuncion
	{
		ZoneID: 0x50ec79a6,
		NameIndex: 73, // "America/Asuncion"
		EraIndex: 162,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 176: Zone Asia/Karachi
	{
		ZoneID: 0x527f5245,
		NameIndex: 278, // "Asia/Karachi"
		EraIndex: 630,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 177: Link Asia/Kashgar -> Asia/Urumqi
	{
		ZoneID: 0x52955193,
		NameIndex: 279, // "Asia/Kashgar"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 509, // Asia/Urumqi
	},
	// 178: Link Canada/Atlantic -> America/Halifax
	{
		ZoneID: 0x536b119c,
		NameIndex: 375, // "Canada/Atlantic"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 456, // America/Halifax
	},
	// 179: Zone Pacific/Gambier
	{
		ZoneID: 0x53720c3a,
		NameIndex: 540, // "Pacific/Gambier"
		EraIndex: 1042,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 180: Zone America/Whitehorse
	{
		ZoneID: 0x54e0e3e8,
		NameIndex: 218, // "America/Whitehorse"
		EraIndex: 472,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 181: Zone America/Martinique
	{
		ZoneID: 0x551e84c5,
		NameIndex: 154, // "America/Martinique"
		EraIndex: 345,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 182: Zone America/Jamaica
	{
		ZoneID: 0x565dad6c,
		NameIndex: 138, // "America/Jamaica"
		EraIndex: 309,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 183: Link US/Samoa -> Pacific/Pago_Pago
	{
		ZoneID: 0x566821cd,
		NameIndex: 590, // "US/Samoa"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 208, // Pacific/Pago_Pago
	},
	// 184: Link Hongkong -> Asia/Hong_Kong
	{
		ZoneID: 0x56d36560,
		NameIndex: 498, // "Hongkong"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 186, // Asia/Hong_Kong
	},
	// 185: Link Europe/Jersey -> Europe/London
	{
		ZoneID: 0x570dae76,
		NameIndex: 446, // "Europe/Jersey"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 186: Zone Asia/Hong_Kong
	{
		ZoneID: 0x577f28ac,
		NameIndex: 269, // "Asia/Hong_Kong"
		EraIndex: 613,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 187: Zone Pacific/Marquesas
	{
		ZoneID: 0x57ca7135,
		NameIndex: 550, // "Pacific/Marquesas"
		EraIndex: 1059,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 188: Zone America/Miquelon
	{
		ZoneID: 0x59674330,
		NameIndex: 162, // "America/Miquelon"
		EraIndex: 369,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 189: Link Antarctica/DumontDUrville -> Pacific/Port_Moresby
	{
		ZoneID: 0x5a3c656c,
		NameIndex: 224, // "Antarctica/DumontDUrville"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 419, // Pacific/Port_Moresby
	},
	// 190: Link Atlantic/Jan_Mayen -> Europe/Berlin
	{
		ZoneID: 0x5a7535b6,
		NameIndex: 340, // "Atlantic/Jan_Mayen"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 158, // Europe/Berlin
	},
	// 191: Zone America/Anchorage
	{
		ZoneID: 0x5a79260e,
		NameIndex: 55, // "America/Anchorage"
		EraIndex: 50,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 192: Link US/Eastern -> America/New_York
	{
		ZoneID: 0x5bb7e78e,
		NameIndex: 584, // "US/Eastern"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 78, // America/New_York
	},
	// 193: Zone Asia/Jerusalem
	{
		ZoneID: 0x5becd23a,
		NameIndex: 275, // "Asia/Jerusalem"
		EraIndex: 623,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 194: Link Europe/Stockholm -> Europe/Berlin
	{
		ZoneID: 0x5bf6fbb8,
		NameIndex: 475, // "Europe/Stockholm"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 158, // Europe/Berlin
	},
	// 195: Zone Europe/Lisbon
	{
		ZoneID: 0x5c00a70b,
		NameIndex: 451, // "Europe/Lisbon"
		EraIndex: 919,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 196: Zone Atlantic/Cape_Verde
	{
		ZoneID: 0x5c5e1772,
		NameIndex: 337, // "Atlantic/Cape_Verde"
		EraIndex: 796,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 197: Zone Europe/London
	{
		ZoneID: 0x5c6a84ae,
		NameIndex: 453, // "Europe/London"
		EraIndex: 924,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 198: Link America/Cordoba -> America/Argentina/Cordoba
	{
		ZoneID: 0x5c8a7600,
		NameIndex: 97, // "America/Cordoba"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 461, // America/Argentina/Cordoba
	},
	// 199: Link Asia/Ujung_Pandang -> Asia/Makassar
	{
		ZoneID: 0x5d001eb3,
		NameIndex: 323, // "Asia/Ujung_Pandang"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 232, // Asia/Makassar
	},
	// 200: Link Africa/Mbabane -> Africa/Johannesburg
	{
		ZoneID: 0x5d3bdd40,
		NameIndex: 40, // "Africa/Mbabane"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 511, // Africa/Johannesburg
	},
	// 201: Zone Europe/Madrid
	{
		ZoneID: 0x5dbd1535,
		NameIndex: 455, // "Europe/Madrid"
		EraIndex: 927,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 202: Zone America/Moncton
	{
		ZoneID: 0x5e07fe24,
		NameIndex: 163, // "America/Moncton"
		EraIndex: 372,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 203: Zone Pacific/Bougainville
	{
		ZoneID: 0x5e10f7a4,
		NameIndex: 530, // "Pacific/Bougainville"
		EraIndex: 1031,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 204: Link Europe/Monaco -> Europe/Paris
	{
		ZoneID: 0x5ebf9f01,
		NameIndex: 459, // "Europe/Monaco"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 587, // Europe/Paris
	},
	// 205: Zone Europe/Moscow
	{
		ZoneID: 0x5ec266fc,
		NameIndex: 460, // "Europe/Moscow"
		EraIndex: 936,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 206: Zone America/Argentina/Jujuy
	{
		ZoneID: 0x5f2f46c5,
		NameIndex: 63, // "America/Argentina/Jujuy"
		EraIndex: 79,
		EraCount: 10,
		TargetIndex: 0,
	},
	// 207: Zone America/Argentina/Salta
	{
		ZoneID: 0x5fc73403,
		NameIndex: 67, // "America/Argentina/Salta"
		EraIndex: 118,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 208: Zone Pacific/Pago_Pago
	{
		ZoneID: 0x603aebd0,
		NameIndex: 556, // "Pacific/Pago_Pago"
		EraIndex: 1069,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 209: Link Pacific/Enderbury -> Pacific/Kanton
	{
		ZoneID: 0x61599a93,
		NameIndex: 535, // "Pacific/Enderbury"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 523, // Pacific/Kanton
	},
	// 210: Zone Africa/Sao_Tome
	{
		ZoneID: 0x61b319d1,
		NameIndex: 49, // "Africa/Sao_Tome"
		EraIndex: 33,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 211: Link Canada/Central -> America/Winnipeg
	{
		ZoneID: 0x626710f5,
		NameIndex: 376, // "Canada/Central"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 316, // America/Winnipeg
	},
	// 212: Link America/Creston -> America/Phoenix
	{
		ZoneID: 0x62a70204,
		NameIndex: 99, // "America/Creston"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 127, // America/Phoenix
	},
	// 213: Zone America/Costa_Rica
	{
		ZoneID: 0x63ff66be,
		NameIndex: 98, // "America/Costa_Rica"
		EraIndex: 211,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 214: Zone Asia/Qostanay
	{
		ZoneID: 0x654fe522,
		NameIndex: 303, // "Asia/Qostanay"
		EraIndex: 691,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 215: Zone America/Indiana/Knox
	{
		ZoneID: 0x6554adc9,
		NameIndex: 128, // "America/Indiana/Knox"
		EraIndex: 272,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 216: Zone Europe/Prague
	{
		ZoneID: 0x65ee5d48,
		NameIndex: 465, // "Europe/Prague"
		EraIndex: 943,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 217: Link Brazil/Acre -> America/Rio_Branco
	{
		ZoneID: 0x66934f93,
		NameIndex: 369, // "Brazil/Acre"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 393, // America/Rio_Branco
	},
	// 218: Link Brazil/East -> America/Sao_Paulo
	{
		ZoneID: 0x669578c5,
		NameIndex: 371, // "Brazil/East"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 50, // America/Sao_Paulo
	},
	// 219: Link Africa/Kinshasa -> Africa/Lagos
	{
		ZoneID: 0x6695d70c,
		NameIndex: 30, // "Africa/Kinshasa"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 220: Link Brazil/West -> America/Manaus
	{
		ZoneID: 0x669f689b,
		NameIndex: 372, // "Brazil/West"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 432, // America/Manaus
	},
	// 221: Link Africa/Mogadishu -> Africa/Nairobi
	{
		ZoneID: 0x66bc159b,
		NameIndex: 41, // "Africa/Mogadishu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 222: Zone America/Puerto_Rico
	{
		ZoneID: 0x6752ca31,
		NameIndex: 186, // "America/Puerto_Rico"
		EraIndex: 418,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 223: Link US/Indiana-Starke -> America/Indiana/Knox
	{
		ZoneID: 0x67977be7,
		NameIndex: 586, // "US/Indiana-Starke"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 215, // America/Indiana/Knox
	},
	// 224: Link America/Buenos_Aires -> America/Argentina/Buenos_Aires
	{
		ZoneID: 0x67d79a05,
		NameIndex: 85, // "America/Buenos_Aires"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 505, // America/Argentina/Buenos_Aires
	},
	// 225: Link Africa/Freetown -> Africa/Abidjan
	{
		ZoneID: 0x6823dd64,
		NameIndex: 22, // "Africa/Freetown"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 226: Link Indian/Christmas -> Asia/Bangkok
	{
		ZoneID: 0x68c207d5,
		NameIndex: 502, // "Indian/Christmas"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 394, // Asia/Bangkok
	},
	// 227: Zone Asia/Novokuznetsk
	{
		ZoneID: 0x69264f93,
		NameIndex: 295, // "Asia/Novokuznetsk"
		EraIndex: 659,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 228: Link America/Indianapolis -> America/Indiana/Indianapolis
	{
		ZoneID: 0x6a009ae1,
		NameIndex: 135, // "America/Indianapolis"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 107, // America/Indiana/Indianapolis
	},
	// 229: Link Europe/Sarajevo -> Europe/Belgrade
	{
		ZoneID: 0x6a576c3f,
		NameIndex: 470, // "Europe/Sarajevo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 535, // Europe/Belgrade
	},
	// 230: Link America/Curacao -> America/Puerto_Rico
	{
		ZoneID: 0x6a879184,
		NameIndex: 101, // "America/Curacao"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 231: Zone America/Tijuana
	{
		ZoneID: 0x6aa1df72,
		NameIndex: 213, // "America/Tijuana"
		EraIndex: 462,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 232: Zone Asia/Makassar
	{
		ZoneID: 0x6aa21c85,
		NameIndex: 291, // "Asia/Makassar"
		EraIndex: 655,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 233: Zone Europe/Helsinki
	{
		ZoneID: 0x6ab2975b,
		NameIndex: 443, // "Europe/Helsinki"
		EraIndex: 892,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 234: Link America/Lower_Princes -> America/Puerto_Rico
	{
		ZoneID: 0x6ae45b62,
		NameIndex: 149, // "America/Lower_Princes"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 235: Zone America/Porto_Velho
	{
		ZoneID: 0x6b1aac77,
		NameIndex: 185, // "America/Porto_Velho"
		EraIndex: 416,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 236: Zone Europe/Samara
	{
		ZoneID: 0x6bc0b139,
		NameIndex: 468, // "Europe/Samara"
		EraIndex: 953,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 237: Link Europe/Skopje -> Europe/Belgrade
	{
		ZoneID: 0x6c76fdd0,
		NameIndex: 473, // "Europe/Skopje"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 535, // Europe/Belgrade
	},
	// 238: Zone America/Edmonton
	{
		ZoneID: 0x6cb9484a,
		NameIndex: 108, // "America/Edmonton"
		EraIndex: 230,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 239: Zone America/Dawson_Creek
	{
		ZoneID: 0x6cf24e5b,
		NameIndex: 104, // "America/Dawson_Creek"
		EraIndex: 222,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 240: Link Asia/Rangoon -> Asia/Yangon
	{
		ZoneID: 0x6d1217c6,
		NameIndex: 305, // "Asia/Rangoon"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 524, // Asia/Yangon
	},
	// 241: Link US/East-Indiana -> America/Indiana/Indianapolis
	{
		ZoneID: 0x6dcf558a,
		NameIndex: 583, // "US/East-Indiana"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 107, // America/Indiana/Indianapolis
	},
	// 242: Zone America/Grand_Turk
	{
		ZoneID: 0x6e216197,
		NameIndex: 118, // "America/Grand_Turk"
		EraIndex: 253,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 243: Link America/Blanc-Sablon -> America/Puerto_Rico
	{
		ZoneID: 0x6e299892,
		NameIndex: 81, // "America/Blanc-Sablon"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 244: Zone Europe/Tirane
	{
		ZoneID: 0x6ea95b47,
		NameIndex: 477, // "Europe/Tirane"
		EraIndex: 988,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 245: Link US/Mountain -> America/Denver
	{
		ZoneID: 0x6eb88247,
		NameIndex: 588, // "US/Mountain"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 350, // America/Denver
	},
	// 246: Link Antarctica/McMurdo -> Pacific/Auckland
	{
		ZoneID: 0x6eeb5585,
		NameIndex: 227, // "Antarctica/McMurdo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 98, // Pacific/Auckland
	},
	// 247: Zone America/Araguaina
	{
		ZoneID: 0x6f9a3aef,
		NameIndex: 58, // "America/Araguaina"
		EraIndex: 54,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 248: Link Africa/Lubumbashi -> Africa/Maputo
	{
		ZoneID: 0x6fd88566,
		NameIndex: 35, // "Africa/Lubumbashi"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 249: Link Indian/Reunion -> Asia/Dubai
	{
		ZoneID: 0x7076c047,
		NameIndex: 510, // "Indian/Reunion"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 60, // Asia/Dubai
	},
	// 250: Zone Asia/Qyzylorda
	{
		ZoneID: 0x71282e81,
		NameIndex: 304, // "Asia/Qyzylorda"
		EraIndex: 698,
		EraCount: 10,
		TargetIndex: 0,
	},
	// 251: Zone Asia/Kolkata
	{
		ZoneID: 0x72c06cd9,
		NameIndex: 283, // "Asia/Kolkata"
		EraIndex: 641,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 252: Zone America/Ciudad_Juarez
	{
		ZoneID: 0x7347fc60,
		NameIndex: 95, // "America/Ciudad_Juarez"
		EraIndex: 204,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 253: Zone Europe/Vienna
	{
		ZoneID: 0x734cc2e5,
		NameIndex: 483, // "Europe/Vienna"
		EraIndex: 997,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 254: Link Africa/Asmara -> Africa/Nairobi
	{
		ZoneID: 0x73b278ef,
		NameIndex: 4, // "Africa/Asmara"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 255: Link Africa/Asmera -> Africa/Nairobi
	{
		ZoneID: 0x73b289f3,
		NameIndex: 5, // "Africa/Asmera"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 256: Zone Asia/Kamchatka
	{
		ZoneID: 0x73baf9d7,
		NameIndex: 277, // "Asia/Kamchatka"
		EraIndex: 625,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 257: Zone America/Santarem
	{
		ZoneID: 0x740caec1,
		NameIndex: 196, // "America/Santarem"
		EraIndex: 440,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 258: Zone America/Santiago
	{
		ZoneID: 0x7410c9bc,
		NameIndex: 197, // "America/Santiago"
		EraIndex: 443,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 259: Link Africa/Bamako -> Africa/Abidjan
	{
		ZoneID: 0x74c1e7a5,
		NameIndex: 6, // "Africa/Bamako"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 260: Link Africa/Bangui -> Africa/Lagos
	{
		ZoneID: 0x74c28ed0,
		NameIndex: 7, // "Africa/Bangui"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 261: Link Africa/Banjul -> Africa/Abidjan
	{
		ZoneID: 0x74c29b96,
		NameIndex: 8, // "Africa/Banjul"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 262: Link Europe/Nicosia -> Asia/Nicosia
	{
		ZoneID: 0x74efab8a,
		NameIndex: 461, // "Europe/Nicosia"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 166, // Asia/Nicosia
	},
	// 263: Zone Europe/Warsaw
	{
		ZoneID: 0x75185c19,
		NameIndex: 486, // "Europe/Warsaw"
		EraIndex: 1014,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 264: Zone America/El_Salvador
	{
		ZoneID: 0x752ad652,
		NameIndex: 110, // "America/El_Salvador"
		EraIndex: 238,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 265: Zone Africa/Bissau
	{
		ZoneID: 0x75564141,
		NameIndex: 9, // "Africa/Bissau"
		EraIndex: 5,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 266: Zone America/Santo_Domingo
	{
		ZoneID: 0x75a0d177,
		NameIndex: 198, // "America/Santo_Domingo"
		EraIndex: 444,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 267: Link US/Michigan -> America/Detroit
	{
		ZoneID: 0x766bb7bc,
		NameIndex: 587, // "US/Michigan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 334, // America/Detroit
	},
	// 268: Link Canada/Saskatchewan -> America/Regina
	{
		ZoneID: 0x77311f49,
		NameIndex: 381, // "Canada/Saskatchewan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 452, // America/Regina
	},
	// 269: Link Africa/Accra -> Africa/Abidjan
	{
		ZoneID: 0x77d5b054,
		NameIndex: 1, // "Africa/Accra"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 270: Zone Africa/Cairo
	{
		ZoneID: 0x77f8e228,
		NameIndex: 13, // "Africa/Cairo"
		EraIndex: 7,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 271: Zone Africa/Ceuta
	{
		ZoneID: 0x77fb46ec,
		NameIndex: 15, // "Africa/Ceuta"
		EraIndex: 12,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 272: Link Africa/Dakar -> Africa/Abidjan
	{
		ZoneID: 0x780b00fd,
		NameIndex: 17, // "Africa/Dakar"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 273: Zone Africa/Lagos
	{
		ZoneID: 0x789bb5d0,
		NameIndex: 31, // "Africa/Lagos"
		EraIndex: 25,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 274: Zone Africa/Windhoek
	{
		ZoneID: 0x789c9bd3,
		NameIndex: 53, // "Africa/Windhoek"
		EraIndex: 44,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 275: Link Canada/Yukon -> America/Whitehorse
	{
		ZoneID: 0x78dd35c2,
		NameIndex: 382, // "Canada/Yukon"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 180, // America/Whitehorse
	},
	// 276: Zone America/Toronto
	{
		ZoneID: 0x792e851b,
		NameIndex: 214, // "America/Toronto"
		EraIndex: 468,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 277: Link America/Tortola -> America/Puerto_Rico
	{
		ZoneID: 0x7931462b,
		NameIndex: 215, // "America/Tortola"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 278: Zone Africa/Tunis
	{
		ZoneID: 0x79378e6d,
		NameIndex: 52, // "Africa/Tunis"
		EraIndex: 43,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 279: Link Africa/Douala -> Africa/Lagos
	{
		ZoneID: 0x7a6df310,
		NameIndex: 20, // "Africa/Douala"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 280: Link Africa/Conakry -> Africa/Abidjan
	{
		ZoneID: 0x7ab36b31,
		NameIndex: 16, // "Africa/Conakry"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 281: Zone Indian/Mauritius
	{
		ZoneID: 0x7b09c02a,
		NameIndex: 508, // "Indian/Mauritius"
		EraIndex: 1023,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 282: Zone Atlantic/Stanley
	{
		ZoneID: 0x7bb3e1c4,
		NameIndex: 345, // "Atlantic/Stanley"
		EraIndex: 803,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 283: Link America/Ensenada -> America/Tijuana
	{
		ZoneID: 0x7bc95445,
		NameIndex: 111, // "America/Ensenada"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 231, // America/Tijuana
	},
	// 284: Link Europe/Zagreb -> Europe/Belgrade
	{
		ZoneID: 0x7c11c9ff,
		NameIndex: 487, // "Europe/Zagreb"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 535, // Europe/Belgrade
	},
	// 285: Link Cuba -> America/Havana
	{
		ZoneID: 0x7c83cba0,
		NameIndex: 385, // "Cuba"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 404, // America/Havana
	},
	// 286: Link Eire -> Europe/Dublin
	{
		ZoneID: 0x7c84b36a,
		NameIndex: 390, // "Eire"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 165, // Europe/Dublin
	},
	// 287: Link GMT0 -> Etc/GMT
	{
		ZoneID: 0x7c8550fd,
		NameIndex: 495, // "GMT0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 513, // Etc/GMT
	},
	// 288: Link Iran -> Asia/Tehran
	{
		ZoneID: 0x7c87090f,
		NameIndex: 511, // "Iran"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 500, // Asia/Tehran
	},
	// 289: Link W-SU -> Europe/Moscow
	{
		ZoneID: 0x7c8d8ef1,
		NameIndex: 593, // "W-SU"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 205, // Europe/Moscow
	},
	// 290: Link Zulu -> Etc/UTC
	{
		ZoneID: 0x7c9069b5,
		NameIndex: 595, // "Zulu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 515, // Etc/UTC
	},
	// 291: Zone Europe/Zurich
	{
		ZoneID: 0x7d8195b9,
		NameIndex: 489, // "Europe/Zurich"
		EraIndex: 1017,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 292: Link Chile/Continental -> America/Santiago
	{
		ZoneID: 0x7e2bdb18,
		NameIndex: 383, // "Chile/Continental"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 258, // America/Santiago
	},
	// 293: Link America/Fort_Wayne -> America/Indiana/Indianapolis
	{
		ZoneID: 0x7eaaaf24,
		NameIndex: 113, // "America/Fort_Wayne"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 107, // America/Indiana/Indianapolis
	},
	// 294: Zone Asia/Kuching
	{
		ZoneID: 0x801b003b,
		NameIndex: 286, // "Asia/Kuching"
		EraIndex: 647,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 295: Zone Atlantic/Madeira
	{
		ZoneID: 0x81b5c037,
		NameIndex: 341, // "Atlantic/Madeira"
		EraIndex: 800,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 296: Link America/Atikokan -> America/Panama
	{
		ZoneID: 0x81b92098,
		NameIndex: 74, // "America/Atikokan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 445, // America/Panama
	},
	// 297: Link Africa/Harare -> Africa/Maputo
	{
		ZoneID: 0x82c39a2d,
		NameIndex: 24, // "Africa/Harare"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 298: Link America/Shiprock -> America/Denver
	{
		ZoneID: 0x82fb7049,
		NameIndex: 201, // "America/Shiprock"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 350, // America/Denver
	},
	// 299: Zone Pacific/Kiritimati
	{
		ZoneID: 0x8305073a,
		NameIndex: 546, // "Pacific/Kiritimati"
		EraIndex: 1050,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 300: Link America/St_Vincent -> America/Puerto_Rico
	{
		ZoneID: 0x8460e523,
		NameIndex: 208, // "America/St_Vincent"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 301: Zone America/Metlakatla
	{
		ZoneID: 0x84de2686,
		NameIndex: 160, // "America/Metlakatla"
		EraIndex: 360,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 302: Zone Asia/Yakutsk
	{
		ZoneID: 0x87bb3a9e,
		NameIndex: 330, // "Asia/Yakutsk"
		EraIndex: 770,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 303: Zone America/Chihuahua
	{
		ZoneID: 0x8827d776,
		NameIndex: 94, // "America/Chihuahua"
		EraIndex: 199,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 304: Zone Pacific/Pitcairn
	{
		ZoneID: 0x8837d8bd,
		NameIndex: 558, // "Pacific/Pitcairn"
		EraIndex: 1071,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 305: Link Asia/Vientiane -> Asia/Bangkok
	{
		ZoneID: 0x89d68d75,
		NameIndex: 328, // "Asia/Vientiane"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 394, // Asia/Bangkok
	},
	// 306: Link Pacific/Chuuk -> Pacific/Port_Moresby
	{
		ZoneID: 0x8a090b23,
		NameIndex: 532, // "Pacific/Chuuk"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 419, // Pacific/Port_Moresby
	},
	// 307: Zone Pacific/Efate
	{
		ZoneID: 0x8a2bce28,
		NameIndex: 534, // "Pacific/Efate"
		EraIndex: 1036,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 308: Link Africa/Kigali -> Africa/Maputo
	{
		ZoneID: 0x8a4dcf2b,
		NameIndex: 29, // "Africa/Kigali"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 309: Link Australia/ACT -> Australia/Sydney
	{
		ZoneID: 0x8a970eb2,
		NameIndex: 346, // "Australia/ACT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 168, // Australia/Sydney
	},
	// 310: Link Australia/LHI -> Australia/Lord_Howe
	{
		ZoneID: 0x8a973e17,
		NameIndex: 355, // "Australia/LHI"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 418, // Australia/Lord_Howe
	},
	// 311: Link Australia/NSW -> Australia/Sydney
	{
		ZoneID: 0x8a974812,
		NameIndex: 359, // "Australia/NSW"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 168, // Australia/Sydney
	},
	// 312: Zone Pacific/Nauru
	{
		ZoneID: 0x8acc41ae,
		NameIndex: 552, // "Pacific/Nauru"
		EraIndex: 1060,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 313: Zone EST5EDT
	{
		ZoneID: 0x8adc72a3,
		NameIndex: 388, // "EST5EDT"
		EraIndex: 832,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 314: Zone Pacific/Palau
	{
		ZoneID: 0x8af04a36,
		NameIndex: 557, // "Pacific/Palau"
		EraIndex: 1070,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 315: Link Pacific/Samoa -> Pacific/Pago_Pago
	{
		ZoneID: 0x8b2699b4,
		NameIndex: 564, // "Pacific/Samoa"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 208, // Pacific/Pago_Pago
	},
	// 316: Zone America/Winnipeg
	{
		ZoneID: 0x8c7dafc7,
		NameIndex: 219, // "America/Winnipeg"
		EraIndex: 475,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 317: Zone Australia/Eucla
	{
		ZoneID: 0x8cf99e44,
		NameIndex: 353, // "Australia/Eucla"
		EraIndex: 815,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 318: Zone America/Argentina/Catamarca
	{
		ZoneID: 0x8d40986b,
		NameIndex: 60, // "America/Argentina/Catamarca"
		EraIndex: 64,
		EraCount: 9,
		TargetIndex: 0,
	},
	// 319: Link Africa/Luanda -> Africa/Lagos
	{
		ZoneID: 0x8d7909cf,
		NameIndex: 34, // "Africa/Luanda"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 320: Link Africa/Lusaka -> Africa/Maputo
	{
		ZoneID: 0x8d82b23b,
		NameIndex: 36, // "Africa/Lusaka"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 321: Link Australia/North -> Australia/Darwin
	{
		ZoneID: 0x8d997165,
		NameIndex: 360, // "Australia/North"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 103, // Australia/Darwin
	},
	// 322: Zone Australia/Perth
	{
		ZoneID: 0x8db8269d,
		NameIndex: 361, // "Australia/Perth"
		EraIndex: 825,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 323: Link Australia/South -> Australia/Adelaide
	{
		ZoneID: 0x8df3f8ad,
		NameIndex: 363, // "Australia/South"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 97, // Australia/Adelaide
	},
	// 324: Zone Pacific/Kwajalein
	{
		ZoneID: 0x8e216759,
		NameIndex: 548, // "Pacific/Kwajalein"
		EraIndex: 1056,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 325: Zone America/Port-au-Prince
	{
		ZoneID: 0x8e4a7bdc,
		NameIndex: 182, // "America/Port-au-Prince"
		EraIndex: 415,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 326: Link Africa/Malabo -> Africa/Lagos
	{
		ZoneID: 0x8e6a1906,
		NameIndex: 37, // "Africa/Malabo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 327: Zone Africa/Maputo
	{
		ZoneID: 0x8e6ca1f0,
		NameIndex: 38, // "Africa/Maputo"
		EraIndex: 26,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 328: Link Africa/Maseru -> Africa/Johannesburg
	{
		ZoneID: 0x8e6e02c7,
		NameIndex: 39, // "Africa/Maseru"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 511, // Africa/Johannesburg
	},
	// 329: Zone Pacific/Norfolk
	{
		ZoneID: 0x8f4eb4be,
		NameIndex: 554, // "Pacific/Norfolk"
		EraIndex: 1063,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 330: Link America/Godthab -> America/Nuuk
	{
		ZoneID: 0x8f7eba1f,
		NameIndex: 116, // "America/Godthab"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 359, // America/Nuuk
	},
	// 331: Link Australia/Yancowinna -> Australia/Broken_Hill
	{
		ZoneID: 0x90bac131,
		NameIndex: 368, // "Australia/Yancowinna"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 438, // Australia/Broken_Hill
	},
	// 332: Link Africa/Niamey -> Africa/Lagos
	{
		ZoneID: 0x914a30fd,
		NameIndex: 45, // "Africa/Niamey"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 333: Zone Asia/Yerevan
	{
		ZoneID: 0x9185c8cc,
		NameIndex: 333, // "Asia/Yerevan"
		EraIndex: 781,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 334: Zone America/Detroit
	{
		ZoneID: 0x925cfbc1,
		NameIndex: 106, // "America/Detroit"
		EraIndex: 225,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 335: Zone Asia/Choibalsan
	{
		ZoneID: 0x928aa4a6,
		NameIndex: 254, // "Asia/Choibalsan"
		EraIndex: 576,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 336: Zone Antarctica/Macquarie
	{
		ZoneID: 0x92f47626,
		NameIndex: 225, // "Antarctica/Macquarie"
		EraIndex: 501,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 337: Zone America/Belize
	{
		ZoneID: 0x93256c81,
		NameIndex: 80, // "America/Belize"
		EraIndex: 175,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 338: Link Mexico/General -> America/Mexico_City
	{
		ZoneID: 0x93711d57,
		NameIndex: 522, // "Mexico/General"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 497, // America/Mexico_City
	},
	// 339: Zone America/Bogota
	{
		ZoneID: 0x93d7bc62,
		NameIndex: 83, // "America/Bogota"
		EraIndex: 180,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 340: Zone Asia/Pyongyang
	{
		ZoneID: 0x93ed1c8e,
		NameIndex: 301, // "Asia/Pyongyang"
		EraIndex: 686,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 341: Zone America/Indiana/Petersburg
	{
		ZoneID: 0x94ac7acc,
		NameIndex: 130, // "America/Indiana/Petersburg"
		EraIndex: 281,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 342: Zone America/Cancun
	{
		ZoneID: 0x953331be,
		NameIndex: 88, // "America/Cancun"
		EraIndex: 190,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 343: Link America/Cayman -> America/Panama
	{
		ZoneID: 0x953961df,
		NameIndex: 92, // "America/Cayman"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 445, // America/Panama
	},
	// 344: Zone America/Glace_Bay
	{
		ZoneID: 0x9681f8dd,
		NameIndex: 115, // "America/Glace_Bay"
		EraIndex: 248,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 345: Zone Asia/Khandyga
	{
		ZoneID: 0x9685a4d9,
		NameIndex: 282, // "Asia/Khandyga"
		EraIndex: 634,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 346: Link America/Grenada -> America/Puerto_Rico
	{
		ZoneID: 0x968ce4d8,
		NameIndex: 119, // "America/Grenada"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 347: Zone America/Cuiaba
	{
		ZoneID: 0x969a52eb,
		NameIndex: 100, // "America/Cuiaba"
		EraIndex: 212,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 348: Zone America/Dawson
	{
		ZoneID: 0x978d8d12,
		NameIndex: 103, // "America/Dawson"
		EraIndex: 218,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 349: Link America/Aruba -> America/Puerto_Rico
	{
		ZoneID: 0x97cf8651,
		NameIndex: 72, // "America/Aruba"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 350: Zone America/Denver
	{
		ZoneID: 0x97d10b2a,
		NameIndex: 105, // "America/Denver"
		EraIndex: 224,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 351: Zone America/Bahia
	{
		ZoneID: 0x97d815fb,
		NameIndex: 76, // "America/Bahia"
		EraIndex: 165,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 352: Zone America/Belem
	{
		ZoneID: 0x97da580b,
		NameIndex: 79, // "America/Belem"
		EraIndex: 173,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 353: Zone America/Boise
	{
		ZoneID: 0x97dfc8d8,
		NameIndex: 84, // "America/Boise"
		EraIndex: 181,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 354: Zone Europe/Andorra
	{
		ZoneID: 0x97f6764b,
		NameIndex: 427, // "Europe/Andorra"
		EraIndex: 861,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 355: Zone America/Adak
	{
		ZoneID: 0x97fe49d7,
		NameIndex: 54, // "America/Adak"
		EraIndex: 46,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 356: Link America/Atka -> America/Adak
	{
		ZoneID: 0x97fe8f27,
		NameIndex: 75, // "America/Atka"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 355, // America/Adak
	},
	// 357: Zone America/Lima
	{
		ZoneID: 0x980468c9,
		NameIndex: 146, // "America/Lima"
		EraIndex: 324,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 358: Zone America/Nome
	{
		ZoneID: 0x98059b15,
		NameIndex: 171, // "America/Nome"
		EraIndex: 385,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 359: Zone America/Nuuk
	{
		ZoneID: 0x9805b5a9,
		NameIndex: 176, // "America/Nuuk"
		EraIndex: 401,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 360: Zone Indian/Maldives
	{
		ZoneID: 0x9869681c,
		NameIndex: 507, // "Indian/Maldives"
		EraIndex: 1022,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 361: Link America/Jujuy -> America/Argentina/Jujuy
	{
		ZoneID: 0x9873dbbd,
		NameIndex: 139, // "America/Jujuy"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 206, // America/Argentina/Jujuy
	},
	// 362: Zone America/Sitka
	{
		ZoneID: 0x99104ce2,
		NameIndex: 202, // "America/Sitka"
		EraIndex: 452,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 363: Zone America/Thule
	{
		ZoneID: 0x9921dd68,
		NameIndex: 211, // "America/Thule"
		EraIndex: 461,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 364: Zone Pacific/Rarotonga
	{
		ZoneID: 0x9981a3b0,
		NameIndex: 562, // "Pacific/Rarotonga"
		EraIndex: 1074,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 365: Zone Asia/Kathmandu
	{
		ZoneID: 0x9a96ce6f,
		NameIndex: 280, // "Asia/Kathmandu"
		EraIndex: 632,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 366: Link Brazil/DeNoronha -> America/Noronha
	{
		ZoneID: 0x9b4cb496,
		NameIndex: 370, // "Brazil/DeNoronha"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 429, // America/Noronha
	},
	// 367: Zone America/North_Dakota/Beulah
	{
		ZoneID: 0x9b52b384,
		NameIndex: 173, // "America/North_Dakota/Beulah"
		EraIndex: 395,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 368: Link America/Rainy_River -> America/Winnipeg
	{
		ZoneID: 0x9cd58a10,
		NameIndex: 188, // "America/Rainy_River"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 316, // America/Winnipeg
	},
	// 369: Zone Europe/Budapest
	{
		ZoneID: 0x9ce0197c,
		NameIndex: 436, // "Europe/Budapest"
		EraIndex: 883,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 370: Zone Asia/Baghdad
	{
		ZoneID: 0x9ceffbed,
		NameIndex: 244, // "Asia/Baghdad"
		EraIndex: 550,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 371: Link Asia/Bahrain -> Asia/Qatar
	{
		ZoneID: 0x9d078487,
		NameIndex: 245, // "Asia/Bahrain"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 66, // Asia/Qatar
	},
	// 372: Link Etc/GMT+0 -> Etc/GMT
	{
		ZoneID: 0x9d13da13,
		NameIndex: 392, // "Etc/GMT+0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 513, // Etc/GMT
	},
	// 373: Zone Etc/GMT+1
	{
		ZoneID: 0x9d13da14,
		NameIndex: 393, // "Etc/GMT+1"
		EraIndex: 834,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 374: Zone Etc/GMT+2
	{
		ZoneID: 0x9d13da15,
		NameIndex: 397, // "Etc/GMT+2"
		EraIndex: 838,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 375: Zone Etc/GMT+3
	{
		ZoneID: 0x9d13da16,
		NameIndex: 398, // "Etc/GMT+3"
		EraIndex: 839,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 376: Zone Etc/GMT+4
	{
		ZoneID: 0x9d13da17,
		NameIndex: 399, // "Etc/GMT+4"
		EraIndex: 840,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 377: Zone Etc/GMT+5
	{
		ZoneID: 0x9d13da18,
		NameIndex: 400, // "Etc/GMT+5"
		EraIndex: 841,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 378: Zone Etc/GMT+6
	{
		ZoneID: 0x9d13da19,
		NameIndex: 401, // "Etc/GMT+6"
		EraIndex: 842,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 379: Zone Etc/GMT+7
	{
		ZoneID: 0x9d13da1a,
		NameIndex: 402, // "Etc/GMT+7"
		EraIndex: 843,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 380: Zone Etc/GMT+8
	{
		ZoneID: 0x9d13da1b,
		NameIndex: 403, // "Etc/GMT+8"
		EraIndex: 844,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 381: Zone Etc/GMT+9
	{
		ZoneID: 0x9d13da1c,
		NameIndex: 404, // "Etc/GMT+9"
		EraIndex: 845,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 382: Link Etc/GMT-0 -> Etc/GMT
	{
		ZoneID: 0x9d13da55,
		NameIndex: 405, // "Etc/GMT-0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 513, // Etc/GMT
	},
	// 383: Zone Etc/GMT-1
	{
		ZoneID: 0x9d13da56,
		NameIndex: 406, // "Etc/GMT-1"
		EraIndex: 846,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 384: Zone Etc/GMT-2
	{
		ZoneID: 0x9d13da57,
		NameIndex: 412, // "Etc/GMT-2"
		EraIndex: 852,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 385: Zone Etc/GMT-3
	{
		ZoneID: 0x9d13da58,
		NameIndex: 413, // "Etc/GMT-3"
		EraIndex: 853,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 386: Zone Etc/GMT-4
	{
		ZoneID: 0x9d13da59,
		NameIndex: 414, // "Etc/GMT-4"
		EraIndex: 854,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 387: Zone Etc/GMT-5
	{
		ZoneID: 0x9d13da5a,
		NameIndex: 415, // "Etc/GMT-5"
		EraIndex: 855,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 388: Zone Etc/GMT-6
	{
		ZoneID: 0x9d13da5b,
		NameIndex: 416, // "Etc/GMT-6"
		EraIndex: 856,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 389: Zone Etc/GMT-7
	{
		ZoneID: 0x9d13da5c,
		NameIndex: 417, // "Etc/GMT-7"
		EraIndex: 857,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 390: Zone Etc/GMT-8
	{
		ZoneID: 0x9d13da5d,
		NameIndex: 418, // "Etc/GMT-8"
		EraIndex: 858,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 391: Zone Etc/GMT-9
	{
		ZoneID: 0x9d13da5e,
		NameIndex: 419, // "Etc/GMT-9"
		EraIndex: 859,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 392: Link America/Nipigon -> America/Toronto
	{
		ZoneID: 0x9d2a8b1a,
		NameIndex: 170, // "America/Nipigon"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 276, // America/Toronto
	},
	// 393: Zone America/Rio_Branco
	{
		ZoneID: 0x9d352764,
		NameIndex: 193, // "America/Rio_Branco"
		EraIndex: 436,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 394: Zone Asia/Bangkok
	{
		ZoneID: 0x9d6e3aaf,
		NameIndex: 247, // "Asia/Bangkok"
		EraIndex: 557,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 395: Zone Africa/El_Aaiun
	{
		ZoneID: 0x9d6fb118,
		NameIndex: 21, // "Africa/El_Aaiun"
		EraIndex: 15,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 396: Zone America/North_Dakota/Center
	{
		ZoneID: 0x9da42814,
		NameIndex: 174, // "America/North_Dakota/Center"
		EraIndex: 397,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 397: Zone Asia/Barnaul
	{
		ZoneID: 0x9dba4997,
		NameIndex: 248, // "Asia/Barnaul"
		EraIndex: 558,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 398: Zone Africa/Tripoli
	{
		ZoneID: 0x9dfebd3d,
		NameIndex: 51, // "Africa/Tripoli"
		EraIndex: 36,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 399: Zone Europe/Istanbul
	{
		ZoneID: 0x9e09d6e6,
		NameIndex: 445, // "Europe/Istanbul"
		EraIndex: 894,
		EraCount: 11,
		TargetIndex: 0,
	},
	// 400: Link Indian/Antananarivo -> Africa/Nairobi
	{
		ZoneID: 0x9ebf5289,
		NameIndex: 500, // "Indian/Antananarivo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 401: Zone Africa/Ndjamena
	{
		ZoneID: 0x9fe09898,
		NameIndex: 44, // "Africa/Ndjamena"
		EraIndex: 30,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 402: Zone America/Guyana
	{
		ZoneID: 0x9ff7bd0b,
		NameIndex: 123, // "America/Guyana"
		EraIndex: 259,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 403: Link Africa/Dar_es_Salaam -> Africa/Nairobi
	{
		ZoneID: 0xa04c47b6,
		NameIndex: 18, // "Africa/Dar_es_Salaam"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 404: Zone America/Havana
	{
		ZoneID: 0xa0e15675,
		NameIndex: 125, // "America/Havana"
		EraIndex: 264,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 405: Zone Asia/Novosibirsk
	{
		ZoneID: 0xa2a435cb,
		NameIndex: 296, // "Asia/Novosibirsk"
		EraIndex: 664,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 406: Link Europe/Kiev -> Europe/Kyiv
	{
		ZoneID: 0xa2c19eb3,
		NameIndex: 448, // "Europe/Kiev"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 407, // Europe/Kyiv
	},
	// 407: Zone Europe/Kyiv
	{
		ZoneID: 0xa2c1e347,
		NameIndex: 450, // "Europe/Kyiv"
		EraIndex: 915,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 408: Link Europe/Oslo -> Europe/Berlin
	{
		ZoneID: 0xa2c3fba1,
		NameIndex: 462, // "Europe/Oslo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 158, // Europe/Berlin
	},
	// 409: Zone Europe/Riga
	{
		ZoneID: 0xa2c57587,
		NameIndex: 466, // "Europe/Riga"
		EraIndex: 945,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 410: Zone Europe/Rome
	{
		ZoneID: 0xa2c58fd7,
		NameIndex: 467, // "Europe/Rome"
		EraIndex: 951,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 411: Zone America/Inuvik
	{
		ZoneID: 0xa42189fc,
		NameIndex: 136, // "America/Inuvik"
		EraIndex: 303,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 412: Zone America/Argentina/La_Rioja
	{
		ZoneID: 0xa46b7eef,
		NameIndex: 64, // "America/Argentina/La_Rioja"
		EraIndex: 89,
		EraCount: 9,
		TargetIndex: 0,
	},
	// 413: Zone Asia/Almaty
	{
		ZoneID: 0xa61f41fa,
		NameIndex: 236, // "Asia/Almaty"
		EraIndex: 514,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 414: Zone Asia/Anadyr
	{
		ZoneID: 0xa63cebd1,
		NameIndex: 238, // "Asia/Anadyr"
		EraIndex: 520,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 415: Zone Asia/Aqtobe
	{
		ZoneID: 0xa67dcc4e,
		NameIndex: 240, // "Asia/Aqtobe"
		EraIndex: 533,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 416: Zone Asia/Atyrau
	{
		ZoneID: 0xa6b6e068,
		NameIndex: 243, // "Asia/Atyrau"
		EraIndex: 543,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 417: Zone America/Juneau
	{
		ZoneID: 0xa6f13e2e,
		NameIndex: 140, // "America/Juneau"
		EraIndex: 312,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 418: Zone Australia/Lord_Howe
	{
		ZoneID: 0xa748b67d,
		NameIndex: 357, // "Australia/Lord_Howe"
		EraIndex: 820,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 419: Zone Pacific/Port_Moresby
	{
		ZoneID: 0xa7ba7f68,
		NameIndex: 561, // "Pacific/Port_Moresby"
		EraIndex: 1073,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 420: Link Asia/Katmandu -> Asia/Kathmandu
	{
		ZoneID: 0xa7ec12c7,
		NameIndex: 281, // "Asia/Katmandu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 365, // Asia/Kathmandu
	},
	// 421: Zone Asia/Beirut
	{
		ZoneID: 0xa7f3d5fd,
		NameIndex: 249, // "Asia/Beirut"
		EraIndex: 565,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 422: Link Singapore -> Asia/Singapore
	{
		ZoneID: 0xa8598c8d,
		NameIndex: 576, // "Singapore"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 494, // Asia/Singapore
	},
	// 423: Zone Africa/Nairobi
	{
		ZoneID: 0xa87ab57e,
		NameIndex: 43, // "Africa/Nairobi"
		EraIndex: 29,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 424: Link Asia/Brunei -> Asia/Kuching
	{
		ZoneID: 0xa8e595f7,
		NameIndex: 251, // "Asia/Brunei"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 294, // Asia/Kuching
	},
	// 425: Link US/Pacific -> America/Los_Angeles
	{
		ZoneID: 0xa950f6ab,
		NameIndex: 589, // "US/Pacific"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 450, // America/Los_Angeles
	},
	// 426: Zone Pacific/Galapagos
	{
		ZoneID: 0xa952f752,
		NameIndex: 539, // "Pacific/Galapagos"
		EraIndex: 1040,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 427: Zone America/Argentina/Mendoza
	{
		ZoneID: 0xa9f72d5c,
		NameIndex: 65, // "America/Argentina/Mendoza"
		EraIndex: 98,
		EraCount: 13,
		TargetIndex: 0,
	},
	// 428: Zone America/La_Paz
	{
		ZoneID: 0xaa29125d,
		NameIndex: 145, // "America/La_Paz"
		EraIndex: 323,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 429: Zone America/Noronha
	{
		ZoneID: 0xab5116fb,
		NameIndex: 172, // "America/Noronha"
		EraIndex: 389,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 430: Link America/Coral_Harbour -> America/Panama
	{
		ZoneID: 0xabcb7569,
		NameIndex: 96, // "America/Coral_Harbour"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 445, // America/Panama
	},
	// 431: Zone America/Maceio
	{
		ZoneID: 0xac80c6d4,
		NameIndex: 150, // "America/Maceio"
		EraIndex: 326,
		EraCount: 8,
		TargetIndex: 0,
	},
	// 432: Zone America/Manaus
	{
		ZoneID: 0xac86bf8b,
		NameIndex: 152, // "America/Manaus"
		EraIndex: 341,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 433: Zone America/Merida
	{
		ZoneID: 0xacd172d8,
		NameIndex: 159, // "America/Merida"
		EraIndex: 357,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 434: Zone Europe/Chisinau
	{
		ZoneID: 0xad58aa18,
		NameIndex: 438, // "Europe/Chisinau"
		EraIndex: 885,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 435: Link America/Nassau -> America/Toronto
	{
		ZoneID: 0xaedef011,
		NameIndex: 168, // "America/Nassau"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 276, // America/Toronto
	},
	// 436: Link America/Anguilla -> America/Puerto_Rico
	{
		ZoneID: 0xafe31333,
		NameIndex: 56, // "America/Anguilla"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 437: Link Europe/Uzhgorod -> Europe/Kyiv
	{
		ZoneID: 0xb066f5d6,
		NameIndex: 480, // "Europe/Uzhgorod"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 407, // Europe/Kyiv
	},
	// 438: Zone Australia/Broken_Hill
	{
		ZoneID: 0xb06eada3,
		NameIndex: 349, // "Australia/Broken_Hill"
		EraIndex: 811,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 439: Zone Asia/Bishkek
	{
		ZoneID: 0xb0728553,
		NameIndex: 250, // "Asia/Bishkek"
		EraIndex: 566,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 440: Link Chile/EasterIsland -> Pacific/Easter
	{
		ZoneID: 0xb0982af8,
		NameIndex: 384, // "Chile/EasterIsland"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 493, // Pacific/Easter
	},
	// 441: Link Pacific/Johnston -> Pacific/Honolulu
	{
		ZoneID: 0xb15d7b36,
		NameIndex: 544, // "Pacific/Johnston"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 552, // Pacific/Honolulu
	},
	// 442: Link Africa/Timbuktu -> Africa/Abidjan
	{
		ZoneID: 0xb164d56f,
		NameIndex: 50, // "Africa/Timbuktu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 443: Link America/St_Thomas -> America/Puerto_Rico
	{
		ZoneID: 0xb1b3d778,
		NameIndex: 207, // "America/St_Thomas"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 444: Zone America/Paramaribo
	{
		ZoneID: 0xb319e4c4,
		NameIndex: 180, // "America/Paramaribo"
		EraIndex: 412,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 445: Zone America/Panama
	{
		ZoneID: 0xb3863854,
		NameIndex: 178, // "America/Panama"
		EraIndex: 411,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 446: Link Canada/Newfoundland -> America/St_Johns
	{
		ZoneID: 0xb396e991,
		NameIndex: 379, // "Canada/Newfoundland"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 7, // America/St_Johns
	},
	// 447: Link Asia/Harbin -> Asia/Shanghai
	{
		ZoneID: 0xb5af1186,
		NameIndex: 266, // "Asia/Harbin"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 572, // Asia/Shanghai
	},
	// 448: Zone Asia/Hebron
	{
		ZoneID: 0xb5eef250,
		NameIndex: 267, // "Asia/Hebron"
		EraIndex: 608,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 449: Zone America/Goose_Bay
	{
		ZoneID: 0xb649541e,
		NameIndex: 117, // "America/Goose_Bay"
		EraIndex: 251,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 450: Zone America/Los_Angeles
	{
		ZoneID: 0xb7f7e8f2,
		NameIndex: 147, // "America/Los_Angeles"
		EraIndex: 325,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 451: Zone America/Recife
	{
		ZoneID: 0xb8730494,
		NameIndex: 190, // "America/Recife"
		EraIndex: 424,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 452: Zone America/Regina
	{
		ZoneID: 0xb875371c,
		NameIndex: 191, // "America/Regina"
		EraIndex: 430,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 453: Zone Asia/Ashgabat
	{
		ZoneID: 0xba87598d,
		NameIndex: 241, // "Asia/Ashgabat"
		EraIndex: 540,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 454: Link Israel -> Asia/Jerusalem
	{
		ZoneID: 0xba88c9e5,
		NameIndex: 512, // "Israel"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 193, // Asia/Jerusalem
	},
	// 455: Link Pacific/Yap -> Pacific/Port_Moresby
	{
		ZoneID: 0xbb40138d,
		NameIndex: 571, // "Pacific/Yap"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 419, // Pacific/Port_Moresby
	},
	// 456: Zone America/Halifax
	{
		ZoneID: 0xbc5b7183,
		NameIndex: 124, // "America/Halifax"
		EraIndex: 262,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 457: Link Europe/Ljubljana -> Europe/Belgrade
	{
		ZoneID: 0xbd98cdb7,
		NameIndex: 452, // "Europe/Ljubljana"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 535, // Europe/Belgrade
	},
	// 458: Link Asia/Kuwait -> Asia/Riyadh
	{
		ZoneID: 0xbe1b2f27,
		NameIndex: 287, // "Asia/Kuwait"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 489, // Asia/Riyadh
	},
	// 459: Link Europe/Tiraspol -> Europe/Chisinau
	{
		ZoneID: 0xbe704472,
		NameIndex: 478, // "Europe/Tiraspol"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 434, // Europe/Chisinau
	},
	// 460: Zone Asia/Srednekolymsk
	{
		ZoneID: 0xbf8e337d,
		NameIndex: 313, // "Asia/Srednekolymsk"
		EraIndex: 725,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 461: Zone America/Argentina/Cordoba
	{
		ZoneID: 0xbfccc308,
		NameIndex: 62, // "America/Argentina/Cordoba"
		EraIndex: 73,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 462: Zone America/Tegucigalpa
	{
		ZoneID: 0xbfd6fd4c,
		NameIndex: 210, // "America/Tegucigalpa"
		EraIndex: 460,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 463: Link America/Antigua -> America/Puerto_Rico
	{
		ZoneID: 0xc067a32f,
		NameIndex: 57, // "America/Antigua"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 464: Link Europe/Busingen -> Europe/Zurich
	{
		ZoneID: 0xc06d2cdf,
		NameIndex: 437, // "Europe/Busingen"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 291, // Europe/Zurich
	},
	// 465: Zone Asia/Manila
	{
		ZoneID: 0xc156c944,
		NameIndex: 292, // "Asia/Manila"
		EraIndex: 656,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 466: Link Africa/Kampala -> Africa/Nairobi
	{
		ZoneID: 0xc1d30e31,
		NameIndex: 27, // "Africa/Kampala"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 467: Link America/Knox_IN -> America/Indiana/Knox
	{
		ZoneID: 0xc1db9a1c,
		NameIndex: 143, // "America/Knox_IN"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 215, // America/Indiana/Knox
	},
	// 468: Zone Africa/Abidjan
	{
		ZoneID: 0xc21305a3,
		NameIndex: 0, // "Africa/Abidjan"
		EraIndex: 0,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 469: Link America/Virgin -> America/Puerto_Rico
	{
		ZoneID: 0xc2183ab5,
		NameIndex: 217, // "America/Virgin"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 470: Link Asia/Phnom_Penh -> Asia/Bangkok
	{
		ZoneID: 0xc224945e,
		NameIndex: 299, // "Asia/Phnom_Penh"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 394, // Asia/Bangkok
	},
	// 471: Link Asia/Muscat -> Asia/Dubai
	{
		ZoneID: 0xc2c3565f,
		NameIndex: 293, // "Asia/Muscat"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 60, // Asia/Dubai
	},
	// 472: Zone America/Punta_Arenas
	{
		ZoneID: 0xc2c3bce7,
		NameIndex: 187, // "America/Punta_Arenas"
		EraIndex: 419,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 473: Link Portugal -> Europe/Lisbon
	{
		ZoneID: 0xc3274593,
		NameIndex: 573, // "Portugal"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 195, // Europe/Lisbon
	},
	// 474: Link Navajo -> America/Denver
	{
		ZoneID: 0xc4ef0e24,
		NameIndex: 525, // "Navajo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 350, // America/Denver
	},
	// 475: Zone Africa/Casablanca
	{
		ZoneID: 0xc59f1b33,
		NameIndex: 14, // "Africa/Casablanca"
		EraIndex: 8,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 476: Zone America/Argentina/Rio_Gallegos
	{
		ZoneID: 0xc5b0f565,
		NameIndex: 66, // "America/Argentina/Rio_Gallegos"
		EraIndex: 111,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 477: Zone Asia/Jayapura
	{
		ZoneID: 0xc6833c2f,
		NameIndex: 274, // "Asia/Jayapura"
		EraIndex: 622,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 478: Zone America/Resolute
	{
		ZoneID: 0xc7093459,
		NameIndex: 192, // "America/Resolute"
		EraIndex: 431,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 479: Link Asia/Chungking -> Asia/Shanghai
	{
		ZoneID: 0xc7121dd0,
		NameIndex: 256, // "Asia/Chungking"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 572, // Asia/Shanghai
	},
	// 480: Link Greenwich -> Etc/GMT
	{
		ZoneID: 0xc84d4221,
		NameIndex: 496, // "Greenwich"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 513, // Etc/GMT
	},
	// 481: Zone America/Rankin_Inlet
	{
		ZoneID: 0xc8de4984,
		NameIndex: 189, // "America/Rankin_Inlet"
		EraIndex: 421,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 482: Link Poland -> Europe/Warsaw
	{
		ZoneID: 0xca913b23,
		NameIndex: 572, // "Poland"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 263, // Europe/Warsaw
	},
	// 483: Link US/Central -> America/Chicago
	{
		ZoneID: 0xcabdcb25,
		NameIndex: 582, // "US/Central"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 167, // America/Chicago
	},
	// 484: Link Europe/Vatican -> Europe/Rome
	{
		ZoneID: 0xcb485dca,
		NameIndex: 482, // "Europe/Vatican"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 410, // Europe/Rome
	},
	// 485: Zone America/Barbados
	{
		ZoneID: 0xcbbc3b04,
		NameIndex: 78, // "America/Barbados"
		EraIndex: 172,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 486: Link America/Porto_Acre -> America/Rio_Branco
	{
		ZoneID: 0xcce5bf54,
		NameIndex: 184, // "America/Porto_Acre"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 393, // America/Rio_Branco
	},
	// 487: Link America/Guadeloupe -> America/Puerto_Rico
	{
		ZoneID: 0xcd1f8a31,
		NameIndex: 120, // "America/Guadeloupe"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 488: Link Antarctica/South_Pole -> Pacific/Auckland
	{
		ZoneID: 0xcd96b290,
		NameIndex: 230, // "Antarctica/South_Pole"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 98, // Pacific/Auckland
	},
	// 489: Zone Asia/Riyadh
	{
		ZoneID: 0xcd973d93,
		NameIndex: 306, // "Asia/Riyadh"
		EraIndex: 708,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 490: Link America/Dominica -> America/Puerto_Rico
	{
		ZoneID: 0xcecb4c4a,
		NameIndex: 107, // "America/Dominica"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 491: Link Europe/San_Marino -> Europe/Rome
	{
		ZoneID: 0xcef7724b,
		NameIndex: 469, // "Europe/San_Marino"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 410, // Europe/Rome
	},
	// 492: Link Asia/Saigon -> Asia/Ho_Chi_Minh
	{
		ZoneID: 0xcf52f713,
		NameIndex: 307, // "Asia/Saigon"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 88, // Asia/Ho_Chi_Minh
	},
	// 493: Zone Pacific/Easter
	{
		ZoneID: 0xcf54f7e7,
		NameIndex: 533, // "Pacific/Easter"
		EraIndex: 1034,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 494: Zone Asia/Singapore
	{
		ZoneID: 0xcf8581fa,
		NameIndex: 312, // "Asia/Singapore"
		EraIndex: 723,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 495: Zone Asia/Krasnoyarsk
	{
		ZoneID: 0xd0376c6a,
		NameIndex: 284, // "Asia/Krasnoyarsk"
		EraIndex: 642,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 496: Link Europe/Belfast -> Europe/London
	{
		ZoneID: 0xd07dd1e5,
		NameIndex: 430, // "Europe/Belfast"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 497: Zone America/Mexico_City
	{
		ZoneID: 0xd0d93f43,
		NameIndex: 161, // "America/Mexico_City"
		EraIndex: 366,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 498: Link Universal -> Etc/UTC
	{
		ZoneID: 0xd0ff523e,
		NameIndex: 592, // "Universal"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 515, // Etc/UTC
	},
	// 499: Zone Asia/Taipei
	{
		ZoneID: 0xd1a844ae,
		NameIndex: 314, // "Asia/Taipei"
		EraIndex: 730,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 500: Zone Asia/Tehran
	{
		ZoneID: 0xd1f02254,
		NameIndex: 317, // "Asia/Tehran"
		EraIndex: 742,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 501: Link Asia/Thimbu -> Asia/Thimphu
	{
		ZoneID: 0xd226e31b,
		NameIndex: 319, // "Asia/Thimbu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 71, // Asia/Thimphu
	},
	// 502: Link Arctic/Longyearbyen -> Europe/Berlin
	{
		ZoneID: 0xd23e7859,
		NameIndex: 234, // "Arctic/Longyearbyen"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 158, // Europe/Berlin
	},
	// 503: Link Australia/Queensland -> Australia/Brisbane
	{
		ZoneID: 0xd326ed0a,
		NameIndex: 362, // "Australia/Queensland"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 173, // Australia/Brisbane
	},
	// 504: Zone Europe/Kaliningrad
	{
		ZoneID: 0xd33b2f28,
		NameIndex: 447, // "Europe/Kaliningrad"
		EraIndex: 905,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 505: Zone America/Argentina/Buenos_Aires
	{
		ZoneID: 0xd43b4c0d,
		NameIndex: 59, // "America/Argentina/Buenos_Aires"
		EraIndex: 60,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 506: Link Turkey -> Europe/Istanbul
	{
		ZoneID: 0xd455e469,
		NameIndex: 577, // "Turkey"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 399, // Europe/Istanbul
	},
	// 507: Zone Africa/Juba
	{
		ZoneID: 0xd51b395c,
		NameIndex: 26, // "Africa/Juba"
		EraIndex: 19,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 508: Link Africa/Lome -> Africa/Abidjan
	{
		ZoneID: 0xd51c3a07,
		NameIndex: 33, // "Africa/Lome"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 509: Zone Asia/Urumqi
	{
		ZoneID: 0xd5379735,
		NameIndex: 326, // "Asia/Urumqi"
		EraIndex: 757,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 510: Zone America/Cambridge_Bay
	{
		ZoneID: 0xd5a44aff,
		NameIndex: 86, // "America/Cambridge_Bay"
		EraIndex: 184,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 511: Zone Africa/Johannesburg
	{
		ZoneID: 0xd5d157a0,
		NameIndex: 25, // "Africa/Johannesburg"
		EraIndex: 18,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 512: Link America/Port_of_Spain -> America/Puerto_Rico
	{
		ZoneID: 0xd8b28d59,
		NameIndex: 183, // "America/Port_of_Spain"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 513: Zone Etc/GMT
	{
		ZoneID: 0xd8e2de58,
		NameIndex: 391, // "Etc/GMT"
		EraIndex: 833,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 514: Link Etc/UCT -> Etc/UTC
	{
		ZoneID: 0xd8e3189c,
		NameIndex: 422, // "Etc/UCT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 515, // Etc/UTC
	},
	// 515: Zone Etc/UTC
	{
		ZoneID: 0xd8e31abc,
		NameIndex: 423, // "Etc/UTC"
		EraIndex: 860,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 516: Zone America/Yakutat
	{
		ZoneID: 0xd8ee31e9,
		NameIndex: 220, // "America/Yakutat"
		EraIndex: 477,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 517: Zone Africa/Algiers
	{
		ZoneID: 0xd94515c1,
		NameIndex: 3, // "Africa/Algiers"
		EraIndex: 1,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 518: Zone PST8PDT
	{
		ZoneID: 0xd99ee2dc,
		NameIndex: 527, // "PST8PDT"
		EraIndex: 1027,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 519: Link Europe/Bratislava -> Europe/Prague
	{
		ZoneID: 0xda493bed,
		NameIndex: 433, // "Europe/Bratislava"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 216, // Europe/Prague
	},
	// 520: Zone Europe/Simferopol
	{
		ZoneID: 0xda9eb724,
		NameIndex: 472, // "Europe/Simferopol"
		EraIndex: 967,
		EraCount: 10,
		TargetIndex: 0,
	},
	// 521: Link Pacific/Funafuti -> Pacific/Tarawa
	{
		ZoneID: 0xdb402d65,
		NameIndex: 538, // "Pacific/Funafuti"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 563, // Pacific/Tarawa
	},
	// 522: Zone America/Matamoros
	{
		ZoneID: 0xdd1b0259,
		NameIndex: 155, // "America/Matamoros"
		EraIndex: 348,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 523: Zone Pacific/Kanton
	{
		ZoneID: 0xdd512f0e,
		NameIndex: 545, // "Pacific/Kanton"
		EraIndex: 1047,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 524: Zone Asia/Yangon
	{
		ZoneID: 0xdd54a8be,
		NameIndex: 331, // "Asia/Yangon"
		EraIndex: 775,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 525: Zone Europe/Vilnius
	{
		ZoneID: 0xdd63b8ce,
		NameIndex: 484, // "Europe/Vilnius"
		EraIndex: 999,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 526: Link Australia/West -> Australia/Perth
	{
		ZoneID: 0xdd858a5d,
		NameIndex: 367, // "Australia/West"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 322, // Australia/Perth
	},
	// 527: Zone Pacific/Kosrae
	{
		ZoneID: 0xde5139a8,
		NameIndex: 547, // "Pacific/Kosrae"
		EraIndex: 1053,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 528: Zone America/Kentucky/Monticello
	{
		ZoneID: 0xde71c439,
		NameIndex: 142, // "America/Kentucky/Monticello"
		EraIndex: 321,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 529: Zone Europe/Brussels
	{
		ZoneID: 0xdee07337,
		NameIndex: 434, // "Europe/Brussels"
		EraIndex: 876,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 530: Zone America/Swift_Current
	{
		ZoneID: 0xdef98e55,
		NameIndex: 209, // "America/Swift_Current"
		EraIndex: 458,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 531: Link America/Rosario -> America/Argentina/Cordoba
	{
		ZoneID: 0xdf448665,
		NameIndex: 194, // "America/Rosario"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 461, // America/Argentina/Cordoba
	},
	// 532: Zone Asia/Irkutsk
	{
		ZoneID: 0xdfbf213f,
		NameIndex: 271, // "Asia/Irkutsk"
		EraIndex: 616,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 533: Zone Europe/Ulyanovsk
	{
		ZoneID: 0xe03783d0,
		NameIndex: 479, // "Europe/Ulyanovsk"
		EraIndex: 990,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 534: Zone Australia/Lindeman
	{
		ZoneID: 0xe05029e2,
		NameIndex: 356, // "Australia/Lindeman"
		EraIndex: 817,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 535: Zone Europe/Belgrade
	{
		ZoneID: 0xe0532b3a,
		NameIndex: 431, // "Europe/Belgrade"
		EraIndex: 872,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 536: Link Africa/Blantyre -> Africa/Maputo
	{
		ZoneID: 0xe08d813b,
		NameIndex: 10, // "Africa/Blantyre"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 537: Zone America/Menominee
	{
		ZoneID: 0xe0e9c583,
		NameIndex: 158, // "America/Menominee"
		EraIndex: 354,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 538: Link Europe/Copenhagen -> Europe/Berlin
	{
		ZoneID: 0xe0ed30bc,
		NameIndex: 439, // "Europe/Copenhagen"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 158, // Europe/Berlin
	},
	// 539: Zone Atlantic/Faroe
	{
		ZoneID: 0xe110a971,
		NameIndex: 339, // "Atlantic/Faroe"
		EraIndex: 798,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 540: Link Pacific/Majuro -> Pacific/Tarawa
	{
		ZoneID: 0xe1f95371,
		NameIndex: 549, // "Pacific/Majuro"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 563, // Pacific/Tarawa
	},
	// 541: Zone Antarctica/Casey
	{
		ZoneID: 0xe2022583,
		NameIndex: 222, // "Antarctica/Casey"
		EraIndex: 482,
		EraCount: 13,
		TargetIndex: 0,
	},
	// 542: Zone Antarctica/Davis
	{
		ZoneID: 0xe2144b45,
		NameIndex: 223, // "Antarctica/Davis"
		EraIndex: 495,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 543: Zone Europe/Astrakhan
	{
		ZoneID: 0xe22256e1,
		NameIndex: 428, // "Europe/Astrakhan"
		EraIndex: 863,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 544: Link Pacific/Midway -> Pacific/Pago_Pago
	{
		ZoneID: 0xe286d38e,
		NameIndex: 551, // "Pacific/Midway"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 208, // Pacific/Pago_Pago
	},
	// 545: Link Antarctica/Syowa -> Asia/Riyadh
	{
		ZoneID: 0xe330c7e1,
		NameIndex: 231, // "Antarctica/Syowa"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 489, // Asia/Riyadh
	},
	// 546: Zone Antarctica/Troll
	{
		ZoneID: 0xe33f085b,
		NameIndex: 232, // "Antarctica/Troll"
		EraIndex: 512,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 547: Zone Europe/Saratov
	{
		ZoneID: 0xe4315da4,
		NameIndex: 471, // "Europe/Saratov"
		EraIndex: 960,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 548: Zone Pacific/Noumea
	{
		ZoneID: 0xe551b788,
		NameIndex: 555, // "Pacific/Noumea"
		EraIndex: 1068,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 549: Link Iceland -> Africa/Abidjan
	{
		ZoneID: 0xe56a35b5,
		NameIndex: 499, // "Iceland"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 550: Link Indian/Mayotte -> Africa/Nairobi
	{
		ZoneID: 0xe6880bca,
		NameIndex: 509, // "Indian/Mayotte"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 551: Link Australia/Tasmania -> Australia/Hobart
	{
		ZoneID: 0xe6d76648,
		NameIndex: 365, // "Australia/Tasmania"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 124, // Australia/Hobart
	},
	// 552: Zone Pacific/Honolulu
	{
		ZoneID: 0xe6e70af9,
		NameIndex: 543, // "Pacific/Honolulu"
		EraIndex: 1046,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 553: Link America/Kralendijk -> America/Puerto_Rico
	{
		ZoneID: 0xe7c456c5,
		NameIndex: 144, // "America/Kralendijk"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 554: Zone America/Argentina/Tucuman
	{
		ZoneID: 0xe96399eb,
		NameIndex: 70, // "America/Argentina/Tucuman"
		EraIndex: 147,
		EraCount: 8,
		TargetIndex: 0,
	},
	// 555: Link Pacific/Ponape -> Pacific/Guadalcanal
	{
		ZoneID: 0xe9f80086,
		NameIndex: 560, // "Pacific/Ponape"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 568, // Pacific/Guadalcanal
	},
	// 556: Link Europe/Zaporozhye -> Europe/Kyiv
	{
		ZoneID: 0xeab9767f,
		NameIndex: 488, // "Europe/Zaporozhye"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 407, // Europe/Kyiv
	},
	// 557: Link Europe/Isle_of_Man -> Europe/London
	{
		ZoneID: 0xeaf84580,
		NameIndex: 444, // "Europe/Isle_of_Man"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 558: Zone Asia/Magadan
	{
		ZoneID: 0xebacc19b,
		NameIndex: 290, // "Asia/Magadan"
		EraIndex: 649,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 559: Zone America/Ojinaga
	{
		ZoneID: 0xebfde83f,
		NameIndex: 177, // "America/Ojinaga"
		EraIndex: 404,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 560: Link Pacific/Saipan -> Pacific/Guam
	{
		ZoneID: 0xeff7a35f,
		NameIndex: 563, // "Pacific/Saipan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 93, // Pacific/Guam
	},
	// 561: Zone CST6CDT
	{
		ZoneID: 0xf0e87d00,
		NameIndex: 374, // "CST6CDT"
		EraIndex: 829,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 562: Zone Pacific/Tahiti
	{
		ZoneID: 0xf24c2446,
		NameIndex: 565, // "Pacific/Tahiti"
		EraIndex: 1076,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 563: Zone Pacific/Tarawa
	{
		ZoneID: 0xf2517e63,
		NameIndex: 566, // "Pacific/Tarawa"
		EraIndex: 1077,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 564: Zone MST7MDT
	{
		ZoneID: 0xf2af9375,
		NameIndex: 519, // "MST7MDT"
		EraIndex: 1026,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 565: Link Canada/Eastern -> America/Toronto
	{
		ZoneID: 0xf3612d5e,
		NameIndex: 377, // "Canada/Eastern"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 276, // America/Toronto
	},
	// 566: Zone Asia/Tashkent
	{
		ZoneID: 0xf3924254,
		NameIndex: 315, // "Asia/Tashkent"
		EraIndex: 731,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 567: Zone Asia/Sakhalin
	{
		ZoneID: 0xf4a1c9bd,
		NameIndex: 308, // "Asia/Sakhalin"
		EraIndex: 709,
		EraCount: 7,
		TargetIndex: 0,
	},
	// 568: Zone Pacific/Guadalcanal
	{
		ZoneID: 0xf4dd25f0,
		NameIndex: 541, // "Pacific/Guadalcanal"
		EraIndex: 1043,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 569: Link Etc/GMT0 -> Etc/GMT
	{
		ZoneID: 0xf53ea988,
		NameIndex: 420, // "Etc/GMT0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 513, // Etc/GMT
	},
	// 570: Link Etc/Zulu -> Etc/UTC
	{
		ZoneID: 0xf549c240,
		NameIndex: 425, // "Etc/Zulu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 515, // Etc/UTC
	},
	// 571: Zone America/Danmarkshavn
	{
		ZoneID: 0xf554d204,
		NameIndex: 102, // "America/Danmarkshavn"
		EraIndex: 215,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 572: Zone Asia/Shanghai
	{
		ZoneID: 0xf895a7f5,
		NameIndex: 311, // "Asia/Shanghai"
		EraIndex: 722,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 573: Zone Europe/Gibraltar
	{
		ZoneID: 0xf8e325fc,
		NameIndex: 441, // "Europe/Gibraltar"
		EraIndex: 890,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 574: Link Asia/Chongqing -> Asia/Shanghai
	{
		ZoneID: 0xf937fb90,
		NameIndex: 255, // "Asia/Chongqing"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 572, // Asia/Shanghai
	},
	// 575: Zone Atlantic/Azores
	{
		ZoneID: 0xf93ed918,
		NameIndex: 334, // "Atlantic/Azores"
		EraIndex: 786,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 576: Link Pacific/Wallis -> Pacific/Tarawa
	{
		ZoneID: 0xf94ddb0f,
		NameIndex: 570, // "Pacific/Wallis"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 563, // Pacific/Tarawa
	},
	// 577: Link America/Thunder_Bay -> America/Toronto
	{
		ZoneID: 0xf962e71b,
		NameIndex: 212, // "America/Thunder_Bay"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 276, // America/Toronto
	},
	// 578: Zone America/Eirunepe
	{
		ZoneID: 0xf9b29683,
		NameIndex: 109, // "America/Eirunepe"
		EraIndex: 232,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 579: Zone America/Montevideo
	{
		ZoneID: 0xfa214780,
		NameIndex: 165, // "America/Montevideo"
		EraIndex: 379,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 580: Link US/Alaska -> America/Anchorage
	{
		ZoneID: 0xfa300bc9,
		NameIndex: 579, // "US/Alaska"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 191, // America/Anchorage
	},
	// 581: Link GB-Eire -> Europe/London
	{
		ZoneID: 0xfa70e300,
		NameIndex: 491, // "GB-Eire"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 582: Zone Europe/Kirov
	{
		ZoneID: 0xfaf5abef,
		NameIndex: 449, // "Europe/Kirov"
		EraIndex: 909,
		EraCount: 6,
		TargetIndex: 0,
	},
	// 583: Zone Europe/Malta
	{
		ZoneID: 0xfb1560f3,
		NameIndex: 456, // "Europe/Malta"
		EraIndex: 929,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 584: Zone Europe/Minsk
	{
		ZoneID: 0xfb19cc66,
		NameIndex: 458, // "Europe/Minsk"
		EraIndex: 932,
		EraCount: 4,
		TargetIndex: 0,
	},
	// 585: Zone Europe/Bucharest
	{
		ZoneID: 0xfb349ec5,
		NameIndex: 435, // "Europe/Bucharest"
		EraIndex: 878,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 586: Zone Africa/Khartoum
	{
		ZoneID: 0xfb3d4205,
		NameIndex: 28, // "Africa/Khartoum"
		EraIndex: 22,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 587: Zone Europe/Paris
	{
		ZoneID: 0xfb4bc2a3,
		NameIndex: 463, // "Europe/Paris"
		EraIndex: 941,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 588: Zone Asia/Yekaterinburg
	{
		ZoneID: 0xfb544c6e,
		NameIndex: 332, // "Asia/Yekaterinburg"
		EraIndex: 776,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 589: Zone Europe/Sofia
	{
		ZoneID: 0xfb898656,
		NameIndex: 474, // "Europe/Sofia"
		EraIndex: 977,
		EraCount: 5,
		TargetIndex: 0,
	},
	// 590: Link Canada/Mountain -> America/Edmonton
	{
		ZoneID: 0xfb8a8217,
		NameIndex: 378, // "Canada/Mountain"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 238, // America/Edmonton
	},
	// 591: Link Europe/Vaduz -> Europe/Zurich
	{
		ZoneID: 0xfbb81bae,
		NameIndex: 481, // "Europe/Vaduz"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 291, // Europe/Zurich
	},
	// 592: Zone Atlantic/Canary
	{
		ZoneID: 0xfc23f2c2,
		NameIndex: 336, // "Atlantic/Canary"
		EraIndex: 793,
		EraCount: 3,
		TargetIndex: 0,
	},
	// 593: Link Mexico/BajaNorte -> America/Tijuana
	{
		ZoneID: 0xfcf7150f,
		NameIndex: 520, // "Mexico/BajaNorte"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 231, // America/Tijuana
	},
	// 594: Link America/Santa_Isabel -> America/Tijuana
	{
		ZoneID: 0xfd18a56c,
		NameIndex: 195, // "America/Santa_Isabel"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 231, // America/Tijuana
	},
	// 595: Zone America/Campo_Grande
	{
		ZoneID: 0xfec3e7a6,
		NameIndex: 87, // "America/Campo_Grande"
		EraIndex: 189,
		EraCount: 1,
		TargetIndex: 0,
	},

}

const ZoneInfoCount = 596

const ZoneInfoChunkSize = 12

// ZoneInfosData contains the ZoneInfoRecords data as a hex encoded string.
const ZoneInfosData = "\xae\x73\x59\x00\xea\x01\x00\x00\x00\x00\xc5\x00" +
		"\xad\x74\x59\x00\x0b\x02\x00\x00\x00\x00\x62\x00" +
		"\xc4\x63\x47\x01\x1d\x01\x00\x00\x00\x00\xee\x01" +
		"\xe4\x6d\xd9\x01\x20\x00\x00\x00\x00\x00\x11\x01" +
		"\xde\x86\x1e\x02\xf7\x01\x00\x00\x00\x00\x0c\x02" +
		"\xdb\xd5\x60\x02\x6e\x01\x00\x00\x00\x00\x31\x00" +
		"\x16\xc5\x1e\x03\x52\x01\x00\x00\x00\x00\x1b\x02" +
		"\x6e\x4e\xb1\x04\xcc\x00\xc8\x01\x02\x00\x00\x00" +
		"\x7b\x50\xc0\x04\xcd\x00\x00\x00\x00\x00\xde\x00" +
		"\x9a\x21\xd7\x04\x2f\x00\x00\x00\x00\x00\xd4\x01" +
		"\xba\xb3\xd8\x04\xce\x00\x00\x00\x00\x00\xde\x00" +
		"\x8e\x95\xf9\x04\xaf\x00\x8f\x01\x02\x00\x00\x00" +
		"\x50\xab\x06\x05\x11\x01\x6d\x02\x01\x00\x00\x00" +
		"\x47\x2a\x23\x05\x0c\x00\x00\x00\x00\x00\x47\x01" +
		"\x9e\x18\x32\x05\x9c\x00\x60\x01\x02\x00\x00\x00" +
		"\x79\x6a\x4e\x05\xcb\x00\x00\x00\x00\x00\xde\x00" +
		"\x65\x1e\xae\x05\x02\x00\x00\x00\x00\x00\xa7\x01" +
		"\xba\x2b\x53\x06\x18\x02\x0d\x04\x02\x00\x00\x00" +
		"\xc4\x21\x5d\x06\x7e\x00\x09\x01\x03\x00\x00\x00" +
		"\x41\x36\xee\x08\x09\x02\x00\x00\x00\x00\x0e\x00" +
		"\x42\xe4\x03\x09\x3c\x01\xde\x02\x08\x00\x00\x00" +
		"\x12\x36\x26\x09\x83\x00\x1d\x01\x04\x00\x00\x00" +
		"\x2f\xde\xc8\x09\x49\x02\x00\x00\x00\x00\x28\x02" +
		"\xfe\x7e\x7b\x0a\x52\x00\xb0\x00\x04\x00\x00\x00" +
		"\x1d\xe9\xf0\x0a\x01\x01\x44\x02\x04\x00\x00\x00" +
		"\x21\xd9\x87\x0b\x75\x01\x3c\x03\x01\x00\x00\x00" +
		"\xa3\xe1\x87\x0b\x82\x01\x3e\x03\x01\x00\x00\x00" +
		"\x71\xe3\x87\x0b\x83\x01\x3f\x03\x01\x00\x00\x00" +
		"\x2d\xeb\x87\x0b\xec\x01\x00\x00\x00\x00\x01\x02" +
		"\x34\xf0\x87\x0b\xf1\x01\xfb\x03\x01\x00\x00\x00" +
		"\xab\x03\x88\x0b\x05\x02\x00\x04\x01\x00\x00\x00" +
		"\x79\x05\x88\x0b\x06\x02\x01\x04\x01\x00\x00\x00" +
		"\x0a\x12\x88\x0b\x0e\x02\x00\x00\x00\x00\x3c\x02" +
		"\x29\x1a\x88\x0b\x3e\x02\x00\x00\x00\x00\xf3\x01" +
		"\x31\x1a\x88\x0b\x3f\x02\x00\x00\x00\x00\x43\x00" +
		"\x71\x25\x88\x0b\x42\x02\x00\x00\x00\x00\x03\x02" +
		"\x91\x27\x88\x0b\x4f\x02\x00\x00\x00\x00\x03\x02" +
		"\x35\x2e\x88\x0b\x52\x02\x38\x04\x01\x00\x00\x00" +
		"\xf7\x59\x82\x0c\x79\x00\x01\x01\x01\x00\x00\x00" +
		"\x96\x64\xaa\x0c\xc9\x01\x00\x00\x00\x00\xe9\x00" +
		"\x85\x03\xe9\x0c\x2a\x00\x1b\x00\x02\x00\x00\x00" +
		"\x8e\x27\x1a\x0d\x85\x01\x00\x00\x00\x00\x0e\x01" +
		"\x28\x70\x2f\x0d\xed\x01\x00\x00\x00\x00\x01\x02" +
		"\x6a\x70\x2f\x0d\xee\x01\x00\x00\x00\x00\x01\x02" +
		"\x8f\x2f\x71\x0d\x02\x02\x00\x00\x00\x00\x44\x00" +
		"\x16\x8b\x99\x0d\x04\x02\x00\x00\x00\x00\x8e\x01" +
		"\xbb\xaf\x57\x0e\x03\x02\x00\x00\x00\x00\x44\x01" +
		"\x03\xd2\x86\x0e\xe5\x00\xfe\x01\x02\x00\x00\x00" +
		"\x6f\xc7\x76\x0f\xdd\x00\xe0\x01\x02\x00\x00\x00" +
		"\xa3\x59\xe5\x0f\x66\x01\x37\x03\x02\x00\x00\x00" +
		"\xc9\xbf\x63\x10\xc7\x00\xc0\x01\x01\x00\x00\x00" +
		"\xc2\x95\x93\x10\xaa\x01\x00\x00\x00\x00\x11\x02" +
		"\x54\xa0\xac\x10\x84\x00\x21\x01\x04\x00\x00\x00" +
		"\x2a\x8d\x3f\x12\xc8\x00\xc1\x01\x03\x00\x00\x00" +
		"\x04\x51\xae\x13\x35\x01\xcc\x02\x05\x00\x00\x00" +
		"\xbc\x21\x8d\x14\xed\x00\x06\x02\x02\x00\x00\x00" +
		"\x0e\x71\x8f\x14\xef\x00\x0e\x02\x07\x00\x00\x00" +
		"\x3b\x86\xae\x14\xfd\x00\x3a\x02\x06\x00\x00\x00" +
		"\x5e\xac\xbc\x14\x02\x01\x00\x00\x00\x00\x3b\x00" +
		"\x8b\x7b\xc0\x14\x04\x01\x4a\x02\x02\x00\x00\x00" +
		"\x77\x9f\xc7\x14\x06\x01\x4f\x02\x01\x00\x00\x00" +
		"\x9a\x32\xf6\x14\x4d\x00\xa9\x00\x03\x00\x00\x00" +
		"\x01\x56\x3b\x15\x14\x01\x70\x02\x01\x00\x00\x00" +
		"\x09\x4f\x45\x15\xf2\x00\x00\x00\x00\x00\xc5\x01" +
		"\xb3\x88\x5f\x15\x20\x01\x00\x00\x00\x00\x41\x00" +
		"\xb9\x88\x5f\x15\x21\x01\x88\x02\x01\x00\x00\x00" +
		"\x0b\x33\xa8\x15\x2e\x01\xb1\x02\x02\x00\x00\x00" +
		"\xda\x82\xce\x15\x36\x01\xd1\x02\x01\x00\x00\x00" +
		"\xa8\x06\xe6\x15\x41\x01\xeb\x02\x01\x00\x00\x00" +
		"\x60\x0e\xe6\x15\x42\x01\xec\x02\x07\x00\x00\x00" +
		"\x2c\x7c\x6d\x16\x3e\x01\x00\x00\x00\x00\xc1\x00" +
		"\xd1\x80\x03\x17\x40\x01\xe9\x02\x02\x00\x00\x00" +
		"\x58\x49\xe6\x17\x7a\x00\x02\x01\x01\x00\x00\x00" +
		"\x35\x0a\x9b\x19\xa7\x00\x00\x00\x00\x00\xde\x00" +
		"\x4b\x02\x21\x1a\x8d\x00\x3e\x01\x03\x00\x00\x00" +
		"\x57\xc0\x76\x1a\x2c\x01\xac\x02\x02\x00\x00\x00" +
		"\x9c\x49\x1a\x1c\xd0\x01\x00\x00\x00\x00\x17\x02" +
		"\x74\x4f\x2b\x1c\x56\x01\x00\x00\x00\x00\xd4\x01" +
		"\x54\x76\x2a\x1e\xa9\x00\x80\x01\x01\x00\x00\x00" +
		"\xce\xc6\x8b\x1f\xc6\x01\x00\x00\x00\x00\x11\x02" +
		"\x4a\x08\xa7\x1f\xeb\x00\x00\x00\x00\x00\xe9\x01" +
		"\xb5\x88\xa7\x1f\xf6\x00\x28\x02\x05\x00\x00\x00" +
		"\x94\xc3\xa8\x1f\x05\x01\x4c\x02\x03\x00\x00\x00" +
		"\x75\x48\xaa\x1f\x09\x01\x57\x02\x09\x00\x00\x00" +
		"\xe3\x0f\xab\x1f\x0e\x01\x66\x02\x02\x00\x00\x00" +
		"\xac\xdd\xae\x1f\x29\x01\x9f\x02\x05\x00\x00\x00" +
		"\xa0\xf0\xae\x1f\x2a\x01\xa4\x02\x08\x00\x00\x00" +
		"\xa8\x1e\x3a\x20\xa6\x00\x00\x00\x00\x00\x14\x01" +
		"\x27\xd1\xf2\x20\x0c\x01\x63\x02\x02\x00\x00\x00" +
		"\x63\xb0\xfb\x20\x03\x01\x48\x02\x02\x00\x00\x00" +
		"\x77\x88\x75\x22\x3d\x00\x00\x00\x00\x00\x3e\x01" +
		"\x5e\x9b\x35\x23\x10\x02\x04\x04\x02\x00\x00\x00" +
		"\xa5\x3b\x38\x23\x19\x02\x0f\x04\x01\x00\x00\x00" +
		"\xed\xf9\x38\x23\x1e\x02\x14\x04\x02\x00\x00\x00" +
		"\x14\xa0\x3c\x23\x29\x02\x26\x04\x01\x00\x00\x00" +
		"\xa9\x10\x40\x23\x38\x02\x00\x00\x00\x00\xa3\x01" +
		"\x2b\x6c\x41\x23\x39\x02\x00\x00\x00\x00\x33\x02" +
		"\xa3\xe8\x28\x24\x5b\x01\x27\x03\x02\x00\x00\x00" +
		"\x86\x2f\x06\x25\x11\x02\x06\x04\x01\x00\x00\x00" +
		"\x36\xa8\x2c\x26\x37\x02\x36\x04\x02\x00\x00\x00" +
		"\xeb\x1d\x9a\x26\xa4\x00\x78\x01\x03\x00\x00\x00" +
		"\x8c\xa9\xda\x26\xa5\x01\x00\x00\x00\x00\x01\x02" +
		"\x24\x6a\x8b\x27\x5f\x01\x00\x00\x00\x00\x7c\x00" +
		"\xff\xbd\x76\x28\x60\x01\x2e\x03\x01\x00\x00\x00" +
		"\x96\x9f\x92\x28\x2f\x02\x00\x00\x00\x00\x38\x02" +
		"\x8b\x4f\x9b\x28\x08\x01\x53\x02\x04\x00\x00\x00" +
		"\x12\xb2\xa0\x28\x85\x00\x25\x01\x05\x00\x00\x00" +
		"\xa4\x69\xa6\x28\x7f\x00\x0c\x01\x04\x00\x00\x00" +
		"\xa8\x34\xde\x29\x49\x01\xfd\x02\x05\x00\x00\x00" +
		"\x58\xae\x09\x2a\x5e\x01\x00\x00\x00\x00\xa8\x00" +
		"\xee\x18\xd0\x2a\x72\x00\xf2\x00\x06\x00\x00\x00" +
		"\x1f\x6b\x6f\x2c\xd8\x00\xd6\x01\x02\x00\x00\x00" +
		"\x93\x91\x99\x2d\xb3\x00\x00\x00\x00\x00\x71\x00" +
		"\xbf\x10\xe3\x2d\x89\x00\x32\x01\x03\x00\x00\x00" +
		"\xab\xfd\x44\x2e\x01\x02\x00\x00\x00\x00\xb6\x00" +
		"\x99\xe9\x0d\x2f\x13\x02\x09\x04\x01\x00\x00\x00" +
		"\xa9\xb9\x8c\x2f\xa8\x01\x00\x00\x00\x00\x03\x02" +
		"\x72\xee\xee\x2f\x81\x00\x13\x01\x06\x00\x00\x00" +
		"\x96\xe0\xc4\x30\xdc\x01\xd6\x03\x06\x00\x00\x00" +
		"\xd4\x01\xea\x30\x13\x00\x00\x00\x00\x00\xa7\x01" +
		"\x4e\xcc\xf0\x30\x44\x01\xf3\x02\x02\x00\x00\x00" +
		"\xa7\x0a\x7c\x31\x17\x00\x00\x00\x00\x00\x47\x01" +
		"\xde\xcd\x0d\x32\x47\x00\x9b\x00\x07\x00\x00\x00" +
		"\xc3\x44\x8a\x32\xfc\x00\x00\x00\x00\x00\xfb\x00" +
		"\x1a\x95\xbf\x32\x62\x01\x30\x03\x01\x00\x00\x00" +
		"\x3c\x5c\xfc\x32\x07\x01\x50\x02\x03\x00\x00\x00" +
		"\x74\x31\x01\x33\x57\x01\x22\x03\x01\x00\x00\x00" +
		"\x01\xaf\xb5\x34\xb5\x00\x9e\x01\x01\x00\x00\x00" +
		"\x94\x78\x2e\x38\x10\x01\x00\x00\x00\x00\x8f\x01" +
		"\xd9\xb4\x4d\x39\x45\x01\x00\x00\x00\x00\x78\x00" +
		"\x63\xd8\x9c\x39\xe2\x00\xf8\x01\x02\x00\x00\x00" +
		"\x60\xa7\xcd\x39\x0b\x00\x00\x00\x00\x00\x11\x01" +
		"\xf4\x64\xe0\x3b\x59\x00\xc2\x00\x03\x00\x00\x00" +
		"\x69\x72\x61\x3c\x5b\x00\xc5\x00\x01\x00\x00\x00" +
		"\x5d\xf9\x1b\x3d\x30\x00\x00\x00\x00\x00\x11\x01" +
		"\xc4\xb1\x4b\x3d\x4f\x01\x16\x03\x03\x00\x00\x00" +
		"\x00\x76\x5e\x3d\x97\x00\x4e\x01\x07\x00\x00\x00" +
		"\x59\x3a\xab\x3d\x99\x00\x00\x00\x00\x00\xde\x00" +
		"\x16\x2c\xb1\x3d\xba\x01\x00\x00\x00\x00\xc5\x00" +
		"\xba\x9d\xc4\x3d\x2e\x00\x00\x00\x00\x00\xd4\x01" +
		"\xee\x47\xcb\x3d\x94\x00\x00\x00\x00\x00\x4a\x00" +
		"\xbd\x09\x10\x3e\x44\x00\x7d\x00\x09\x00\x00\x00" +
		"\x8c\x23\x11\x3e\x45\x00\x86\x00\x0d\x00\x00\x00" +
		"\x89\xf3\xd0\x3e\xe5\x01\xee\x03\x08\x00\x00\x00" +
		"\x0f\x7e\x43\x3f\x70\x00\xef\x00\x03\x00\x00\x00" +
		"\xc4\x1c\x8f\x3f\x8a\x01\x43\x03\x01\x00\x00\x00" +
		"\xc5\x1c\x8f\x3f\x8b\x01\x44\x03\x01\x00\x00\x00" +
		"\xc6\x1c\x8f\x3f\x8c\x01\x45\x03\x01\x00\x00\x00" +
		"\x46\x25\x8f\x3f\x97\x01\x4f\x03\x01\x00\x00\x00" +
		"\x47\x25\x8f\x3f\x98\x01\x50\x03\x01\x00\x00\x00" +
		"\x48\x25\x8f\x3f\x99\x01\x51\x03\x01\x00\x00\x00" +
		"\x49\x25\x8f\x3f\x9a\x01\x52\x03\x01\x00\x00\x00" +
		"\x4a\x25\x8f\x3f\x9b\x01\x53\x03\x01\x00\x00\x00" +
		"\x4f\x2f\x96\x40\xe4\x00\xfa\x01\x04\x00\x00\x00" +
		"\x7b\x3c\xfa\x40\x7c\x01\x00\x00\x00\x00\x6f\x00" +
		"\x27\xfa\x18\x43\xad\x01\x66\x03\x02\x00\x00\x00" +
		"\x89\xb3\x51\x43\xf9\x01\x00\x00\x00\x00\x68\x01" +
		"\x69\xfa\x13\x44\x86\x00\x2a\x01\x05\x00\x00\x00" +
		"\x20\x4c\x64\x44\xb0\x01\x6a\x03\x02\x00\x00\x00" +
		"\xf7\xc5\x1f\x45\x58\x01\x00\x00\x00\x00\xd4\x01" +
		"\x3c\x7c\x6f\x45\xf5\x01\xfc\x03\x02\x00\x00\x00" +
		"\xe2\x25\xe7\x45\xfa\x01\x00\x00\x00\x00\x3c\x00" +
		"\xb6\xde\xf4\x45\xf8\x01\x00\x00\x00\x00\xa7\x01" +
		"\x54\xe0\xb4\x46\x9d\x00\x00\x00\x00\x00\xab\x01" +
		"\x21\xf9\x85\x47\x47\x01\xf6\x02\x07\x00\x00\x00" +
		"\x62\x5f\x27\x4a\xb8\x01\x79\x03\x01\x00\x00\x00" +
		"\x78\xcf\x0f\x4b\x26\x01\x91\x02\x02\x00\x00\x00" +
		"\xd4\xb5\x92\x4b\x5d\x00\xc6\x00\x01\x00\x00\x00" +
		"\x76\x97\x1e\x4d\x6c\x01\x3a\x03\x02\x00\x00\x00" +
		"\xda\xaf\x42\x4d\x0c\x02\x00\x00\x00\x00\x73\x00" +
		"\x70\x26\xc5\x4e\x45\x02\x00\x00\x00\x00\x7f\x00" +
		"\xd4\x6f\x96\x4f\xe9\x00\x00\x00\x00\x00\xfd\x01" +
		"\xef\x13\xe0\x4f\x44\x02\x00\x00\x00\x00\x63\x01" +
		"\xc0\xc9\xed\x4f\x5c\x01\x29\x03\x02\x00\x00\x00" +
		"\x63\xe9\x36\x50\x5a\x00\x00\x00\x00\x00\x3e\x01" +
		"\xa6\x79\xec\x50\x49\x00\xa2\x00\x03\x00\x00\x00" +
		"\x45\x52\x7f\x52\x16\x01\x76\x02\x02\x00\x00\x00" +
		"\x93\x51\x95\x52\x17\x01\x00\x00\x00\x00\xfd\x01" +
		"\x9c\x11\x6b\x53\x77\x01\x00\x00\x00\x00\xc8\x01" +
		"\x3a\x0c\x72\x53\x1c\x02\x12\x04\x01\x00\x00\x00" +
		"\xe8\xe3\xe0\x54\xda\x00\xd8\x01\x03\x00\x00\x00" +
		"\xc5\x84\x1e\x55\x9a\x00\x59\x01\x03\x00\x00\x00" +
		"\x6c\xad\x5d\x56\x8a\x00\x35\x01\x03\x00\x00\x00" +
		"\xcd\x21\x68\x56\x4e\x02\x00\x00\x00\x00\xd0\x00" +
		"\x60\x65\xd3\x56\xf2\x01\x00\x00\x00\x00\xba\x00" +
		"\x76\xae\x0d\x57\xbe\x01\x00\x00\x00\x00\xc5\x00" +
		"\xac\x28\x7f\x57\x0d\x01\x65\x02\x01\x00\x00\x00" +
		"\x35\x71\xca\x57\x26\x02\x23\x04\x01\x00\x00\x00" +
		"\x30\x43\x67\x59\xa2\x00\x71\x01\x03\x00\x00\x00" +
		"\x6c\x65\x3c\x5a\xe0\x00\x00\x00\x00\x00\xa3\x01" +
		"\xb6\x35\x75\x5a\x54\x01\x00\x00\x00\x00\x9e\x00" +
		"\x0e\x26\x79\x5a\x37\x00\x32\x00\x04\x00\x00\x00" +
		"\x8e\xe7\xb7\x5b\x48\x02\x00\x00\x00\x00\x4e\x00" +
		"\x3a\xd2\xec\x5b\x13\x01\x6f\x02\x01\x00\x00\x00" +
		"\xb8\xfb\xf6\x5b\xdb\x01\x00\x00\x00\x00\x9e\x00" +
		"\x0b\xa7\x00\x5c\xc3\x01\x97\x03\x05\x00\x00\x00" +
		"\x72\x17\x5e\x5c\x51\x01\x1c\x03\x02\x00\x00\x00" +
		"\xae\x84\x6a\x5c\xc5\x01\x9c\x03\x03\x00\x00\x00" +
		"\x00\x76\x8a\x5c\x61\x00\x00\x00\x00\x00\xcd\x01" +
		"\xb3\x1e\x00\x5d\x43\x01\x00\x00\x00\x00\xe8\x00" +
		"\x40\xdd\x3b\x5d\x28\x00\x00\x00\x00\x00\xff\x01" +
		"\x35\x15\xbd\x5d\xc7\x01\x9f\x03\x02\x00\x00\x00" +
		"\x24\xfe\x07\x5e\xa3\x00\x74\x01\x04\x00\x00\x00" +
		"\xa4\xf7\x10\x5e\x12\x02\x07\x04\x02\x00\x00\x00" +
		"\x01\x9f\xbf\x5e\xcb\x01\x00\x00\x00\x00\x4b\x02" +
		"\xfc\x66\xc2\x5e\xcc\x01\xa8\x03\x05\x00\x00\x00" +
		"\xc5\x46\x2f\x5f\x3f\x00\x4f\x00\x0a\x00\x00\x00" +
		"\x03\x34\xc7\x5f\x43\x00\x76\x00\x07\x00\x00\x00" +
		"\xd0\xeb\x3a\x60\x2c\x02\x2d\x04\x01\x00\x00\x00" +
		"\x93\x9a\x59\x61\x17\x02\x00\x00\x00\x00\x0b\x02" +
		"\xd1\x19\xb3\x61\x31\x00\x21\x00\x03\x00\x00\x00" +
		"\xf5\x10\x67\x62\x78\x01\x00\x00\x00\x00\x3c\x01" +
		"\x04\x02\xa7\x62\x63\x00\x00\x00\x00\x00\x7f\x00" +
		"\xbe\x66\xff\x63\x62\x00\xd3\x00\x01\x00\x00\x00" +
		"\x22\xe5\x4f\x65\x2f\x01\xb3\x02\x07\x00\x00\x00" +
		"\xc9\xad\x54\x65\x80\x00\x10\x01\x03\x00\x00\x00" +
		"\x48\x5d\xee\x65\xd1\x01\xaf\x03\x02\x00\x00\x00" +
		"\x93\x4f\x93\x66\x71\x01\x00\x00\x00\x00\x89\x01" +
		"\xc5\x78\x95\x66\x73\x01\x00\x00\x00\x00\x32\x00" +
		"\x0c\xd7\x95\x66\x1e\x00\x00\x00\x00\x00\x11\x01" +
		"\x9b\x68\x9f\x66\x74\x01\x00\x00\x00\x00\xb0\x01" +
		"\x9b\x15\xbc\x66\x29\x00\x00\x00\x00\x00\xa7\x01" +
		"\x31\xca\x52\x67\xba\x00\xa2\x01\x01\x00\x00\x00" +
		"\xe7\x7b\x97\x67\x4a\x02\x00\x00\x00\x00\xd7\x00" +
		"\x05\x9a\xd7\x67\x55\x00\x00\x00\x00\x00\xf9\x01" +
		"\x64\xdd\x23\x68\x16\x00\x00\x00\x00\x00\xd4\x01" +
		"\xd5\x07\xc2\x68\xf6\x01\x00\x00\x00\x00\x8a\x01" +
		"\x93\x4f\x26\x69\x27\x01\x93\x02\x05\x00\x00\x00" +
		"\xe1\x9a\x00\x6a\x87\x00\x00\x00\x00\x00\x6b\x00" +
		"\x3f\x6c\x57\x6a\xd6\x01\x00\x00\x00\x00\x17\x02" +
		"\x84\x91\x87\x6a\x65\x00\x00\x00\x00\x00\xde\x00" +
		"\x72\xdf\xa1\x6a\xd5\x00\xce\x01\x06\x00\x00\x00" +
		"\x85\x1c\xa2\x6a\x23\x01\x8f\x02\x01\x00\x00\x00" +
		"\x5b\x97\xb2\x6a\xbb\x01\x7c\x03\x02\x00\x00\x00" +
		"\x62\x5b\xe4\x6a\x95\x00\x00\x00\x00\x00\xde\x00" +
		"\x77\xac\x1a\x6b\xb9\x00\xa0\x01\x02\x00\x00\x00" +
		"\x39\xb1\xc0\x6b\xd4\x01\xb9\x03\x07\x00\x00\x00" +
		"\xd0\xfd\x76\x6c\xd9\x01\x00\x00\x00\x00\x17\x02" +
		"\x4a\x48\xb9\x6c\x6c\x00\xe6\x00\x02\x00\x00\x00" +
		"\x5b\x4e\xf2\x6c\x68\x00\xde\x00\x02\x00\x00\x00" +
		"\xc6\x17\x12\x6d\x31\x01\x00\x00\x00\x00\x0c\x02" +
		"\x8a\x55\xcf\x6d\x47\x02\x00\x00\x00\x00\x6b\x00" +
		"\x97\x61\x21\x6e\x76\x00\xfd\x00\x04\x00\x00\x00" +
		"\x92\x98\x29\x6e\x51\x00\x00\x00\x00\x00\xde\x00" +
		"\x47\x5b\xa9\x6e\xdd\x01\xdc\x03\x02\x00\x00\x00" +
		"\x47\x82\xb8\x6e\x4c\x02\x00\x00\x00\x00\x5e\x01" +
		"\x85\x55\xeb\x6e\xe3\x00\x00\x00\x00\x00\x62\x00" +
		"\xef\x3a\x9a\x6f\x3a\x00\x36\x00\x06\x00\x00\x00" +
		"\x66\x85\xd8\x6f\x23\x00\x00\x00\x00\x00\x47\x01" +
		"\x47\xc0\x76\x70\xfe\x01\x00\x00\x00\x00\x3c\x00" +
		"\x81\x2e\x28\x71\x30\x01\xba\x02\x0a\x00\x00\x00" +
		"\xd9\x6c\xc0\x72\x1b\x01\x81\x02\x01\x00\x00\x00" +
		"\x60\xfc\x47\x73\x5f\x00\xcc\x00\x07\x00\x00\x00" +
		"\xe5\xc2\x4c\x73\xe3\x01\xe5\x03\x02\x00\x00\x00" +
		"\xef\x78\xb2\x73\x04\x00\x00\x00\x00\x00\xa7\x01" +
		"\xf3\x89\xb2\x73\x05\x00\x00\x00\x00\x00\xa7\x01" +
		"\xd7\xf9\xba\x73\x15\x01\x71\x02\x05\x00\x00\x00" +
		"\xc1\xae\x0c\x74\xc4\x00\xb8\x01\x03\x00\x00\x00" +
		"\xbc\xc9\x10\x74\xc5\x00\xbb\x01\x01\x00\x00\x00" +
		"\xa5\xe7\xc1\x74\x06\x00\x00\x00\x00\x00\xd4\x01" +
		"\xd0\x8e\xc2\x74\x07\x00\x00\x00\x00\x00\x11\x01" +
		"\x96\x9b\xc2\x74\x08\x00\x00\x00\x00\x00\xd4\x01" +
		"\x8a\xab\xef\x74\xcd\x01\x00\x00\x00\x00\xa6\x00" +
		"\x19\x5c\x18\x75\xe6\x01\xf6\x03\x03\x00\x00\x00" +
		"\x52\xd6\x2a\x75\x6e\x00\xee\x00\x01\x00\x00\x00" +
		"\x41\x41\x56\x75\x09\x00\x05\x00\x02\x00\x00\x00" +
		"\x77\xd1\xa0\x75\xc6\x00\xbc\x01\x04\x00\x00\x00" +
		"\xbc\xb7\x6b\x76\x4b\x02\x00\x00\x00\x00\x4e\x01" +
		"\x49\x1f\x31\x77\x7d\x01\x00\x00\x00\x00\xc4\x01" +
		"\x54\xb0\xd5\x77\x01\x00\x00\x00\x00\x00\xd4\x01" +
		"\x28\xe2\xf8\x77\x0d\x00\x07\x00\x01\x00\x00\x00" +
		"\xec\x46\xfb\x77\x0f\x00\x0c\x00\x03\x00\x00\x00" +
		"\xfd\x00\x0b\x78\x11\x00\x00\x00\x00\x00\xd4\x01" +
		"\xd0\xb5\x9b\x78\x1f\x00\x19\x00\x01\x00\x00\x00" +
		"\xd3\x9b\x9c\x78\x35\x00\x2c\x00\x02\x00\x00\x00" +
		"\xc2\x35\xdd\x78\x7e\x01\x00\x00\x00\x00\xb4\x00" +
		"\x1b\x85\x2e\x79\xd6\x00\xd4\x01\x02\x00\x00\x00" +
		"\x2b\x46\x31\x79\xd7\x00\x00\x00\x00\x00\xde\x00" +
		"\x6d\x8e\x37\x79\x34\x00\x2b\x00\x01\x00\x00\x00" +
		"\x10\xf3\x6d\x7a\x14\x00\x00\x00\x00\x00\x11\x01" +
		"\x31\x6b\xb3\x7a\x10\x00\x00\x00\x00\x00\xd4\x01" +
		"\x2a\xc0\x09\x7b\xfc\x01\xff\x03\x01\x00\x00\x00" +
		"\xc4\xe1\xb3\x7b\x59\x01\x23\x03\x04\x00\x00\x00" +
		"\x45\x54\xc9\x7b\x6f\x00\x00\x00\x00\x00\xe7\x00" +
		"\xff\xc9\x11\x7c\xe7\x01\x00\x00\x00\x00\x17\x02" +
		"\xa0\xcb\x83\x7c\x81\x01\x00\x00\x00\x00\x94\x01" +
		"\x6a\xb3\x84\x7c\x86\x01\x00\x00\x00\x00\xa5\x00" +
		"\xfd\x50\x85\x7c\xef\x01\x00\x00\x00\x00\x01\x02" +
		"\x0f\x09\x87\x7c\xff\x01\x00\x00\x00\x00\xf4\x01" +
		"\xf1\x8e\x8d\x7c\x51\x02\x00\x00\x00\x00\xcd\x00" +
		"\xb5\x69\x90\x7c\x53\x02\x00\x00\x00\x00\x03\x02" +
		"\xb9\x95\x81\x7d\xe9\x01\xf9\x03\x02\x00\x00\x00" +
		"\x18\xdb\x2b\x7e\x7f\x01\x00\x00\x00\x00\x02\x01" +
		"\x24\xaf\xaa\x7e\x71\x00\x00\x00\x00\x00\x6b\x00" +
		"\x3b\x00\x1b\x80\x1e\x01\x87\x02\x01\x00\x00\x00" +
		"\x37\xc0\xb5\x81\x55\x01\x20\x03\x02\x00\x00\x00" +
		"\x98\x20\xb9\x81\x4a\x00\x00\x00\x00\x00\xbd\x01" +
		"\x2d\x9a\xc3\x82\x18\x00\x00\x00\x00\x00\x47\x01" +
		"\x49\x70\xfb\x82\xc9\x00\x00\x00\x00\x00\x5e\x01" +
		"\x3a\x07\x05\x83\x22\x02\x1a\x04\x03\x00\x00\x00" +
		"\x23\xe5\x60\x84\xd0\x00\x00\x00\x00\x00\xde\x00" +
		"\x86\x26\xde\x84\xa0\x00\x68\x01\x06\x00\x00\x00" +
		"\x9e\x3a\xbb\x87\x4a\x01\x02\x03\x05\x00\x00\x00" +
		"\x76\xd7\x27\x88\x5e\x00\xc7\x00\x05\x00\x00\x00" +
		"\xbd\xd8\x37\x88\x2e\x02\x2f\x04\x02\x00\x00\x00" +
		"\x75\x8d\xd6\x89\x48\x01\x00\x00\x00\x00\x8a\x01" +
		"\x23\x0b\x09\x8a\x14\x02\x00\x00\x00\x00\xa3\x01" +
		"\x28\xce\x2b\x8a\x16\x02\x0c\x04\x01\x00\x00\x00" +
		"\x2b\xcf\x4d\x8a\x1d\x00\x00\x00\x00\x00\x47\x01" +
		"\xb2\x0e\x97\x8a\x5a\x01\x00\x00\x00\x00\xa8\x00" +
		"\x17\x3e\x97\x8a\x63\x01\x00\x00\x00\x00\xa2\x01" +
		"\x12\x48\x97\x8a\x67\x01\x00\x00\x00\x00\xa8\x00" +
		"\xae\x41\xcc\x8a\x28\x02\x24\x04\x02\x00\x00\x00" +
		"\xa3\x72\xdc\x8a\x84\x01\x40\x03\x01\x00\x00\x00" +
		"\x36\x4a\xf0\x8a\x2d\x02\x2e\x04\x01\x00\x00\x00" +
		"\xb4\x99\x26\x8b\x34\x02\x00\x00\x00\x00\xd0\x00" +
		"\xc7\xaf\x7d\x8c\xdb\x00\xdb\x01\x02\x00\x00\x00" +
		"\x44\x9e\xf9\x8c\x61\x01\x2f\x03\x01\x00\x00\x00" +
		"\x6b\x98\x40\x8d\x3c\x00\x40\x00\x09\x00\x00\x00" +
		"\xcf\x09\x79\x8d\x22\x00\x00\x00\x00\x00\x11\x01" +
		"\x3b\xb2\x82\x8d\x24\x00\x00\x00\x00\x00\x47\x01" +
		"\x65\x71\x99\x8d\x68\x01\x00\x00\x00\x00\x67\x00" +
		"\x9d\x26\xb8\x8d\x69\x01\x39\x03\x01\x00\x00\x00" +
		"\xad\xf8\xf3\x8d\x6b\x01\x00\x00\x00\x00\x61\x00" +
		"\x59\x67\x21\x8e\x24\x02\x20\x04\x03\x00\x00\x00" +
		"\xdc\x7b\x4a\x8e\xb6\x00\x9f\x01\x01\x00\x00\x00" +
		"\x06\x19\x6a\x8e\x25\x00\x00\x00\x00\x00\x11\x01" +
		"\xf0\xa1\x6c\x8e\x26\x00\x1a\x00\x01\x00\x00\x00" +
		"\xc7\x02\x6e\x8e\x27\x00\x00\x00\x00\x00\xff\x01" +
		"\xbe\xb4\x4e\x8f\x2a\x02\x27\x04\x05\x00\x00\x00" +
		"\x1f\xba\x7e\x8f\x74\x00\x00\x00\x00\x00\x67\x01" +
		"\x31\xc1\xba\x90\x70\x01\x00\x00\x00\x00\xb6\x01" +
		"\xfd\x30\x4a\x91\x2d\x00\x00\x00\x00\x00\x11\x01" +
		"\xcc\xc8\x85\x91\x4d\x01\x0d\x03\x05\x00\x00\x00" +
		"\xc1\xfb\x5c\x92\x6a\x00\xe1\x00\x05\x00\x00\x00" +
		"\xa6\xa4\x8a\x92\xfe\x00\x40\x02\x04\x00\x00\x00" +
		"\x26\x76\xf4\x92\xe1\x00\xf5\x01\x03\x00\x00\x00" +
		"\x81\x6c\x25\x93\x50\x00\xaf\x00\x01\x00\x00\x00" +
		"\x57\x1d\x71\x93\x0a\x02\x00\x00\x00\x00\xf1\x01" +
		"\x62\xbc\xd7\x93\x53\x00\xb4\x00\x01\x00\x00\x00" +
		"\x8e\x1c\xed\x93\x2d\x01\xae\x02\x03\x00\x00\x00" +
		"\xcc\x7a\xac\x94\x82\x00\x19\x01\x04\x00\x00\x00" +
		"\xbe\x31\x33\x95\x58\x00\xbe\x00\x04\x00\x00\x00" +
		"\xdf\x61\x39\x95\x5c\x00\x00\x00\x00\x00\xbd\x01" +
		"\xdd\xf8\x81\x96\x73\x00\xf8\x00\x03\x00\x00\x00" +
		"\xd9\xa4\x85\x96\x1a\x01\x7a\x02\x07\x00\x00\x00" +
		"\xd8\xe4\x8c\x96\x77\x00\x00\x00\x00\x00\xde\x00" +
		"\xeb\x52\x9a\x96\x64\x00\xd4\x00\x03\x00\x00\x00" +
		"\x12\x8d\x8d\x97\x67\x00\xda\x00\x04\x00\x00\x00" +
		"\x51\x86\xcf\x97\x48\x00\x00\x00\x00\x00\xde\x00" +
		"\x2a\x0b\xd1\x97\x69\x00\xe0\x00\x01\x00\x00\x00" +
		"\xfb\x15\xd8\x97\x4c\x00\xa5\x00\x04\x00\x00\x00" +
		"\x0b\x58\xda\x97\x4f\x00\xad\x00\x02\x00\x00\x00" +
		"\xd8\xc8\xdf\x97\x54\x00\xb5\x00\x03\x00\x00\x00" +
		"\x4b\x76\xf6\x97\xab\x01\x5d\x03\x02\x00\x00\x00" +
		"\xd7\x49\xfe\x97\x36\x00\x2e\x00\x04\x00\x00\x00" +
		"\x27\x8f\xfe\x97\x4b\x00\x00\x00\x00\x00\x63\x01" +
		"\xc9\x68\x04\x98\x92\x00\x44\x01\x01\x00\x00\x00" +
		"\x15\x9b\x05\x98\xab\x00\x81\x01\x04\x00\x00\x00" +
		"\xa9\xb5\x05\x98\xb0\x00\x91\x01\x03\x00\x00\x00" +
		"\x1c\x68\x69\x98\xfb\x01\xfe\x03\x01\x00\x00\x00" +
		"\xbd\xdb\x73\x98\x8b\x00\x00\x00\x00\x00\xce\x00" +
		"\xe2\x4c\x10\x99\xca\x00\xc4\x01\x04\x00\x00\x00" +
		"\x68\xdd\x21\x99\xd3\x00\xcd\x01\x01\x00\x00\x00" +
		"\xb0\xa3\x81\x99\x32\x02\x32\x04\x02\x00\x00\x00" +
		"\x6f\xce\x96\x9a\x18\x01\x78\x02\x02\x00\x00\x00" +
		"\x96\xb4\x4c\x9b\x72\x01\x00\x00\x00\x00\xad\x01" +
		"\x84\xb3\x52\x9b\xad\x00\x8b\x01\x02\x00\x00\x00" +
		"\x10\x8a\xd5\x9c\xbc\x00\x00\x00\x00\x00\x3c\x01" +
		"\x7c\x19\xe0\x9c\xb4\x01\x73\x03\x02\x00\x00\x00" +
		"\xed\xfb\xef\x9c\xf4\x00\x26\x02\x02\x00\x00\x00" +
		"\x87\x84\x07\x9d\xf5\x00\x00\x00\x00\x00\x42\x00" +
		"\x13\xda\x13\x9d\x88\x01\x00\x00\x00\x00\x01\x02" +
		"\x14\xda\x13\x9d\x89\x01\x42\x03\x01\x00\x00\x00" +
		"\x15\xda\x13\x9d\x8d\x01\x46\x03\x01\x00\x00\x00" +
		"\x16\xda\x13\x9d\x8e\x01\x47\x03\x01\x00\x00\x00" +
		"\x17\xda\x13\x9d\x8f\x01\x48\x03\x01\x00\x00\x00" +
		"\x18\xda\x13\x9d\x90\x01\x49\x03\x01\x00\x00\x00" +
		"\x19\xda\x13\x9d\x91\x01\x4a\x03\x01\x00\x00\x00" +
		"\x1a\xda\x13\x9d\x92\x01\x4b\x03\x01\x00\x00\x00" +
		"\x1b\xda\x13\x9d\x93\x01\x4c\x03\x01\x00\x00\x00" +
		"\x1c\xda\x13\x9d\x94\x01\x4d\x03\x01\x00\x00\x00" +
		"\x55\xda\x13\x9d\x95\x01\x00\x00\x00\x00\x01\x02" +
		"\x56\xda\x13\x9d\x96\x01\x4e\x03\x01\x00\x00\x00" +
		"\x57\xda\x13\x9d\x9c\x01\x54\x03\x01\x00\x00\x00" +
		"\x58\xda\x13\x9d\x9d\x01\x55\x03\x01\x00\x00\x00" +
		"\x59\xda\x13\x9d\x9e\x01\x56\x03\x01\x00\x00\x00" +
		"\x5a\xda\x13\x9d\x9f\x01\x57\x03\x01\x00\x00\x00" +
		"\x5b\xda\x13\x9d\xa0\x01\x58\x03\x01\x00\x00\x00" +
		"\x5c\xda\x13\x9d\xa1\x01\x59\x03\x01\x00\x00\x00" +
		"\x5d\xda\x13\x9d\xa2\x01\x5a\x03\x01\x00\x00\x00" +
		"\x5e\xda\x13\x9d\xa3\x01\x5b\x03\x01\x00\x00\x00" +
		"\x1a\x8b\x2a\x9d\xaa\x00\x00\x00\x00\x00\x14\x01" +
		"\x64\x27\x35\x9d\xc1\x00\xb4\x01\x04\x00\x00\x00" +
		"\xaf\x3a\x6e\x9d\xf7\x00\x2d\x02\x01\x00\x00\x00" +
		"\x18\xb1\x6f\x9d\x15\x00\x0f\x00\x03\x00\x00\x00" +
		"\x14\x28\xa4\x9d\xae\x00\x8d\x01\x02\x00\x00\x00" +
		"\x97\x49\xba\x9d\xf8\x00\x2e\x02\x07\x00\x00\x00" +
		"\x3d\xbd\xfe\x9d\x33\x00\x24\x00\x07\x00\x00\x00" +
		"\xe6\xd6\x09\x9e\xbd\x01\x7e\x03\x0b\x00\x00\x00" +
		"\x89\x52\xbf\x9e\xf4\x01\x00\x00\x00\x00\xa7\x01" +
		"\x98\x98\xe0\x9f\x2c\x00\x1e\x00\x03\x00\x00\x00" +
		"\x0b\xbd\xf7\x9f\x7b\x00\x03\x01\x03\x00\x00\x00" +
		"\xb6\x47\x4c\xa0\x12\x00\x00\x00\x00\x00\xa7\x01" +
		"\x75\x56\xe1\xa0\x7d\x00\x08\x01\x01\x00\x00\x00" +
		"\xcb\x35\xa4\xa2\x28\x01\x98\x02\x07\x00\x00\x00" +
		"\xb3\x9e\xc1\xa2\xc0\x01\x00\x00\x00\x00\x97\x01" +
		"\x47\xe3\xc1\xa2\xc2\x01\x93\x03\x04\x00\x00\x00" +
		"\xa1\xfb\xc3\xa2\xce\x01\x00\x00\x00\x00\x9e\x00" +
		"\x87\x75\xc5\xa2\xd2\x01\xb1\x03\x06\x00\x00\x00" +
		"\xd7\x8f\xc5\xa2\xd3\x01\xb7\x03\x02\x00\x00\x00" +
		"\xfc\x89\x21\xa4\x88\x00\x2f\x01\x03\x00\x00\x00" +
		"\xef\x7e\x6b\xa4\x40\x00\x59\x00\x09\x00\x00\x00" +
		"\xfa\x41\x1f\xa6\xec\x00\x02\x02\x04\x00\x00\x00" +
		"\xd1\xeb\x3c\xa6\xee\x00\x08\x02\x06\x00\x00\x00" +
		"\x4e\xcc\x7d\xa6\xf0\x00\x15\x02\x07\x00\x00\x00" +
		"\x68\xe0\xb6\xa6\xf3\x00\x1f\x02\x07\x00\x00\x00" +
		"\x2e\x3e\xf1\xa6\x8c\x00\x38\x01\x06\x00\x00\x00" +
		"\x7d\xb6\x48\xa7\x65\x01\x34\x03\x03\x00\x00\x00" +
		"\x68\x7f\xba\xa7\x31\x02\x31\x04\x01\x00\x00\x00" +
		"\xc7\x12\xec\xa7\x19\x01\x00\x00\x00\x00\x6d\x01" +
		"\xfd\xd5\xf3\xa7\xf9\x00\x35\x02\x01\x00\x00\x00" +
		"\x8d\x8c\x59\xa8\x40\x02\x00\x00\x00\x00\xee\x01" +
		"\x7e\xb5\x7a\xa8\x2b\x00\x1d\x00\x01\x00\x00\x00" +
		"\xf7\x95\xe5\xa8\xfb\x00\x00\x00\x00\x00\x26\x01" +
		"\xab\xf6\x50\xa9\x4d\x02\x00\x00\x00\x00\xc2\x01" +
		"\x52\xf7\x52\xa9\x1b\x02\x10\x04\x02\x00\x00\x00" +
		"\x5c\x2d\xf7\xa9\x41\x00\x62\x00\x0d\x00\x00\x00" +
		"\x5d\x12\x29\xaa\x91\x00\x43\x01\x01\x00\x00\x00" +
		"\xfb\x16\x51\xab\xac\x00\x85\x01\x06\x00\x00\x00" +
		"\x69\x75\xcb\xab\x60\x00\x00\x00\x00\x00\xbd\x01" +
		"\xd4\xc6\x80\xac\x96\x00\x46\x01\x08\x00\x00\x00" +
		"\x8b\xbf\x86\xac\x98\x00\x55\x01\x04\x00\x00\x00" +
		"\xd8\x72\xd1\xac\x9f\x00\x65\x01\x03\x00\x00\x00" +
		"\x18\xaa\x58\xad\xb6\x01\x75\x03\x04\x00\x00\x00" +
		"\x11\xf0\xde\xae\xa8\x00\x00\x00\x00\x00\x14\x01" +
		"\x33\x13\xe3\xaf\x38\x00\x00\x00\x00\x00\xde\x00" +
		"\xd6\xf5\x66\xb0\xe0\x01\x00\x00\x00\x00\x97\x01" +
		"\xa3\xad\x6e\xb0\x5d\x01\x2b\x03\x03\x00\x00\x00" +
		"\x53\x85\x72\xb0\xfa\x00\x36\x02\x04\x00\x00\x00" +
		"\xf8\x2a\x98\xb0\x80\x01\x00\x00\x00\x00\xed\x01" +
		"\x36\x7b\x5d\xb1\x20\x02\x00\x00\x00\x00\x28\x02" +
		"\x6f\xd5\x64\xb1\x32\x00\x00\x00\x00\x00\xd4\x01" +
		"\x78\xd7\xb3\xb1\xcf\x00\x00\x00\x00\x00\xde\x00" +
		"\xc4\xe4\x19\xb3\xb4\x00\x9c\x01\x02\x00\x00\x00" +
		"\x54\x38\x86\xb3\xb2\x00\x9b\x01\x01\x00\x00\x00" +
		"\x91\xe9\x96\xb3\x7b\x01\x00\x00\x00\x00\x07\x00" +
		"\x86\x11\xaf\xb5\x0a\x01\x00\x00\x00\x00\x3c\x02" +
		"\x50\xf2\xee\xb5\x0b\x01\x60\x02\x03\x00\x00\x00" +
		"\x1e\x54\x49\xb6\x75\x00\xfb\x00\x02\x00\x00\x00" +
		"\xf2\xe8\xf7\xb7\x93\x00\x45\x01\x01\x00\x00\x00" +
		"\x94\x04\x73\xb8\xbe\x00\xa8\x01\x06\x00\x00\x00" +
		"\x1c\x37\x75\xb8\xbf\x00\xae\x01\x01\x00\x00\x00" +
		"\x8d\x59\x87\xba\xf1\x00\x1c\x02\x03\x00\x00\x00" +
		"\xe5\xc9\x88\xba\x00\x02\x00\x00\x00\x00\xc1\x00" +
		"\x8d\x13\x40\xbb\x3b\x02\x00\x00\x00\x00\xa3\x01" +
		"\x83\x71\x5b\xbc\x7c\x00\x06\x01\x02\x00\x00\x00" +
		"\xb7\xcd\x98\xbd\xc4\x01\x00\x00\x00\x00\x17\x02" +
		"\x27\x2f\x1b\xbe\x1f\x01\x00\x00\x00\x00\xe9\x01" +
		"\x72\x44\x70\xbe\xde\x01\x00\x00\x00\x00\xb2\x01" +
		"\x7d\x33\x8e\xbf\x39\x01\xd5\x02\x05\x00\x00\x00" +
		"\x08\xc3\xcc\xbf\x3e\x00\x49\x00\x06\x00\x00\x00" +
		"\x4c\xfd\xd6\xbf\xd2\x00\xcc\x01\x01\x00\x00\x00" +
		"\x2f\xa3\x67\xc0\x39\x00\x00\x00\x00\x00\xde\x00" +
		"\xdf\x2c\x6d\xc0\xb5\x01\x00\x00\x00\x00\x23\x01" +
		"\x44\xc9\x56\xc1\x24\x01\x90\x02\x01\x00\x00\x00" +
		"\x31\x0e\xd3\xc1\x1b\x00\x00\x00\x00\x00\xa7\x01" +
		"\x1c\x9a\xdb\xc1\x8f\x00\x00\x00\x00\x00\xd7\x00" +
		"\xa3\x05\x13\xc2\x00\x00\x00\x00\x01\x00\x00\x00" +
		"\xb5\x3a\x18\xc2\xd9\x00\x00\x00\x00\x00\xde\x00" +
		"\x5e\x94\x24\xc2\x2b\x01\x00\x00\x00\x00\x8a\x01" +
		"\x5f\x56\xc3\xc2\x25\x01\x00\x00\x00\x00\x3c\x00" +
		"\xe7\xbc\xc3\xc2\xbb\x00\xa3\x01\x02\x00\x00\x00" +
		"\x93\x45\x27\xc3\x3d\x02\x00\x00\x00\x00\xc3\x00" +
		"\x24\x0e\xef\xc4\x0d\x02\x00\x00\x00\x00\x5e\x01" +
		"\x33\x1b\x9f\xc5\x0e\x00\x08\x00\x04\x00\x00\x00" +
		"\x65\xf5\xb0\xc5\x42\x00\x6f\x00\x07\x00\x00\x00" +
		"\x2f\x3c\x83\xc6\x12\x01\x6e\x02\x01\x00\x00\x00" +
		"\x59\x34\x09\xc7\xc0\x00\xaf\x01\x05\x00\x00\x00" +
		"\xd0\x1d\x12\xc7\x00\x01\x00\x00\x00\x00\x3c\x02" +
		"\x21\x42\x4d\xc8\xf0\x01\x00\x00\x00\x00\x01\x02" +
		"\x84\x49\xde\xc8\xbd\x00\xa5\x01\x03\x00\x00\x00" +
		"\x23\x3b\x91\xca\x3c\x02\x00\x00\x00\x00\x07\x01" +
		"\x25\xcb\xbd\xca\x46\x02\x00\x00\x00\x00\xa7\x00" +
		"\xca\x5d\x48\xcb\xe2\x01\x00\x00\x00\x00\x9a\x01" +
		"\x04\x3b\xbc\xcb\x4e\x00\xac\x00\x01\x00\x00\x00" +
		"\x54\xbf\xe5\xcc\xb8\x00\x00\x00\x00\x00\x89\x01" +
		"\x31\x8a\x1f\xcd\x78\x00\x00\x00\x00\x00\xde\x00" +
		"\x90\xb2\x96\xcd\xe6\x00\x00\x00\x00\x00\x62\x00" +
		"\x93\x3d\x97\xcd\x32\x01\xc4\x02\x01\x00\x00\x00" +
		"\x4a\x4c\xcb\xce\x6b\x00\x00\x00\x00\x00\xde\x00" +
		"\x4b\x72\xf7\xce\xd5\x01\x00\x00\x00\x00\x9a\x01" +
		"\x13\xf7\x52\xcf\x33\x01\x00\x00\x00\x00\x58\x00" +
		"\xe7\xf7\x54\xcf\x15\x02\x0a\x04\x02\x00\x00\x00" +
		"\xfa\x81\x85\xcf\x38\x01\xd3\x02\x02\x00\x00\x00" +
		"\x6a\x6c\x37\xd0\x1c\x01\x82\x02\x05\x00\x00\x00" +
		"\xe5\xd1\x7d\xd0\xae\x01\x00\x00\x00\x00\xc5\x00" +
		"\x43\x3f\xd9\xd0\xa1\x00\x6e\x01\x03\x00\x00\x00" +
		"\x3e\x52\xff\xd0\x50\x02\x00\x00\x00\x00\x03\x02" +
		"\xae\x44\xa8\xd1\x3a\x01\xda\x02\x01\x00\x00\x00" +
		"\x54\x22\xf0\xd1\x3d\x01\xe6\x02\x03\x00\x00\x00" +
		"\x1b\xe3\x26\xd2\x3f\x01\x00\x00\x00\x00\x47\x00" +
		"\x59\x78\x3e\xd2\xea\x00\x00\x00\x00\x00\x9e\x00" +
		"\x0a\xed\x26\xd3\x6a\x01\x00\x00\x00\x00\xad\x00" +
		"\x28\x2f\x3b\xd3\xbf\x01\x89\x03\x04\x00\x00\x00" +
		"\x0d\x4c\x3b\xd4\x3b\x00\x3c\x00\x04\x00\x00\x00" +
		"\x69\xe4\x55\xd4\x41\x02\x00\x00\x00\x00\x8f\x01" +
		"\x5c\x39\x1b\xd5\x1a\x00\x13\x00\x03\x00\x00\x00" +
		"\x07\x3a\x1c\xd5\x21\x00\x00\x00\x00\x00\xd4\x01" +
		"\x35\x97\x37\xd5\x46\x01\xf5\x02\x01\x00\x00\x00" +
		"\xff\x4a\xa4\xd5\x56\x00\xb8\x00\x05\x00\x00\x00" +
		"\xa0\x57\xd1\xd5\x19\x00\x12\x00\x01\x00\x00\x00" +
		"\x59\x8d\xb2\xd8\xb7\x00\x00\x00\x00\x00\xde\x00" +
		"\x58\xde\xe2\xd8\x87\x01\x41\x03\x01\x00\x00\x00" +
		"\x9c\x18\xe3\xd8\xa6\x01\x00\x00\x00\x00\x03\x02" +
		"\xbc\x1a\xe3\xd8\xa7\x01\x5c\x03\x01\x00\x00\x00" +
		"\xe9\x31\xee\xd8\xdc\x00\xdd\x01\x03\x00\x00\x00" +
		"\xc1\x15\x45\xd9\x03\x00\x01\x00\x04\x00\x00\x00" +
		"\xdc\xe2\x9e\xd9\x0f\x02\x03\x04\x01\x00\x00\x00" +
		"\xed\x3b\x49\xda\xb1\x01\x00\x00\x00\x00\xd8\x00" +
		"\x24\xb7\x9e\xda\xd8\x01\xc7\x03\x0a\x00\x00\x00" +
		"\x65\x2d\x40\xdb\x1a\x02\x00\x00\x00\x00\x33\x02" +
		"\x59\x02\x1b\xdd\x9b\x00\x5c\x01\x04\x00\x00\x00" +
		"\x0e\x2f\x51\xdd\x21\x02\x17\x04\x03\x00\x00\x00" +
		"\xbe\xa8\x54\xdd\x4b\x01\x07\x03\x01\x00\x00\x00" +
		"\xce\xb8\x63\xdd\xe4\x01\xe7\x03\x07\x00\x00\x00" +
		"\x5d\x8a\x85\xdd\x6f\x01\x00\x00\x00\x00\x42\x01" +
		"\xa8\x39\x51\xde\x23\x02\x1d\x04\x03\x00\x00\x00" +
		"\x39\xc4\x71\xde\x8e\x00\x41\x01\x02\x00\x00\x00" +
		"\x37\x73\xe0\xde\xb2\x01\x6c\x03\x02\x00\x00\x00" +
		"\x55\x8e\xf9\xde\xd1\x00\xca\x01\x02\x00\x00\x00" +
		"\x65\x86\x44\xdf\xc2\x00\x00\x00\x00\x00\xcd\x01" +
		"\x3f\x21\xbf\xdf\x0f\x01\x68\x02\x05\x00\x00\x00" +
		"\xd0\x83\x37\xe0\xdf\x01\xde\x03\x07\x00\x00\x00" +
		"\xe2\x29\x50\xe0\x64\x01\x31\x03\x03\x00\x00\x00" +
		"\x3a\x2b\x53\xe0\xaf\x01\x68\x03\x02\x00\x00\x00" +
		"\x3b\x81\x8d\xe0\x0a\x00\x00\x00\x00\x00\x47\x01" +
		"\x83\xc5\xe9\xe0\x9e\x00\x62\x01\x03\x00\x00\x00" +
		"\xbc\x30\xed\xe0\xb7\x01\x00\x00\x00\x00\x9e\x00" +
		"\x71\xa9\x10\xe1\x53\x01\x1e\x03\x02\x00\x00\x00" +
		"\x71\x53\xf9\xe1\x25\x02\x00\x00\x00\x00\x33\x02" +
		"\x83\x25\x02\xe2\xde\x00\xe2\x01\x0d\x00\x00\x00" +
		"\x45\x4b\x14\xe2\xdf\x00\xef\x01\x06\x00\x00\x00" +
		"\xe1\x56\x22\xe2\xac\x01\x5f\x03\x07\x00\x00\x00" +
		"\x8e\xd3\x86\xe2\x27\x02\x00\x00\x00\x00\xd0\x00" +
		"\xe1\xc7\x30\xe3\xe7\x00\x00\x00\x00\x00\xe9\x01" +
		"\x5b\x08\x3f\xe3\xe8\x00\x00\x02\x02\x00\x00\x00" +
		"\xa4\x5d\x31\xe4\xd7\x01\xc0\x03\x07\x00\x00\x00" +
		"\x88\xb7\x51\xe5\x2b\x02\x2c\x04\x01\x00\x00\x00" +
		"\xb5\x35\x6a\xe5\xf3\x01\x00\x00\x00\x00\xd4\x01" +
		"\xca\x0b\x88\xe6\xfd\x01\x00\x00\x00\x00\xa7\x01" +
		"\x48\x66\xd7\xe6\x6d\x01\x00\x00\x00\x00\x7c\x00" +
		"\xf9\x0a\xe7\xe6\x1f\x02\x16\x04\x01\x00\x00\x00" +
		"\xc5\x56\xc4\xe7\x90\x00\x00\x00\x00\x00\xde\x00" +
		"\xeb\x99\x63\xe9\x46\x00\x93\x00\x08\x00\x00\x00" +
		"\x86\x00\xf8\xe9\x30\x02\x00\x00\x00\x00\x38\x02" +
		"\x7f\x76\xb9\xea\xe8\x01\x00\x00\x00\x00\x97\x01" +
		"\x80\x45\xf8\xea\xbc\x01\x00\x00\x00\x00\xc5\x00" +
		"\x9b\xc1\xac\xeb\x22\x01\x89\x02\x06\x00\x00\x00" +
		"\x3f\xe8\xfd\xeb\xb1\x00\x94\x01\x07\x00\x00\x00" +
		"\x5f\xa3\xf7\xef\x33\x02\x00\x00\x00\x00\x5d\x00" +
		"\x00\x7d\xe8\xf0\x76\x01\x3d\x03\x01\x00\x00\x00" +
		"\x46\x24\x4c\xf2\x35\x02\x34\x04\x01\x00\x00\x00" +
		"\x63\x7e\x51\xf2\x36\x02\x35\x04\x01\x00\x00\x00" +
		"\x75\x93\xaf\xf2\x07\x02\x02\x04\x01\x00\x00\x00" +
		"\x5e\x2d\x61\xf3\x79\x01\x00\x00\x00\x00\x14\x01" +
		"\x54\x42\x92\xf3\x3b\x01\xdb\x02\x03\x00\x00\x00" +
		"\xbd\xc9\xa1\xf4\x34\x01\xc5\x02\x07\x00\x00\x00" +
		"\xf0\x25\xdd\xf4\x1d\x02\x13\x04\x01\x00\x00\x00" +
		"\x88\xa9\x3e\xf5\xa4\x01\x00\x00\x00\x00\x01\x02" +
		"\x40\xc2\x49\xf5\xa9\x01\x00\x00\x00\x00\x03\x02" +
		"\x04\xd2\x54\xf5\x66\x00\xd7\x00\x03\x00\x00\x00" +
		"\xf5\xa7\x95\xf8\x37\x01\xd2\x02\x01\x00\x00\x00" +
		"\xfc\x25\xe3\xf8\xb9\x01\x7a\x03\x02\x00\x00\x00" +
		"\x90\xfb\x37\xf9\xff\x00\x00\x00\x00\x00\x3c\x02" +
		"\x18\xd9\x3e\xf9\x4e\x01\x12\x03\x04\x00\x00\x00" +
		"\x0f\xdb\x4d\xf9\x3a\x02\x00\x00\x00\x00\x33\x02" +
		"\x1b\xe7\x62\xf9\xd4\x00\x00\x00\x00\x00\x14\x01" +
		"\x83\x96\xb2\xf9\x6d\x00\xe8\x00\x06\x00\x00\x00" +
		"\x80\x47\x21\xfa\xa5\x00\x7b\x01\x05\x00\x00\x00" +
		"\xc9\x0b\x30\xfa\x43\x02\x00\x00\x00\x00\xbf\x00" +
		"\x00\xe3\x70\xfa\xeb\x01\x00\x00\x00\x00\xc5\x00" +
		"\xef\xab\xf5\xfa\xc1\x01\x8d\x03\x06\x00\x00\x00" +
		"\xf3\x60\x15\xfb\xc8\x01\xa1\x03\x03\x00\x00\x00" +
		"\x66\xcc\x19\xfb\xca\x01\xa4\x03\x04\x00\x00\x00" +
		"\xc5\x9e\x34\xfb\xb3\x01\x6e\x03\x05\x00\x00\x00" +
		"\x05\x42\x3d\xfb\x1c\x00\x16\x00\x03\x00\x00\x00" +
		"\xa3\xc2\x4b\xfb\xcf\x01\xad\x03\x02\x00\x00\x00" +
		"\x6e\x4c\x54\xfb\x4c\x01\x08\x03\x05\x00\x00\x00" +
		"\x56\x86\x89\xfb\xda\x01\xd1\x03\x05\x00\x00\x00" +
		"\x17\x82\x8a\xfb\x7a\x01\x00\x00\x00\x00\xee\x00" +
		"\xae\x1b\xb8\xfb\xe1\x01\x00\x00\x00\x00\x23\x01" +
		"\xc2\xf2\x23\xfc\x50\x01\x19\x03\x03\x00\x00\x00" +
		"\x0f\x15\xf7\xfc\x08\x02\x00\x00\x00\x00\xe7\x00" +
		"\x6c\xa5\x18\xfd\xc3\x00\x00\x00\x00\x00\xe7\x00" +
		"\xa6\xe7\xc3\xfe\x57\x00\xbd\x00\x01\x00\x00\x00"

// ---------------------------------------------------------------------------
// Unsupported zones: 0
// ---------------------------------------------------------------------------



// ---------------------------------------------------------------------------
// Notable zones: 9
// ---------------------------------------------------------------------------

// Africa/Casablanca {
//   Morocco {SAVE (delta_offset) '-1:00' different from 1:00}
// }
// Africa/Ceuta {
//   EU {Added anchor rule at year 0}
// }
// Africa/El_Aaiun {
//   Morocco {SAVE (delta_offset) '-1:00' different from 1:00}
// }
// Africa/Johannesburg {RULES not fixed but FORMAT is missing '%' or '/'}
// Africa/Juba {
//   Sudan {Added anchor rule at year 0}
// }
// Africa/Khartoum {
//   Sudan {Added anchor rule at year 0}
// }
// Africa/Monrovia {STDOFF (-0:44:30) not at :00 or :30 mark}
// Africa/Windhoek {
//   Namibia {
//     Added anchor rule at year 0,
//     LETTER 'CAT' not single character,
//     LETTER 'WAT' not single character,
//     SAVE (delta_offset) '-1:00' different from 1:00,
//   }
// }
// America/Argentina/San_Luis {
//   SanLuis {Added anchor rule at year 0}
// }
// America/Asuncion {
//   Para {Added anchor rule at year 0}
// }
// America/Belize {
//   Belize {
//     LETTER 'CDT' not single character,
//     LETTER 'CST' not single character,
//   }
// }
// America/Bogota {
//   CO {Added anchor rule at year 0}
// }
// America/Costa_Rica {
//   CR {Added anchor rule at year 0}
// }
// America/Danmarkshavn {
//   EU {Added anchor rule at year 0}
// }
// America/El_Salvador {
//   Salv {Added anchor rule at year 0}
// }
// America/Goose_Bay {
//   StJohns {
//     AT '0:01' not on 15-minute boundary,
//     LETTER 'DD' not single character,
//     SAVE (delta_offset) '2:00' different from 1:00,
//   }
// }
// America/Guatemala {
//   Guat {Added anchor rule at year 0}
// }
// America/Guayaquil {
//   Ecuador {Added anchor rule at year 0}
// }
// America/Guyana {STDOFF (-3:45) not at :00 or :30 mark}
// America/Managua {
//   Nic {Added anchor rule at year 0}
// }
// America/Moncton {
//   Moncton {AT '0:01' not on 15-minute boundary}
// }
// America/Montevideo {
//   Uruguay {
//     SAVE (delta_offset) '0:30' different from 1:00,
//     SAVE (delta_offset) '1:30' different from 1:00,
//   }
// }
// America/Nuuk {
//   EU {Added anchor rule at year 0}
// }
// America/Port-au-Prince {
//   Haiti {Added anchor rule at year 0}
// }
// America/Santo_Domingo {
//   DR {
//     LETTER '-0430' not single character,
//     LETTER 'EST' not single character,
//     SAVE (delta_offset) '0:30' different from 1:00,
//   }
// }
// America/Scoresbysund {
//   EU {Added anchor rule at year 0}
// }
// America/St_Johns {
//   StJohns {
//     AT '0:01' not on 15-minute boundary,
//     LETTER 'DD' not single character,
//     SAVE (delta_offset) '2:00' different from 1:00,
//   }
// }
// America/Tegucigalpa {
//   Hond {Added anchor rule at year 0}
// }
// America/Thule {
//   Thule {Added anchor rule at year 0}
// }
// Antarctica/Casey {UNTIL '0:01' not on 15-minute boundary}
// Antarctica/Troll {
//   Troll {
//     Added anchor rule at year 0,
//     LETTER '+00' not single character,
//     LETTER '+02' not single character,
//     SAVE (delta_offset) '2:00' different from 1:00,
//   }
// }
// Asia/Almaty {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Amman {
//   Jordan {Added anchor rule at year 0}
// }
// Asia/Aqtau {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Aqtobe {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Ashgabat {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Atyrau {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Baghdad {
//   Iraq {Added anchor rule at year 0}
// }
// Asia/Baku {
//   Azer {Added anchor rule at year 0}
//   EUAsia {Added anchor rule at year 0}
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Bishkek {
//   Kyrgyz {Added anchor rule at year 0}
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Choibalsan {
//   Mongol {Added anchor rule at year 0}
// }
// Asia/Dhaka {
//   Dhaka {Added anchor rule at year 0}
// }
// Asia/Dushanbe {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Famagusta {
//   Cyprus {Added anchor rule at year 0}
//   EUAsia {Added anchor rule at year 0}
// }
// Asia/Gaza {
//   UNTIL '0:01' not on 15-minute boundary,
//   Jordan {Added anchor rule at year 0}
//   Palestine {
//     AT '0:01' not on 15-minute boundary,
//     Added anchor rule at year 0,
//   }
// }
// Asia/Hebron {
//   Jordan {Added anchor rule at year 0}
//   Palestine {
//     AT '0:01' not on 15-minute boundary,
//     Added anchor rule at year 0,
//   }
// }
// Asia/Hovd {
//   Mongol {Added anchor rule at year 0}
// }
// Asia/Karachi {
//   Pakistan {Added anchor rule at year 0}
// }
// Asia/Kathmandu {STDOFF (5:45) not at :00 or :30 mark}
// Asia/Nicosia {
//   Cyprus {Added anchor rule at year 0}
//   EUAsia {Added anchor rule at year 0}
// }
// Asia/Oral {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Qostanay {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Qyzylorda {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Samarkand {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Shanghai {
//   PRC {Added anchor rule at year 0}
// }
// Asia/Tashkent {
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Tbilisi {
//   E-EurAsia {Added anchor rule at year 0}
//   RussiaAsia {Added anchor rule at year 0}
// }
// Asia/Ulaanbaatar {
//   Mongol {Added anchor rule at year 0}
// }
// Asia/Yerevan {
//   Armenia {Added anchor rule at year 0}
//   RussiaAsia {Added anchor rule at year 0}
// }
// Atlantic/Azores {
//   EU {Added anchor rule at year 0}
//   W-Eur {Added anchor rule at year 0}
// }
// Atlantic/Canary {
//   EU {Added anchor rule at year 0}
// }
// Atlantic/Faroe {
//   EU {Added anchor rule at year 0}
// }
// Atlantic/Madeira {
//   EU {Added anchor rule at year 0}
// }
// Australia/Adelaide {
//   AS {Added anchor rule at year 0}
// }
// Australia/Brisbane {
//   AQ {Added anchor rule at year 0}
// }
// Australia/Broken_Hill {
//   AN {Added anchor rule at year 0}
//   AS {Added anchor rule at year 0}
// }
// Australia/Eucla {
//   STDOFF (8:45) not at :00 or :30 mark,
//   AW {Added anchor rule at year 0}
// }
// Australia/Lindeman {
//   AQ {Added anchor rule at year 0}
//   Holiday {Added anchor rule at year 0}
// }
// Australia/Lord_Howe {
//   LH {
//     Added anchor rule at year 0,
//     SAVE (delta_offset) '0:30' different from 1:00,
//   }
// }
// Australia/Melbourne {
//   AV {Added anchor rule at year 0}
// }
// Australia/Perth {
//   AW {Added anchor rule at year 0}
// }
// Australia/Sydney {
//   AN {Added anchor rule at year 0}
// }
// EET {
//   EU {Added anchor rule at year 0}
// }
// Europe/Andorra {
//   EU {Added anchor rule at year 0}
// }
// Europe/Athens {
//   EU {Added anchor rule at year 0}
// }
// Europe/Belgrade {
//   EU {Added anchor rule at year 0}
// }
// Europe/Berlin {
//   EU {Added anchor rule at year 0}
// }
// Europe/Brussels {
//   EU {Added anchor rule at year 0}
// }
// Europe/Bucharest {
//   E-Eur {Added anchor rule at year 0}
//   EU {Added anchor rule at year 0}
// }
// Europe/Budapest {
//   EU {Added anchor rule at year 0}
// }
// Europe/Chisinau {
//   E-Eur {Added anchor rule at year 0}
//   Moldova {Added anchor rule at year 0}
// }
// Europe/Dublin {
//   Eire {
//     Added anchor rule at year 0,
//     SAVE (delta_offset) '-1:00' different from 1:00,
//   }
// }
// Europe/Gibraltar {
//   EU {Added anchor rule at year 0}
// }
// Europe/Helsinki {
//   EU {Added anchor rule at year 0}
// }
// Europe/Istanbul {
//   EU {Added anchor rule at year 0}
// }
// Europe/Kyiv {
//   EU {Added anchor rule at year 0}
// }
// Europe/Lisbon {
//   EU {Added anchor rule at year 0}
//   W-Eur {Added anchor rule at year 0}
// }
// Europe/London {
//   EU {Added anchor rule at year 0}
//   GB-Eire {
//     LETTER 'BST' not single character,
//     LETTER 'GMT' not single character,
//   }
// }
// Europe/Madrid {
//   EU {Added anchor rule at year 0}
// }
// Europe/Malta {
//   EU {Added anchor rule at year 0}
//   Malta {Added anchor rule at year 0}
// }
// Europe/Paris {
//   EU {Added anchor rule at year 0}
// }
// Europe/Prague {
//   EU {Added anchor rule at year 0}
// }
// Europe/Riga {
//   EU {Added anchor rule at year 0}
//   Latvia {Added anchor rule at year 0}
// }
// Europe/Rome {
//   EU {Added anchor rule at year 0}
// }
// Europe/Simferopol {
//   EU {Added anchor rule at year 0}
// }
// Europe/Sofia {
//   Bulg {Added anchor rule at year 0}
//   E-Eur {Added anchor rule at year 0}
//   EU {Added anchor rule at year 0}
// }
// Europe/Tallinn {
//   EU {Added anchor rule at year 0}
// }
// Europe/Tirane {
//   EU {Added anchor rule at year 0}
// }
// Europe/Vienna {
//   EU {Added anchor rule at year 0}
// }
// Europe/Vilnius {
//   EU {Added anchor rule at year 0}
// }
// Europe/Warsaw {
//   EU {Added anchor rule at year 0}
//   W-Eur {Added anchor rule at year 0}
// }
// Europe/Zurich {
//   EU {Added anchor rule at year 0}
// }
// Indian/Mauritius {
//   Mauritius {Added anchor rule at year 0}
// }
// Pacific/Apia {
//   WS {Added anchor rule at year 0}
// }
// Pacific/Chatham {
//   STDOFF (12:45) not at :00 or :30 mark,
//   Chatham {Added anchor rule at year 0}
// }
// Pacific/Efate {
//   Vanuatu {Added anchor rule at year 0}
// }
// Pacific/Fiji {
//   Fiji {Added anchor rule at year 0}
// }
// Pacific/Galapagos {
//   Ecuador {Added anchor rule at year 0}
// }
// Pacific/Guam {
//   Guam {
//     AT '0:01' not on 15-minute boundary,
//     AT '2:01' not on 15-minute boundary,
//   }
// }
// Pacific/Kiritimati {
//   STDOFF '-10:40' not on 15-minute boundary,
//   STDOFF (-10:40) not at :00 or :30 mark,
// }
// Pacific/Norfolk {
//   AN {Added anchor rule at year 0}
// }
// Pacific/Noumea {
//   NC {Added anchor rule at year 0}
// }
// Pacific/Rarotonga {
//   Cook {
//     Added anchor rule at year 0,
//     SAVE (delta_offset) '0:30' different from 1:00,
//   }
// }
// Pacific/Tongatapu {
//   Tonga {Added anchor rule at year 0}
// }
// WET {
//   EU {Added anchor rule at year 0}
// }


// ---------------------------------------------------------------------------
// Unsuported links: 0
// ---------------------------------------------------------------------------



// ---------------------------------------------------------------------------
// Notable links: 0
// ---------------------------------------------------------------------------


