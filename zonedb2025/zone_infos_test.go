// This file was generated by the following script:
//
//   $ /home/brian/src/AceTimeSuite/compiler/src/acetimecompiler/tzcompiler.py
//     --input_dir /home/brian/src/AceTimeSuite/libraries/acetimego/zonedb2025/tzfiles
//     --output_dir /home/brian/src/AceTimeSuite/libraries/acetimego/zonedb2025
//     --tz_version 2025b
//     --actions zonedb
//     --language go
//     --scope complete
//     --db_namespace zonedb2025
//     --start_year 2025
//     --until_year 2200
//
// using the TZ Database files
//
//   africa
//   antarctica
//   asia
//   australasia
//   backward
//   etcetera
//   europe
//   northamerica
//   southamerica
//
// from https://github.com/eggert/tz/releases/tag/2025b
//
// Supported Zones: 597 (340 zones, 257 links)
// Unsupported Zones: 0 (0 zones, 0 links)
//
// Requested Years: [2025,2200]
// Accurate Years: [2025,32767]
//
// Original Years:  [1844,2087]
// Generated Years: [1967,2087]
// Lower/Upper Truncated: [True,False]
//
// Estimator Years: [1967,2090]
// Max Buffer Size: 7
//
// Records:
//   Infos: 597
//   Eras: 345
//   Policies: 66
//   Rules: 343
//
// Memory:
//   Rules: 4116
//   Policies: 264
//   Eras: 4830
//   Zones: 4080
//   Links: 3084
//   Registry: 0
//   Formats: 248
//   Letters: 23
//   Fragments: 0
//   Names: 9694
//   TOTAL: 26339
//
// DO NOT EDIT

package zonedb2025

import (
	"github.com/bxparks/acetimego/zoneinfo"
)

// ---------------------------------------------------------------------------
// ZoneEraRecords is an array of ZoneEraRecord items concatenated together
// across all zones.
//
// Supported zones: 340
// numEras: 345
// ---------------------------------------------------------------------------

var ZoneEraRecords = []zoneinfo.ZoneEraRecord{
	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Abidjan
	// EraIndex: 0
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              0:00    -    GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "GMT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Algiers
	// EraIndex: 1
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    -    CET
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 12, // "CET"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Bissau
	// EraIndex: 2
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              0:00    -    GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "GMT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Cairo
	// EraIndex: 3
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    Egypt    EE%sT
	{
		PolicyIndex: 25, // PolicyName: Egypt
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Casablanca
	// EraIndex: 4
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              1:00    Morocco    %z
	{
		PolicyIndex: 43, // PolicyName: Morocco
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Ceuta
	// EraIndex: 5
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/El_Aaiun
	// EraIndex: 6
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              1:00    Morocco    %z
	{
		PolicyIndex: 43, // PolicyName: Morocco
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Johannesburg
	// EraIndex: 7
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    SA    SAST
	{
		PolicyIndex: 55, // PolicyName: SA
		FormatIndex: 38, // "SAST"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Juba
	// EraIndex: 8
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    -    CAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "CAT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Khartoum
	// EraIndex: 9
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    -    CAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "CAT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Lagos
	// EraIndex: 10
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    -    WAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 41, // "WAT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Maputo
	// EraIndex: 11
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    -    CAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 10, // "CAT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Monrovia
	// EraIndex: 12
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              0:00    -    GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "GMT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Nairobi
	// EraIndex: 13
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:00    -    EAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 16, // "EAT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Ndjamena
	// EraIndex: 14
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    -    WAT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 41, // "WAT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Sao_Tome
	// EraIndex: 15
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              0:00    -    GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "GMT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Tripoli
	// EraIndex: 16
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    -    EET
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 18, // "EET"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Tunis
	// EraIndex: 17
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    Tunisia    CE%sT
	{
		PolicyIndex: 61, // PolicyName: Tunisia
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Africa/Windhoek
	// EraIndex: 18
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    Namibia    %s
	{
		PolicyIndex: 46, // PolicyName: Namibia
		FormatIndex: 1, // "%"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Adak
	// EraIndex: 19
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -10:00    US    H%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 22, // "H%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -2400, // -36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Anchorage
	// EraIndex: 20
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 6, // "AK%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -2160, // -32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Araguaina
	// EraIndex: 21
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Buenos_Aires
	// EraIndex: 22
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    Arg    %z
	{
		PolicyIndex: 7, // PolicyName: Arg
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Catamarca
	// EraIndex: 23
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Cordoba
	// EraIndex: 24
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    Arg    %z
	{
		PolicyIndex: 7, // PolicyName: Arg
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Jujuy
	// EraIndex: 25
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/La_Rioja
	// EraIndex: 26
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Mendoza
	// EraIndex: 27
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Rio_Gallegos
	// EraIndex: 28
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Salta
	// EraIndex: 29
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/San_Juan
	// EraIndex: 30
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/San_Luis
	// EraIndex: 31
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Tucuman
	// EraIndex: 32
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    Arg    %z
	{
		PolicyIndex: 7, // PolicyName: Arg
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Argentina/Ushuaia
	// EraIndex: 33
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Asuncion
	// EraIndex: 34
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -4:00    Para    %z    2024 Oct 15
	{
		PolicyIndex: 51, // PolicyName: Para
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 2024,
		UntilMonth: 10,
		UntilDay: 15,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Bahia
	// EraIndex: 36
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Bahia_Banderas
	// EraIndex: 37
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Mexico    C%sT
	{
		PolicyIndex: 40, // PolicyName: Mexico
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Barbados
	// EraIndex: 38
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Barb    A%sT
	{
		PolicyIndex: 11, // PolicyName: Barb
		FormatIndex: 3, // "A%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Belem
	// EraIndex: 39
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Belize
	// EraIndex: 40
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Belize    %s
	{
		PolicyIndex: 12, // PolicyName: Belize
		FormatIndex: 1, // "%"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Boa_Vista
	// EraIndex: 41
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Bogota
	// EraIndex: 42
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    CO    %z
	{
		PolicyIndex: 14, // PolicyName: CO
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Boise
	// EraIndex: 43
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    US    M%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 31, // "M%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Cambridge_Bay
	// EraIndex: 44
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    Canada    M%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 31, // "M%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Campo_Grande
	// EraIndex: 45
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Brazil    %z
	{
		PolicyIndex: 13, // PolicyName: Brazil
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Cancun
	// EraIndex: 46
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 19, // "EST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Caracas
	// EraIndex: 47
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Cayenne
	// EraIndex: 48
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Chicago
	// EraIndex: 49
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Chihuahua
	// EraIndex: 50
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "CST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Ciudad_Juarez
	// EraIndex: 51
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    US    M%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 31, // "M%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Costa_Rica
	// EraIndex: 52
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    CR    C%sT
	{
		PolicyIndex: 15, // PolicyName: CR
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Coyhaique
	// EraIndex: 53
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -4:00    Chile    %z    2025 Mar 20
	{
		PolicyIndex: 18, // PolicyName: Chile
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 2025,
		UntilMonth: 3,
		UntilDay: 20,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Cuiaba
	// EraIndex: 55
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Brazil    %z
	{
		PolicyIndex: 13, // PolicyName: Brazil
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Danmarkshavn
	// EraIndex: 56
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             0:00    -    GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "GMT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Dawson
	// EraIndex: 57
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 33, // "MST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Dawson_Creek
	// EraIndex: 58
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 33, // "MST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Denver
	// EraIndex: 59
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    US    M%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 31, // "M%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Detroit
	// EraIndex: 60
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Edmonton
	// EraIndex: 61
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    Canada    M%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 31, // "M%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Eirunepe
	// EraIndex: 62
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/El_Salvador
	// EraIndex: 63
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Salv    C%sT
	{
		PolicyIndex: 56, // PolicyName: Salv
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Fort_Nelson
	// EraIndex: 64
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 33, // "MST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Fortaleza
	// EraIndex: 65
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Glace_Bay
	// EraIndex: 66
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Canada    A%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 3, // "A%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Goose_Bay
	// EraIndex: 67
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Canada    A%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 3, // "A%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Grand_Turk
	// EraIndex: 68
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Guatemala
	// EraIndex: 69
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Guat    C%sT
	{
		PolicyIndex: 28, // PolicyName: Guat
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Guayaquil
	// EraIndex: 70
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    Ecuador    %z
	{
		PolicyIndex: 24, // PolicyName: Ecuador
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Guyana
	// EraIndex: 71
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Halifax
	// EraIndex: 72
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Canada    A%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 3, // "A%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Havana
	// EraIndex: 73
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    Cuba    C%sT
	{
		PolicyIndex: 20, // PolicyName: Cuba
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Hermosillo
	// EraIndex: 74
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 33, // "MST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Indianapolis
	// EraIndex: 75
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Knox
	// EraIndex: 76
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Marengo
	// EraIndex: 77
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Petersburg
	// EraIndex: 78
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Tell_City
	// EraIndex: 79
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Vevay
	// EraIndex: 80
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Vincennes
	// EraIndex: 81
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Indiana/Winamac
	// EraIndex: 82
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Inuvik
	// EraIndex: 83
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    Canada    M%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 31, // "M%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Iqaluit
	// EraIndex: 84
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    Canada    E%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Jamaica
	// EraIndex: 85
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 19, // "EST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Juneau
	// EraIndex: 86
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 6, // "AK%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -2160, // -32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Kentucky/Louisville
	// EraIndex: 87
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Kentucky/Monticello
	// EraIndex: 88
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/La_Paz
	// EraIndex: 89
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Lima
	// EraIndex: 90
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    Peru    %z
	{
		PolicyIndex: 52, // PolicyName: Peru
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Los_Angeles
	// EraIndex: 91
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -8:00    US    P%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 36, // "P%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1920, // -28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Maceio
	// EraIndex: 92
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Managua
	// EraIndex: 93
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Nic    C%sT
	{
		PolicyIndex: 47, // PolicyName: Nic
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Manaus
	// EraIndex: 94
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Martinique
	// EraIndex: 95
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    AST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "AST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Matamoros
	// EraIndex: 96
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Mazatlan
	// EraIndex: 97
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    Mexico    M%sT
	{
		PolicyIndex: 40, // PolicyName: Mexico
		FormatIndex: 31, // "M%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Menominee
	// EraIndex: 98
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Merida
	// EraIndex: 99
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Mexico    C%sT
	{
		PolicyIndex: 40, // PolicyName: Mexico
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Metlakatla
	// EraIndex: 100
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 6, // "AK%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -2160, // -32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Mexico_City
	// EraIndex: 101
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Mexico    C%sT
	{
		PolicyIndex: 40, // PolicyName: Mexico
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Miquelon
	// EraIndex: 102
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    Canada    %z
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Moncton
	// EraIndex: 103
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Canada    A%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 3, // "A%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Monterrey
	// EraIndex: 104
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Mexico    C%sT
	{
		PolicyIndex: 40, // PolicyName: Mexico
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Montevideo
	// EraIndex: 105
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    Uruguay    %z
	{
		PolicyIndex: 63, // PolicyName: Uruguay
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/New_York
	// EraIndex: 106
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    US    E%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Nome
	// EraIndex: 107
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 6, // "AK%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -2160, // -32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Noronha
	// EraIndex: 108
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -2:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -480, // -7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/North_Dakota/Beulah
	// EraIndex: 109
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/North_Dakota/Center
	// EraIndex: 110
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/North_Dakota/New_Salem
	// EraIndex: 111
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Nuuk
	// EraIndex: 112
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -2:00    EU    %z
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -480, // -7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Ojinaga
	// EraIndex: 113
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    US    C%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Panama
	// EraIndex: 114
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    -    EST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 19, // "EST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Paramaribo
	// EraIndex: 115
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Phoenix
	// EraIndex: 116
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 33, // "MST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Port-au-Prince
	// EraIndex: 117
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    Haiti    E%sT
	{
		PolicyIndex: 30, // PolicyName: Haiti
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Porto_Velho
	// EraIndex: 118
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Puerto_Rico
	// EraIndex: 119
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    AST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "AST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Punta_Arenas
	// EraIndex: 120
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Rankin_Inlet
	// EraIndex: 121
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Canada    C%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Recife
	// EraIndex: 122
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Regina
	// EraIndex: 123
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "CST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Resolute
	// EraIndex: 124
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Canada    C%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Rio_Branco
	// EraIndex: 125
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Santarem
	// EraIndex: 126
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Santiago
	// EraIndex: 127
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Chile    %z
	{
		PolicyIndex: 18, // PolicyName: Chile
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Santo_Domingo
	// EraIndex: 128
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    -    AST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 7, // "AST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Sao_Paulo
	// EraIndex: 129
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    Brazil    %z
	{
		PolicyIndex: 13, // PolicyName: Brazil
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Scoresbysund
	// EraIndex: 130
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             -1:00    EU    %z    2024 Mar 31
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -240, // -3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 2024,
		UntilMonth: 3,
		UntilDay: 31,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	//             -2:00    EU    %z
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -480, // -7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Sitka
	// EraIndex: 132
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 6, // "AK%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -2160, // -32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/St_Johns
	// EraIndex: 133
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:30    Canada    N%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 34, // "N%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -840, // -12600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Swift_Current
	// EraIndex: 134
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    -    CST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 13, // "CST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Tegucigalpa
	// EraIndex: 135
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Hond    C%sT
	{
		PolicyIndex: 32, // PolicyName: Hond
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Thule
	// EraIndex: 136
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    Thule    A%sT
	{
		PolicyIndex: 58, // PolicyName: Thule
		FormatIndex: 3, // "A%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Tijuana
	// EraIndex: 137
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -8:00    US    P%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 36, // "P%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1920, // -28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Toronto
	// EraIndex: 138
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -5:00    Canada    E%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 15, // "E%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Vancouver
	// EraIndex: 139
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -8:00    Canada    P%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 36, // "P%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1920, // -28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Whitehorse
	// EraIndex: 140
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -7:00    -    MST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 33, // "MST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Winnipeg
	// EraIndex: 141
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Canada    C%sT
	{
		PolicyIndex: 16, // PolicyName: Canada
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: America/Yakutat
	// EraIndex: 142
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              -9:00    US    AK%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 6, // "AK%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -2160, // -32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Casey
	// EraIndex: 143
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              8:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Davis
	// EraIndex: 144
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             7:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Macquarie
	// EraIndex: 145
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    AT    AE%sT
	{
		PolicyIndex: 4, // PolicyName: AT
		FormatIndex: 5, // "AE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Mawson
	// EraIndex: 146
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Palmer
	// EraIndex: 147
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Rothera
	// EraIndex: 148
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Troll
	// EraIndex: 149
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             0:00    Troll    %s
	{
		PolicyIndex: 60, // PolicyName: Troll
		FormatIndex: 1, // "%"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Antarctica/Vostok
	// EraIndex: 150
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Almaty
	// EraIndex: 151
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             6:00    -    %z    2024 Mar  1  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1440, // 21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 2024,
		UntilMonth: 3,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Amman
	// EraIndex: 153
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Anadyr
	// EraIndex: 154
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             12:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2880, // 43200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Aqtau
	// EraIndex: 155
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Aqtobe
	// EraIndex: 156
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Ashgabat
	// EraIndex: 157
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Atyrau
	// EraIndex: 158
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Baghdad
	// EraIndex: 159
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:00    Iraq    %z
	{
		PolicyIndex: 34, // PolicyName: Iraq
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Baku
	// EraIndex: 160
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             4:00    Azer    %z
	{
		PolicyIndex: 10, // PolicyName: Azer
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 960, // 14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Bangkok
	// EraIndex: 161
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             7:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Barnaul
	// EraIndex: 162
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              7:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Beirut
	// EraIndex: 163
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    Lebanon    EE%sT
	{
		PolicyIndex: 37, // PolicyName: Lebanon
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Bishkek
	// EraIndex: 164
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             6:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1440, // 21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Chita
	// EraIndex: 165
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              9:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2160, // 32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Colombo
	// EraIndex: 166
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:30    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1320, // 19800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Damascus
	// EraIndex: 167
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Dhaka
	// EraIndex: 168
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             6:00    Dhaka    %z
	{
		PolicyIndex: 21, // PolicyName: Dhaka
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1440, // 21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Dili
	// EraIndex: 169
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             9:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2160, // 32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Dubai
	// EraIndex: 170
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 960, // 14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Dushanbe
	// EraIndex: 171
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Famagusta
	// EraIndex: 172
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    EUAsia    EE%sT
	{
		PolicyIndex: 23, // PolicyName: EUAsia
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Gaza
	// EraIndex: 173
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00 Palestine    EE%sT
	{
		PolicyIndex: 50, // PolicyName: Palestine
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Hebron
	// EraIndex: 174
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00 Palestine    EE%sT
	{
		PolicyIndex: 50, // PolicyName: Palestine
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Ho_Chi_Minh
	// EraIndex: 175
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             7:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Hong_Kong
	// EraIndex: 176
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    HK    HK%sT
	{
		PolicyIndex: 29, // PolicyName: HK
		FormatIndex: 23, // "HK%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Hovd
	// EraIndex: 177
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             7:00    Mongol    %z
	{
		PolicyIndex: 42, // PolicyName: Mongol
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Irkutsk
	// EraIndex: 178
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              8:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Jakarta
	// EraIndex: 179
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             7:00    -    WIB
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 43, // "WIB"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Jayapura
	// EraIndex: 180
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             9:00    -    WIT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 44, // "WIT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2160, // 32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Jerusalem
	// EraIndex: 181
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    Zion    I%sT
	{
		PolicyIndex: 66, // PolicyName: Zion
		FormatIndex: 25, // "I%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Kabul
	// EraIndex: 182
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             4:30    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1080, // 16200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Kamchatka
	// EraIndex: 183
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             12:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2880, // 43200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Karachi
	// EraIndex: 184
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00 Pakistan    PK%sT
	{
		PolicyIndex: 49, // PolicyName: Pakistan
		FormatIndex: 37, // "PK%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Kathmandu
	// EraIndex: 185
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:45    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1380, // 20700 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Khandyga
	// EraIndex: 186
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              9:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2160, // 32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Kolkata
	// EraIndex: 187
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:30    -    IST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 26, // "IST"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1320, // 19800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Krasnoyarsk
	// EraIndex: 188
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              7:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Kuching
	// EraIndex: 189
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Macau
	// EraIndex: 190
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    Macau    C%sT
	{
		PolicyIndex: 38, // PolicyName: Macau
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Magadan
	// EraIndex: 191
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2640, // 39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Makassar
	// EraIndex: 192
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    -    WITA
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 45, // "WITA"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Manila
	// EraIndex: 193
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    Phil    P%sT
	{
		PolicyIndex: 53, // PolicyName: Phil
		FormatIndex: 36, // "P%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Nicosia
	// EraIndex: 194
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    EUAsia    EE%sT
	{
		PolicyIndex: 23, // PolicyName: EUAsia
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Novokuznetsk
	// EraIndex: 195
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              7:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Novosibirsk
	// EraIndex: 196
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              7:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Omsk
	// EraIndex: 197
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              6:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1440, // 21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Oral
	// EraIndex: 198
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Pontianak
	// EraIndex: 199
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             7:00    -    WIB
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 43, // "WIB"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Pyongyang
	// EraIndex: 200
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             9:00    -    KST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 30, // "KST"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2160, // 32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Qatar
	// EraIndex: 201
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Qostanay
	// EraIndex: 202
	// EraCount: 2
	// ---------------------------------------------------------------------------

	//             6:00    -    %z    2024 Mar  1  0:00
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1440, // 21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 2024,
		UntilMonth: 3,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Qyzylorda
	// EraIndex: 204
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Riyadh
	// EraIndex: 205
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Sakhalin
	// EraIndex: 206
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2640, // 39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Samarkand
	// EraIndex: 207
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Seoul
	// EraIndex: 208
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             9:00    ROK    K%sT
	{
		PolicyIndex: 54, // PolicyName: ROK
		FormatIndex: 29, // "K%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2160, // 32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Shanghai
	// EraIndex: 209
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    PRC    C%sT
	{
		PolicyIndex: 48, // PolicyName: PRC
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Singapore
	// EraIndex: 210
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Srednekolymsk
	// EraIndex: 211
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2640, // 39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Taipei
	// EraIndex: 212
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    Taiwan    C%sT
	{
		PolicyIndex: 57, // PolicyName: Taiwan
		FormatIndex: 9, // "C%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Tashkent
	// EraIndex: 213
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Tbilisi
	// EraIndex: 214
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 960, // 14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Tehran
	// EraIndex: 215
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:30    Iran    %z
	{
		PolicyIndex: 33, // PolicyName: Iran
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 840, // 12600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Thimphu
	// EraIndex: 216
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             6:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1440, // 21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Tokyo
	// EraIndex: 217
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             9:00    Japan    J%sT
	{
		PolicyIndex: 35, // PolicyName: Japan
		FormatIndex: 28, // "J%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2160, // 32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Tomsk
	// EraIndex: 218
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              7:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Ulaanbaatar
	// EraIndex: 219
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             8:00    Mongol    %z
	{
		PolicyIndex: 42, // PolicyName: Mongol
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Urumqi
	// EraIndex: 220
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             6:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1440, // 21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Ust-Nera
	// EraIndex: 221
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Vladivostok
	// EraIndex: 222
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Yakutsk
	// EraIndex: 223
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              9:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2160, // 32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Yangon
	// EraIndex: 224
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             6:30    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1560, // 23400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Yekaterinburg
	// EraIndex: 225
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Asia/Yerevan
	// EraIndex: 226
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             4:00    Armenia    %z
	{
		PolicyIndex: 8, // PolicyName: Armenia
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 960, // 14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Azores
	// EraIndex: 227
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -1:00    EU    %z
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -240, // -3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Bermuda
	// EraIndex: 228
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -4:00    US    A%sT
	{
		PolicyIndex: 62, // PolicyName: US
		FormatIndex: 3, // "A%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Canary
	// EraIndex: 229
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              0:00    EU    WE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 42, // "WE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Cape_Verde
	// EraIndex: 230
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -1:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -240, // -3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Faroe
	// EraIndex: 231
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              0:00    EU    WE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 42, // "WE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Madeira
	// EraIndex: 232
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              0:00    EU    WE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 42, // "WE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/South_Georgia
	// EraIndex: 233
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -2:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -480, // -7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Atlantic/Stanley
	// EraIndex: 234
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Adelaide
	// EraIndex: 235
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             9:30    AS    AC%sT
	{
		PolicyIndex: 3, // PolicyName: AS
		FormatIndex: 4, // "AC%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2280, // 34200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Brisbane
	// EraIndex: 236
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    AQ    AE%sT
	{
		PolicyIndex: 2, // PolicyName: AQ
		FormatIndex: 5, // "AE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Broken_Hill
	// EraIndex: 237
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             9:30    AS    AC%sT
	{
		PolicyIndex: 3, // PolicyName: AS
		FormatIndex: 4, // "AC%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2280, // 34200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Darwin
	// EraIndex: 238
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              9:30    Aus    AC%sT
	{
		PolicyIndex: 9, // PolicyName: Aus
		FormatIndex: 4, // "AC%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2280, // 34200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Eucla
	// EraIndex: 239
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              8:45    AW    %z
	{
		PolicyIndex: 6, // PolicyName: AW
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2100, // 31500 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Hobart
	// EraIndex: 240
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    AT    AE%sT
	{
		PolicyIndex: 4, // PolicyName: AT
		FormatIndex: 5, // "AE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Lindeman
	// EraIndex: 241
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    Holiday    AE%sT
	{
		PolicyIndex: 31, // PolicyName: Holiday
		FormatIndex: 5, // "AE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Lord_Howe
	// EraIndex: 242
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:30    LH    %z
	{
		PolicyIndex: 36, // PolicyName: LH
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2520, // 37800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Melbourne
	// EraIndex: 243
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    AV    AE%sT
	{
		PolicyIndex: 5, // PolicyName: AV
		FormatIndex: 5, // "AE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Perth
	// EraIndex: 244
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              8:00    AW    AW%sT
	{
		PolicyIndex: 6, // PolicyName: AW
		FormatIndex: 8, // "AW%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Australia/Sydney
	// EraIndex: 245
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    AN    AE%sT
	{
		PolicyIndex: 1, // PolicyName: AN
		FormatIndex: 5, // "AE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT
	// EraIndex: 246
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 0 - GMT
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 20, // "GMT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+1
	// EraIndex: 247
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -1 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -240, // -3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+10
	// EraIndex: 248
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -10 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -2400, // -36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+11
	// EraIndex: 249
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -11 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -2640, // -39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+12
	// EraIndex: 250
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -12 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -2880, // -43200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+2
	// EraIndex: 251
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -2 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -480, // -7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+3
	// EraIndex: 252
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -3 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -720, // -10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+4
	// EraIndex: 253
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -4 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -960, // -14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+5
	// EraIndex: 254
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -5 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1200, // -18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+6
	// EraIndex: 255
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -6 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+7
	// EraIndex: 256
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -7 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1680, // -25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+8
	// EraIndex: 257
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -8 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1920, // -28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT+9
	// EraIndex: 258
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// -9 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -2160, // -32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-1
	// EraIndex: 259
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 1 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-10
	// EraIndex: 260
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 10 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-11
	// EraIndex: 261
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 11 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2640, // 39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-12
	// EraIndex: 262
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 12 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2880, // 43200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-13
	// EraIndex: 263
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 13 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 3120, // 46800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-14
	// EraIndex: 264
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 14 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 3360, // 50400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-2
	// EraIndex: 265
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 2 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-3
	// EraIndex: 266
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 3 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-4
	// EraIndex: 267
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 4 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 960, // 14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-5
	// EraIndex: 268
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 5 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-6
	// EraIndex: 269
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 6 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1440, // 21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-7
	// EraIndex: 270
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 7 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1680, // 25200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-8
	// EraIndex: 271
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 8 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1920, // 28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/GMT-9
	// EraIndex: 272
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 9 - %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2160, // 32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Etc/UTC
	// EraIndex: 273
	// EraCount: 1
	// ---------------------------------------------------------------------------

	// 0 - UTC
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 40, // "UTC"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Andorra
	// EraIndex: 274
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Astrakhan
	// EraIndex: 275
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 960, // 14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Athens
	// EraIndex: 276
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Belgrade
	// EraIndex: 277
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Berlin
	// EraIndex: 278
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Brussels
	// EraIndex: 279
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Bucharest
	// EraIndex: 280
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Budapest
	// EraIndex: 281
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Chisinau
	// EraIndex: 282
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    Moldova    EE%sT
	{
		PolicyIndex: 41, // PolicyName: Moldova
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Dublin
	// EraIndex: 283
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              1:00    Eire    IST/GMT
	{
		PolicyIndex: 26, // PolicyName: Eire
		FormatIndex: 27, // "IST/GMT"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Gibraltar
	// EraIndex: 284
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Helsinki
	// EraIndex: 285
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Istanbul
	// EraIndex: 286
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Kaliningrad
	// EraIndex: 287
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              2:00    -    EET
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 18, // "EET"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Kirov
	// EraIndex: 288
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              3:00    -    MSK
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "MSK"
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Kyiv
	// EraIndex: 289
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Lisbon
	// EraIndex: 290
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              0:00    EU    WE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 42, // "WE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/London
	// EraIndex: 291
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              0:00    EU    GMT/BST
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 21, // "GMT/BST"
		DeltaMinutes: 0,
		OffsetSecondsCode: 0, // 0 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Madrid
	// EraIndex: 292
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Malta
	// EraIndex: 293
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Minsk
	// EraIndex: 294
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             3:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Moscow
	// EraIndex: 295
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              3:00    -    MSK
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "MSK"
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Paris
	// EraIndex: 296
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Prague
	// EraIndex: 297
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Riga
	// EraIndex: 298
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Rome
	// EraIndex: 299
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Samara
	// EraIndex: 300
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 960, // 14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Saratov
	// EraIndex: 301
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 960, // 14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Simferopol
	// EraIndex: 302
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              3:00    -    MSK
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "MSK"
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Sofia
	// EraIndex: 303
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Tallinn
	// EraIndex: 304
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Tirane
	// EraIndex: 305
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Ulyanovsk
	// EraIndex: 306
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              4:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 960, // 14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Vienna
	// EraIndex: 307
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Vilnius
	// EraIndex: 308
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             2:00    EU    EE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 17, // "EE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 480, // 7200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Volgograd
	// EraIndex: 309
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              3:00    -    MSK
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 32, // "MSK"
		DeltaMinutes: 0,
		OffsetSecondsCode: 720, // 10800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Warsaw
	// EraIndex: 310
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Europe/Zurich
	// EraIndex: 311
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             1:00    EU    CE%sT
	{
		PolicyIndex: 22, // PolicyName: EU
		FormatIndex: 11, // "CE%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 240, // 3600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Indian/Chagos
	// EraIndex: 312
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             6:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1440, // 21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Indian/Maldives
	// EraIndex: 313
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             5:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 1200, // 18000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Indian/Mauritius
	// EraIndex: 314
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             4:00 Mauritius    %z
	{
		PolicyIndex: 39, // PolicyName: Mauritius
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 960, // 14400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Apia
	// EraIndex: 315
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              13:00    WS    %z
	{
		PolicyIndex: 65, // PolicyName: WS
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 3120, // 46800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Auckland
	// EraIndex: 316
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             12:00    NZ    NZ%sT
	{
		PolicyIndex: 45, // PolicyName: NZ
		FormatIndex: 35, // "NZ%T"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2880, // 43200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Bougainville
	// EraIndex: 317
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2640, // 39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Chatham
	// EraIndex: 318
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             12:45    Chatham    %z
	{
		PolicyIndex: 17, // PolicyName: Chatham
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 3060, // 45900 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Easter
	// EraIndex: 319
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Chile    %z
	{
		PolicyIndex: 18, // PolicyName: Chile
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Efate
	// EraIndex: 320
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    Vanuatu    %z
	{
		PolicyIndex: 64, // PolicyName: Vanuatu
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2640, // 39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Fakaofo
	// EraIndex: 321
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             13:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 3120, // 46800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Fiji
	// EraIndex: 322
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             12:00    Fiji    %z
	{
		PolicyIndex: 27, // PolicyName: Fiji
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2880, // 43200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Galapagos
	// EraIndex: 323
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -6:00    Ecuador    %z
	{
		PolicyIndex: 24, // PolicyName: Ecuador
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1440, // -21600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Gambier
	// EraIndex: 324
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              -9:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -2160, // -32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Guadalcanal
	// EraIndex: 325
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2640, // 39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Guam
	// EraIndex: 326
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    -    ChST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 14, // "ChST"
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Honolulu
	// EraIndex: 327
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -10:00    -    HST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 24, // "HST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -2400, // -36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Kanton
	// EraIndex: 328
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              13:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 3120, // 46800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Kiritimati
	// EraIndex: 329
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              14:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 3360, // 50400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Kosrae
	// EraIndex: 330
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              11:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2640, // 39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Kwajalein
	// EraIndex: 331
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              12:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2880, // 43200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Marquesas
	// EraIndex: 332
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              -9:30    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -2280, // -34200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Nauru
	// EraIndex: 333
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             12:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2880, // 43200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Niue
	// EraIndex: 334
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -11:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -2640, // -39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Norfolk
	// EraIndex: 335
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    AN    %z
	{
		PolicyIndex: 1, // PolicyName: AN
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2640, // 39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Noumea
	// EraIndex: 336
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             11:00    NC    %z
	{
		PolicyIndex: 44, // PolicyName: NC
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2640, // 39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Pago_Pago
	// EraIndex: 337
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -11:00    -    SST
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 39, // "SST"
		DeltaMinutes: 0,
		OffsetSecondsCode: -2640, // -39600 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Palau
	// EraIndex: 338
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//               9:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2160, // 32400 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Pitcairn
	// EraIndex: 339
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -8:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -1920, // -28800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Port_Moresby
	// EraIndex: 340
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             10:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2400, // 36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Rarotonga
	// EraIndex: 341
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -10:00    Cook    %z
	{
		PolicyIndex: 19, // PolicyName: Cook
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -2400, // -36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Tahiti
	// EraIndex: 342
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             -10:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: -2400, // -36000 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Tarawa
	// EraIndex: 343
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//              12:00    -    %z
	{
		PolicyIndex: 0, // PolicyName: (none)
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 2880, // 43200 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},

	// ---------------------------------------------------------------------------
	// ZoneName: Pacific/Tongatapu
	// EraIndex: 344
	// EraCount: 1
	// ---------------------------------------------------------------------------

	//             13:00    Tonga    %z
	{
		PolicyIndex: 59, // PolicyName: Tonga
		FormatIndex: 0, // ""
		DeltaMinutes: 0,
		OffsetSecondsCode: 3120, // 46800 / 15
		OffsetSecondsRemainder: 0,
		UntilYear: 32767,
		UntilMonth: 1,
		UntilDay: 1,
		UntilSecondsCode: 0, // 0 / 15
		UntilSecondsModifier: 0, // SuffixW + remainder=0
	},


}

// ---------------------------------------------------------------------------
// ZoneInfoRecords is an array of ZoneInfoRecord items concatenated together
// across all zones.
//
// Total: 597 (340 zones, 257 links)
// ---------------------------------------------------------------------------

var ZoneInfoRecords = []zoneinfo.ZoneInfoRecord{
	// 0: Link GB -> Europe/London
	{
		ZoneID: 0x005973ae,
		NameIndex: 491, // "GB"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 1: Link NZ -> Pacific/Auckland
	{
		ZoneID: 0x005974ad,
		NameIndex: 524, // "NZ"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 98, // Pacific/Auckland
	},
	// 2: Link Asia/Kuala_Lumpur -> Asia/Singapore
	{
		ZoneID: 0x014763c4,
		NameIndex: 286, // "Asia/Kuala_Lumpur"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 495, // Asia/Singapore
	},
	// 3: Link Africa/Libreville -> Africa/Lagos
	{
		ZoneID: 0x01d96de4,
		NameIndex: 32, // "Africa/Libreville"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 4: Link Indian/Cocos -> Asia/Yangon
	{
		ZoneID: 0x021e86de,
		NameIndex: 504, // "Indian/Cocos"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 525, // Asia/Yangon
	},
	// 5: Link Australia/Victoria -> Australia/Melbourne
	{
		ZoneID: 0x0260d5db,
		NameIndex: 367, // "Australia/Victoria"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 49, // Australia/Melbourne
	},
	// 6: Link Atlantic/Faeroe -> Atlantic/Faroe
	{
		ZoneID: 0x031ec516,
		NameIndex: 339, // "Atlantic/Faeroe"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 540, // Atlantic/Faroe
	},
	// 7: Zone America/St_Johns
	{
		ZoneID: 0x04b14e6e,
		NameIndex: 205, // "America/St_Johns"
		EraIndex: 133,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 8: Link America/St_Kitts -> America/Puerto_Rico
	{
		ZoneID: 0x04c0507b,
		NameIndex: 206, // "America/St_Kitts"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 9: Link Africa/Ouagadougou -> Africa/Abidjan
	{
		ZoneID: 0x04d7219a,
		NameIndex: 47, // "Africa/Ouagadougou"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 10: Link America/St_Lucia -> America/Puerto_Rico
	{
		ZoneID: 0x04d8b3ba,
		NameIndex: 207, // "America/St_Lucia"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 11: Zone America/North_Dakota/New_Salem
	{
		ZoneID: 0x04f9958e,
		NameIndex: 176, // "America/North_Dakota/New_Salem"
		EraIndex: 111,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 12: Zone Asia/Jakarta
	{
		ZoneID: 0x0506ab50,
		NameIndex: 274, // "Asia/Jakarta"
		EraIndex: 179,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 13: Link Africa/Bujumbura -> Africa/Maputo
	{
		ZoneID: 0x05232a47,
		NameIndex: 12, // "Africa/Bujumbura"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 14: Zone America/Mazatlan
	{
		ZoneID: 0x0532189e,
		NameIndex: 157, // "America/Mazatlan"
		EraIndex: 97,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 15: Link America/St_Barthelemy -> America/Puerto_Rico
	{
		ZoneID: 0x054e6a79,
		NameIndex: 204, // "America/St_Barthelemy"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 16: Link Africa/Addis_Ababa -> Africa/Nairobi
	{
		ZoneID: 0x05ae1e65,
		NameIndex: 2, // "Africa/Addis_Ababa"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 17: Zone Pacific/Fakaofo
	{
		ZoneID: 0x06532bba,
		NameIndex: 537, // "Pacific/Fakaofo"
		EraIndex: 321,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 18: Zone America/Hermosillo
	{
		ZoneID: 0x065d21c4,
		NameIndex: 127, // "America/Hermosillo"
		EraIndex: 74,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 19: Link Mexico/BajaSur -> America/Mazatlan
	{
		ZoneID: 0x08ee3641,
		NameIndex: 522, // "Mexico/BajaSur"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 14, // America/Mazatlan
	},
	// 20: Zone Asia/Tbilisi
	{
		ZoneID: 0x0903e442,
		NameIndex: 317, // "Asia/Tbilisi"
		EraIndex: 214,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 21: Zone America/Indiana/Tell_City
	{
		ZoneID: 0x09263612,
		NameIndex: 132, // "America/Indiana/Tell_City"
		EraIndex: 79,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 22: Link US/Hawaii -> Pacific/Honolulu
	{
		ZoneID: 0x09c8de2f,
		NameIndex: 586, // "US/Hawaii"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 553, // Pacific/Honolulu
	},
	// 23: Zone America/Boa_Vista
	{
		ZoneID: 0x0a7b7efe,
		NameIndex: 82, // "America/Boa_Vista"
		EraIndex: 41,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 24: Zone Asia/Colombo
	{
		ZoneID: 0x0af0e91d,
		NameIndex: 258, // "Asia/Colombo"
		EraIndex: 166,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 25: Link CET -> Europe/Brussels
	{
		ZoneID: 0x0b87d921,
		NameIndex: 374, // "CET"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 530, // Europe/Brussels
	},
	// 26: Link EET -> Europe/Athens
	{
		ZoneID: 0x0b87e1a3,
		NameIndex: 387, // "EET"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 155, // Europe/Athens
	},
	// 27: Link EST -> America/Panama
	{
		ZoneID: 0x0b87e371,
		NameIndex: 388, // "EST"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 445, // America/Panama
	},
	// 28: Link GMT -> Etc/GMT
	{
		ZoneID: 0x0b87eb2d,
		NameIndex: 493, // "GMT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 514, // Etc/GMT
	},
	// 29: Link HST -> Pacific/Honolulu
	{
		ZoneID: 0x0b87f034,
		NameIndex: 498, // "HST"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 553, // Pacific/Honolulu
	},
	// 30: Link MET -> Europe/Brussels
	{
		ZoneID: 0x0b8803ab,
		NameIndex: 518, // "MET"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 530, // Europe/Brussels
	},
	// 31: Link MST -> America/Phoenix
	{
		ZoneID: 0x0b880579,
		NameIndex: 519, // "MST"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 127, // America/Phoenix
	},
	// 32: Link PRC -> Asia/Shanghai
	{
		ZoneID: 0x0b88120a,
		NameIndex: 527, // "PRC"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 573, // Asia/Shanghai
	},
	// 33: Link ROC -> Asia/Taipei
	{
		ZoneID: 0x0b881a29,
		NameIndex: 575, // "ROC"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 500, // Asia/Taipei
	},
	// 34: Link ROK -> Asia/Seoul
	{
		ZoneID: 0x0b881a31,
		NameIndex: 576, // "ROK"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 67, // Asia/Seoul
	},
	// 35: Link UCT -> Etc/UTC
	{
		ZoneID: 0x0b882571,
		NameIndex: 579, // "UCT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 516, // Etc/UTC
	},
	// 36: Link UTC -> Etc/UTC
	{
		ZoneID: 0x0b882791,
		NameIndex: 592, // "UTC"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 516, // Etc/UTC
	},
	// 37: Link WET -> Europe/Lisbon
	{
		ZoneID: 0x0b882e35,
		NameIndex: 595, // "WET"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 195, // Europe/Lisbon
	},
	// 38: Zone America/Guatemala
	{
		ZoneID: 0x0c8259f7,
		NameIndex: 122, // "America/Guatemala"
		EraIndex: 69,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 39: Link Europe/Mariehamn -> Europe/Helsinki
	{
		ZoneID: 0x0caa6496,
		NameIndex: 458, // "Europe/Mariehamn"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 233, // Europe/Helsinki
	},
	// 40: Zone Africa/Monrovia
	{
		ZoneID: 0x0ce90385,
		NameIndex: 42, // "Africa/Monrovia"
		EraIndex: 12,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 41: Link Egypt -> Africa/Cairo
	{
		ZoneID: 0x0d1a278e,
		NameIndex: 390, // "Egypt"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 270, // Africa/Cairo
	},
	// 42: Link GMT+0 -> Etc/GMT
	{
		ZoneID: 0x0d2f7028,
		NameIndex: 494, // "GMT+0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 514, // Etc/GMT
	},
	// 43: Link GMT-0 -> Etc/GMT
	{
		ZoneID: 0x0d2f706a,
		NameIndex: 495, // "GMT-0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 514, // Etc/GMT
	},
	// 44: Link Japan -> Asia/Tokyo
	{
		ZoneID: 0x0d712f8f,
		NameIndex: 515, // "Japan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 68, // Asia/Tokyo
	},
	// 45: Link Libya -> Africa/Tripoli
	{
		ZoneID: 0x0d998b16,
		NameIndex: 517, // "Libya"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 398, // Africa/Tripoli
	},
	// 46: Link Kwajalein -> Pacific/Kwajalein
	{
		ZoneID: 0x0e57afbb,
		NameIndex: 516, // "Kwajalein"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 324, // Pacific/Kwajalein
	},
	// 47: Zone Antarctica/Rothera
	{
		ZoneID: 0x0e86d203,
		NameIndex: 230, // "Antarctica/Rothera"
		EraIndex: 148,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 48: Link America/Yellowknife -> America/Edmonton
	{
		ZoneID: 0x0f76c76f,
		NameIndex: 222, // "America/Yellowknife"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 238, // America/Edmonton
	},
	// 49: Zone Australia/Melbourne
	{
		ZoneID: 0x0fe559a3,
		NameIndex: 359, // "Australia/Melbourne"
		EraIndex: 243,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 50: Zone America/Sao_Paulo
	{
		ZoneID: 0x1063bfc9,
		NameIndex: 200, // "America/Sao_Paulo"
		EraIndex: 129,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 51: Link Europe/Amsterdam -> Europe/Brussels
	{
		ZoneID: 0x109395c2,
		NameIndex: 427, // "Europe/Amsterdam"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 530, // Europe/Brussels
	},
	// 52: Zone America/Indiana/Vevay
	{
		ZoneID: 0x10aca054,
		NameIndex: 133, // "America/Indiana/Vevay"
		EraIndex: 80,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 53: Zone America/Scoresbysund
	{
		ZoneID: 0x123f8d2a,
		NameIndex: 201, // "America/Scoresbysund"
		EraIndex: 130,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 54: Zone Asia/Samarkand
	{
		ZoneID: 0x13ae5104,
		NameIndex: 310, // "Asia/Samarkand"
		EraIndex: 207,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 55: Zone Asia/Amman
	{
		ZoneID: 0x148d21bc,
		NameIndex: 238, // "Asia/Amman"
		EraIndex: 153,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 56: Zone Asia/Aqtau
	{
		ZoneID: 0x148f710e,
		NameIndex: 240, // "Asia/Aqtau"
		EraIndex: 155,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 57: Zone Asia/Chita
	{
		ZoneID: 0x14ae863b,
		NameIndex: 254, // "Asia/Chita"
		EraIndex: 165,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 58: Link Asia/Dacca -> Asia/Dhaka
	{
		ZoneID: 0x14bcac5e,
		NameIndex: 259, // "Asia/Dacca"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 59, // Asia/Dhaka
	},
	// 59: Zone Asia/Dhaka
	{
		ZoneID: 0x14c07b8b,
		NameIndex: 261, // "Asia/Dhaka"
		EraIndex: 168,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 60: Zone Asia/Dubai
	{
		ZoneID: 0x14c79f77,
		NameIndex: 263, // "Asia/Dubai"
		EraIndex: 170,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 61: Zone America/Bahia_Banderas
	{
		ZoneID: 0x14f6329a,
		NameIndex: 77, // "America/Bahia_Banderas"
		EraIndex: 37,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 62: Zone Asia/Kabul
	{
		ZoneID: 0x153b5601,
		NameIndex: 277, // "Asia/Kabul"
		EraIndex: 182,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 63: Link Asia/Ashkhabad -> Asia/Ashgabat
	{
		ZoneID: 0x15454f09,
		NameIndex: 243, // "Asia/Ashkhabad"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 453, // Asia/Ashgabat
	},
	// 64: Link Asia/Macao -> Asia/Macau
	{
		ZoneID: 0x155f88b3,
		NameIndex: 289, // "Asia/Macao"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 65, // Asia/Macau
	},
	// 65: Zone Asia/Macau
	{
		ZoneID: 0x155f88b9,
		NameIndex: 290, // "Asia/Macau"
		EraIndex: 190,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 66: Zone Asia/Qatar
	{
		ZoneID: 0x15a8330b,
		NameIndex: 303, // "Asia/Qatar"
		EraIndex: 201,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 67: Zone Asia/Seoul
	{
		ZoneID: 0x15ce82da,
		NameIndex: 311, // "Asia/Seoul"
		EraIndex: 208,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 68: Zone Asia/Tokyo
	{
		ZoneID: 0x15e606a8,
		NameIndex: 322, // "Asia/Tokyo"
		EraIndex: 217,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 69: Zone Asia/Tomsk
	{
		ZoneID: 0x15e60e60,
		NameIndex: 323, // "Asia/Tomsk"
		EraIndex: 218,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 70: Link Asia/Tel_Aviv -> Asia/Jerusalem
	{
		ZoneID: 0x166d7c2c,
		NameIndex: 319, // "Asia/Tel_Aviv"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 193, // Asia/Jerusalem
	},
	// 71: Zone Asia/Thimphu
	{
		ZoneID: 0x170380d1,
		NameIndex: 321, // "Asia/Thimphu"
		EraIndex: 216,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 72: Zone America/Guayaquil
	{
		ZoneID: 0x17e64958,
		NameIndex: 123, // "America/Guayaquil"
		EraIndex: 70,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 73: Link America/Montserrat -> America/Puerto_Rico
	{
		ZoneID: 0x199b0a35,
		NameIndex: 168, // "America/Montserrat"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 74: Zone America/Kentucky/Louisville
	{
		ZoneID: 0x1a21024b,
		NameIndex: 142, // "America/Kentucky/Louisville"
		EraIndex: 87,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 75: Zone Asia/Pontianak
	{
		ZoneID: 0x1a76c057,
		NameIndex: 301, // "Asia/Pontianak"
		EraIndex: 199,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 76: Link Europe/Podgorica -> Europe/Belgrade
	{
		ZoneID: 0x1c1a499c,
		NameIndex: 465, // "Europe/Podgorica"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 536, // Europe/Belgrade
	},
	// 77: Link Atlantic/Reykjavik -> Africa/Abidjan
	{
		ZoneID: 0x1c2b4f74,
		NameIndex: 343, // "Atlantic/Reykjavik"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 78: Zone America/New_York
	{
		ZoneID: 0x1e2a7654,
		NameIndex: 170, // "America/New_York"
		EraIndex: 106,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 79: Link Europe/Luxembourg -> Europe/Brussels
	{
		ZoneID: 0x1f8bc6ce,
		NameIndex: 455, // "Europe/Luxembourg"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 530, // Europe/Brussels
	},
	// 80: Link Asia/Aden -> Asia/Riyadh
	{
		ZoneID: 0x1fa7084a,
		NameIndex: 236, // "Asia/Aden"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 490, // Asia/Riyadh
	},
	// 81: Zone Asia/Baku
	{
		ZoneID: 0x1fa788b5,
		NameIndex: 247, // "Asia/Baku"
		EraIndex: 160,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 82: Zone Asia/Dili
	{
		ZoneID: 0x1fa8c394,
		NameIndex: 262, // "Asia/Dili"
		EraIndex: 169,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 83: Zone Asia/Gaza
	{
		ZoneID: 0x1faa4875,
		NameIndex: 266, // "Asia/Gaza"
		EraIndex: 173,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 84: Zone Asia/Hovd
	{
		ZoneID: 0x1fab0fe3,
		NameIndex: 271, // "Asia/Hovd"
		EraIndex: 177,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 85: Zone Asia/Omsk
	{
		ZoneID: 0x1faeddac,
		NameIndex: 298, // "Asia/Omsk"
		EraIndex: 197,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 86: Zone Asia/Oral
	{
		ZoneID: 0x1faef0a0,
		NameIndex: 299, // "Asia/Oral"
		EraIndex: 198,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 87: Link America/Montreal -> America/Toronto
	{
		ZoneID: 0x203a1ea8,
		NameIndex: 167, // "America/Montreal"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 276, // America/Toronto
	},
	// 88: Zone Asia/Ho_Chi_Minh
	{
		ZoneID: 0x20f2d127,
		NameIndex: 269, // "Asia/Ho_Chi_Minh"
		EraIndex: 175,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 89: Zone Asia/Damascus
	{
		ZoneID: 0x20fbb063,
		NameIndex: 260, // "Asia/Damascus"
		EraIndex: 167,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 90: Link America/Argentina/ComodRivadavia -> America/Argentina/Catamarca
	{
		ZoneID: 0x22758877,
		NameIndex: 61, // "America/Argentina/ComodRivadavia"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 318, // America/Argentina/Catamarca
	},
	// 91: Zone Pacific/Apia
	{
		ZoneID: 0x23359b5e,
		NameIndex: 529, // "Pacific/Apia"
		EraIndex: 315,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 92: Zone Pacific/Fiji
	{
		ZoneID: 0x23383ba5,
		NameIndex: 538, // "Pacific/Fiji"
		EraIndex: 322,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 93: Zone Pacific/Guam
	{
		ZoneID: 0x2338f9ed,
		NameIndex: 543, // "Pacific/Guam"
		EraIndex: 326,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 94: Zone Pacific/Niue
	{
		ZoneID: 0x233ca014,
		NameIndex: 554, // "Pacific/Niue"
		EraIndex: 334,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 95: Link Pacific/Truk -> Pacific/Port_Moresby
	{
		ZoneID: 0x234010a9,
		NameIndex: 569, // "Pacific/Truk"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 419, // Pacific/Port_Moresby
	},
	// 96: Link Pacific/Wake -> Pacific/Tarawa
	{
		ZoneID: 0x23416c2b,
		NameIndex: 570, // "Pacific/Wake"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 564, // Pacific/Tarawa
	},
	// 97: Zone Australia/Adelaide
	{
		ZoneID: 0x2428e8a3,
		NameIndex: 348, // "Australia/Adelaide"
		EraIndex: 235,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 98: Zone Pacific/Auckland
	{
		ZoneID: 0x25062f86,
		NameIndex: 530, // "Pacific/Auckland"
		EraIndex: 316,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 99: Zone Pacific/Tongatapu
	{
		ZoneID: 0x262ca836,
		NameIndex: 568, // "Pacific/Tongatapu"
		EraIndex: 344,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 100: Zone America/Monterrey
	{
		ZoneID: 0x269a1deb,
		NameIndex: 165, // "America/Monterrey"
		EraIndex: 104,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 101: Link Etc/Greenwich -> Etc/GMT
	{
		ZoneID: 0x26daa98c,
		NameIndex: 422, // "Etc/Greenwich"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 514, // Etc/GMT
	},
	// 102: Link Australia/Currie -> Australia/Hobart
	{
		ZoneID: 0x278b6a24,
		NameIndex: 352, // "Australia/Currie"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 124, // Australia/Hobart
	},
	// 103: Zone Australia/Darwin
	{
		ZoneID: 0x2876bdff,
		NameIndex: 353, // "Australia/Darwin"
		EraIndex: 238,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 104: Link Pacific/Pohnpei -> Pacific/Guadalcanal
	{
		ZoneID: 0x28929f96,
		NameIndex: 560, // "Pacific/Pohnpei"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 569, // Pacific/Guadalcanal
	},
	// 105: Zone Asia/Famagusta
	{
		ZoneID: 0x289b4f8b,
		NameIndex: 265, // "Asia/Famagusta"
		EraIndex: 172,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 106: Zone America/Indiana/Vincennes
	{
		ZoneID: 0x28a0b212,
		NameIndex: 134, // "America/Indiana/Vincennes"
		EraIndex: 81,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 107: Zone America/Indiana/Indianapolis
	{
		ZoneID: 0x28a669a4,
		NameIndex: 128, // "America/Indiana/Indianapolis"
		EraIndex: 75,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 108: Zone Asia/Vladivostok
	{
		ZoneID: 0x29de34a8,
		NameIndex: 330, // "Asia/Vladivostok"
		EraIndex: 222,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 109: Link Australia/Canberra -> Australia/Sydney
	{
		ZoneID: 0x2a09ae58,
		NameIndex: 351, // "Australia/Canberra"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 168, // Australia/Sydney
	},
	// 110: Zone America/Fortaleza
	{
		ZoneID: 0x2ad018ee,
		NameIndex: 115, // "America/Fortaleza"
		EraIndex: 65,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 111: Zone America/Vancouver
	{
		ZoneID: 0x2c6f6b1f,
		NameIndex: 217, // "America/Vancouver"
		EraIndex: 139,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 112: Link America/Pangnirtung -> America/Iqaluit
	{
		ZoneID: 0x2d999193,
		NameIndex: 180, // "America/Pangnirtung"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 113, // America/Iqaluit
	},
	// 113: Zone America/Iqaluit
	{
		ZoneID: 0x2de310bf,
		NameIndex: 138, // "America/Iqaluit"
		EraIndex: 84,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 114: Link Jamaica -> America/Jamaica
	{
		ZoneID: 0x2e44fdab,
		NameIndex: 514, // "Jamaica"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 182, // America/Jamaica
	},
	// 115: Zone Pacific/Chatham
	{
		ZoneID: 0x2f0de999,
		NameIndex: 532, // "Pacific/Chatham"
		EraIndex: 318,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 116: Link Etc/Universal -> Etc/UTC
	{
		ZoneID: 0x2f8cb9a9,
		NameIndex: 425, // "Etc/Universal"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 516, // Etc/UTC
	},
	// 117: Zone America/Indiana/Marengo
	{
		ZoneID: 0x2feeee72,
		NameIndex: 130, // "America/Indiana/Marengo"
		EraIndex: 77,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 118: Zone Europe/Tallinn
	{
		ZoneID: 0x30c4e096,
		NameIndex: 477, // "Europe/Tallinn"
		EraIndex: 304,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 119: Link Africa/Djibouti -> Africa/Nairobi
	{
		ZoneID: 0x30ea01d4,
		NameIndex: 19, // "Africa/Djibouti"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 120: Zone Asia/Ulaanbaatar
	{
		ZoneID: 0x30f0cc4e,
		NameIndex: 325, // "Asia/Ulaanbaatar"
		EraIndex: 219,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 121: Link Africa/Gaborone -> Africa/Maputo
	{
		ZoneID: 0x317c0aa7,
		NameIndex: 23, // "Africa/Gaborone"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 122: Zone America/Argentina/Ushuaia
	{
		ZoneID: 0x320dcdde,
		NameIndex: 71, // "America/Argentina/Ushuaia"
		EraIndex: 33,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 123: Link Asia/Calcutta -> Asia/Kolkata
	{
		ZoneID: 0x328a44c3,
		NameIndex: 253, // "Asia/Calcutta"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 251, // Asia/Kolkata
	},
	// 124: Zone Australia/Hobart
	{
		ZoneID: 0x32bf951a,
		NameIndex: 355, // "Australia/Hobart"
		EraIndex: 240,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 125: Zone Asia/Dushanbe
	{
		ZoneID: 0x32fc5c3c,
		NameIndex: 264, // "Asia/Dushanbe"
		EraIndex: 171,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 126: Zone Atlantic/South_Georgia
	{
		ZoneID: 0x33013174,
		NameIndex: 344, // "Atlantic/South_Georgia"
		EraIndex: 233,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 127: Zone America/Phoenix
	{
		ZoneID: 0x34b5af01,
		NameIndex: 182, // "America/Phoenix"
		EraIndex: 116,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 128: Link Asia/Istanbul -> Europe/Istanbul
	{
		ZoneID: 0x382e7894,
		NameIndex: 273, // "Asia/Istanbul"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 399, // Europe/Istanbul
	},
	// 129: Link Asia/Ulan_Bator -> Asia/Ulaanbaatar
	{
		ZoneID: 0x394db4d9,
		NameIndex: 326, // "Asia/Ulan_Bator"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 120, // Asia/Ulaanbaatar
	},
	// 130: Zone Antarctica/Mawson
	{
		ZoneID: 0x399cd863,
		NameIndex: 227, // "Antarctica/Mawson"
		EraIndex: 146,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 131: Link Africa/Brazzaville -> Africa/Lagos
	{
		ZoneID: 0x39cda760,
		NameIndex: 11, // "Africa/Brazzaville"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 132: Zone America/Caracas
	{
		ZoneID: 0x3be064f4,
		NameIndex: 89, // "America/Caracas"
		EraIndex: 47,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 133: Zone America/Cayenne
	{
		ZoneID: 0x3c617269,
		NameIndex: 91, // "America/Cayenne"
		EraIndex: 48,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 134: Link Africa/Porto-Novo -> Africa/Lagos
	{
		ZoneID: 0x3d1bf95d,
		NameIndex: 48, // "Africa/Porto-Novo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 135: Zone Atlantic/Bermuda
	{
		ZoneID: 0x3d4bb1c4,
		NameIndex: 336, // "Atlantic/Bermuda"
		EraIndex: 228,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 136: Zone America/Managua
	{
		ZoneID: 0x3d5e7600,
		NameIndex: 152, // "America/Managua"
		EraIndex: 93,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 137: Link America/Marigot -> America/Puerto_Rico
	{
		ZoneID: 0x3dab3a59,
		NameIndex: 154, // "America/Marigot"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 138: Link Europe/Guernsey -> Europe/London
	{
		ZoneID: 0x3db12c16,
		NameIndex: 443, // "Europe/Guernsey"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 139: Link Africa/Nouakchott -> Africa/Abidjan
	{
		ZoneID: 0x3dc49dba,
		NameIndex: 46, // "Africa/Nouakchott"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 140: Link America/Louisville -> America/Kentucky/Louisville
	{
		ZoneID: 0x3dcb47ee,
		NameIndex: 149, // "America/Louisville"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 74, // America/Kentucky/Louisville
	},
	// 141: Zone America/Argentina/San_Juan
	{
		ZoneID: 0x3e1009bd,
		NameIndex: 68, // "America/Argentina/San_Juan"
		EraIndex: 30,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 142: Zone America/Argentina/San_Luis
	{
		ZoneID: 0x3e11238c,
		NameIndex: 69, // "America/Argentina/San_Luis"
		EraIndex: 31,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 143: Zone Europe/Volgograd
	{
		ZoneID: 0x3ed0f389,
		NameIndex: 486, // "Europe/Volgograd"
		EraIndex: 309,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 144: Zone America/Fort_Nelson
	{
		ZoneID: 0x3f437e0f,
		NameIndex: 113, // "America/Fort_Nelson"
		EraIndex: 64,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 145: Zone Etc/GMT+10
	{
		ZoneID: 0x3f8f1cc4,
		NameIndex: 395, // "Etc/GMT+10"
		EraIndex: 248,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 146: Zone Etc/GMT+11
	{
		ZoneID: 0x3f8f1cc5,
		NameIndex: 396, // "Etc/GMT+11"
		EraIndex: 249,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 147: Zone Etc/GMT+12
	{
		ZoneID: 0x3f8f1cc6,
		NameIndex: 397, // "Etc/GMT+12"
		EraIndex: 250,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 148: Zone Etc/GMT-10
	{
		ZoneID: 0x3f8f2546,
		NameIndex: 408, // "Etc/GMT-10"
		EraIndex: 260,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 149: Zone Etc/GMT-11
	{
		ZoneID: 0x3f8f2547,
		NameIndex: 409, // "Etc/GMT-11"
		EraIndex: 261,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 150: Zone Etc/GMT-12
	{
		ZoneID: 0x3f8f2548,
		NameIndex: 410, // "Etc/GMT-12"
		EraIndex: 262,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 151: Zone Etc/GMT-13
	{
		ZoneID: 0x3f8f2549,
		NameIndex: 411, // "Etc/GMT-13"
		EraIndex: 263,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 152: Zone Etc/GMT-14
	{
		ZoneID: 0x3f8f254a,
		NameIndex: 412, // "Etc/GMT-14"
		EraIndex: 264,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 153: Zone Antarctica/Palmer
	{
		ZoneID: 0x40962f4f,
		NameIndex: 229, // "Antarctica/Palmer"
		EraIndex: 147,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 154: Link Canada/Pacific -> America/Vancouver
	{
		ZoneID: 0x40fa3c7b,
		NameIndex: 381, // "Canada/Pacific"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 111, // America/Vancouver
	},
	// 155: Zone Europe/Athens
	{
		ZoneID: 0x4318fa27,
		NameIndex: 430, // "Europe/Athens"
		EraIndex: 276,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 156: Link Indian/Kerguelen -> Indian/Maldives
	{
		ZoneID: 0x4351b389,
		NameIndex: 506, // "Indian/Kerguelen"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 360, // Indian/Maldives
	},
	// 157: Zone America/Indiana/Winamac
	{
		ZoneID: 0x4413fa69,
		NameIndex: 135, // "America/Indiana/Winamac"
		EraIndex: 82,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 158: Zone Europe/Berlin
	{
		ZoneID: 0x44644c20,
		NameIndex: 433, // "Europe/Berlin"
		EraIndex: 278,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 159: Link Atlantic/St_Helena -> Africa/Abidjan
	{
		ZoneID: 0x451fc5f7,
		NameIndex: 345, // "Atlantic/St_Helena"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 160: Zone Indian/Chagos
	{
		ZoneID: 0x456f7c3c,
		NameIndex: 502, // "Indian/Chagos"
		EraIndex: 312,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 161: Link Indian/Mahe -> Asia/Dubai
	{
		ZoneID: 0x45e725e2,
		NameIndex: 507, // "Indian/Mahe"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 60, // Asia/Dubai
	},
	// 162: Link Indian/Comoro -> Africa/Nairobi
	{
		ZoneID: 0x45f4deb6,
		NameIndex: 505, // "Indian/Comoro"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 163: Link America/Mendoza -> America/Argentina/Mendoza
	{
		ZoneID: 0x46b4e054,
		NameIndex: 158, // "America/Mendoza"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 427, // America/Argentina/Mendoza
	},
	// 164: Zone Asia/Ust-Nera
	{
		ZoneID: 0x4785f921,
		NameIndex: 328, // "Asia/Ust-Nera"
		EraIndex: 221,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 165: Zone Europe/Dublin
	{
		ZoneID: 0x4a275f62,
		NameIndex: 441, // "Europe/Dublin"
		EraIndex: 283,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 166: Zone Asia/Nicosia
	{
		ZoneID: 0x4b0fcf78,
		NameIndex: 295, // "Asia/Nicosia"
		EraIndex: 194,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 167: Zone America/Chicago
	{
		ZoneID: 0x4b92b5d4,
		NameIndex: 93, // "America/Chicago"
		EraIndex: 49,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 168: Zone Australia/Sydney
	{
		ZoneID: 0x4d1e9776,
		NameIndex: 365, // "Australia/Sydney"
		EraIndex: 245,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 169: Link NZ-CHAT -> Pacific/Chatham
	{
		ZoneID: 0x4d42afda,
		NameIndex: 525, // "NZ-CHAT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 115, // Pacific/Chatham
	},
	// 170: Link US/Arizona -> America/Phoenix
	{
		ZoneID: 0x4ec52670,
		NameIndex: 582, // "US/Arizona"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 127, // America/Phoenix
	},
	// 171: Zone Antarctica/Vostok
	{
		ZoneID: 0x4f966fd4,
		NameIndex: 234, // "Antarctica/Vostok"
		EraIndex: 150,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 172: Link US/Aleutian -> America/Adak
	{
		ZoneID: 0x4fe013ef,
		NameIndex: 581, // "US/Aleutian"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 355, // America/Adak
	},
	// 173: Zone Australia/Brisbane
	{
		ZoneID: 0x4fedc9c0,
		NameIndex: 349, // "Australia/Brisbane"
		EraIndex: 236,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 174: Link America/Catamarca -> America/Argentina/Catamarca
	{
		ZoneID: 0x5036e963,
		NameIndex: 90, // "America/Catamarca"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 318, // America/Argentina/Catamarca
	},
	// 175: Zone America/Asuncion
	{
		ZoneID: 0x50ec79a6,
		NameIndex: 73, // "America/Asuncion"
		EraIndex: 34,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 176: Zone Asia/Karachi
	{
		ZoneID: 0x527f5245,
		NameIndex: 279, // "Asia/Karachi"
		EraIndex: 184,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 177: Link Asia/Kashgar -> Asia/Urumqi
	{
		ZoneID: 0x52955193,
		NameIndex: 280, // "Asia/Kashgar"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 510, // Asia/Urumqi
	},
	// 178: Link Canada/Atlantic -> America/Halifax
	{
		ZoneID: 0x536b119c,
		NameIndex: 376, // "Canada/Atlantic"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 456, // America/Halifax
	},
	// 179: Zone Pacific/Gambier
	{
		ZoneID: 0x53720c3a,
		NameIndex: 541, // "Pacific/Gambier"
		EraIndex: 324,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 180: Zone America/Whitehorse
	{
		ZoneID: 0x54e0e3e8,
		NameIndex: 219, // "America/Whitehorse"
		EraIndex: 140,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 181: Zone America/Martinique
	{
		ZoneID: 0x551e84c5,
		NameIndex: 155, // "America/Martinique"
		EraIndex: 95,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 182: Zone America/Jamaica
	{
		ZoneID: 0x565dad6c,
		NameIndex: 139, // "America/Jamaica"
		EraIndex: 85,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 183: Link US/Samoa -> Pacific/Pago_Pago
	{
		ZoneID: 0x566821cd,
		NameIndex: 591, // "US/Samoa"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 208, // Pacific/Pago_Pago
	},
	// 184: Link Hongkong -> Asia/Hong_Kong
	{
		ZoneID: 0x56d36560,
		NameIndex: 499, // "Hongkong"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 186, // Asia/Hong_Kong
	},
	// 185: Link Europe/Jersey -> Europe/London
	{
		ZoneID: 0x570dae76,
		NameIndex: 447, // "Europe/Jersey"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 186: Zone Asia/Hong_Kong
	{
		ZoneID: 0x577f28ac,
		NameIndex: 270, // "Asia/Hong_Kong"
		EraIndex: 176,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 187: Zone Pacific/Marquesas
	{
		ZoneID: 0x57ca7135,
		NameIndex: 551, // "Pacific/Marquesas"
		EraIndex: 332,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 188: Zone America/Miquelon
	{
		ZoneID: 0x59674330,
		NameIndex: 163, // "America/Miquelon"
		EraIndex: 102,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 189: Link Antarctica/DumontDUrville -> Pacific/Port_Moresby
	{
		ZoneID: 0x5a3c656c,
		NameIndex: 225, // "Antarctica/DumontDUrville"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 419, // Pacific/Port_Moresby
	},
	// 190: Link Atlantic/Jan_Mayen -> Europe/Berlin
	{
		ZoneID: 0x5a7535b6,
		NameIndex: 341, // "Atlantic/Jan_Mayen"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 158, // Europe/Berlin
	},
	// 191: Zone America/Anchorage
	{
		ZoneID: 0x5a79260e,
		NameIndex: 55, // "America/Anchorage"
		EraIndex: 20,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 192: Link US/Eastern -> America/New_York
	{
		ZoneID: 0x5bb7e78e,
		NameIndex: 585, // "US/Eastern"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 78, // America/New_York
	},
	// 193: Zone Asia/Jerusalem
	{
		ZoneID: 0x5becd23a,
		NameIndex: 276, // "Asia/Jerusalem"
		EraIndex: 181,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 194: Link Europe/Stockholm -> Europe/Berlin
	{
		ZoneID: 0x5bf6fbb8,
		NameIndex: 476, // "Europe/Stockholm"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 158, // Europe/Berlin
	},
	// 195: Zone Europe/Lisbon
	{
		ZoneID: 0x5c00a70b,
		NameIndex: 452, // "Europe/Lisbon"
		EraIndex: 290,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 196: Zone Atlantic/Cape_Verde
	{
		ZoneID: 0x5c5e1772,
		NameIndex: 338, // "Atlantic/Cape_Verde"
		EraIndex: 230,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 197: Zone Europe/London
	{
		ZoneID: 0x5c6a84ae,
		NameIndex: 454, // "Europe/London"
		EraIndex: 291,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 198: Link America/Cordoba -> America/Argentina/Cordoba
	{
		ZoneID: 0x5c8a7600,
		NameIndex: 97, // "America/Cordoba"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 461, // America/Argentina/Cordoba
	},
	// 199: Link Asia/Ujung_Pandang -> Asia/Makassar
	{
		ZoneID: 0x5d001eb3,
		NameIndex: 324, // "Asia/Ujung_Pandang"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 232, // Asia/Makassar
	},
	// 200: Link Africa/Mbabane -> Africa/Johannesburg
	{
		ZoneID: 0x5d3bdd40,
		NameIndex: 40, // "Africa/Mbabane"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 512, // Africa/Johannesburg
	},
	// 201: Zone Europe/Madrid
	{
		ZoneID: 0x5dbd1535,
		NameIndex: 456, // "Europe/Madrid"
		EraIndex: 292,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 202: Zone America/Moncton
	{
		ZoneID: 0x5e07fe24,
		NameIndex: 164, // "America/Moncton"
		EraIndex: 103,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 203: Zone Pacific/Bougainville
	{
		ZoneID: 0x5e10f7a4,
		NameIndex: 531, // "Pacific/Bougainville"
		EraIndex: 317,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 204: Link Europe/Monaco -> Europe/Paris
	{
		ZoneID: 0x5ebf9f01,
		NameIndex: 460, // "Europe/Monaco"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 588, // Europe/Paris
	},
	// 205: Zone Europe/Moscow
	{
		ZoneID: 0x5ec266fc,
		NameIndex: 461, // "Europe/Moscow"
		EraIndex: 295,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 206: Zone America/Argentina/Jujuy
	{
		ZoneID: 0x5f2f46c5,
		NameIndex: 63, // "America/Argentina/Jujuy"
		EraIndex: 25,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 207: Zone America/Argentina/Salta
	{
		ZoneID: 0x5fc73403,
		NameIndex: 67, // "America/Argentina/Salta"
		EraIndex: 29,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 208: Zone Pacific/Pago_Pago
	{
		ZoneID: 0x603aebd0,
		NameIndex: 557, // "Pacific/Pago_Pago"
		EraIndex: 337,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 209: Link Pacific/Enderbury -> Pacific/Kanton
	{
		ZoneID: 0x61599a93,
		NameIndex: 536, // "Pacific/Enderbury"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 524, // Pacific/Kanton
	},
	// 210: Zone Africa/Sao_Tome
	{
		ZoneID: 0x61b319d1,
		NameIndex: 49, // "Africa/Sao_Tome"
		EraIndex: 15,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 211: Link Canada/Central -> America/Winnipeg
	{
		ZoneID: 0x626710f5,
		NameIndex: 377, // "Canada/Central"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 316, // America/Winnipeg
	},
	// 212: Link America/Creston -> America/Phoenix
	{
		ZoneID: 0x62a70204,
		NameIndex: 100, // "America/Creston"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 127, // America/Phoenix
	},
	// 213: Zone America/Costa_Rica
	{
		ZoneID: 0x63ff66be,
		NameIndex: 98, // "America/Costa_Rica"
		EraIndex: 52,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 214: Zone Asia/Qostanay
	{
		ZoneID: 0x654fe522,
		NameIndex: 304, // "Asia/Qostanay"
		EraIndex: 202,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 215: Zone America/Indiana/Knox
	{
		ZoneID: 0x6554adc9,
		NameIndex: 129, // "America/Indiana/Knox"
		EraIndex: 76,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 216: Zone Europe/Prague
	{
		ZoneID: 0x65ee5d48,
		NameIndex: 466, // "Europe/Prague"
		EraIndex: 297,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 217: Link Brazil/Acre -> America/Rio_Branco
	{
		ZoneID: 0x66934f93,
		NameIndex: 370, // "Brazil/Acre"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 393, // America/Rio_Branco
	},
	// 218: Link Brazil/East -> America/Sao_Paulo
	{
		ZoneID: 0x669578c5,
		NameIndex: 372, // "Brazil/East"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 50, // America/Sao_Paulo
	},
	// 219: Link Africa/Kinshasa -> Africa/Lagos
	{
		ZoneID: 0x6695d70c,
		NameIndex: 30, // "Africa/Kinshasa"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 220: Link Brazil/West -> America/Manaus
	{
		ZoneID: 0x669f689b,
		NameIndex: 373, // "Brazil/West"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 432, // America/Manaus
	},
	// 221: Link Africa/Mogadishu -> Africa/Nairobi
	{
		ZoneID: 0x66bc159b,
		NameIndex: 41, // "Africa/Mogadishu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 222: Zone America/Puerto_Rico
	{
		ZoneID: 0x6752ca31,
		NameIndex: 187, // "America/Puerto_Rico"
		EraIndex: 119,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 223: Link US/Indiana-Starke -> America/Indiana/Knox
	{
		ZoneID: 0x67977be7,
		NameIndex: 587, // "US/Indiana-Starke"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 215, // America/Indiana/Knox
	},
	// 224: Link America/Buenos_Aires -> America/Argentina/Buenos_Aires
	{
		ZoneID: 0x67d79a05,
		NameIndex: 85, // "America/Buenos_Aires"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 506, // America/Argentina/Buenos_Aires
	},
	// 225: Link Africa/Freetown -> Africa/Abidjan
	{
		ZoneID: 0x6823dd64,
		NameIndex: 22, // "Africa/Freetown"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 226: Link Indian/Christmas -> Asia/Bangkok
	{
		ZoneID: 0x68c207d5,
		NameIndex: 503, // "Indian/Christmas"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 394, // Asia/Bangkok
	},
	// 227: Zone Asia/Novokuznetsk
	{
		ZoneID: 0x69264f93,
		NameIndex: 296, // "Asia/Novokuznetsk"
		EraIndex: 195,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 228: Link America/Indianapolis -> America/Indiana/Indianapolis
	{
		ZoneID: 0x6a009ae1,
		NameIndex: 136, // "America/Indianapolis"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 107, // America/Indiana/Indianapolis
	},
	// 229: Link Europe/Sarajevo -> Europe/Belgrade
	{
		ZoneID: 0x6a576c3f,
		NameIndex: 471, // "Europe/Sarajevo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 536, // Europe/Belgrade
	},
	// 230: Link America/Curacao -> America/Puerto_Rico
	{
		ZoneID: 0x6a879184,
		NameIndex: 102, // "America/Curacao"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 231: Zone America/Tijuana
	{
		ZoneID: 0x6aa1df72,
		NameIndex: 214, // "America/Tijuana"
		EraIndex: 137,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 232: Zone Asia/Makassar
	{
		ZoneID: 0x6aa21c85,
		NameIndex: 292, // "Asia/Makassar"
		EraIndex: 192,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 233: Zone Europe/Helsinki
	{
		ZoneID: 0x6ab2975b,
		NameIndex: 444, // "Europe/Helsinki"
		EraIndex: 285,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 234: Link America/Lower_Princes -> America/Puerto_Rico
	{
		ZoneID: 0x6ae45b62,
		NameIndex: 150, // "America/Lower_Princes"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 235: Zone America/Porto_Velho
	{
		ZoneID: 0x6b1aac77,
		NameIndex: 186, // "America/Porto_Velho"
		EraIndex: 118,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 236: Zone Europe/Samara
	{
		ZoneID: 0x6bc0b139,
		NameIndex: 469, // "Europe/Samara"
		EraIndex: 300,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 237: Link Europe/Skopje -> Europe/Belgrade
	{
		ZoneID: 0x6c76fdd0,
		NameIndex: 474, // "Europe/Skopje"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 536, // Europe/Belgrade
	},
	// 238: Zone America/Edmonton
	{
		ZoneID: 0x6cb9484a,
		NameIndex: 109, // "America/Edmonton"
		EraIndex: 61,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 239: Zone America/Dawson_Creek
	{
		ZoneID: 0x6cf24e5b,
		NameIndex: 105, // "America/Dawson_Creek"
		EraIndex: 58,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 240: Link Asia/Rangoon -> Asia/Yangon
	{
		ZoneID: 0x6d1217c6,
		NameIndex: 306, // "Asia/Rangoon"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 525, // Asia/Yangon
	},
	// 241: Link US/East-Indiana -> America/Indiana/Indianapolis
	{
		ZoneID: 0x6dcf558a,
		NameIndex: 584, // "US/East-Indiana"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 107, // America/Indiana/Indianapolis
	},
	// 242: Zone America/Grand_Turk
	{
		ZoneID: 0x6e216197,
		NameIndex: 119, // "America/Grand_Turk"
		EraIndex: 68,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 243: Link America/Blanc-Sablon -> America/Puerto_Rico
	{
		ZoneID: 0x6e299892,
		NameIndex: 81, // "America/Blanc-Sablon"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 244: Zone Europe/Tirane
	{
		ZoneID: 0x6ea95b47,
		NameIndex: 478, // "Europe/Tirane"
		EraIndex: 305,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 245: Link US/Mountain -> America/Denver
	{
		ZoneID: 0x6eb88247,
		NameIndex: 589, // "US/Mountain"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 350, // America/Denver
	},
	// 246: Link Antarctica/McMurdo -> Pacific/Auckland
	{
		ZoneID: 0x6eeb5585,
		NameIndex: 228, // "Antarctica/McMurdo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 98, // Pacific/Auckland
	},
	// 247: Zone America/Araguaina
	{
		ZoneID: 0x6f9a3aef,
		NameIndex: 58, // "America/Araguaina"
		EraIndex: 21,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 248: Link Africa/Lubumbashi -> Africa/Maputo
	{
		ZoneID: 0x6fd88566,
		NameIndex: 35, // "Africa/Lubumbashi"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 249: Link Indian/Reunion -> Asia/Dubai
	{
		ZoneID: 0x7076c047,
		NameIndex: 511, // "Indian/Reunion"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 60, // Asia/Dubai
	},
	// 250: Zone Asia/Qyzylorda
	{
		ZoneID: 0x71282e81,
		NameIndex: 305, // "Asia/Qyzylorda"
		EraIndex: 204,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 251: Zone Asia/Kolkata
	{
		ZoneID: 0x72c06cd9,
		NameIndex: 284, // "Asia/Kolkata"
		EraIndex: 187,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 252: Zone America/Ciudad_Juarez
	{
		ZoneID: 0x7347fc60,
		NameIndex: 95, // "America/Ciudad_Juarez"
		EraIndex: 51,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 253: Zone Europe/Vienna
	{
		ZoneID: 0x734cc2e5,
		NameIndex: 484, // "Europe/Vienna"
		EraIndex: 307,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 254: Link Africa/Asmara -> Africa/Nairobi
	{
		ZoneID: 0x73b278ef,
		NameIndex: 4, // "Africa/Asmara"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 255: Link Africa/Asmera -> Africa/Nairobi
	{
		ZoneID: 0x73b289f3,
		NameIndex: 5, // "Africa/Asmera"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 256: Zone Asia/Kamchatka
	{
		ZoneID: 0x73baf9d7,
		NameIndex: 278, // "Asia/Kamchatka"
		EraIndex: 183,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 257: Zone America/Santarem
	{
		ZoneID: 0x740caec1,
		NameIndex: 197, // "America/Santarem"
		EraIndex: 126,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 258: Zone America/Santiago
	{
		ZoneID: 0x7410c9bc,
		NameIndex: 198, // "America/Santiago"
		EraIndex: 127,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 259: Link Africa/Bamako -> Africa/Abidjan
	{
		ZoneID: 0x74c1e7a5,
		NameIndex: 6, // "Africa/Bamako"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 260: Link Africa/Bangui -> Africa/Lagos
	{
		ZoneID: 0x74c28ed0,
		NameIndex: 7, // "Africa/Bangui"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 261: Link Africa/Banjul -> Africa/Abidjan
	{
		ZoneID: 0x74c29b96,
		NameIndex: 8, // "Africa/Banjul"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 262: Link Europe/Nicosia -> Asia/Nicosia
	{
		ZoneID: 0x74efab8a,
		NameIndex: 462, // "Europe/Nicosia"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 166, // Asia/Nicosia
	},
	// 263: Zone Europe/Warsaw
	{
		ZoneID: 0x75185c19,
		NameIndex: 487, // "Europe/Warsaw"
		EraIndex: 310,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 264: Zone America/El_Salvador
	{
		ZoneID: 0x752ad652,
		NameIndex: 111, // "America/El_Salvador"
		EraIndex: 63,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 265: Zone Africa/Bissau
	{
		ZoneID: 0x75564141,
		NameIndex: 9, // "Africa/Bissau"
		EraIndex: 2,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 266: Zone America/Santo_Domingo
	{
		ZoneID: 0x75a0d177,
		NameIndex: 199, // "America/Santo_Domingo"
		EraIndex: 128,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 267: Link US/Michigan -> America/Detroit
	{
		ZoneID: 0x766bb7bc,
		NameIndex: 588, // "US/Michigan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 334, // America/Detroit
	},
	// 268: Link Canada/Saskatchewan -> America/Regina
	{
		ZoneID: 0x77311f49,
		NameIndex: 382, // "Canada/Saskatchewan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 452, // America/Regina
	},
	// 269: Link Africa/Accra -> Africa/Abidjan
	{
		ZoneID: 0x77d5b054,
		NameIndex: 1, // "Africa/Accra"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 270: Zone Africa/Cairo
	{
		ZoneID: 0x77f8e228,
		NameIndex: 13, // "Africa/Cairo"
		EraIndex: 3,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 271: Zone Africa/Ceuta
	{
		ZoneID: 0x77fb46ec,
		NameIndex: 15, // "Africa/Ceuta"
		EraIndex: 5,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 272: Link Africa/Dakar -> Africa/Abidjan
	{
		ZoneID: 0x780b00fd,
		NameIndex: 17, // "Africa/Dakar"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 273: Zone Africa/Lagos
	{
		ZoneID: 0x789bb5d0,
		NameIndex: 31, // "Africa/Lagos"
		EraIndex: 10,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 274: Zone Africa/Windhoek
	{
		ZoneID: 0x789c9bd3,
		NameIndex: 53, // "Africa/Windhoek"
		EraIndex: 18,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 275: Link Canada/Yukon -> America/Whitehorse
	{
		ZoneID: 0x78dd35c2,
		NameIndex: 383, // "Canada/Yukon"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 180, // America/Whitehorse
	},
	// 276: Zone America/Toronto
	{
		ZoneID: 0x792e851b,
		NameIndex: 215, // "America/Toronto"
		EraIndex: 138,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 277: Link America/Tortola -> America/Puerto_Rico
	{
		ZoneID: 0x7931462b,
		NameIndex: 216, // "America/Tortola"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 278: Zone Africa/Tunis
	{
		ZoneID: 0x79378e6d,
		NameIndex: 52, // "Africa/Tunis"
		EraIndex: 17,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 279: Link Africa/Douala -> Africa/Lagos
	{
		ZoneID: 0x7a6df310,
		NameIndex: 20, // "Africa/Douala"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 280: Link Africa/Conakry -> Africa/Abidjan
	{
		ZoneID: 0x7ab36b31,
		NameIndex: 16, // "Africa/Conakry"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 281: Zone Indian/Mauritius
	{
		ZoneID: 0x7b09c02a,
		NameIndex: 509, // "Indian/Mauritius"
		EraIndex: 314,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 282: Zone Atlantic/Stanley
	{
		ZoneID: 0x7bb3e1c4,
		NameIndex: 346, // "Atlantic/Stanley"
		EraIndex: 234,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 283: Link America/Ensenada -> America/Tijuana
	{
		ZoneID: 0x7bc95445,
		NameIndex: 112, // "America/Ensenada"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 231, // America/Tijuana
	},
	// 284: Link Europe/Zagreb -> Europe/Belgrade
	{
		ZoneID: 0x7c11c9ff,
		NameIndex: 488, // "Europe/Zagreb"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 536, // Europe/Belgrade
	},
	// 285: Link Cuba -> America/Havana
	{
		ZoneID: 0x7c83cba0,
		NameIndex: 386, // "Cuba"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 404, // America/Havana
	},
	// 286: Link Eire -> Europe/Dublin
	{
		ZoneID: 0x7c84b36a,
		NameIndex: 391, // "Eire"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 165, // Europe/Dublin
	},
	// 287: Link GMT0 -> Etc/GMT
	{
		ZoneID: 0x7c8550fd,
		NameIndex: 496, // "GMT0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 514, // Etc/GMT
	},
	// 288: Link Iran -> Asia/Tehran
	{
		ZoneID: 0x7c87090f,
		NameIndex: 512, // "Iran"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 501, // Asia/Tehran
	},
	// 289: Link W-SU -> Europe/Moscow
	{
		ZoneID: 0x7c8d8ef1,
		NameIndex: 594, // "W-SU"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 205, // Europe/Moscow
	},
	// 290: Link Zulu -> Etc/UTC
	{
		ZoneID: 0x7c9069b5,
		NameIndex: 596, // "Zulu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 516, // Etc/UTC
	},
	// 291: Zone Europe/Zurich
	{
		ZoneID: 0x7d8195b9,
		NameIndex: 490, // "Europe/Zurich"
		EraIndex: 311,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 292: Link Chile/Continental -> America/Santiago
	{
		ZoneID: 0x7e2bdb18,
		NameIndex: 384, // "Chile/Continental"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 258, // America/Santiago
	},
	// 293: Link America/Fort_Wayne -> America/Indiana/Indianapolis
	{
		ZoneID: 0x7eaaaf24,
		NameIndex: 114, // "America/Fort_Wayne"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 107, // America/Indiana/Indianapolis
	},
	// 294: Zone Asia/Kuching
	{
		ZoneID: 0x801b003b,
		NameIndex: 287, // "Asia/Kuching"
		EraIndex: 189,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 295: Zone Atlantic/Madeira
	{
		ZoneID: 0x81b5c037,
		NameIndex: 342, // "Atlantic/Madeira"
		EraIndex: 232,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 296: Link America/Atikokan -> America/Panama
	{
		ZoneID: 0x81b92098,
		NameIndex: 74, // "America/Atikokan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 445, // America/Panama
	},
	// 297: Link Africa/Harare -> Africa/Maputo
	{
		ZoneID: 0x82c39a2d,
		NameIndex: 24, // "Africa/Harare"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 298: Link America/Shiprock -> America/Denver
	{
		ZoneID: 0x82fb7049,
		NameIndex: 202, // "America/Shiprock"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 350, // America/Denver
	},
	// 299: Zone Pacific/Kiritimati
	{
		ZoneID: 0x8305073a,
		NameIndex: 547, // "Pacific/Kiritimati"
		EraIndex: 329,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 300: Link America/St_Vincent -> America/Puerto_Rico
	{
		ZoneID: 0x8460e523,
		NameIndex: 209, // "America/St_Vincent"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 301: Zone America/Metlakatla
	{
		ZoneID: 0x84de2686,
		NameIndex: 161, // "America/Metlakatla"
		EraIndex: 100,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 302: Zone Asia/Yakutsk
	{
		ZoneID: 0x87bb3a9e,
		NameIndex: 331, // "Asia/Yakutsk"
		EraIndex: 223,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 303: Zone America/Chihuahua
	{
		ZoneID: 0x8827d776,
		NameIndex: 94, // "America/Chihuahua"
		EraIndex: 50,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 304: Zone Pacific/Pitcairn
	{
		ZoneID: 0x8837d8bd,
		NameIndex: 559, // "Pacific/Pitcairn"
		EraIndex: 339,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 305: Link Asia/Vientiane -> Asia/Bangkok
	{
		ZoneID: 0x89d68d75,
		NameIndex: 329, // "Asia/Vientiane"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 394, // Asia/Bangkok
	},
	// 306: Link Pacific/Chuuk -> Pacific/Port_Moresby
	{
		ZoneID: 0x8a090b23,
		NameIndex: 533, // "Pacific/Chuuk"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 419, // Pacific/Port_Moresby
	},
	// 307: Zone Pacific/Efate
	{
		ZoneID: 0x8a2bce28,
		NameIndex: 535, // "Pacific/Efate"
		EraIndex: 320,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 308: Link Africa/Kigali -> Africa/Maputo
	{
		ZoneID: 0x8a4dcf2b,
		NameIndex: 29, // "Africa/Kigali"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 309: Link Australia/ACT -> Australia/Sydney
	{
		ZoneID: 0x8a970eb2,
		NameIndex: 347, // "Australia/ACT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 168, // Australia/Sydney
	},
	// 310: Link Australia/LHI -> Australia/Lord_Howe
	{
		ZoneID: 0x8a973e17,
		NameIndex: 356, // "Australia/LHI"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 418, // Australia/Lord_Howe
	},
	// 311: Link Australia/NSW -> Australia/Sydney
	{
		ZoneID: 0x8a974812,
		NameIndex: 360, // "Australia/NSW"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 168, // Australia/Sydney
	},
	// 312: Zone Pacific/Nauru
	{
		ZoneID: 0x8acc41ae,
		NameIndex: 553, // "Pacific/Nauru"
		EraIndex: 333,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 313: Link EST5EDT -> America/New_York
	{
		ZoneID: 0x8adc72a3,
		NameIndex: 389, // "EST5EDT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 78, // America/New_York
	},
	// 314: Zone Pacific/Palau
	{
		ZoneID: 0x8af04a36,
		NameIndex: 558, // "Pacific/Palau"
		EraIndex: 338,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 315: Link Pacific/Samoa -> Pacific/Pago_Pago
	{
		ZoneID: 0x8b2699b4,
		NameIndex: 565, // "Pacific/Samoa"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 208, // Pacific/Pago_Pago
	},
	// 316: Zone America/Winnipeg
	{
		ZoneID: 0x8c7dafc7,
		NameIndex: 220, // "America/Winnipeg"
		EraIndex: 141,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 317: Zone Australia/Eucla
	{
		ZoneID: 0x8cf99e44,
		NameIndex: 354, // "Australia/Eucla"
		EraIndex: 239,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 318: Zone America/Argentina/Catamarca
	{
		ZoneID: 0x8d40986b,
		NameIndex: 60, // "America/Argentina/Catamarca"
		EraIndex: 23,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 319: Link Africa/Luanda -> Africa/Lagos
	{
		ZoneID: 0x8d7909cf,
		NameIndex: 34, // "Africa/Luanda"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 320: Link Africa/Lusaka -> Africa/Maputo
	{
		ZoneID: 0x8d82b23b,
		NameIndex: 36, // "Africa/Lusaka"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 321: Link Australia/North -> Australia/Darwin
	{
		ZoneID: 0x8d997165,
		NameIndex: 361, // "Australia/North"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 103, // Australia/Darwin
	},
	// 322: Zone Australia/Perth
	{
		ZoneID: 0x8db8269d,
		NameIndex: 362, // "Australia/Perth"
		EraIndex: 244,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 323: Link Australia/South -> Australia/Adelaide
	{
		ZoneID: 0x8df3f8ad,
		NameIndex: 364, // "Australia/South"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 97, // Australia/Adelaide
	},
	// 324: Zone Pacific/Kwajalein
	{
		ZoneID: 0x8e216759,
		NameIndex: 549, // "Pacific/Kwajalein"
		EraIndex: 331,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 325: Zone America/Port-au-Prince
	{
		ZoneID: 0x8e4a7bdc,
		NameIndex: 183, // "America/Port-au-Prince"
		EraIndex: 117,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 326: Link Africa/Malabo -> Africa/Lagos
	{
		ZoneID: 0x8e6a1906,
		NameIndex: 37, // "Africa/Malabo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 327: Zone Africa/Maputo
	{
		ZoneID: 0x8e6ca1f0,
		NameIndex: 38, // "Africa/Maputo"
		EraIndex: 11,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 328: Link Africa/Maseru -> Africa/Johannesburg
	{
		ZoneID: 0x8e6e02c7,
		NameIndex: 39, // "Africa/Maseru"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 512, // Africa/Johannesburg
	},
	// 329: Zone Pacific/Norfolk
	{
		ZoneID: 0x8f4eb4be,
		NameIndex: 555, // "Pacific/Norfolk"
		EraIndex: 335,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 330: Link America/Godthab -> America/Nuuk
	{
		ZoneID: 0x8f7eba1f,
		NameIndex: 117, // "America/Godthab"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 359, // America/Nuuk
	},
	// 331: Link Australia/Yancowinna -> Australia/Broken_Hill
	{
		ZoneID: 0x90bac131,
		NameIndex: 369, // "Australia/Yancowinna"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 438, // Australia/Broken_Hill
	},
	// 332: Link Africa/Niamey -> Africa/Lagos
	{
		ZoneID: 0x914a30fd,
		NameIndex: 45, // "Africa/Niamey"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 273, // Africa/Lagos
	},
	// 333: Zone Asia/Yerevan
	{
		ZoneID: 0x9185c8cc,
		NameIndex: 334, // "Asia/Yerevan"
		EraIndex: 226,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 334: Zone America/Detroit
	{
		ZoneID: 0x925cfbc1,
		NameIndex: 107, // "America/Detroit"
		EraIndex: 60,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 335: Link Asia/Choibalsan -> Asia/Ulaanbaatar
	{
		ZoneID: 0x928aa4a6,
		NameIndex: 255, // "Asia/Choibalsan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 120, // Asia/Ulaanbaatar
	},
	// 336: Zone Antarctica/Macquarie
	{
		ZoneID: 0x92f47626,
		NameIndex: 226, // "Antarctica/Macquarie"
		EraIndex: 145,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 337: Zone America/Belize
	{
		ZoneID: 0x93256c81,
		NameIndex: 80, // "America/Belize"
		EraIndex: 40,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 338: Link Mexico/General -> America/Mexico_City
	{
		ZoneID: 0x93711d57,
		NameIndex: 523, // "Mexico/General"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 498, // America/Mexico_City
	},
	// 339: Zone America/Bogota
	{
		ZoneID: 0x93d7bc62,
		NameIndex: 83, // "America/Bogota"
		EraIndex: 42,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 340: Zone Asia/Pyongyang
	{
		ZoneID: 0x93ed1c8e,
		NameIndex: 302, // "Asia/Pyongyang"
		EraIndex: 200,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 341: Zone America/Indiana/Petersburg
	{
		ZoneID: 0x94ac7acc,
		NameIndex: 131, // "America/Indiana/Petersburg"
		EraIndex: 78,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 342: Zone America/Cancun
	{
		ZoneID: 0x953331be,
		NameIndex: 88, // "America/Cancun"
		EraIndex: 46,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 343: Link America/Cayman -> America/Panama
	{
		ZoneID: 0x953961df,
		NameIndex: 92, // "America/Cayman"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 445, // America/Panama
	},
	// 344: Zone America/Glace_Bay
	{
		ZoneID: 0x9681f8dd,
		NameIndex: 116, // "America/Glace_Bay"
		EraIndex: 66,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 345: Zone Asia/Khandyga
	{
		ZoneID: 0x9685a4d9,
		NameIndex: 283, // "Asia/Khandyga"
		EraIndex: 186,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 346: Link America/Grenada -> America/Puerto_Rico
	{
		ZoneID: 0x968ce4d8,
		NameIndex: 120, // "America/Grenada"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 347: Zone America/Cuiaba
	{
		ZoneID: 0x969a52eb,
		NameIndex: 101, // "America/Cuiaba"
		EraIndex: 55,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 348: Zone America/Dawson
	{
		ZoneID: 0x978d8d12,
		NameIndex: 104, // "America/Dawson"
		EraIndex: 57,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 349: Link America/Aruba -> America/Puerto_Rico
	{
		ZoneID: 0x97cf8651,
		NameIndex: 72, // "America/Aruba"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 350: Zone America/Denver
	{
		ZoneID: 0x97d10b2a,
		NameIndex: 106, // "America/Denver"
		EraIndex: 59,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 351: Zone America/Bahia
	{
		ZoneID: 0x97d815fb,
		NameIndex: 76, // "America/Bahia"
		EraIndex: 36,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 352: Zone America/Belem
	{
		ZoneID: 0x97da580b,
		NameIndex: 79, // "America/Belem"
		EraIndex: 39,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 353: Zone America/Boise
	{
		ZoneID: 0x97dfc8d8,
		NameIndex: 84, // "America/Boise"
		EraIndex: 43,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 354: Zone Europe/Andorra
	{
		ZoneID: 0x97f6764b,
		NameIndex: 428, // "Europe/Andorra"
		EraIndex: 274,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 355: Zone America/Adak
	{
		ZoneID: 0x97fe49d7,
		NameIndex: 54, // "America/Adak"
		EraIndex: 19,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 356: Link America/Atka -> America/Adak
	{
		ZoneID: 0x97fe8f27,
		NameIndex: 75, // "America/Atka"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 355, // America/Adak
	},
	// 357: Zone America/Lima
	{
		ZoneID: 0x980468c9,
		NameIndex: 147, // "America/Lima"
		EraIndex: 90,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 358: Zone America/Nome
	{
		ZoneID: 0x98059b15,
		NameIndex: 172, // "America/Nome"
		EraIndex: 107,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 359: Zone America/Nuuk
	{
		ZoneID: 0x9805b5a9,
		NameIndex: 177, // "America/Nuuk"
		EraIndex: 112,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 360: Zone Indian/Maldives
	{
		ZoneID: 0x9869681c,
		NameIndex: 508, // "Indian/Maldives"
		EraIndex: 313,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 361: Link America/Jujuy -> America/Argentina/Jujuy
	{
		ZoneID: 0x9873dbbd,
		NameIndex: 140, // "America/Jujuy"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 206, // America/Argentina/Jujuy
	},
	// 362: Zone America/Sitka
	{
		ZoneID: 0x99104ce2,
		NameIndex: 203, // "America/Sitka"
		EraIndex: 132,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 363: Zone America/Thule
	{
		ZoneID: 0x9921dd68,
		NameIndex: 212, // "America/Thule"
		EraIndex: 136,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 364: Zone Pacific/Rarotonga
	{
		ZoneID: 0x9981a3b0,
		NameIndex: 563, // "Pacific/Rarotonga"
		EraIndex: 341,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 365: Zone Asia/Kathmandu
	{
		ZoneID: 0x9a96ce6f,
		NameIndex: 281, // "Asia/Kathmandu"
		EraIndex: 185,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 366: Link Brazil/DeNoronha -> America/Noronha
	{
		ZoneID: 0x9b4cb496,
		NameIndex: 371, // "Brazil/DeNoronha"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 429, // America/Noronha
	},
	// 367: Zone America/North_Dakota/Beulah
	{
		ZoneID: 0x9b52b384,
		NameIndex: 174, // "America/North_Dakota/Beulah"
		EraIndex: 109,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 368: Link America/Rainy_River -> America/Winnipeg
	{
		ZoneID: 0x9cd58a10,
		NameIndex: 189, // "America/Rainy_River"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 316, // America/Winnipeg
	},
	// 369: Zone Europe/Budapest
	{
		ZoneID: 0x9ce0197c,
		NameIndex: 437, // "Europe/Budapest"
		EraIndex: 281,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 370: Zone Asia/Baghdad
	{
		ZoneID: 0x9ceffbed,
		NameIndex: 245, // "Asia/Baghdad"
		EraIndex: 159,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 371: Link Asia/Bahrain -> Asia/Qatar
	{
		ZoneID: 0x9d078487,
		NameIndex: 246, // "Asia/Bahrain"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 66, // Asia/Qatar
	},
	// 372: Link Etc/GMT+0 -> Etc/GMT
	{
		ZoneID: 0x9d13da13,
		NameIndex: 393, // "Etc/GMT+0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 514, // Etc/GMT
	},
	// 373: Zone Etc/GMT+1
	{
		ZoneID: 0x9d13da14,
		NameIndex: 394, // "Etc/GMT+1"
		EraIndex: 247,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 374: Zone Etc/GMT+2
	{
		ZoneID: 0x9d13da15,
		NameIndex: 398, // "Etc/GMT+2"
		EraIndex: 251,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 375: Zone Etc/GMT+3
	{
		ZoneID: 0x9d13da16,
		NameIndex: 399, // "Etc/GMT+3"
		EraIndex: 252,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 376: Zone Etc/GMT+4
	{
		ZoneID: 0x9d13da17,
		NameIndex: 400, // "Etc/GMT+4"
		EraIndex: 253,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 377: Zone Etc/GMT+5
	{
		ZoneID: 0x9d13da18,
		NameIndex: 401, // "Etc/GMT+5"
		EraIndex: 254,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 378: Zone Etc/GMT+6
	{
		ZoneID: 0x9d13da19,
		NameIndex: 402, // "Etc/GMT+6"
		EraIndex: 255,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 379: Zone Etc/GMT+7
	{
		ZoneID: 0x9d13da1a,
		NameIndex: 403, // "Etc/GMT+7"
		EraIndex: 256,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 380: Zone Etc/GMT+8
	{
		ZoneID: 0x9d13da1b,
		NameIndex: 404, // "Etc/GMT+8"
		EraIndex: 257,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 381: Zone Etc/GMT+9
	{
		ZoneID: 0x9d13da1c,
		NameIndex: 405, // "Etc/GMT+9"
		EraIndex: 258,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 382: Link Etc/GMT-0 -> Etc/GMT
	{
		ZoneID: 0x9d13da55,
		NameIndex: 406, // "Etc/GMT-0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 514, // Etc/GMT
	},
	// 383: Zone Etc/GMT-1
	{
		ZoneID: 0x9d13da56,
		NameIndex: 407, // "Etc/GMT-1"
		EraIndex: 259,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 384: Zone Etc/GMT-2
	{
		ZoneID: 0x9d13da57,
		NameIndex: 413, // "Etc/GMT-2"
		EraIndex: 265,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 385: Zone Etc/GMT-3
	{
		ZoneID: 0x9d13da58,
		NameIndex: 414, // "Etc/GMT-3"
		EraIndex: 266,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 386: Zone Etc/GMT-4
	{
		ZoneID: 0x9d13da59,
		NameIndex: 415, // "Etc/GMT-4"
		EraIndex: 267,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 387: Zone Etc/GMT-5
	{
		ZoneID: 0x9d13da5a,
		NameIndex: 416, // "Etc/GMT-5"
		EraIndex: 268,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 388: Zone Etc/GMT-6
	{
		ZoneID: 0x9d13da5b,
		NameIndex: 417, // "Etc/GMT-6"
		EraIndex: 269,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 389: Zone Etc/GMT-7
	{
		ZoneID: 0x9d13da5c,
		NameIndex: 418, // "Etc/GMT-7"
		EraIndex: 270,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 390: Zone Etc/GMT-8
	{
		ZoneID: 0x9d13da5d,
		NameIndex: 419, // "Etc/GMT-8"
		EraIndex: 271,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 391: Zone Etc/GMT-9
	{
		ZoneID: 0x9d13da5e,
		NameIndex: 420, // "Etc/GMT-9"
		EraIndex: 272,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 392: Link America/Nipigon -> America/Toronto
	{
		ZoneID: 0x9d2a8b1a,
		NameIndex: 171, // "America/Nipigon"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 276, // America/Toronto
	},
	// 393: Zone America/Rio_Branco
	{
		ZoneID: 0x9d352764,
		NameIndex: 194, // "America/Rio_Branco"
		EraIndex: 125,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 394: Zone Asia/Bangkok
	{
		ZoneID: 0x9d6e3aaf,
		NameIndex: 248, // "Asia/Bangkok"
		EraIndex: 161,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 395: Zone Africa/El_Aaiun
	{
		ZoneID: 0x9d6fb118,
		NameIndex: 21, // "Africa/El_Aaiun"
		EraIndex: 6,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 396: Zone America/North_Dakota/Center
	{
		ZoneID: 0x9da42814,
		NameIndex: 175, // "America/North_Dakota/Center"
		EraIndex: 110,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 397: Zone Asia/Barnaul
	{
		ZoneID: 0x9dba4997,
		NameIndex: 249, // "Asia/Barnaul"
		EraIndex: 162,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 398: Zone Africa/Tripoli
	{
		ZoneID: 0x9dfebd3d,
		NameIndex: 51, // "Africa/Tripoli"
		EraIndex: 16,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 399: Zone Europe/Istanbul
	{
		ZoneID: 0x9e09d6e6,
		NameIndex: 446, // "Europe/Istanbul"
		EraIndex: 286,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 400: Link Indian/Antananarivo -> Africa/Nairobi
	{
		ZoneID: 0x9ebf5289,
		NameIndex: 501, // "Indian/Antananarivo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 401: Zone Africa/Ndjamena
	{
		ZoneID: 0x9fe09898,
		NameIndex: 44, // "Africa/Ndjamena"
		EraIndex: 14,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 402: Zone America/Guyana
	{
		ZoneID: 0x9ff7bd0b,
		NameIndex: 124, // "America/Guyana"
		EraIndex: 71,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 403: Link Africa/Dar_es_Salaam -> Africa/Nairobi
	{
		ZoneID: 0xa04c47b6,
		NameIndex: 18, // "Africa/Dar_es_Salaam"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 404: Zone America/Havana
	{
		ZoneID: 0xa0e15675,
		NameIndex: 126, // "America/Havana"
		EraIndex: 73,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 405: Zone Asia/Novosibirsk
	{
		ZoneID: 0xa2a435cb,
		NameIndex: 297, // "Asia/Novosibirsk"
		EraIndex: 196,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 406: Link Europe/Kiev -> Europe/Kyiv
	{
		ZoneID: 0xa2c19eb3,
		NameIndex: 449, // "Europe/Kiev"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 407, // Europe/Kyiv
	},
	// 407: Zone Europe/Kyiv
	{
		ZoneID: 0xa2c1e347,
		NameIndex: 451, // "Europe/Kyiv"
		EraIndex: 289,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 408: Link Europe/Oslo -> Europe/Berlin
	{
		ZoneID: 0xa2c3fba1,
		NameIndex: 463, // "Europe/Oslo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 158, // Europe/Berlin
	},
	// 409: Zone Europe/Riga
	{
		ZoneID: 0xa2c57587,
		NameIndex: 467, // "Europe/Riga"
		EraIndex: 298,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 410: Zone Europe/Rome
	{
		ZoneID: 0xa2c58fd7,
		NameIndex: 468, // "Europe/Rome"
		EraIndex: 299,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 411: Zone America/Inuvik
	{
		ZoneID: 0xa42189fc,
		NameIndex: 137, // "America/Inuvik"
		EraIndex: 83,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 412: Zone America/Argentina/La_Rioja
	{
		ZoneID: 0xa46b7eef,
		NameIndex: 64, // "America/Argentina/La_Rioja"
		EraIndex: 26,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 413: Zone Asia/Almaty
	{
		ZoneID: 0xa61f41fa,
		NameIndex: 237, // "Asia/Almaty"
		EraIndex: 151,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 414: Zone Asia/Anadyr
	{
		ZoneID: 0xa63cebd1,
		NameIndex: 239, // "Asia/Anadyr"
		EraIndex: 154,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 415: Zone Asia/Aqtobe
	{
		ZoneID: 0xa67dcc4e,
		NameIndex: 241, // "Asia/Aqtobe"
		EraIndex: 156,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 416: Zone Asia/Atyrau
	{
		ZoneID: 0xa6b6e068,
		NameIndex: 244, // "Asia/Atyrau"
		EraIndex: 158,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 417: Zone America/Juneau
	{
		ZoneID: 0xa6f13e2e,
		NameIndex: 141, // "America/Juneau"
		EraIndex: 86,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 418: Zone Australia/Lord_Howe
	{
		ZoneID: 0xa748b67d,
		NameIndex: 358, // "Australia/Lord_Howe"
		EraIndex: 242,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 419: Zone Pacific/Port_Moresby
	{
		ZoneID: 0xa7ba7f68,
		NameIndex: 562, // "Pacific/Port_Moresby"
		EraIndex: 340,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 420: Link Asia/Katmandu -> Asia/Kathmandu
	{
		ZoneID: 0xa7ec12c7,
		NameIndex: 282, // "Asia/Katmandu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 365, // Asia/Kathmandu
	},
	// 421: Zone Asia/Beirut
	{
		ZoneID: 0xa7f3d5fd,
		NameIndex: 250, // "Asia/Beirut"
		EraIndex: 163,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 422: Link Singapore -> Asia/Singapore
	{
		ZoneID: 0xa8598c8d,
		NameIndex: 577, // "Singapore"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 495, // Asia/Singapore
	},
	// 423: Zone Africa/Nairobi
	{
		ZoneID: 0xa87ab57e,
		NameIndex: 43, // "Africa/Nairobi"
		EraIndex: 13,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 424: Link Asia/Brunei -> Asia/Kuching
	{
		ZoneID: 0xa8e595f7,
		NameIndex: 252, // "Asia/Brunei"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 294, // Asia/Kuching
	},
	// 425: Link US/Pacific -> America/Los_Angeles
	{
		ZoneID: 0xa950f6ab,
		NameIndex: 590, // "US/Pacific"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 450, // America/Los_Angeles
	},
	// 426: Zone Pacific/Galapagos
	{
		ZoneID: 0xa952f752,
		NameIndex: 540, // "Pacific/Galapagos"
		EraIndex: 323,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 427: Zone America/Argentina/Mendoza
	{
		ZoneID: 0xa9f72d5c,
		NameIndex: 65, // "America/Argentina/Mendoza"
		EraIndex: 27,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 428: Zone America/La_Paz
	{
		ZoneID: 0xaa29125d,
		NameIndex: 146, // "America/La_Paz"
		EraIndex: 89,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 429: Zone America/Noronha
	{
		ZoneID: 0xab5116fb,
		NameIndex: 173, // "America/Noronha"
		EraIndex: 108,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 430: Link America/Coral_Harbour -> America/Panama
	{
		ZoneID: 0xabcb7569,
		NameIndex: 96, // "America/Coral_Harbour"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 445, // America/Panama
	},
	// 431: Zone America/Maceio
	{
		ZoneID: 0xac80c6d4,
		NameIndex: 151, // "America/Maceio"
		EraIndex: 92,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 432: Zone America/Manaus
	{
		ZoneID: 0xac86bf8b,
		NameIndex: 153, // "America/Manaus"
		EraIndex: 94,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 433: Zone America/Merida
	{
		ZoneID: 0xacd172d8,
		NameIndex: 160, // "America/Merida"
		EraIndex: 99,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 434: Zone Europe/Chisinau
	{
		ZoneID: 0xad58aa18,
		NameIndex: 439, // "Europe/Chisinau"
		EraIndex: 282,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 435: Link America/Nassau -> America/Toronto
	{
		ZoneID: 0xaedef011,
		NameIndex: 169, // "America/Nassau"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 276, // America/Toronto
	},
	// 436: Link America/Anguilla -> America/Puerto_Rico
	{
		ZoneID: 0xafe31333,
		NameIndex: 56, // "America/Anguilla"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 437: Link Europe/Uzhgorod -> Europe/Kyiv
	{
		ZoneID: 0xb066f5d6,
		NameIndex: 481, // "Europe/Uzhgorod"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 407, // Europe/Kyiv
	},
	// 438: Zone Australia/Broken_Hill
	{
		ZoneID: 0xb06eada3,
		NameIndex: 350, // "Australia/Broken_Hill"
		EraIndex: 237,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 439: Zone Asia/Bishkek
	{
		ZoneID: 0xb0728553,
		NameIndex: 251, // "Asia/Bishkek"
		EraIndex: 164,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 440: Link Chile/EasterIsland -> Pacific/Easter
	{
		ZoneID: 0xb0982af8,
		NameIndex: 385, // "Chile/EasterIsland"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 494, // Pacific/Easter
	},
	// 441: Link Pacific/Johnston -> Pacific/Honolulu
	{
		ZoneID: 0xb15d7b36,
		NameIndex: 545, // "Pacific/Johnston"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 553, // Pacific/Honolulu
	},
	// 442: Link Africa/Timbuktu -> Africa/Abidjan
	{
		ZoneID: 0xb164d56f,
		NameIndex: 50, // "Africa/Timbuktu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 443: Link America/St_Thomas -> America/Puerto_Rico
	{
		ZoneID: 0xb1b3d778,
		NameIndex: 208, // "America/St_Thomas"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 444: Zone America/Paramaribo
	{
		ZoneID: 0xb319e4c4,
		NameIndex: 181, // "America/Paramaribo"
		EraIndex: 115,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 445: Zone America/Panama
	{
		ZoneID: 0xb3863854,
		NameIndex: 179, // "America/Panama"
		EraIndex: 114,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 446: Link Canada/Newfoundland -> America/St_Johns
	{
		ZoneID: 0xb396e991,
		NameIndex: 380, // "Canada/Newfoundland"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 7, // America/St_Johns
	},
	// 447: Link Asia/Harbin -> Asia/Shanghai
	{
		ZoneID: 0xb5af1186,
		NameIndex: 267, // "Asia/Harbin"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 573, // Asia/Shanghai
	},
	// 448: Zone Asia/Hebron
	{
		ZoneID: 0xb5eef250,
		NameIndex: 268, // "Asia/Hebron"
		EraIndex: 174,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 449: Zone America/Goose_Bay
	{
		ZoneID: 0xb649541e,
		NameIndex: 118, // "America/Goose_Bay"
		EraIndex: 67,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 450: Zone America/Los_Angeles
	{
		ZoneID: 0xb7f7e8f2,
		NameIndex: 148, // "America/Los_Angeles"
		EraIndex: 91,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 451: Zone America/Recife
	{
		ZoneID: 0xb8730494,
		NameIndex: 191, // "America/Recife"
		EraIndex: 122,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 452: Zone America/Regina
	{
		ZoneID: 0xb875371c,
		NameIndex: 192, // "America/Regina"
		EraIndex: 123,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 453: Zone Asia/Ashgabat
	{
		ZoneID: 0xba87598d,
		NameIndex: 242, // "Asia/Ashgabat"
		EraIndex: 157,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 454: Link Israel -> Asia/Jerusalem
	{
		ZoneID: 0xba88c9e5,
		NameIndex: 513, // "Israel"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 193, // Asia/Jerusalem
	},
	// 455: Link Pacific/Yap -> Pacific/Port_Moresby
	{
		ZoneID: 0xbb40138d,
		NameIndex: 572, // "Pacific/Yap"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 419, // Pacific/Port_Moresby
	},
	// 456: Zone America/Halifax
	{
		ZoneID: 0xbc5b7183,
		NameIndex: 125, // "America/Halifax"
		EraIndex: 72,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 457: Link Europe/Ljubljana -> Europe/Belgrade
	{
		ZoneID: 0xbd98cdb7,
		NameIndex: 453, // "Europe/Ljubljana"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 536, // Europe/Belgrade
	},
	// 458: Link Asia/Kuwait -> Asia/Riyadh
	{
		ZoneID: 0xbe1b2f27,
		NameIndex: 288, // "Asia/Kuwait"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 490, // Asia/Riyadh
	},
	// 459: Link Europe/Tiraspol -> Europe/Chisinau
	{
		ZoneID: 0xbe704472,
		NameIndex: 479, // "Europe/Tiraspol"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 434, // Europe/Chisinau
	},
	// 460: Zone Asia/Srednekolymsk
	{
		ZoneID: 0xbf8e337d,
		NameIndex: 314, // "Asia/Srednekolymsk"
		EraIndex: 211,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 461: Zone America/Argentina/Cordoba
	{
		ZoneID: 0xbfccc308,
		NameIndex: 62, // "America/Argentina/Cordoba"
		EraIndex: 24,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 462: Zone America/Tegucigalpa
	{
		ZoneID: 0xbfd6fd4c,
		NameIndex: 211, // "America/Tegucigalpa"
		EraIndex: 135,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 463: Link America/Antigua -> America/Puerto_Rico
	{
		ZoneID: 0xc067a32f,
		NameIndex: 57, // "America/Antigua"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 464: Link Europe/Busingen -> Europe/Zurich
	{
		ZoneID: 0xc06d2cdf,
		NameIndex: 438, // "Europe/Busingen"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 291, // Europe/Zurich
	},
	// 465: Zone Asia/Manila
	{
		ZoneID: 0xc156c944,
		NameIndex: 293, // "Asia/Manila"
		EraIndex: 193,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 466: Link Africa/Kampala -> Africa/Nairobi
	{
		ZoneID: 0xc1d30e31,
		NameIndex: 27, // "Africa/Kampala"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 467: Link America/Knox_IN -> America/Indiana/Knox
	{
		ZoneID: 0xc1db9a1c,
		NameIndex: 144, // "America/Knox_IN"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 215, // America/Indiana/Knox
	},
	// 468: Zone Africa/Abidjan
	{
		ZoneID: 0xc21305a3,
		NameIndex: 0, // "Africa/Abidjan"
		EraIndex: 0,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 469: Link America/Virgin -> America/Puerto_Rico
	{
		ZoneID: 0xc2183ab5,
		NameIndex: 218, // "America/Virgin"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 470: Link Asia/Phnom_Penh -> Asia/Bangkok
	{
		ZoneID: 0xc224945e,
		NameIndex: 300, // "Asia/Phnom_Penh"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 394, // Asia/Bangkok
	},
	// 471: Link Asia/Muscat -> Asia/Dubai
	{
		ZoneID: 0xc2c3565f,
		NameIndex: 294, // "Asia/Muscat"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 60, // Asia/Dubai
	},
	// 472: Zone America/Punta_Arenas
	{
		ZoneID: 0xc2c3bce7,
		NameIndex: 188, // "America/Punta_Arenas"
		EraIndex: 120,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 473: Link Portugal -> Europe/Lisbon
	{
		ZoneID: 0xc3274593,
		NameIndex: 574, // "Portugal"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 195, // Europe/Lisbon
	},
	// 474: Link Navajo -> America/Denver
	{
		ZoneID: 0xc4ef0e24,
		NameIndex: 526, // "Navajo"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 350, // America/Denver
	},
	// 475: Zone Africa/Casablanca
	{
		ZoneID: 0xc59f1b33,
		NameIndex: 14, // "Africa/Casablanca"
		EraIndex: 4,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 476: Zone America/Argentina/Rio_Gallegos
	{
		ZoneID: 0xc5b0f565,
		NameIndex: 66, // "America/Argentina/Rio_Gallegos"
		EraIndex: 28,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 477: Zone Asia/Jayapura
	{
		ZoneID: 0xc6833c2f,
		NameIndex: 275, // "Asia/Jayapura"
		EraIndex: 180,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 478: Zone America/Resolute
	{
		ZoneID: 0xc7093459,
		NameIndex: 193, // "America/Resolute"
		EraIndex: 124,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 479: Link Asia/Chungking -> Asia/Shanghai
	{
		ZoneID: 0xc7121dd0,
		NameIndex: 257, // "Asia/Chungking"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 573, // Asia/Shanghai
	},
	// 480: Link Greenwich -> Etc/GMT
	{
		ZoneID: 0xc84d4221,
		NameIndex: 497, // "Greenwich"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 514, // Etc/GMT
	},
	// 481: Zone America/Rankin_Inlet
	{
		ZoneID: 0xc8de4984,
		NameIndex: 190, // "America/Rankin_Inlet"
		EraIndex: 121,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 482: Link Poland -> Europe/Warsaw
	{
		ZoneID: 0xca913b23,
		NameIndex: 573, // "Poland"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 263, // Europe/Warsaw
	},
	// 483: Link US/Central -> America/Chicago
	{
		ZoneID: 0xcabdcb25,
		NameIndex: 583, // "US/Central"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 167, // America/Chicago
	},
	// 484: Link Europe/Vatican -> Europe/Rome
	{
		ZoneID: 0xcb485dca,
		NameIndex: 483, // "Europe/Vatican"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 410, // Europe/Rome
	},
	// 485: Zone America/Barbados
	{
		ZoneID: 0xcbbc3b04,
		NameIndex: 78, // "America/Barbados"
		EraIndex: 38,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 486: Zone America/Coyhaique
	{
		ZoneID: 0xcc3314ee,
		NameIndex: 99, // "America/Coyhaique"
		EraIndex: 53,
		EraCount: 2,
		TargetIndex: 0,
	},
	// 487: Link America/Porto_Acre -> America/Rio_Branco
	{
		ZoneID: 0xcce5bf54,
		NameIndex: 185, // "America/Porto_Acre"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 393, // America/Rio_Branco
	},
	// 488: Link America/Guadeloupe -> America/Puerto_Rico
	{
		ZoneID: 0xcd1f8a31,
		NameIndex: 121, // "America/Guadeloupe"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 489: Link Antarctica/South_Pole -> Pacific/Auckland
	{
		ZoneID: 0xcd96b290,
		NameIndex: 231, // "Antarctica/South_Pole"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 98, // Pacific/Auckland
	},
	// 490: Zone Asia/Riyadh
	{
		ZoneID: 0xcd973d93,
		NameIndex: 307, // "Asia/Riyadh"
		EraIndex: 205,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 491: Link America/Dominica -> America/Puerto_Rico
	{
		ZoneID: 0xcecb4c4a,
		NameIndex: 108, // "America/Dominica"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 492: Link Europe/San_Marino -> Europe/Rome
	{
		ZoneID: 0xcef7724b,
		NameIndex: 470, // "Europe/San_Marino"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 410, // Europe/Rome
	},
	// 493: Link Asia/Saigon -> Asia/Ho_Chi_Minh
	{
		ZoneID: 0xcf52f713,
		NameIndex: 308, // "Asia/Saigon"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 88, // Asia/Ho_Chi_Minh
	},
	// 494: Zone Pacific/Easter
	{
		ZoneID: 0xcf54f7e7,
		NameIndex: 534, // "Pacific/Easter"
		EraIndex: 319,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 495: Zone Asia/Singapore
	{
		ZoneID: 0xcf8581fa,
		NameIndex: 313, // "Asia/Singapore"
		EraIndex: 210,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 496: Zone Asia/Krasnoyarsk
	{
		ZoneID: 0xd0376c6a,
		NameIndex: 285, // "Asia/Krasnoyarsk"
		EraIndex: 188,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 497: Link Europe/Belfast -> Europe/London
	{
		ZoneID: 0xd07dd1e5,
		NameIndex: 431, // "Europe/Belfast"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 498: Zone America/Mexico_City
	{
		ZoneID: 0xd0d93f43,
		NameIndex: 162, // "America/Mexico_City"
		EraIndex: 101,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 499: Link Universal -> Etc/UTC
	{
		ZoneID: 0xd0ff523e,
		NameIndex: 593, // "Universal"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 516, // Etc/UTC
	},
	// 500: Zone Asia/Taipei
	{
		ZoneID: 0xd1a844ae,
		NameIndex: 315, // "Asia/Taipei"
		EraIndex: 212,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 501: Zone Asia/Tehran
	{
		ZoneID: 0xd1f02254,
		NameIndex: 318, // "Asia/Tehran"
		EraIndex: 215,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 502: Link Asia/Thimbu -> Asia/Thimphu
	{
		ZoneID: 0xd226e31b,
		NameIndex: 320, // "Asia/Thimbu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 71, // Asia/Thimphu
	},
	// 503: Link Arctic/Longyearbyen -> Europe/Berlin
	{
		ZoneID: 0xd23e7859,
		NameIndex: 235, // "Arctic/Longyearbyen"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 158, // Europe/Berlin
	},
	// 504: Link Australia/Queensland -> Australia/Brisbane
	{
		ZoneID: 0xd326ed0a,
		NameIndex: 363, // "Australia/Queensland"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 173, // Australia/Brisbane
	},
	// 505: Zone Europe/Kaliningrad
	{
		ZoneID: 0xd33b2f28,
		NameIndex: 448, // "Europe/Kaliningrad"
		EraIndex: 287,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 506: Zone America/Argentina/Buenos_Aires
	{
		ZoneID: 0xd43b4c0d,
		NameIndex: 59, // "America/Argentina/Buenos_Aires"
		EraIndex: 22,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 507: Link Turkey -> Europe/Istanbul
	{
		ZoneID: 0xd455e469,
		NameIndex: 578, // "Turkey"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 399, // Europe/Istanbul
	},
	// 508: Zone Africa/Juba
	{
		ZoneID: 0xd51b395c,
		NameIndex: 26, // "Africa/Juba"
		EraIndex: 8,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 509: Link Africa/Lome -> Africa/Abidjan
	{
		ZoneID: 0xd51c3a07,
		NameIndex: 33, // "Africa/Lome"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 510: Zone Asia/Urumqi
	{
		ZoneID: 0xd5379735,
		NameIndex: 327, // "Asia/Urumqi"
		EraIndex: 220,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 511: Zone America/Cambridge_Bay
	{
		ZoneID: 0xd5a44aff,
		NameIndex: 86, // "America/Cambridge_Bay"
		EraIndex: 44,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 512: Zone Africa/Johannesburg
	{
		ZoneID: 0xd5d157a0,
		NameIndex: 25, // "Africa/Johannesburg"
		EraIndex: 7,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 513: Link America/Port_of_Spain -> America/Puerto_Rico
	{
		ZoneID: 0xd8b28d59,
		NameIndex: 184, // "America/Port_of_Spain"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 514: Zone Etc/GMT
	{
		ZoneID: 0xd8e2de58,
		NameIndex: 392, // "Etc/GMT"
		EraIndex: 246,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 515: Link Etc/UCT -> Etc/UTC
	{
		ZoneID: 0xd8e3189c,
		NameIndex: 423, // "Etc/UCT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 516, // Etc/UTC
	},
	// 516: Zone Etc/UTC
	{
		ZoneID: 0xd8e31abc,
		NameIndex: 424, // "Etc/UTC"
		EraIndex: 273,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 517: Zone America/Yakutat
	{
		ZoneID: 0xd8ee31e9,
		NameIndex: 221, // "America/Yakutat"
		EraIndex: 142,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 518: Zone Africa/Algiers
	{
		ZoneID: 0xd94515c1,
		NameIndex: 3, // "Africa/Algiers"
		EraIndex: 1,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 519: Link PST8PDT -> America/Los_Angeles
	{
		ZoneID: 0xd99ee2dc,
		NameIndex: 528, // "PST8PDT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 450, // America/Los_Angeles
	},
	// 520: Link Europe/Bratislava -> Europe/Prague
	{
		ZoneID: 0xda493bed,
		NameIndex: 434, // "Europe/Bratislava"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 216, // Europe/Prague
	},
	// 521: Zone Europe/Simferopol
	{
		ZoneID: 0xda9eb724,
		NameIndex: 473, // "Europe/Simferopol"
		EraIndex: 302,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 522: Link Pacific/Funafuti -> Pacific/Tarawa
	{
		ZoneID: 0xdb402d65,
		NameIndex: 539, // "Pacific/Funafuti"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 564, // Pacific/Tarawa
	},
	// 523: Zone America/Matamoros
	{
		ZoneID: 0xdd1b0259,
		NameIndex: 156, // "America/Matamoros"
		EraIndex: 96,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 524: Zone Pacific/Kanton
	{
		ZoneID: 0xdd512f0e,
		NameIndex: 546, // "Pacific/Kanton"
		EraIndex: 328,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 525: Zone Asia/Yangon
	{
		ZoneID: 0xdd54a8be,
		NameIndex: 332, // "Asia/Yangon"
		EraIndex: 224,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 526: Zone Europe/Vilnius
	{
		ZoneID: 0xdd63b8ce,
		NameIndex: 485, // "Europe/Vilnius"
		EraIndex: 308,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 527: Link Australia/West -> Australia/Perth
	{
		ZoneID: 0xdd858a5d,
		NameIndex: 368, // "Australia/West"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 322, // Australia/Perth
	},
	// 528: Zone Pacific/Kosrae
	{
		ZoneID: 0xde5139a8,
		NameIndex: 548, // "Pacific/Kosrae"
		EraIndex: 330,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 529: Zone America/Kentucky/Monticello
	{
		ZoneID: 0xde71c439,
		NameIndex: 143, // "America/Kentucky/Monticello"
		EraIndex: 88,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 530: Zone Europe/Brussels
	{
		ZoneID: 0xdee07337,
		NameIndex: 435, // "Europe/Brussels"
		EraIndex: 279,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 531: Zone America/Swift_Current
	{
		ZoneID: 0xdef98e55,
		NameIndex: 210, // "America/Swift_Current"
		EraIndex: 134,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 532: Link America/Rosario -> America/Argentina/Cordoba
	{
		ZoneID: 0xdf448665,
		NameIndex: 195, // "America/Rosario"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 461, // America/Argentina/Cordoba
	},
	// 533: Zone Asia/Irkutsk
	{
		ZoneID: 0xdfbf213f,
		NameIndex: 272, // "Asia/Irkutsk"
		EraIndex: 178,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 534: Zone Europe/Ulyanovsk
	{
		ZoneID: 0xe03783d0,
		NameIndex: 480, // "Europe/Ulyanovsk"
		EraIndex: 306,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 535: Zone Australia/Lindeman
	{
		ZoneID: 0xe05029e2,
		NameIndex: 357, // "Australia/Lindeman"
		EraIndex: 241,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 536: Zone Europe/Belgrade
	{
		ZoneID: 0xe0532b3a,
		NameIndex: 432, // "Europe/Belgrade"
		EraIndex: 277,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 537: Link Africa/Blantyre -> Africa/Maputo
	{
		ZoneID: 0xe08d813b,
		NameIndex: 10, // "Africa/Blantyre"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 327, // Africa/Maputo
	},
	// 538: Zone America/Menominee
	{
		ZoneID: 0xe0e9c583,
		NameIndex: 159, // "America/Menominee"
		EraIndex: 98,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 539: Link Europe/Copenhagen -> Europe/Berlin
	{
		ZoneID: 0xe0ed30bc,
		NameIndex: 440, // "Europe/Copenhagen"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 158, // Europe/Berlin
	},
	// 540: Zone Atlantic/Faroe
	{
		ZoneID: 0xe110a971,
		NameIndex: 340, // "Atlantic/Faroe"
		EraIndex: 231,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 541: Link Pacific/Majuro -> Pacific/Tarawa
	{
		ZoneID: 0xe1f95371,
		NameIndex: 550, // "Pacific/Majuro"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 564, // Pacific/Tarawa
	},
	// 542: Zone Antarctica/Casey
	{
		ZoneID: 0xe2022583,
		NameIndex: 223, // "Antarctica/Casey"
		EraIndex: 143,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 543: Zone Antarctica/Davis
	{
		ZoneID: 0xe2144b45,
		NameIndex: 224, // "Antarctica/Davis"
		EraIndex: 144,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 544: Zone Europe/Astrakhan
	{
		ZoneID: 0xe22256e1,
		NameIndex: 429, // "Europe/Astrakhan"
		EraIndex: 275,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 545: Link Pacific/Midway -> Pacific/Pago_Pago
	{
		ZoneID: 0xe286d38e,
		NameIndex: 552, // "Pacific/Midway"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 208, // Pacific/Pago_Pago
	},
	// 546: Link Antarctica/Syowa -> Asia/Riyadh
	{
		ZoneID: 0xe330c7e1,
		NameIndex: 232, // "Antarctica/Syowa"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 490, // Asia/Riyadh
	},
	// 547: Zone Antarctica/Troll
	{
		ZoneID: 0xe33f085b,
		NameIndex: 233, // "Antarctica/Troll"
		EraIndex: 149,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 548: Zone Europe/Saratov
	{
		ZoneID: 0xe4315da4,
		NameIndex: 472, // "Europe/Saratov"
		EraIndex: 301,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 549: Zone Pacific/Noumea
	{
		ZoneID: 0xe551b788,
		NameIndex: 556, // "Pacific/Noumea"
		EraIndex: 336,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 550: Link Iceland -> Africa/Abidjan
	{
		ZoneID: 0xe56a35b5,
		NameIndex: 500, // "Iceland"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 468, // Africa/Abidjan
	},
	// 551: Link Indian/Mayotte -> Africa/Nairobi
	{
		ZoneID: 0xe6880bca,
		NameIndex: 510, // "Indian/Mayotte"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 423, // Africa/Nairobi
	},
	// 552: Link Australia/Tasmania -> Australia/Hobart
	{
		ZoneID: 0xe6d76648,
		NameIndex: 366, // "Australia/Tasmania"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 124, // Australia/Hobart
	},
	// 553: Zone Pacific/Honolulu
	{
		ZoneID: 0xe6e70af9,
		NameIndex: 544, // "Pacific/Honolulu"
		EraIndex: 327,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 554: Link America/Kralendijk -> America/Puerto_Rico
	{
		ZoneID: 0xe7c456c5,
		NameIndex: 145, // "America/Kralendijk"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 222, // America/Puerto_Rico
	},
	// 555: Zone America/Argentina/Tucuman
	{
		ZoneID: 0xe96399eb,
		NameIndex: 70, // "America/Argentina/Tucuman"
		EraIndex: 32,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 556: Link Pacific/Ponape -> Pacific/Guadalcanal
	{
		ZoneID: 0xe9f80086,
		NameIndex: 561, // "Pacific/Ponape"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 569, // Pacific/Guadalcanal
	},
	// 557: Link Europe/Zaporozhye -> Europe/Kyiv
	{
		ZoneID: 0xeab9767f,
		NameIndex: 489, // "Europe/Zaporozhye"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 407, // Europe/Kyiv
	},
	// 558: Link Europe/Isle_of_Man -> Europe/London
	{
		ZoneID: 0xeaf84580,
		NameIndex: 445, // "Europe/Isle_of_Man"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 559: Zone Asia/Magadan
	{
		ZoneID: 0xebacc19b,
		NameIndex: 291, // "Asia/Magadan"
		EraIndex: 191,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 560: Zone America/Ojinaga
	{
		ZoneID: 0xebfde83f,
		NameIndex: 178, // "America/Ojinaga"
		EraIndex: 113,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 561: Link Pacific/Saipan -> Pacific/Guam
	{
		ZoneID: 0xeff7a35f,
		NameIndex: 564, // "Pacific/Saipan"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 93, // Pacific/Guam
	},
	// 562: Link CST6CDT -> America/Chicago
	{
		ZoneID: 0xf0e87d00,
		NameIndex: 375, // "CST6CDT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 167, // America/Chicago
	},
	// 563: Zone Pacific/Tahiti
	{
		ZoneID: 0xf24c2446,
		NameIndex: 566, // "Pacific/Tahiti"
		EraIndex: 342,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 564: Zone Pacific/Tarawa
	{
		ZoneID: 0xf2517e63,
		NameIndex: 567, // "Pacific/Tarawa"
		EraIndex: 343,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 565: Link MST7MDT -> America/Denver
	{
		ZoneID: 0xf2af9375,
		NameIndex: 520, // "MST7MDT"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 350, // America/Denver
	},
	// 566: Link Canada/Eastern -> America/Toronto
	{
		ZoneID: 0xf3612d5e,
		NameIndex: 378, // "Canada/Eastern"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 276, // America/Toronto
	},
	// 567: Zone Asia/Tashkent
	{
		ZoneID: 0xf3924254,
		NameIndex: 316, // "Asia/Tashkent"
		EraIndex: 213,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 568: Zone Asia/Sakhalin
	{
		ZoneID: 0xf4a1c9bd,
		NameIndex: 309, // "Asia/Sakhalin"
		EraIndex: 206,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 569: Zone Pacific/Guadalcanal
	{
		ZoneID: 0xf4dd25f0,
		NameIndex: 542, // "Pacific/Guadalcanal"
		EraIndex: 325,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 570: Link Etc/GMT0 -> Etc/GMT
	{
		ZoneID: 0xf53ea988,
		NameIndex: 421, // "Etc/GMT0"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 514, // Etc/GMT
	},
	// 571: Link Etc/Zulu -> Etc/UTC
	{
		ZoneID: 0xf549c240,
		NameIndex: 426, // "Etc/Zulu"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 516, // Etc/UTC
	},
	// 572: Zone America/Danmarkshavn
	{
		ZoneID: 0xf554d204,
		NameIndex: 103, // "America/Danmarkshavn"
		EraIndex: 56,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 573: Zone Asia/Shanghai
	{
		ZoneID: 0xf895a7f5,
		NameIndex: 312, // "Asia/Shanghai"
		EraIndex: 209,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 574: Zone Europe/Gibraltar
	{
		ZoneID: 0xf8e325fc,
		NameIndex: 442, // "Europe/Gibraltar"
		EraIndex: 284,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 575: Link Asia/Chongqing -> Asia/Shanghai
	{
		ZoneID: 0xf937fb90,
		NameIndex: 256, // "Asia/Chongqing"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 573, // Asia/Shanghai
	},
	// 576: Zone Atlantic/Azores
	{
		ZoneID: 0xf93ed918,
		NameIndex: 335, // "Atlantic/Azores"
		EraIndex: 227,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 577: Link Pacific/Wallis -> Pacific/Tarawa
	{
		ZoneID: 0xf94ddb0f,
		NameIndex: 571, // "Pacific/Wallis"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 564, // Pacific/Tarawa
	},
	// 578: Link America/Thunder_Bay -> America/Toronto
	{
		ZoneID: 0xf962e71b,
		NameIndex: 213, // "America/Thunder_Bay"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 276, // America/Toronto
	},
	// 579: Zone America/Eirunepe
	{
		ZoneID: 0xf9b29683,
		NameIndex: 110, // "America/Eirunepe"
		EraIndex: 62,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 580: Zone America/Montevideo
	{
		ZoneID: 0xfa214780,
		NameIndex: 166, // "America/Montevideo"
		EraIndex: 105,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 581: Link US/Alaska -> America/Anchorage
	{
		ZoneID: 0xfa300bc9,
		NameIndex: 580, // "US/Alaska"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 191, // America/Anchorage
	},
	// 582: Link GB-Eire -> Europe/London
	{
		ZoneID: 0xfa70e300,
		NameIndex: 492, // "GB-Eire"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 197, // Europe/London
	},
	// 583: Zone Europe/Kirov
	{
		ZoneID: 0xfaf5abef,
		NameIndex: 450, // "Europe/Kirov"
		EraIndex: 288,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 584: Zone Europe/Malta
	{
		ZoneID: 0xfb1560f3,
		NameIndex: 457, // "Europe/Malta"
		EraIndex: 293,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 585: Zone Europe/Minsk
	{
		ZoneID: 0xfb19cc66,
		NameIndex: 459, // "Europe/Minsk"
		EraIndex: 294,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 586: Zone Europe/Bucharest
	{
		ZoneID: 0xfb349ec5,
		NameIndex: 436, // "Europe/Bucharest"
		EraIndex: 280,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 587: Zone Africa/Khartoum
	{
		ZoneID: 0xfb3d4205,
		NameIndex: 28, // "Africa/Khartoum"
		EraIndex: 9,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 588: Zone Europe/Paris
	{
		ZoneID: 0xfb4bc2a3,
		NameIndex: 464, // "Europe/Paris"
		EraIndex: 296,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 589: Zone Asia/Yekaterinburg
	{
		ZoneID: 0xfb544c6e,
		NameIndex: 333, // "Asia/Yekaterinburg"
		EraIndex: 225,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 590: Zone Europe/Sofia
	{
		ZoneID: 0xfb898656,
		NameIndex: 475, // "Europe/Sofia"
		EraIndex: 303,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 591: Link Canada/Mountain -> America/Edmonton
	{
		ZoneID: 0xfb8a8217,
		NameIndex: 379, // "Canada/Mountain"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 238, // America/Edmonton
	},
	// 592: Link Europe/Vaduz -> Europe/Zurich
	{
		ZoneID: 0xfbb81bae,
		NameIndex: 482, // "Europe/Vaduz"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 291, // Europe/Zurich
	},
	// 593: Zone Atlantic/Canary
	{
		ZoneID: 0xfc23f2c2,
		NameIndex: 337, // "Atlantic/Canary"
		EraIndex: 229,
		EraCount: 1,
		TargetIndex: 0,
	},
	// 594: Link Mexico/BajaNorte -> America/Tijuana
	{
		ZoneID: 0xfcf7150f,
		NameIndex: 521, // "Mexico/BajaNorte"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 231, // America/Tijuana
	},
	// 595: Link America/Santa_Isabel -> America/Tijuana
	{
		ZoneID: 0xfd18a56c,
		NameIndex: 196, // "America/Santa_Isabel"
		EraIndex: 0,
		EraCount: 0, // IsLink=true
		TargetIndex: 231, // America/Tijuana
	},
	// 596: Zone America/Campo_Grande
	{
		ZoneID: 0xfec3e7a6,
		NameIndex: 87, // "America/Campo_Grande"
		EraIndex: 45,
		EraCount: 1,
		TargetIndex: 0,
	},

}
