...
func TestZonedDateTimeFromEpochSeconds_UnixMax(t *testing.T) {
	...
	tz := TimeZoneForZoneInfo(&zonedbtest.ZoneEtc_UTC)
...
func TestZonedDateTimeFromLocalDateTime(t *testing.T) {
	...
	zdt := ZonedDateTimeFromLocalDateTime(&ldt, 0 /*fold*/, &tz)
	if zdt.IsError() {
		t.Fatal(zdt)
	}
...
func TestZonedDateTimeConvertToTimeZone(t *testing.T) {
	...
	tzLosAngeles := TimeZoneForZoneInfo(&zonedbtest.ZoneAmerica_Los_Angeles)
	tzNewYork := TimeZoneForZoneInfo(&zonedbtest.ZoneAmerica_New_York)
